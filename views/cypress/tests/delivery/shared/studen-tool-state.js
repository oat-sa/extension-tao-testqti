...
export function studentToolTest() {
  function openTool(name) {
    cy.get(`.tools-box-list [data-control=${name}]:not(.active) a`)
      .first()
  ...
  function closeTool(name) {
    cy.get(`.tools-box-list [data-control=${name}].active a`)
      .first()
      .should("have.length", 1)
      .and("be.visible")
      .click();
  ...
  function createCalculatorAlias() {
    cy.get(".test-runner-scope .widget-calculator")
      .and("not.be.visible")
      .as("calcContainer");
  ...
  describe("Calculator/Answer masking/Answer elimination", () => {
    ...
    describe("Answer masking tool", function () {
      ...
      it("turns on/off", function () {
        ...
        cy.get(".qti-choice.masked .answer-mask.masked").should(
          "have.length",
          4
        );
        closeTool(toolName.mask);
    ...
    describe("Answer elimination tool", function () {
      it("turns on/off", function () {
        ...
        cy.get('.qti-choice [data-eliminable="container"]')
  ...
  describe("Calculator BODMAS/Area masking/Highlighter/Line reader", () => {
    ...
    describe("Highlighter tool", () => {
      ...
      it("highlights (selection first)", function () {
        ...
        highlight();
        cy.get(".qti-itemBody .txt-user-highlight")
          .contains("lorem")
          .and("has.css", "background-color", "rgb(255, 255, 0)");
        clearHighlights();
        cy.get(".qti-itemBody").should(
          "not.contain",
          "span.txt-user-highlight"
        );
      });
  ...
  describe("Scientific calculator/Zoom/Magnifier", () => {
    ...
    describe("Scientific calculator tool", () => {
      function createAliases() {
        cy.get(".test-runner-scope .widget-calculator")
          .and("not.be.visible")
          .as("calcContainer");
        cy.get("@calcContainer")
          .find(".dynamic-component-container")
          .as("calc");
        cy.get("@calc").find('a[title="Close"]').as("closer");
        cy.get("@calc").find(".calculator-screen").as("display");
      }
    ...
    describe("Zoom tool", () => {
      ...
      function checkZoomValue(zoomValue) {
        cy.get(".qti-item")
          ...
          .and("have.attr", "style")
          .and("contain", `scaleX(${zoomValue})`)
          .and("contain", `scaleY(${zoomValue})`);
      }
    ...
    describe("Magnifier tool", function () {
      function creatAliases() {
        ...
        cy.get("@magnifierContainer")
          .find(".magnifier")
          .first()
          .as("magnifier");
        cy.get("@magnifier").should("be.visible");
        cy.get("@magnifier")
          .find('[data-control="closeMagnifier"]')
          .first()
          .as("closer");
      ...
      function checkMagnifying(expectedMultiplicity) {
        cy.get("@inner")
          ...
          .and("be.visible")
          .and("have.attr", "style")
          .and("contain", `scale(${expectedMultiplicity})`);
      }

      function zoomOut() {
        cy.get("@magnifier")
          .find(':not(.inner) .control[data-control="zoomOut"]')
          .click();
      }

      function zoomIn() {
        cy.get("@magnifier")
          .find(':not(.inner) .control[data-control="zoomIn"]')
          .click();
      }
