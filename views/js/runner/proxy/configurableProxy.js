...
define([
    ...
    /**
     * Builds a QTI proxy definition
     * Related to remote services calls
     * @param {Function} configFactory - A config factory that build the config storage
     * @returns {proxy}
     */
    return function configurableProxyFactory(configFactory) {
        if (!_.isFunction(configFactory)) {
            throw new TypeError('The configFactory should be a function');
        }
