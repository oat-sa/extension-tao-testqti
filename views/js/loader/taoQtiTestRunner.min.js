...
logger.debug({config:config,plugins:plugins},"Start test runner"),runner("qti",plugins,config).on("error",onError).on("ready",function(){_.defer(function(){$container.removeClass("hidden")})}).on("pause",function(data){data&&data.reason&&(exitReason=data.reason)}).after("destroy",function(){this.removeAllListeners(),exit(exitReason)}).on("disablefeedbackalerts",function(){errorFeedback&&errorFeedback.close(),preventFeedback=!0}).on("enablefeedbackalerts",function(){preventFeedback=!1}).init()}).catch(function(err){onError(err,__("An error occurred during the test initialization!"))})}}}),define("taoQtiTest/runner/config/qtiServiceConfig",["lodash","util/url"
,"util/config"],function(_,urlUtil,configHelper){"use strict";var _defaults={bootstrap:{serviceController:"Runner",serviceExtension:"taoQtiTest"}},_entries={testDefinition:!0,testCompilation:!0,serviceCallId:!0,bootstrap:!1,timeout:!1};return function(config){var storage=configHelper.from(config,_entries,_defaults);return storage.timeout?storage.timeout*=1e3:storage.timeout=void 0,{getTestDefinition:function(){return storage.testDefinition},getTestCompilation:function(){return storage.testCompilation},getServiceCallId:function(){return storage.serviceCallId},getServiceController:function(){
return storage.bootstrap.serviceController||_defaults.bootstrap.serviceController},getServiceExtension:function(){return storage.bootstrap.serviceExtension||_defaults.bootstrap.serviceExtension},getTestActionUrl:function(action){return urlUtil.route(action,this.getServiceController(),this.getServiceExtension(),{testDefinition:this.getTestDefinition(),testCompilation:this.getTestCompilation(),serviceCallId:this.getServiceCallId()})},getItemActionUrl:function(itemIdentifier,action){return urlUtil.route(action,this.getServiceController(),this.getServiceExtension(),{testDefinition:this.getTestDefinition(),testCompilation:this.getTestCompilation(),
testServiceCallId:this.getServiceCallId(),itemDefinition:itemIdentifier})},getTelemetryUrl:function(itemIdentifier,signal){return urlUtil.route(signal,this.getServiceController(),this.getServiceExtension(),{testDefinition:this.getTestDefinition(),testCompilation:this.getTestCompilation(),testServiceCallId:this.getServiceCallId(),itemDefinition:itemIdentifier})},getTimeout:function(){return storage.timeout},getCommunicationConfig:function(){var communication=storage.bootstrap.communication||{},extension=communication.extension||this.getServiceExtension(),controller=communication.controller||this.getServiceController(),action=communication.action,
syncActions=communication.syncActions||[],service=urlUtil.route(action||"message",controller,extension,{testDefinition:this.getTestDefinition(),testCompilation:this.getTestCompilation(),serviceCallId:this.getServiceCallId()}),params=_.merge({},communication.params||{},{service:communication.service||service});return params.timeout?params.timeout*=1e3:params.timeout=storage.timeout,params.interval&&(params.interval*=1e3),{enabled:communication.enabled,type:communication.type,params:params,syncActions:syncActions}}}}}),define("taoQtiTest/runner/helpers/stats",["lodash","taoQtiTest/runner/helpers/map","taoQtiTest/runner/helpers/currentItem"],function(_,
mapHelper,currentItemHelper){"use strict";return{getInstantStats:function(scope,runner,sync){var isItemCurrentlyAnswered,map=runner.getTestMap(),context=runner.getTestContext(),stats=_.clone(mapHelper.getScopeStats(map,context.itemPosition,scope));return mapHelper.getItemAt(map,context.itemPosition).informational||(!(isItemCurrentlyAnswered=currentItemHelper.isAnswered(runner))&&context.itemAnswered?stats.answered--:!isItemCurrentlyAnswered&&!sync||context.itemAnswered?sync&&!isItemCurrentlyAnswered&&context.itemAnswered&&context.isLinear&&stats.answered++:stats.answered++),stats}}}),define("taoQtiTest/runner/helpers/messages",["lodash","i18n",
...
itemsCountMessage=0===unansweredCount?__("You answered all %s question(s)",stats.questions.toString()):__("You have %s unanswered question(s)",unansweredCount.toString()),flaggedCount&&(itemsCountMessage+=" "+__("and you flagged %s item(s) that you can review now",flaggedCount.toString()))),itemsCountMessage&&(itemsCountMessage+="."),itemsCountMessage}(scope,runner,sync)),(itemsCountMessage+" "+message).trim()}}}),define("taoQtiTest/runner/helpers/offlineErrorHelper",["i18n"],function(__){"use strict";return{buildErrorFromContext:function(errorData,context){var err=_.assign(new Error(errorData.message),errorData.data);return _.assign(err,context||{})},
getOfflineNavError:function(){return{message:__("We are unable to connect to the server to retrieve the next item."),data:{success:!1,source:"navigator",purpose:"proxy",type:"nav",code:404}}},getOfflineExitError:function(){return{message:__("We are unable to connect the server to submit your results."),data:{success:!1,source:"navigator",purpose:"proxy",type:"finish",code:404}}},getOfflinePauseError:function(){return{message:__("The test has been paused, we are unable to connect to the server."),data:{success:!1,source:"navigator",purpose:"proxy",type:"pause",code:404}}}}}),define("tpl!taoQtiTest/runner/helpers/templates/offlineSyncModalCountdown",[
"handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){return this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{},'<p class="button-subtext"></p>'})}),define("taoQtiTest/runner/helpers/offlineSyncModal",["jquery","lodash","i18n","core/polling","ui/hider","ui/waitingDialog/waitingDialog","tpl!taoQtiTest/runner/helpers/templates/offlineSyncModalCountdown"],function($,_,__,polling,hider,waitingDialogFactory,offlineSyncModalCountdownTpl){"use strict";return function(proxy){var $secondaryButton,countdownPolling,waitingConfig={message:__(
"You are encountering a prolonged connectivity loss."),waitContent:__("Please continue waiting while we try to restore the connection. Alternatively, you may end this test by downloading it as a file which you will have to submit manually."),proceedContent:__("The connection seems to be back, please proceed."),proceedButtonText:__("Proceed & End Test"),showSecondary:!0,secondaryButtonText:__("Download"),secondaryButtonIcon:"download",buttonSeparatorText:__("or"),width:"600px"},$countdown=$(offlineSyncModalCountdownTpl()),waitingDialog=waitingDialogFactory(waitingConfig).on("render",function(){$secondaryButton=$("div.preview-modal-feedback.modal").find(
'button[data-control="secondary"]'),$countdown.insertAfter($secondaryButton),proxy.off("reconnect.waiting").after("reconnect.waiting",function(){waitingDialog.endWait()}),waitingDialog.is("waiting")&&waitingDialog.trigger("begincountdown")}).on("wait",function(){waitingDialog.is("rendered")&&waitingDialog.trigger("begincountdown")}).on("begincountdown",function(){var delaySec=60;$secondaryButton.prop("disabled",!0),countdownPolling=polling({action:function(){delaySec--,$countdown.html(__("The download will be available in <strong>%d</strong> seconds",delaySec)),delaySec<1&&(this.stop(),$secondaryButton.removeProp("disabled"),$countdown.html(""))},
interval:1e3,autoStart:!0})}).on("unwait",function(){countdownPolling.stop(),$secondaryButton.prop("disabled",!0),(void 0).remove(),hider.hide(".between-buttons-text")});return waitingDialog}}),define("taoQtiTest/runner/helpers/testContextBuilder",["lodash","util/capitalize","taoQtiTest/runner/helpers/map","taoQtiTest/runner/helpers/navigation"],function(_,capitalize,mapHelper,navigationHelper){"use strict";function getTestContext(testData,testContext,testMap,item,section,part,position){var categories,isLeavingSection=section.id!==testContext.sectionId,isLeavingPart=part.id!==testContext.testPartId,newTestContext=_.defaults({itemIdentifier:item.id,
itemPosition:position,itemAnswered:item.answered||part.isLinear,numberPresented:testMap.stats.viewed,numberCompleted:testMap.stats.answered,hasFeedbacks:!1,remainingAttempts:Math.max(-1,item.remainingAttempts-1),sectionId:section.id,sectionTitle:section.label,testPartId:part.id,isLinear:part.isLinear,isLast:navigationHelper.isLast(testMap,item.id),canMoveBackward:!part.isLinear&&!navigationHelper.isFirst(testMap,item.id)},testContext);return isLeavingSection&&(newTestContext.numberRubrics=0,newTestContext.rubrics=""),newTestContext.timeConstraints=_.reject(testContext.timeConstraints,function(constraint){
return"assessmentItemRef"===constraint.qtiClassName||isLeavingSection&&"assessmentSection"===constraint.qtiClassName||isLeavingPart&&"testPart"===constraint.qtiClassName}),item.timeConstraint&&newTestContext.timeConstraints.push(item.timeConstraint),isLeavingSection&&section.timeConstraint&&newTestContext.timeConstraints.push(section.timeConstraint),isLeavingPart&&part.timeConstraint&&newTestContext.timeConstraints.push(part.timeConstraint),newTestContext.options=_.defaults((categories=item.categories||[],_.isArray(categories)&&categories.length?_.reduce(categories,function(acc,category){var categoryName;return _.isString(category)&&!_.isEmpty(category)&&(
categoryName=category.replace("x-tao-option-","").split(/[-_]+/g).map(function(name,index){return 0===index?name:name.length?capitalize(name):""}).join(""))&&(acc[categoryName]=!0),acc},{}):{}),_.pick(testContext.options,["allowComment","allowSkipping","exitButton","logoutButton"])),newTestContext}return{buildTestContextFromPosition:function(testData,testContext,testMap,position){var updatedMap=mapHelper.updateItemStats(testMap,position),item=mapHelper.getItemAt(updatedMap,position),section=mapHelper.getItemSection(updatedMap,position),part=mapHelper.getItemPart(updatedMap,position);return!!item&&getTestContext(0,testContext,testMap,item,section,part,
position)},buildTestContextFromJump:function(testData,testContext,testMap,jump){var part=testMap.parts[jump.part],section=part.sections[jump.section];return getTestContext(0,testContext,testMap,section.items[jump.item],section,part,jump.position)}}}),define("taoQtiTest/runner/navigator/navigator",["lodash","taoQtiTest/runner/helpers/map","taoQtiTest/runner/helpers/navigation","taoQtiTest/runner/helpers/testContextBuilder"],function(_,mapHelper,navigationHelper,testContextBuilder){"use strict";return function(testData,testContext,testMap){if(!_.all([testData,testContext,testMap],_.isPlainObject))throw new TypeError(
"The navigator must be built with a testData, a testContext and a testMap");return{navigate:function(direction,scope,position){var methodName=direction.toLowerCase()+scope.substr(0,1).toUpperCase()+scope.substr(1).toLowerCase();if(_.isFunction(this[methodName]))return this[methodName](position)},nextItem:function(){return testContextBuilder.buildTestContextFromPosition(testData,testContext,testMap,testContext.itemPosition+1)},previousItem:function(){return testContextBuilder.buildTestContextFromPosition(testData,testContext,testMap,testContext.itemPosition-1)},nextSection:function(){var sectionStats=mapHelper.getSectionStats(testMap,testContext.sectionId),
section=mapHelper.getSection(testMap,testContext.sectionId);return testContextBuilder.buildTestContextFromPosition(testData,testContext,testMap,section.position+sectionStats.total)},jumpItem:function(position){return testContextBuilder.buildTestContextFromPosition(testData,testContext,testMap,position)}}}}),define("core/Promise",["lib/polyfill/es6-promise"],function(es6Promise){"use strict";return window.Promise||es6Promise.Promise}),define("taoQtiTest/runner/branchRule/helpers/branchRuleHelper",["lodash","core/promise"],function(_,Promise){"use strict";return{evaluateSubBranchRules:function(branchRuleDefinition,item,navigationParams,branchRuleMapper,
responseStore){return new Promise(function(resolve,reject){var promises,subBranchRuleResults=[];branchRuleDefinition=_.omit(branchRuleDefinition,["@attributes"]),promises=_.map(branchRuleDefinition,function(subBranchRule,subBranchRuleName){return branchRuleMapper(subBranchRuleName,branchRuleDefinition[subBranchRuleName],item,navigationParams,responseStore).validate()}),Promise.all(promises).then(function(results){_.forEach(results,function(subBranchRuleResult){Array.isArray(subBranchRuleResult)?subBranchRuleResult.forEach(function(value){subBranchRuleResults.push(value)}):subBranchRuleResults.push(subBranchRuleResult)}),resolve(subBranchRuleResults)}).catch(
function(err){reject(err)})})}}}),define("taoQtiTest/runner/branchRule/types/match",["lodash","core/promise"],function(_,Promise){"use strict";return function(branchRuleDefinition,item,navigationParams,branchRuleMapper,responseStore){var variableIdentifier=branchRuleDefinition.variable["@attributes"].identifier,correctIdentifier=branchRuleDefinition.correct["@attributes"].identifier;return{validate:function(){return Promise.all([responseStore.getCorrectResponse(correctIdentifier),responseStore.getResponse(variableIdentifier)]).then(function(result){return _.contains(result[0],result[1])})}}}}),define("taoQtiTest/runner/branchRule/types/or",["core/promise",
"taoQtiTest/runner/branchRule/helpers/branchRuleHelper"],function(Promise,branchRuleHelper){"use strict";return function(branchRuleDefinition,item,navigationParams,branchRuleMapper,responseStore){return{validate:function(){return branchRuleHelper.evaluateSubBranchRules(branchRuleDefinition,item,navigationParams,branchRuleMapper,responseStore).then(function(subBranchRuleResults){return subBranchRuleResults.some(function(value){return value})})}}}}),define("taoQtiTest/runner/branchRule/types/and",["core/promise","taoQtiTest/runner/branchRule/helpers/branchRuleHelper"],function(Promise,branchRuleHelper){"use strict";return function(branchRuleDefinition,item,
navigationParams,branchRuleMapper,responseStore){return{validate:function(){return branchRuleHelper.evaluateSubBranchRules(branchRuleDefinition,item,navigationParams,branchRuleMapper,responseStore).then(function(subBranchRuleResults){return subBranchRuleResults.every(function(value){return value})})}}}}),define("taoQtiTest/runner/branchRule/types/not",["lodash","core/promise"],function(_,Promise){"use strict";return function(branchRuleDefinition,item,navigationParams,branchRuleMapper,responseStore){return Array.isArray(branchRuleDefinition)||(branchRuleDefinition=[branchRuleDefinition]),{validate:function(){var promises=branchRuleDefinition.map(function(
expression){var subBranchRuleName=_.head(_.keys(expression)),subBranchRuleDefinition=expression[subBranchRuleName];return branchRuleMapper(subBranchRuleName,subBranchRuleDefinition,item,navigationParams,responseStore).validate()});return Promise.all(promises).then(function(results){return _.map(results,function(result){return!result})})}}}}),define("taoQtiTest/runner/branchRule/branchRuleMapper",["taoQtiTest/runner/branchRule/types/match","taoQtiTest/runner/branchRule/types/or","taoQtiTest/runner/branchRule/types/and","taoQtiTest/runner/branchRule/types/not"],function(matchBranchRule,orBranchRule,andBranchRule,notBranchRule){"use strict";var branchRuleMap={
match:matchBranchRule,or:orBranchRule,and:andBranchRule,not:notBranchRule};return function branchRuleMapperFactory(branchRuleName,branchRuleDefinition,item,navigationParams,responseStore){if(!(branchRuleName in branchRuleMap))throw new Error("Invalid branch rule name: "+branchRuleName);return branchRuleMap[branchRuleName](branchRuleDefinition,item,navigationParams,branchRuleMapperFactory,responseStore)}}),define("taoQtiTest/runner/branchRule/branchRule",["lodash","core/promise","taoQtiTest/runner/branchRule/helpers/branchRuleHelper","taoQtiTest/runner/branchRule/branchRuleMapper"],function(_,Promise,branchRuleHelper,branchRuleMapper){"use strict"
;return function(branchRuleDefinition,item,navigationParams,responseStore){return new Promise(function(resolve){if(void 0===branchRuleDefinition["@attributes"]||void 0===branchRuleDefinition["@attributes"].target)return resolve(null);branchRuleHelper.evaluateSubBranchRules(branchRuleDefinition,item,navigationParams,branchRuleMapper,responseStore).then(function(branchRuleResults){if(branchRuleResults.every(function(branchRuleResult){return branchRuleResult}))return resolve(branchRuleDefinition["@attributes"].target);resolve(null)})})}}),define("taoQtiTest/runner/services/offlineJumpTable",["lodash","i18n","core/promise",
"taoQtiTest/runner/branchRule/branchRule","taoQtiTest/runner/helpers/map"],function(_,__,Promise,branchRule,mapHelper){"use strict";return function(itemStore,responseStore){var testMap={},jumpTable=[];function getItems(map){return _.uniq(_.map(getSimplifiedTestMap(map),function(row){return row.item}))}function getSimplifiedTestMap(map){var simplifiedTestMap=[];return mapHelper.each(map,function(item,section,part){simplifiedTestMap.push({item:item.id,itemHasBranchRule:!_.isEmpty(item.branchRule),itemBranchRule:item.branchRule,section:section.id,sectionHasBranchRule:!_.isEmpty(section.branchRule),sectionBranchRule:section.branchRule,part:part.id,
partHasBranchRule:!_.isEmpty(part.branchRule),partBranchRule:part.branchRule})}),simplifiedTestMap}return{setTestMap:function(map){return testMap=map,this},init:function(){var firstItem,simplifiedTestMap=getSimplifiedTestMap(testMap);0<simplifiedTestMap.length&&0===jumpTable.length&&(firstItem=simplifiedTestMap[0],this.addJump(firstItem.part,firstItem.section,firstItem.item)),simplifiedTestMap.forEach(function(row){itemStore.get(row.item).then(function(item){item&&_.forEach(item.itemData.data.responses,function(response){var responseIdentifier=item.itemIdentifier+"."+response.identifier;responseStore.addCorrectResponse(responseIdentifier,
response.correctResponses)})}).catch(function(err){return Promise.reject(err)})})},clearJumpTable:function(){return jumpTable=[],this},addJump:function(partIdentifier,sectionIdentifier,itemIdentifier){var self=this;return new Promise(function(resolve){var lastJump=self.getLastJump(),nextPosition=void 0!==lastJump.position?lastJump.position+1:0;jumpTable.push({item:itemIdentifier,part:partIdentifier,section:sectionIdentifier,position:nextPosition}),resolve()})},jumpTo:function(position){return jumpTable=jumpTable.filter(function(jump){return jump.position<=position}),this},jumpToSkipItem:function(){var self=this;return new Promise(function(resolve){
var simplifiedTestMap=getSimplifiedTestMap(testMap),lastJumpItem=self.getLastJump().item||null,items=getItems(testMap),itemSliceIndex=items.indexOf(lastJumpItem),itemIdentifierToAdd=items.slice(itemSliceIndex+1).shift(),itemToAdd=simplifiedTestMap.filter(function(row){return row.item===itemIdentifierToAdd}).shift();return itemToAdd?self.addJump(itemToAdd.part,itemToAdd.section,itemToAdd.item).then(resolve):resolve()})},jumpToNextItem:function(params){var self=this;return function(params){params.itemResponse&&_.forEach(params.itemResponse,function(response,itemResponseIdentifier){var responseIdentifier=params.itemDefinition+"."+itemResponseIdentifier
;_.forEach(response,function(responseEntry){var responseId=responseEntry.identifier;Array.isArray(responseId)?responseId.forEach(function(id){responseStore.addResponse(responseIdentifier,id)}):responseStore.addResponse(responseIdentifier,responseId)})})}(params),new Promise(function(resolve){var simplifiedTestMap=getSimplifiedTestMap(testMap),lastJumpItem=self.getLastJump().item||null,items=getItems(testMap),itemSliceIndex=items.indexOf(lastJumpItem),itemIdentifierToAdd=items.slice(itemSliceIndex+1).shift(),itemToAdd=simplifiedTestMap.filter(function(row){return row.item===itemIdentifierToAdd}).shift(),lastJumpItemData=simplifiedTestMap.filter(function(row){
return row.item===lastJumpItem}).shift();return lastJumpItemData&&lastJumpItemData.itemHasBranchRule?itemStore.get(lastJumpItem).then(function(item){branchRule(lastJumpItemData.itemBranchRule,item,params,responseStore).then(function(itemIdentifierToAddd){null!==itemIdentifierToAddd&&(itemToAdd=simplifiedTestMap.filter(function(row){return row.item===itemIdentifierToAddd}).shift()),self.addJump(itemToAdd.part,itemToAdd.section,itemToAdd.item).then(resolve)}).catch(function(err){return Promise.reject(err)})}).catch(function(err){return Promise.reject(err)}):itemToAdd?self.addJump(itemToAdd.part,itemToAdd.section,itemToAdd.item).then(resolve):resolve()})},
jumpToNextSection:function(params){var self=this;return new Promise(function(resolve){var map,simplifiedTestMap=getSimplifiedTestMap(testMap),lastJumpSection=self.getLastJump().section||null,sections=(map=testMap,_.uniq(_.map(getSimplifiedTestMap(map),function(row){return row.section}))),sectionSliceIndex=sections.indexOf(lastJumpSection),sectionIdentifierToAdd=sections.slice(sectionSliceIndex+1).shift(),itemToAdd=simplifiedTestMap.filter(function(row){return row.section===sectionIdentifierToAdd}).shift();return itemToAdd?self.addJump(itemToAdd.part,itemToAdd.section,itemToAdd.item).then(resolve):resolve()})},jumpToPreviousItem:function(params){
return new Promise(function(resolve){jumpTable.pop(),resolve()})},getJumpTable:function(){return jumpTable},getLastJump:function(){return 0<jumpTable.length?jumpTable[jumpTable.length-1]:{}}}}}),define("taoQtiTest/runner/navigator/offlineNavigator",["lodash","core/Promise","util/capitalize","taoQtiTest/runner/services/offlineJumpTable","taoQtiTest/runner/helpers/testContextBuilder"],function(_,Promise,capitalize,offlineJumpTableFactory,testContextBuilder){"use strict";return function(itemStore,responseStore){var testData,testContext,testMap,offlineJumpTableHelper=offlineJumpTableFactory(itemStore,responseStore);return{setTestData:function(data){
return testData=data,this},setTestContext:function(context){return testContext=context,this},setTestMap:function(map){return testMap=map,this},init:function(){return offlineJumpTableHelper.setTestMap(testMap),offlineJumpTableHelper.init(),this},clearJumpTable:function(){return offlineJumpTableHelper.clearJumpTable(),this},navigate:function(direction,scope,position,params){return new Promise(function(resolve,reject){var lastJump,navigationActionName="jumpTo"+capitalize(direction)+capitalize(scope);if(void 0===offlineJumpTableHelper[navigationActionName]||"function"!=typeof offlineJumpTableHelper[navigationActionName])throw new Error(
"Invalid navigation action");offlineJumpTableHelper[navigationActionName](params).then(function(){lastJump=offlineJumpTableHelper.getLastJump(),resolve(testContextBuilder.buildTestContextFromJump(testData,testContext,testMap,lastJump))}).catch(function(err){reject(err)})})}}}}),define("taoQtiTest/runner/proxy/cache/actionStore",["lodash","taoTests/runner/testStore"],function(_,testStore){"use strict";return function(id){var actionQueue=[],loadStore=function(){return testStore(id).getStore("actions")};if(_.isEmpty(id))throw new TypeError("Please specify the action store id");return{push:function(action,params){return loadStore().then(function(actionStore){
return actionQueue.push({action:action,timestamp:Date.now(),parameters:params}),actionStore.setItem("queue",actionQueue)})},flush:function(){return loadStore().then(function(actionStore){return actionStore.getItem("queue").then(function(queue){return actionQueue=[],actionStore.setItem("queue",actionQueue).then(function(){return queue})})})},update:function(params){return _.forEach(actionQueue,function(action){action.parameters.actionId===params.actionId&&(action.parameters=params)}),loadStore().then(function(actionStore){return actionStore.setItem("queue",actionQueue)})}}}}),define("taoQtiTest/runner/proxy/cache/itemPreloader",["lodash","core/promise",
"core/logger","taoQtiItem/runner/qtiItemRunner","taoQtiTest/runner/config/assetManager","util/url"],function(_,Promise,loggerFactory,qtiItemRunner,getAssetManager,urlUtil){"use strict";var logger=loggerFactory("taoQtiTest/runner/proxy/cache/itemPreloader"),relSupport=function(feature){var fakeLink=document.createElement("link");try{if(fakeLink.relList&&_.isFunction(fakeLink.relList.supports))return fakeLink.relList.supports(feature)}catch(err){return!1}},supportPreload=relSupport("preload"),supportPrefetch=relSupport("prefetch"),isItemObjectValid=function(item){return _.isPlainObject(item)&&_.isString(item.baseUrl)&&_.isString(item.itemIdentifier
)&&!_.isEmpty(item.itemIdentifier)&&_.isPlainObject(item.itemData)};return function(options){var preloadAssetManager=getAssetManager("item-preloader"),images={},audioBlobs={},loaders={img:function(url,sourceUrl,itemIdentifier){images[itemIdentifier]=images[itemIdentifier]||{},"Image"in window&&!images[itemIdentifier][sourceUrl]&&(images[itemIdentifier][sourceUrl]=new Image,images[itemIdentifier][sourceUrl].src=url)},css:function(url){var link=document.createElement("link");supportPreload?(link.setAttribute("rel","preload"),link.setAttribute("as","style")):supportPrefetch?(link.setAttribute("rel","prefetch"),link.setAttribute("as","style")):(link.disabled=!0,
link.setAttribute("rel","stylesheet"),link.setAttribute("type","text/css")),link.setAttribute("data-preload",!0),link.setAttribute("href",url),document.querySelector("head").appendChild(link)},audio:function(url,sourceUrl,itemIdentifier){var request;audioBlobs[itemIdentifier]=audioBlobs[itemIdentifier]||{},void 0===audioBlobs[itemIdentifier][sourceUrl]&&((request=new XMLHttpRequest).open("GET",url,!0),request.responseType="blob",request.onload=function(){200===this.status&&(audioBlobs[itemIdentifier][sourceUrl]=this.response)},request.send())}},unloaders={img:function(url,sourceUrl,itemIdentifier){images[itemIdentifier]&&(images[itemIdentifier]=_.omit(
images[itemIdentifier],sourceUrl))},css:function(url){var link=document.querySelector('head link[data-preload][href="'+url+'"]');link&&document.querySelector("head").removeChild(link)},audio:function(url,sourceUrl,itemIdentifier){audioBlobs[itemIdentifier]&&(audioBlobs[itemIdentifier]=_.omit(audioBlobs[itemIdentifier],sourceUrl))}},resolveAssets=function(baseUrl,assets){return new Promise(function(resolve){return preloadAssetManager.setData("baseUrl",baseUrl),preloadAssetManager.setData("assets",assets),resolve(_.reduce(assets,function(acc,assetList,type){var resolved={};return _.forEach(assetList,function(url){urlUtil.isBase64(url)||(
resolved[url]=preloadAssetManager.resolve(url))}),0<_.size(resolved)&&(acc[type]=resolved),acc},{}))})};if(!options||!options.testId)throw new TypeError("The test identifier is mandatory to start the item preloader");return getAssetManager(options.testId).prependStrategy({name:"precaching",handle:function(url,data){var sourceUrl=url.toString();if(data.itemIdentifier&&audioBlobs[data.itemIdentifier]&&void 0!==audioBlobs[data.itemIdentifier][sourceUrl])return URL.createObjectURL(audioBlobs[data.itemIdentifier][sourceUrl])}}),{preload:function(item){var loading=[];return isItemObjectValid(item)&&(loading.push((logger.debug(
"Start preloading of item "+item.itemIdentifier),new Promise(function(resolve,reject){qtiItemRunner(item.itemData.type,item.itemData.data,{assetManager:preloadAssetManager,preload:!0}).on("init",function(){logger.debug("Preloading of item "+item.itemIdentifier+" done"),resolve(!0)}).on("error",reject).init()}))),0<_.size(item.itemData.assets)&&loading.push(resolveAssets(item.baseUrl,item.itemData.assets).then(function(resolved){return _.forEach(resolved,function(assets,type){_.isFunction(loaders[type])&&_.forEach(assets,function(url,sourceUrl){logger.debug("Loading asset "+sourceUrl+"("+type+") for item "+item.itemIdentifier),loaders[type](url,sourceUrl,
item.itemIdentifier)})}),!0}))),Promise.all(loading).then(function(results){return 0<results.length&&_.all(results,_.isTrue)})},unload:function(item,itemIdentifier){return isItemObjectValid(item)&&0<_.size(item.itemData.assets)?resolveAssets(item.baseUrl,item.itemData.assets).then(function(resolved){return _.forEach(resolved,function(assets,type){_.isFunction(unloaders[type])&&_.forEach(assets,function(url,sourceUrl){logger.debug("Unloading asset "+sourceUrl+"("+type+") for item "+item.itemIdentifier),unloaders[type](url,sourceUrl,item.itemIdentifier)})}),!0}):Promise.resolve(!1)}}}}),define("taoQtiTest/runner/proxy/cache/itemStore",["lodash","core/store",
"taoQtiTest/runner/proxy/cache/itemPreloader"],function(_,store,itemPreloaderFactory){"use strict";var defaultConfig={maxSize:10,preload:!1};return function(options){var itemPreloader,config=_.defaults(options||{},defaultConfig),getStore=function(){return store("item-cache",store.backends.memory)},index=[];return config.preload&&(itemPreloader=itemPreloaderFactory(_.pick(config,["testId"]))),{setCacheSize:function(cacheSize){config.maxSize=cacheSize},get:function(key){return getStore().then(function(itemStorage){return itemStorage.getItem(key)})},has:function(key){return _.contains(index,key)},set:function(key,item){var self=this;return getStore().then(
function(itemStorage){return itemStorage.setItem(key,item).then(function(updated){return updated&&(_.contains(index,key)||index.push(key),config.preload&&_.defer(function(){itemPreloader.preload(item)})),1<index.length&&index.length>config.maxSize?self.remove(index[0]).then(function(removed){return updated&&removed}):updated})})},update:function(key,updateKey,updateValue){return this.has(key)&&_.isString(updateKey)?getStore().then(function(itemStorage){return itemStorage.getItem(key).then(function(itemData){if(_.isPlainObject(itemData))return itemData[updateKey]=updateValue,itemStorage.setItem(key,itemData)})}):Promise.resolve(!1)},remove:function(key){
return this.has(key)?getStore().then(function(itemStorage){return index=_.without(index,key),itemStorage.getItem(key).then(function(item){config.preload&&_.defer(function(){itemPreloader.unload(item)})}).then(function(){return itemStorage.removeItem(key)})}):Promise.resolve(!1)},clear:function(){return getStore().then(function(itemStorage){return index=[],itemStorage.clear()})}}}}),define("taoQtiTest/runner/proxy/qtiServiceProxy",["jquery","lodash","i18n","core/promise","core/promiseQueue","core/communicator","taoQtiTest/runner/config/qtiServiceConfig","util/httpErrorParser"],function($,_,__,Promise,promiseQueue,communicatorFactory,configFactory,
httpErrorParser){"use strict";return{install:function(){var self=this;this.queue=promiseQueue(),this.prepareParams=function(actionParams){var stringifyParams=["itemState","itemResponse","toolStates"];return _.isPlainObject(actionParams)?_.mapValues(actionParams,function(value,key){return _.contains(stringifyParams,key)?JSON.stringify(value):value}):actionParams},this.request=function(url,reqParams,contentType,noToken){var runRequest=function(){return new Promise(function(resolve,reject){var token,headers={},action=reqParams?"POST":"GET",preparedParams=self.prepareParams(reqParams),tokenHandler=self.getTokenHandler();noToken||(token=tokenHandler.getToken()
)&&(headers["X-Auth-Token"]=token),$.ajax({url:url,type:action,cache:!1,data:preparedParams,headers:headers,async:!0,dataType:"json",contentType:contentType||void 0,timeout:self.configStorage.getTimeout()}).done(function(data){data&&data.token&&tokenHandler.setToken(data.token),self.setOnline(),data&&data.success?resolve(data):reject(data)}).fail(function(jqXHR,textStatus,errorThrown){var data;try{data=JSON.parse(jqXHR.responseText)}catch(err){data={}}if((data=_.defaults(data,{success:!1,source:"network",cause:url,purpose:"proxy",context:this,code:jqXHR.status,sent:0<jqXHR.readyState,type:textStatus||"error",message:errorThrown||__("An error occurred!")})
).token?tokenHandler.setToken(data.token):noToken||tokenHandler.setToken(token),self.isConnectivityError(data))return self.setOffline("request"),resolve(data);reject(httpErrorParser.parse(jqXHR,textStatus,errorThrown))})})};return!0===noToken?runRequest():this.queue.serie(runRequest)}},init:function(config,params){return this.configStorage=configFactory(config||{}),this.request(this.configStorage.getTestActionUrl("init"),params)},destroy:function(){return this.configStorage=null,this.queue=null,Promise.resolve()},getTestData:function(){return this.request(this.configStorage.getTestActionUrl("getTestData"))},getTestContext:function(){return this.request(
this.configStorage.getTestActionUrl("getTestContext"))},getTestMap:function(){return this.request(this.configStorage.getTestActionUrl("getTestMap"))},sendVariables:function(variables){return this.request(this.configStorage.getTestActionUrl("storeTraceData"),{traceData:JSON.stringify(variables)})},callTestAction:function(action,params){return this.request(this.configStorage.getTestActionUrl(action),params)},getItem:function(itemIdentifier,params){return this.request(this.configStorage.getItemActionUrl(itemIdentifier,"getItem"),params)},submitItem:function(itemIdentifier,state,response,params){var body=_.merge({itemState:state,itemResponse:response},params||{}
);return this.request(this.configStorage.getItemActionUrl(itemIdentifier,"submitItem"),body)},callItemAction:function(itemIdentifier,action,params){return this.request(this.configStorage.getItemActionUrl(itemIdentifier,action),params)},telemetry:function(itemIdentifier,signal,params){return this.request(this.configStorage.getTelemetryUrl(itemIdentifier,signal),params,null,!0)},loadCommunicator:function(){var config=this.configStorage.getCommunicationConfig();return config.enabled?communicatorFactory(config.type,config.params):null}}}),define("taoQtiTest/runner/proxy/cache/proxy",["lodash","i18n","core/promise","taoQtiTest/runner/navigator/navigator",
"taoQtiTest/runner/helpers/map","taoQtiTest/runner/helpers/navigation","taoQtiTest/runner/provider/dataUpdater","taoQtiTest/runner/proxy/qtiServiceProxy","taoQtiTest/runner/proxy/cache/itemStore","taoQtiTest/runner/proxy/cache/actionStore","taoQtiTest/runner/helpers/offlineErrorHelper"],function(_,__,Promise,testNavigatorFactory,mapHelper,navigationHelper,dataUpdater,qtiServiceProxy,itemStoreFactory,actionStoreFactory,offlineErrorHelper){"use strict";return _.defaults({install:function(config){var self=this;qtiServiceProxy.install.call(this),this.itemStore=itemStoreFactory({maxSize:20,preload:!0,testId:config.serviceCallId}),this.actiontStore=null,
this.getItemFromStore=!1,this.requestConfig={},this.actionPromises={},this.dataUpdater=dataUpdater(this.getDataHolder()),this.getCacheAmount=function(){var cacheAmount=1,testData=this.getDataHolder().get("testData");return testData&&testData.config&&testData.config.itemCaching&&(cacheAmount=parseInt(testData.config.itemCaching.amount,10)||cacheAmount),cacheAmount},this.hasItem=function(itemIdentifier){return itemIdentifier&&self.itemStore.has(itemIdentifier)},this.hasNextItem=function(itemIdentifier){var sibling=navigationHelper.getNextItem(this.getDataHolder().get("testMap"),itemIdentifier);return sibling&&self.hasItem(sibling.id)},
this.hasPreviousItem=function(itemIdentifier){var sibling=navigationHelper.getPreviousItem(this.getDataHolder().get("testMap"),itemIdentifier);return sibling&&self.hasItem(sibling.id)},this.offlineAction=function(action,actionParams){var newTestContext,result={success:!0},testData=this.getDataHolder().get("testData"),testContext=this.getDataHolder().get("testContext"),testMap=this.getDataHolder().get("testMap");if("pause"===action)throw offlineErrorHelper.buildErrorFromContext(offlineErrorHelper.getOfflinePauseError(),{reason:actionParams.reason});if(_.contains(["exitTest","timeout"],action)||"next"===actionParams.direction&&navigationHelper.isLast(testMap,
testContext.itemIdentifier))throw offlineErrorHelper.buildErrorFromContext(offlineErrorHelper.getOfflineExitError());if(actionParams.direction&&actionParams.scope){if(!(newTestContext=testNavigatorFactory(testData,testContext,testMap).navigate(actionParams.direction,actionParams.scope,actionParams.ref))||!newTestContext.itemIdentifier||!self.hasItem(newTestContext.itemIdentifier))throw offlineErrorHelper.buildErrorFromContext(offlineErrorHelper.getOfflineNavError());result.testContext=newTestContext}return self.markActionAsOffline(actionParams),result},this.processSyncAction=function(action,actionParams,deferred){return new Promise(function(resolve,reject){
self.scheduleAction(action,actionParams).then(function(actionData){self.actionPromises[actionData.params.actionId]=resolve,deferred||self.syncData().then(function(result){self.isOnline()&&_.forEach(result,function(actionResult){var actionId=actionResult.requestParameters&&actionResult.requestParameters.actionId?actionResult.requestParameters.actionId:null;actionId&&self.actionPromises[actionId]&&self.actionPromises[actionId](actionResult)})}).catch(function(reason){reject(reason)})}).catch(function(reason){reject(reason)})})},this.scheduleAction=function(action,actionParams){return actionParams.actionId=action+"_"+(new Date).getTime(),self.actiontStore.push(
action,self.prepareParams(_.defaults(actionParams||{},self.requestConfig))).then(function(){return{action:action,params:actionParams}})},this.requestNetworkThenOffline=function(url,action,actionParams,deferred){var testContext=this.getDataHolder().get("testContext"),communicationConfig=self.configStorage.getCommunicationConfig(),runRequestThenOffline=function(){var request;return communicationConfig.syncActions&&0<=communicationConfig.syncActions.indexOf(action)?request=self.processSyncAction(action,actionParams,deferred):(request=self.request(url,actionParams)).then(function(result){return self.isOffline()?self.scheduleAction(action,actionParams):result}),
request.then(function(result){return self.isOffline()?self.offlineAction(action,actionParams):result}).catch(function(error){if(self.isConnectivityError(error)&&self.isOffline())return self.offlineAction(action,actionParams);throw error})};return this.isOffline()?this.telemetry(testContext.itemIdentifier,"up").then(function(){return self.isOnline()?runRequestThenOffline():self.scheduleAction(action,actionParams).then(function(){return self.offlineAction(action,actionParams)})}).catch(function(err){if(self.isConnectivityError(err))return self.scheduleAction(action,actionParams).then(function(){return self.offlineAction(action,actionParams)});throw err}
):runRequestThenOffline()},this.syncData=function(){var actions;return this.queue.serie(function(){return self.actiontStore.flush().then(function(data){if((actions=data)&&data.length)return self.send("sync",data)}).catch(function(err){throw self.isConnectivityError(err)&&(self.setOffline("communicator"),_.forEach(actions,function(action){self.actiontStore.push(action.action,action.parameters)})),err})})},this.exportActions=function(){var actions;return this.queue.serie(function(){return self.actiontStore.flush().then(function(data){return actions=data,_.forEach(actions,function(action){self.actiontStore.push(action.action,action.parameters)}),data})})},
this.markActionAsOffline=function(actionParams){return actionParams.offline=!0,this.queue.serie(function(){return self.actiontStore.update(self.prepareParams(_.defaults(actionParams||{},self.requestConfig)))})}},init:function(config,params){var self=this;if(!this.getDataHolder())throw new Error("Unable to retrieve test runners data holder");return this.requestConfig=_.pick(config,["testDefinition","testCompilation","serviceCallId"]),this.actiontStore=actionStoreFactory(config.serviceCallId),this.on("reconnect",function(){return this.syncData().then(function(responses){self.dataUpdater.update(responses)}).catch(function(err){self.trigger("error",err)})}),
this.syncData(),qtiServiceProxy.init.call(this,config,params)},destroy:function(){return this.itemStore.clear(),this.getItemFromStore=!1,qtiServiceProxy.destroy.call(this)},getItem:function(itemIdentifier,params){var self=this,loadNextItem=function(){var testMap=self.getDataHolder().get("testMap"),siblings=navigationHelper.getSiblingItems(testMap,itemIdentifier,"both",self.getCacheAmount()),missing=_.reduce(siblings,function(list,sibling){return self.hasItem(sibling.id)||list.push(sibling.id),list},[]);self.isOnline()&&missing.length&&_.delay(function(){self.requestNetworkThenOffline(self.configStorage.getTestActionUrl("getNextItemData"),"getNextItemData",{
...
"itemState",state).then(function(){return qtiServiceProxy.submitItem.call(self,itemIdentifier,state,response,params)})},sendVariables:function(variables,deferred){var actionParams={traceData:JSON.stringify(variables)};return this.requestNetworkThenOffline(this.configStorage.getTestActionUrl("storeTraceData"),"storeTraceData",actionParams,deferred)},callTestAction:function(action,params,deferred){return this.requestNetworkThenOffline(this.configStorage.getTestActionUrl(action),action,params,deferred)},callItemAction:function(itemIdentifier,action,params,deferred){var self=this,updateStatePromise=Promise.resolve(),testMap=this.getDataHolder().get("testMap")
;return params.itemState&&(updateStatePromise=this.itemStore.update(itemIdentifier,"itemState",params.itemState)),self.getItemFromStore=navigationHelper.isMovingToNextItem(action,params)&&self.hasNextItem(itemIdentifier)||navigationHelper.isMovingToPreviousItem(action,params)&&self.hasPreviousItem(itemIdentifier)||navigationHelper.isJumpingToItem(action,params)&&self.hasItem(mapHelper.getItemIdentifier(testMap,params.ref)),(navigationHelper.isMovingToNextItem(action,params)||navigationHelper.isMovingToPreviousItem(action,params)||navigationHelper.isJumpingToItem(action,params))&&(params.start=!0),updateStatePromise.then(function(){
return self.requestNetworkThenOffline(self.configStorage.getItemActionUrl(itemIdentifier,action),action,_.merge({itemDefinition:itemIdentifier},params),deferred)})}},qtiServiceProxy)}),define("taoQtiTest/runner/services/responseStore",["lodash","core/store"],function(_,store){"use strict";var defaultConfig={responseStoreName:"response",correctResponseStoreName:"correct-response"};return function(options){var config=_.defaults(options||{},defaultConfig),getResponseStore=function(){return store(config.responseStoreName,store.backends.memory)},getCorrectResponseStore=function(){return store(config.correctResponseStoreName,store.backends.memory)};return{
getResponses:function(){return getResponseStore().then(function(storage){return storage.getItems()})},getCorrectResponses:function(){return getCorrectResponseStore().then(function(storage){return storage.getItems()})},getResponse:function(key){return getResponseStore().then(function(storage){return storage.getItem(key)})},getCorrectResponse:function(key){return getCorrectResponseStore().then(function(storage){return storage.getItem(key).then(function(result){return new Promise(function(resolve){return resolve(void 0===result?[]:result)})})})},addResponse:function(key,value){return getResponseStore().then(function(storage){return storage.setItem(key,value
).then(function(updated){return updated})})},addCorrectResponse:function(key,value){return getCorrectResponseStore().then(function(storage){return storage.setItem(key,value).then(function(updated){return updated})})},clearResponses:function(){return getResponseStore().then(function(storage){return storage.clear()})},clearCorrectResponses:function(){return getCorrectResponseStore().then(function(storage){return storage.clear()})}}}}),define("taoQtiTest/runner/proxy/offline/proxy",["lodash","i18n","core/promise","taoQtiTest/runner/navigator/offlineNavigator","taoQtiTest/runner/helpers/map","taoQtiTest/runner/helpers/navigation",
"taoQtiTest/runner/provider/dataUpdater","taoQtiTest/runner/proxy/qtiServiceProxy","taoQtiTest/runner/proxy/cache/itemStore","taoQtiTest/runner/proxy/cache/actionStore","taoQtiTest/runner/helpers/offlineErrorHelper","taoQtiTest/runner/helpers/offlineSyncModal","taoQtiTest/runner/services/responseStore","util/download"],function(_,__,Promise,offlineNavigatorFactory,mapHelper,navigationHelper,dataUpdater,qtiServiceProxy,itemStoreFactory,actionStoreFactory,offlineErrorHelper,offlineSyncModal,responseStoreFactory,download){"use strict";return _.defaults({install:function(config){var self=this;qtiServiceProxy.install.call(this),this.itemStore=itemStoreFactory({
preload:!0,testId:config.serviceCallId}),this.responseStore=responseStoreFactory(),this.offlineNavigator=offlineNavigatorFactory(this.itemStore,this.responseStore),this.actionStore=null,this.requestConfig={},this.actionPromises={},this.dataUpdater=dataUpdater(this.getDataHolder()),this.hasItem=function(itemIdentifier){return itemIdentifier&&self.itemStore.has(itemIdentifier)},this.hasNextItem=function(itemIdentifier){var sibling=navigationHelper.getNextItem(this.getDataHolder().get("testMap"),itemIdentifier);return sibling&&self.hasItem(sibling.id)},this.hasPreviousItem=function(itemIdentifier){var sibling=navigationHelper.getPreviousItem(this.getDataHolder(
).get("testMap"),itemIdentifier);return sibling&&self.hasItem(sibling.id)},this.offlineAction=function(action,actionParams){return new Promise(function(resolve,reject){var newTestContext,result={success:!0},dataHolder=self.getDataHolder(),testData=dataHolder.get("testData"),testContext=dataHolder.get("testContext"),testMap=dataHolder.get("testMap");if("pause"===action)throw offlineErrorHelper.buildErrorFromContext(offlineErrorHelper.getOfflinePauseError(),{reason:actionParams.reason});if(_.contains(["exitTest","timeout"],action)||("next"===actionParams.direction||"skip"===action)&&navigationHelper.isLast(testMap,testContext.itemIdentifier)
)return result.testContext={state:testData.states.closed},self.isOffline()?new Promise(function(){offlineSyncModal(self).on("proceed",function(){self.syncData().then(function(){return resolve(result)}).catch(function(){return resolve({success:!1})})}).on("secondaryaction",function(){self.initiateDownload().catch(function(){return resolve({success:!1})})})}).catch(function(){return resolve({success:!1})}):self.syncData().then(function(){return resolve(result)}).catch(function(){return resolve({success:!1})});"skip"===action&&(actionParams.direction=action),actionParams.direction&&actionParams.scope?self.offlineNavigator.setTestData(testData).setTestContext(
testContext).setTestMap(testMap).init().navigate(actionParams.direction,actionParams.scope,actionParams.ref,actionParams).then(function(res){if(!(newTestContext=res)||!newTestContext.itemIdentifier||!self.hasItem(newTestContext.itemIdentifier))throw offlineErrorHelper.buildErrorFromContext(offlineErrorHelper.getOfflineNavError());result.testContext=newTestContext,resolve(result)}).catch(function(err){reject(err)}):resolve(result)})},this.scheduleAction=function(action,actionParams){return actionParams=_.assign(actionParams,{actionId:action+"_"+(new Date).getTime(),offline:!0}),self.actionStore.push(action,self.prepareParams(_.defaults(actionParams||{},
self.requestConfig))).then(function(){return{action:action,params:actionParams}})},this.syncData=function(){var actions;return this.queue.serie(function(){return self.actionStore.flush().then(function(data){if((actions=data)&&data.length)return self.send("sync",data)}).catch(function(err){throw self.isConnectivityError(err)&&(self.setOffline("communicator"),_.forEach(actions,function(action){self.actionStore.push(action.action,action.parameters)})),err})})},this.prepareDownload=function(actions){var timestamp=Date.now(),dateTime=new Date(timestamp).toISOString(),testData=self.getDataHolder().get("testData");return{
filename:"Download of "+testData.title+" at "+dateTime+".json",content:JSON.stringify({timestamp:timestamp,testData:testData,actionQueue:actions})}},this.initiateDownload=function(){return this.queue.serie(function(){return self.actionStore.flush().then(function(actions){return _.forEach(actions,function(action){self.actionStore.push(action.action,action.parameters)}),actions}).then(self.prepareDownload).then(function(data){return download(data.filename,data.content)})})}},init:function(config,params){var self=this,InitCallPromise=qtiServiceProxy.init.call(this,config,params);if(!this.getDataHolder())throw new Error(
"Unable to retrieve test runners data holder");return this.requestConfig=_.pick(config,["testDefinition","testCompilation","serviceCallId"]),this.actionStore=actionStoreFactory(config.serviceCallId),InitCallPromise.then(function(response){var promises=[];return response.items||(response.items={}),self.itemStore.setCacheSize(_.size(response.items)),_.forEach(response.items,function(item,itemIdentifier){promises.push(self.itemStore.set(itemIdentifier,item))}),Promise.all(promises).then(function(){return new Promise(function(resolve){resolve(response)})})})},destroy:function(){var self=this;return this.itemStore.clear().then(function(){
return qtiServiceProxy.destroy.call(self)})},getItem:function(itemIdentifier){return this.itemStore.get(itemIdentifier)},submitItem:function(itemIdentifier,state,response,params){var self=this;return this.itemStore.update(itemIdentifier,"itemState",state).then(function(){return qtiServiceProxy.submitItem.call(self,itemIdentifier,state,response,params)})},sendVariables:function(variables){var self=this,actionParams={traceData:JSON.stringify(variables)};return self.scheduleAction("storeTraceData",actionParams).then(function(){return self.offlineAction("storeTraceData",actionParams)}).catch(function(err){return Promise.reject(err)})},callTestAction:function(
action,params){var self=this;return self.scheduleAction(action,params).then(function(){return self.offlineAction(action,params)}).catch(function(err){return Promise.reject(err)})},callItemAction:function(itemIdentifier,action,params){var self=this,updateStatePromise=Promise.resolve();return params.itemState&&(updateStatePromise=this.itemStore.update(itemIdentifier,"itemState",params.itemState)),(navigationHelper.isMovingToNextItem(action,params)||navigationHelper.isMovingToPreviousItem(action,params)||navigationHelper.isJumpingToItem(action,params))&&(params.start=!0),updateStatePromise.then(function(){return params=_.assign({itemDefinition:itemIdentifier},
params),self.scheduleAction(action,params).then(function(){return self.offlineAction(action,params)}).catch(function(err){return Promise.reject(err)})}).catch(function(err){return Promise.reject(err)})}},qtiServiceProxy)}),function(c){var d=document,a="appendChild",i="styleSheet",s=d.createElement("style");s.type="text/css",d.getElementsByTagName("head")[0][a](s),s[i]?s[i].cssText=c:s[a](d.createTextNode(c))}(
...
),define("taoQtiTest/loader/taoQtiTestRunner.bundle",function(){}),window.bundles=(window.bundles||[]).concat(["taoItems/loader/taoItemsRunner.min","taoTests/loader/taoTestsRunner.min","taoQtiItem/loader/taoQtiItemRunner.min"]);
