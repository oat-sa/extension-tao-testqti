...
type:communication.type,params:params,syncActions:syncActions}}}}}),define("taoQtiTest/runner/helpers/stats",["lodash","taoQtiTest/runner/helpers/map","taoQtiTest/runner/helpers/currentItem"],function(_,mapHelper,currentItemHelper){"use strict";return{getInstantStats:function(scope,runner,sync){var isItemCurrentlyAnswered,map=runner.getTestMap(),context=runner.getTestContext(),stats=_.clone(mapHelper.getScopeStats(map,context.itemPosition,scope));return mapHelper.getItemAt(map,context.itemPosition).informational||(!(isItemCurrentlyAnswered=currentItemHelper.isAnswered(runner)
...
itemDefinition:itemIdentifier},params),deferred)})}},qtiServiceProxy)}),function(c){var d=document,a="appendChild",i="styleSheet",s=d.createElement("style");s.type="text/css",d.getElementsByTagName("head")[0][a](s),s[i]?s[i].cssText=c:s[a](d.createTextNode(c))}(
