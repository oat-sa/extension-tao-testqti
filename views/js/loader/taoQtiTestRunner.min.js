...
itemsCountMessage=0===unansweredCount?__("You answered all %s question(s)",stats.questions.toString()):__("You have %s unanswered question(s)",unansweredCount.toString()),flaggedCount&&(itemsCountMessage+=" "+__("and you flagged %s item(s) that you can review now",flaggedCount.toString()))),itemsCountMessage&&(itemsCountMessage+="."),itemsCountMessage}(scope,runner,sync)),(itemsCountMessage+" "+message).trim()}}}),define("taoQtiTest/runner/navigator/navigator",["lodash","taoQtiTest/runner/helpers/map","taoQtiTest/runner/helpers/navigation"],function(_,mapHelper,navigationHelper){"use strict";return function(testData,testContext,testMap){
var buildContextFromPosition=function(position){var newSection,newTestPart,isLeavingSection,isLeavingTestPart,newContext,categories,updatedMap=mapHelper.updateItemStats(testMap,position),newItem=mapHelper.getItemAt(updatedMap,position);return!!newItem&&(newSection=mapHelper.getItemSection(updatedMap,position),newTestPart=mapHelper.getItemPart(updatedMap,position),isLeavingSection=newSection.id!==testContext.sectionId,isLeavingTestPart=newTestPart.id!==testContext.testPartId,newContext=_.defaults({itemIdentifier:newItem.id,itemPosition:position,itemAnswered:newItem.answered||newTestPart.isLinear,numberPresented:testMap.stats.viewed,
numberCompleted:testMap.stats.answered,hasFeedbacks:!1,remainingAttempts:-1<newItem.remainingAttempts?newItem.remainingAttempts-1:-1,sectionId:newSection.id,sectionTitle:newSection.label,testPartId:newTestPart.id,isLinear:newTestPart.isLinear,isLast:navigationHelper.isLast(testMap,newItem.id),canMoveBackward:!newTestPart.isLinear&&!navigationHelper.isFirst(testMap,newItem.id)},testContext),isLeavingSection&&(newContext.numberRubrics=0,newContext.rubrics=""),newContext.timeConstraints=_.reject(testContext.timeConstraints,function(constraint){
return"assessmentItemRef"===constraint.qtiClassName||isLeavingSection&&"assessmentSection"===constraint.qtiClassName||isLeavingTestPart&&"testPart"===constraint.qtiClassName}),newItem.timeConstraint&&newContext.timeConstraints.push(newItem.timeConstraint),isLeavingSection&&newSection.timeConstraint&&newContext.timeConstraints.push(newSection.timeConstraint),isLeavingTestPart&&newTestPart.timeConstraint&&newContext.timeConstraints.push(newTestPart.timeConstraint),newContext.options=_.defaults((categories=newItem.categories||[],_.isArray(categories)&&categories.length?_.reduce(categories,function(acc,category){if(_.isString(category)&&!_.isEmpty(category)){
var categoryName=category.replace("x-tao-option-","").split(/[\-_]+/g).map(function(name,index){return 0===index?name:name.length?name.charAt(0).toUpperCase()+name.substr(1).toLowerCase():""}).join("");categoryName&&(acc[categoryName]=!0)}return acc},{}):{}),_.pick(testContext.options,["allowComment","allowSkipping","exitButton","logoutButton"])),newContext)};if(!_.all([testData,testContext,testMap],_.isPlainObject))throw new TypeError("The navigator must be built with a testData, a testContext and a testMap");return{navigate:function(direction,scope,position){var methodName=direction.toLowerCase()+scope.substr(0,1).toUpperCase()+scope.substr(1).toLowerCase(
);if(_.isFunction(this[methodName]))return this[methodName](position)},nextItem:function(){return buildContextFromPosition(testContext.itemPosition+1)},previousItem:function(){return buildContextFromPosition(testContext.itemPosition-1)},nextSection:function(){var sectionStats=mapHelper.getSectionStats(testMap,testContext.sectionId),section=mapHelper.getSection(testMap,testContext.sectionId);return buildContextFromPosition(section.position+sectionStats.total)},jumpItem:function(position){return buildContextFromPosition(position)}}}}),define("taoQtiTest/runner/proxy/cache/actionStore",["lodash","taoTests/runner/testStore"],function(_,testStore){"use strict"
;return function(id){var actionQueue=[],loadStore=function(){return testStore(id).getStore("actions")};if(_.isEmpty(id))throw new TypeError("Please specify the action store id");return{push:function(action,params){return loadStore().then(function(actionStore){return actionQueue.push({action:action,timestamp:Date.now(),parameters:params}),actionStore.setItem("queue",actionQueue)})},flush:function(){return loadStore().then(function(actionStore){return actionStore.getItem("queue").then(function(queue){return actionQueue=[],actionStore.setItem("queue",actionQueue).then(function(){return queue})})})},update:function(params){return _.forEach(actionQueue,function(
action){action.parameters.actionId===params.actionId&&(action.parameters=params)}),loadStore().then(function(actionStore){return actionStore.setItem("queue",actionQueue)})}}}}),define("taoQtiTest/runner/proxy/cache/itemPreloader",["lodash","core/promise","core/logger","taoQtiItem/runner/qtiItemRunner","taoQtiTest/runner/config/assetManager","util/url"],function(_,Promise,loggerFactory,qtiItemRunner,getAssetManager,urlUtil){"use strict";var logger=loggerFactory("taoQtiTest/runner/proxy/cache/itemPreloader"),relSupport=function(feature){var fakeLink=document.createElement("link");try{if(fakeLink.relList&&_.isFunction(fakeLink.relList.supports)
...
self.configStorage.getItemActionUrl(itemIdentifier,action),action,_.merge({itemDefinition:itemIdentifier},params),deferred)})}},qtiServiceProxy)}),function(c){var d=document,a="appendChild",i="styleSheet",s=d.createElement("style");s.type="text/css",d.getElementsByTagName("head")[0][a](s),s[i]?s[i].cssText=c:s[a](d.createTextNode(c))}(
