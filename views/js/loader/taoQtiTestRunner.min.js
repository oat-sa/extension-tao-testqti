...
;return branchRuleMapper(subBranchRuleName,subBranchRuleDefinition,item,navigationParams,responseStore).validate()});return Promise.all(promises).then(function(results){return _.map(results,function(result){return!result})})}}}}),define("taoQtiTest/runner/branchRule/branchRuleMapper",["taoQtiTest/runner/branchRule/types/match","taoQtiTest/runner/branchRule/types/or","taoQtiTest/runner/branchRule/types/and","taoQtiTest/runner/branchRule/types/not"],function(matchBranchRule,orBranchRule,andBranchRule,notBranchRule){"use strict";var branchRuleMap={match:matchBranchRule,or:orBranchRule,and:andBranchRule,not:notBranchRule};return function branchRuleMapperFactory(
branchRuleName,branchRuleDefinition,item,navigationParams,responseStore){if(!(branchRuleName in branchRuleMap))throw new Error("Invalid branch rule name: "+branchRuleName);return branchRuleMap[branchRuleName](branchRuleDefinition,item,navigationParams,branchRuleMapperFactory,responseStore)}}),define("taoQtiTest/runner/branchRule/branchRule",["lodash","core/promise","taoQtiTest/runner/branchRule/helpers/branchRuleHelper","taoQtiTest/runner/branchRule/branchRuleMapper"],function(_,Promise,branchRuleHelper,branchRuleMapper){"use strict";return function(branchRuleDefinition,item,navigationParams,responseStore){return new Promise(function(resolve){if(
void 0===branchRuleDefinition["@attributes"]||void 0===branchRuleDefinition["@attributes"].target)return resolve(null);branchRuleHelper.evaluateSubBranchRules(branchRuleDefinition,item,navigationParams,branchRuleMapper,responseStore).then(function(branchRuleResults){if(branchRuleResults.every(function(branchRuleResult){return branchRuleResult}))return resolve(branchRuleDefinition["@attributes"].target);resolve(null)})})}}),define("taoQtiTest/runner/services/offlineJumpTable",["lodash","i18n","core/promise","taoQtiTest/runner/branchRule/branchRule","taoQtiTest/runner/helpers/map"],function(_,__,Promise,branchRule,mapHelper){"use strict";return function(
itemStore,responseStore){var testMap={},jumpTable=[];function getItems(map){return _.uniq(_.map(getSimplifiedTestMap(map),function(row){return row.item}))}function getSimplifiedTestMap(map){var simplifiedTestMap=[];return mapHelper.each(map,function(item,section,part){simplifiedTestMap.push({item:item.id,itemHasBranchRule:!_.isEmpty(item.branchRule),itemBranchRule:item.branchRule,section:section.id,sectionHasBranchRule:!_.isEmpty(section.branchRule),sectionBranchRule:section.branchRule,part:part.id,partHasBranchRule:!_.isEmpty(part.branchRule),partBranchRule:part.branchRule})}),simplifiedTestMap}return{setTestMap:function(map){return testMap=map,this},
init:function(){var firstItem,simplifiedTestMap=getSimplifiedTestMap(testMap);0<simplifiedTestMap.length&&0===jumpTable.length&&(firstItem=simplifiedTestMap[0],this.addJump(firstItem.part,firstItem.section,firstItem.item)),simplifiedTestMap.forEach(function(row){itemStore.get(row.item).then(function(item){item&&_.forEach(item.itemData.data.responses,function(response){var responseIdentifier=item.itemIdentifier+"."+response.identifier;responseStore.addCorrectResponse(responseIdentifier,response.correctResponses)})}).catch(function(err){return Promise.reject(err)})})},clearJumpTable:function(){return jumpTable=[],this},addJump:function(partIdentifier,
sectionIdentifier,itemIdentifier){var self=this;return new Promise(function(resolve){var lastJump=self.getLastJump(),nextPosition=void 0!==lastJump.position?lastJump.position+1:0;jumpTable.push({item:itemIdentifier,part:partIdentifier,section:sectionIdentifier,position:nextPosition}),resolve()})},jumpTo:function(position){return jumpTable=jumpTable.filter(function(jump){return jump.position<=position}),this},jumpToSkipItem:function(){var self=this;return new Promise(function(resolve){var simplifiedTestMap=getSimplifiedTestMap(testMap),lastJumpItem=self.getLastJump().item||null,items=getItems(testMap),itemSliceIndex=items.indexOf(lastJumpItem),
itemIdentifierToAdd=items.slice(itemSliceIndex+1).shift(),itemToAdd=simplifiedTestMap.filter(function(row){return row.item===itemIdentifierToAdd}).shift();return itemToAdd?self.addJump(itemToAdd.part,itemToAdd.section,itemToAdd.item).then(resolve):resolve()})},jumpToNextItem:function(params){var self=this;return function(params){params.itemResponse&&_.forEach(params.itemResponse,function(response,itemResponseIdentifier){var responseIdentifier=params.itemDefinition+"."+itemResponseIdentifier;_.forEach(response,function(responseEntry){var responseId=responseEntry.identifier;Array.isArray(responseId)?responseId.forEach(function(id){responseStore.addResponse(
responseIdentifier,id)}):responseStore.addResponse(responseIdentifier,responseId)})})}(params),new Promise(function(resolve){var simplifiedTestMap=getSimplifiedTestMap(testMap),lastJumpItem=self.getLastJump().item||null,items=getItems(testMap),itemSliceIndex=items.indexOf(lastJumpItem),itemIdentifierToAdd=items.slice(itemSliceIndex+1).shift(),itemToAdd=simplifiedTestMap.filter(function(row){return row.item===itemIdentifierToAdd}).shift(),lastJumpItemData=simplifiedTestMap.filter(function(row){return row.item===lastJumpItem}).shift();return lastJumpItemData&&lastJumpItemData.itemHasBranchRule?itemStore.get(lastJumpItem).then(function(item){branchRule(
lastJumpItemData.itemBranchRule,item,params,responseStore).then(function(itemIdentifierToAddd){null!==itemIdentifierToAddd&&(itemToAdd=simplifiedTestMap.filter(function(row){return row.item===itemIdentifierToAddd}).shift()),self.addJump(itemToAdd.part,itemToAdd.section,itemToAdd.item).then(resolve)}).catch(function(err){return Promise.reject(err)})}).catch(function(err){return Promise.reject(err)}):itemToAdd?self.addJump(itemToAdd.part,itemToAdd.section,itemToAdd.item).then(resolve):resolve()})},jumpToNextSection:function(params){var self=this;return new Promise(function(resolve){var simplifiedTestMap=getSimplifiedTestMap(testMap),
lastJumpSection=self.getLastJump().section||null,sections=function(map){return _.uniq(_.map(getSimplifiedTestMap(map),function(row){return row.section}))}(testMap),sectionSliceIndex=sections.indexOf(lastJumpSection),sectionIdentifierToAdd=sections.slice(sectionSliceIndex+1).shift(),itemToAdd=simplifiedTestMap.filter(function(row){return row.section===sectionIdentifierToAdd}).shift();return itemToAdd?self.addJump(itemToAdd.part,itemToAdd.section,itemToAdd.item).then(resolve):resolve()})},jumpToPreviousItem:function(params){return new Promise(function(resolve){jumpTable.pop(),resolve()})},getJumpTable:function(){return jumpTable},getLastJump:function(){
return 0<jumpTable.length?jumpTable[jumpTable.length-1]:{}}}}}),define("taoQtiTest/runner/navigator/offlineNavigator",["lodash","core/promise","util/capitalize","taoQtiTest/runner/services/offlineJumpTable","taoQtiTest/runner/helpers/testContextBuilder"],function(_,Promise,capitalize,offlineJumpTableFactory,testContextBuilder){"use strict";return function(itemStore,responseStore){var testData,testContext,testMap,offlineJumpTableHelper=offlineJumpTableFactory(itemStore,responseStore);return{setTestData:function(data){return testData=data,this},setTestContext:function(context){return testContext=context,this},setTestMap:function(map){return testMap=map,this},
init:function(){return offlineJumpTableHelper.setTestMap(testMap),offlineJumpTableHelper.init(),this},clearJumpTable:function(){return offlineJumpTableHelper.clearJumpTable(),this},navigate:function(direction,scope,position,params){return new Promise(function(resolve,reject){var lastJump,navigationActionName="jumpTo"+capitalize(direction)+capitalize(scope);if(void 0===offlineJumpTableHelper[navigationActionName]||"function"!=typeof offlineJumpTableHelper[navigationActionName])throw new Error("Invalid navigation action");offlineJumpTableHelper[navigationActionName](params).then(function(){lastJump=offlineJumpTableHelper.getLastJump(),resolve(
testContextBuilder.buildTestContextFromJump(testData,testContext,testMap,lastJump))}).catch(function(err){reject(err)})})}}}}),define("taoQtiTest/runner/proxy/cache/actionStore",["lodash","taoTests/runner/testStore"],function(_,testStore){"use strict";return function(id){var actionQueue=[],loadStore=function(){return testStore(id).getStore("actions")};if(_.isEmpty(id))throw new TypeError("Please specify the action store id");return{push:function(action,params){return loadStore().then(function(actionStore){return actionQueue.push({action:action,timestamp:Date.now(),parameters:params}),actionStore.setItem("queue",actionQueue)})},flush:function(){
return loadStore().then(function(actionStore){return actionStore.getItem("queue").then(function(queue){return actionQueue=[],actionStore.setItem("queue",actionQueue).then(function(){return queue})})})},update:function(params){return _.forEach(actionQueue,function(action){action.parameters.actionId===params.actionId&&(action.parameters=params)}),loadStore().then(function(actionStore){return actionStore.setItem("queue",actionQueue)})}}}}),define("taoQtiTest/runner/proxy/cache/itemPreloader",["lodash","core/promise","core/logger","taoQtiItem/runner/qtiItemRunner","taoQtiTest/runner/config/assetManager","util/url"],function(_,Promise,loggerFactory,qtiItemRunner,
getAssetManager,urlUtil){"use strict";var logger=loggerFactory("taoQtiTest/runner/proxy/cache/itemPreloader"),relSupport=function(feature){var fakeLink=document.createElement("link");try{if(fakeLink.relList&&_.isFunction(fakeLink.relList.supports))return fakeLink.relList.supports(feature)}catch(err){return!1}},supportPreload=relSupport("preload"),supportPrefetch=relSupport("prefetch"),isItemObjectValid=function(item){return _.isPlainObject(item)&&_.isString(item.baseUrl)&&_.isString(item.itemIdentifier)&&!_.isEmpty(item.itemIdentifier)&&_.isPlainObject(item.itemData)};return function(options){var preloadAssetManager=getAssetManager("item-preloader"),
images={},audioBlobs={},loaders={img:function(url,sourceUrl,itemIdentifier){images[itemIdentifier]=images[itemIdentifier]||{},"Image"in window&&!images[itemIdentifier][sourceUrl]&&(images[itemIdentifier][sourceUrl]=new Image,images[itemIdentifier][sourceUrl].src=url)},css:function(url){var link=document.createElement("link");supportPreload?(link.setAttribute("rel","preload"),link.setAttribute("as","style")):supportPrefetch?(link.setAttribute("rel","prefetch"),link.setAttribute("as","style")):(link.disabled=!0,link.setAttribute("rel","stylesheet"),link.setAttribute("type","text/css")),link.setAttribute("data-preload",!0),link.setAttribute("href",url),
...
itemDefinition:itemIdentifier},params),deferred)})}},qtiServiceProxy)}),define("taoQtiTest/runner/services/responseStore",["lodash","core/store"],function(_,store){"use strict";var defaultConfig={responseStoreName:"response",correctResponseStoreName:"correct-response"};return function(options){var config=_.defaults(options||{},defaultConfig),getResponseStore=function(){return store(config.responseStoreName,store.backends.memory)},getCorrectResponseStore=function(){return store(config.correctResponseStoreName,store.backends.memory)};return{getResponses:function(){return getResponseStore().then(function(storage){return storage.getItems()})},
getCorrectResponses:function(){return getCorrectResponseStore().then(function(storage){return storage.getItems()})},getResponse:function(key){return getResponseStore().then(function(storage){return storage.getItem(key)})},getCorrectResponse:function(key){return getCorrectResponseStore().then(function(storage){return storage.getItem(key).then(function(result){return new Promise(function(resolve){return resolve(void 0===result?[]:result)})})})},addResponse:function(key,value){return getResponseStore().then(function(storage){return storage.setItem(key,value).then(function(updated){return updated})})},addCorrectResponse:function(key,value){
return getCorrectResponseStore().then(function(storage){return storage.setItem(key,value).then(function(updated){return updated})})},clearResponses:function(){return getResponseStore().then(function(storage){return storage.clear()})},clearCorrectResponses:function(){return getCorrectResponseStore().then(function(storage){return storage.clear()})}}}}),define("taoQtiTest/runner/proxy/offline/proxy",["lodash","i18n","core/promise","taoQtiTest/runner/navigator/offlineNavigator","taoQtiTest/runner/helpers/map","taoQtiTest/runner/helpers/navigation","taoQtiTest/runner/provider/dataUpdater","taoQtiTest/runner/proxy/qtiServiceProxy",
"taoQtiTest/runner/proxy/cache/itemStore","taoQtiTest/runner/proxy/cache/actionStore","taoQtiTest/runner/helpers/offlineErrorHelper","taoQtiTest/runner/helpers/offlineSyncModal","taoQtiTest/runner/services/responseStore","util/download"],function(_,__,Promise,offlineNavigatorFactory,mapHelper,navigationHelper,dataUpdater,qtiServiceProxy,itemStoreFactory,actionStoreFactory,offlineErrorHelper,offlineSyncModal,responseStoreFactory,download){"use strict";return _.defaults({install:function(config){var self=this;qtiServiceProxy.install.call(this),this.itemStore=itemStoreFactory({preload:!0,testId:config.serviceCallId}),this.responseStore=responseStoreFactory(),
this.offlineNavigator=offlineNavigatorFactory(this.itemStore,this.responseStore),this.actionStore=null,this.requestConfig={},this.actionPromises={},this.dataUpdater=dataUpdater(this.getDataHolder()),this.hasItem=function(itemIdentifier){return itemIdentifier&&self.itemStore.has(itemIdentifier)},this.hasNextItem=function(itemIdentifier){var sibling=navigationHelper.getNextItem(this.getDataHolder().get("testMap"),itemIdentifier);return sibling&&self.hasItem(sibling.id)},this.hasPreviousItem=function(itemIdentifier){var sibling=navigationHelper.getPreviousItem(this.getDataHolder().get("testMap"),itemIdentifier);return sibling&&self.hasItem(sibling.id)},
this.offlineAction=function(action,actionParams){return new Promise(function(resolve,reject){var newTestContext,result={success:!0},dataHolder=self.getDataHolder(),testData=dataHolder.get("testData"),testContext=dataHolder.get("testContext"),testMap=dataHolder.get("testMap");if("pause"===action)throw offlineErrorHelper.buildErrorFromContext(offlineErrorHelper.getOfflinePauseError(),{reason:actionParams.reason});if(_.contains(["exitTest","timeout"],action)||("next"===actionParams.direction||"skip"===action)&&navigationHelper.isLast(testMap,testContext.itemIdentifier))return result.testContext={state:testData.states.closed},self.isOffline()?new Promise(
function(){offlineSyncModal(self).on("proceed",function(){self.syncData().then(function(){return resolve(result)}).catch(function(){return resolve({success:!1})})}).on("secondaryaction",function(){self.initiateDownload().catch(function(){return resolve({success:!1})})})}).catch(function(){return resolve({success:!1})}):self.syncData().then(function(){return resolve(result)}).catch(function(){return resolve({success:!1})});"skip"===action&&(actionParams.direction=action),actionParams.direction&&actionParams.scope?self.offlineNavigator.setTestData(testData).setTestContext(testContext).setTestMap(testMap).init().navigate(actionParams.direction,
actionParams.scope,actionParams.ref,actionParams).then(function(res){if(!(newTestContext=res)||!newTestContext.itemIdentifier||!self.hasItem(newTestContext.itemIdentifier))throw offlineErrorHelper.buildErrorFromContext(offlineErrorHelper.getOfflineNavError());result.testContext=newTestContext,resolve(result)}).catch(function(err){reject(err)}):resolve(result)})},this.scheduleAction=function(action,actionParams){return actionParams=_.assign(actionParams,{actionId:action+"_"+(new Date).getTime(),offline:!0}),self.actionStore.push(action,self.prepareParams(_.defaults(actionParams||{},self.requestConfig))).then(function(){return{action:action,params:actionParams
}})},this.syncData=function(){var actions;return this.queue.serie(function(){return self.actionStore.flush().then(function(data){if((actions=data)&&data.length)return self.send("sync",data)}).catch(function(err){throw self.isConnectivityError(err)&&(self.setOffline("communicator"),_.forEach(actions,function(action){self.actionStore.push(action.action,action.parameters)})),err})})},this.prepareDownload=function(actions){var timestamp=Date.now(),dateTime=new Date(timestamp).toISOString(),testData=self.getDataHolder().get("testData");return{filename:"Download of "+testData.title+" at "+dateTime+".json",content:JSON.stringify({timestamp:timestamp,
testData:testData,actionQueue:actions})}},this.initiateDownload=function(){return this.queue.serie(function(){return self.actionStore.flush().then(function(actions){return _.forEach(actions,function(action){self.actionStore.push(action.action,action.parameters)}),actions}).then(self.prepareDownload).then(function(data){return download(data.filename,data.content)})})}},init:function(config,params){var self=this,InitCallPromise=qtiServiceProxy.init.call(this,config,params);if(!this.getDataHolder())throw new Error("Unable to retrieve test runners data holder");return this.requestConfig=_.pick(config,["testDefinition","testCompilation","serviceCallId"]),
this.actionStore=actionStoreFactory(config.serviceCallId),InitCallPromise.then(function(response){var promises=[];return response.items||(response.items={}),self.itemStore.setCacheSize(_.size(response.items)),_.forEach(response.items,function(item,itemIdentifier){promises.push(self.itemStore.set(itemIdentifier,item))}),Promise.all(promises).then(function(){return new Promise(function(resolve){resolve(response)})})})},destroy:function(){var self=this;return this.itemStore.clear().then(function(){return qtiServiceProxy.destroy.call(self)})},getItem:function(itemIdentifier){return this.itemStore.get(itemIdentifier)},submitItem:function(itemIdentifier,state,
response,params){var self=this;return this.itemStore.update(itemIdentifier,"itemState",state).then(function(){return qtiServiceProxy.submitItem.call(self,itemIdentifier,state,response,params)})},sendVariables:function(variables){var self=this,actionParams={traceData:JSON.stringify(variables)};return self.scheduleAction("storeTraceData",actionParams).then(function(){return self.offlineAction("storeTraceData",actionParams)}).catch(function(err){return Promise.reject(err)})},callTestAction:function(action,params){var self=this;return self.scheduleAction(action,params).then(function(){return self.offlineAction(action,params)}).catch(function(err){
return Promise.reject(err)})},callItemAction:function(itemIdentifier,action,params){var self=this,updateStatePromise=Promise.resolve();return params.itemState&&(updateStatePromise=this.itemStore.update(itemIdentifier,"itemState",params.itemState)),(navigationHelper.isMovingToNextItem(action,params)||navigationHelper.isMovingToPreviousItem(action,params)||navigationHelper.isJumpingToItem(action,params))&&(params.start=!0),updateStatePromise.then(function(){return params=_.assign({itemDefinition:itemIdentifier},params),self.scheduleAction(action,params).then(function(){return self.offlineAction(action,params)}).catch(function(err){return Promise.reject(err)})}
).catch(function(err){return Promise.reject(err)})}},qtiServiceProxy)}),function(c){var d=document,a="appendChild",i="styleSheet",s=d.createElement("style");s.type="text/css",d.getElementsByTagName("head")[0][a](s),s[i]?s[i].cssText=c:s[a](d.createTextNode(c))}(
