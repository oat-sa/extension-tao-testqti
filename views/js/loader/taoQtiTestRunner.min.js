...
exitReason=data.reason)}).after("destroy",function(){this.removeAllListeners(),exit(exitReason)}).on("disablefeedbackalerts",function(){errorFeedback&&errorFeedback.close(),preventFeedback=!0}).on("enablefeedbackalerts",function(){preventFeedback=!1}).init()}).catch(function(err){onError(err,__("An error occurred during the test initialization!"))})}}}),define("taoQtiTest/runner/config/qtiServiceConfig",["lodash","util/url","util/config"],function(_,urlUtil,configHelper){"use strict";var _defaults={bootstrap:{serviceController:"Runner",serviceExtension:"taoQtiTest"}},_entries={testDefinition:!0,testCompilation:!0,serviceCallId:!0,bootstrap:!1,timeout:!1}
;return function(config){var storage=configHelper.from(config,_entries,_defaults);return storage.timeout?storage.timeout*=1e3:storage.timeout=void 0,{getTestDefinition:function(){return storage.testDefinition},getTestCompilation:function(){return storage.testCompilation},getServiceCallId:function(){return storage.serviceCallId},getServiceController:function(){return storage.bootstrap.serviceController||_defaults.bootstrap.serviceController},getServiceExtension:function(){return storage.bootstrap.serviceExtension||_defaults.bootstrap.serviceExtension},getTestActionUrl:function(action){return urlUtil.route(action,this.getServiceController(),
this.getServiceExtension(),{testDefinition:this.getTestDefinition(),testCompilation:this.getTestCompilation(),serviceCallId:this.getServiceCallId()})},getItemActionUrl:function(itemIdentifier,action){return urlUtil.route(action,this.getServiceController(),this.getServiceExtension(),{testDefinition:this.getTestDefinition(),testCompilation:this.getTestCompilation(),testServiceCallId:this.getServiceCallId(),itemDefinition:itemIdentifier})},getTelemetryUrl:function(itemIdentifier,signal){return urlUtil.route(signal,this.getServiceController(),this.getServiceExtension(),{testDefinition:this.getTestDefinition(),testCompilation:this.getTestCompilation(),
testServiceCallId:this.getServiceCallId(),itemDefinition:itemIdentifier})},getTimeout:function(){return storage.timeout},getCommunicationConfig:function(){var communication=storage.bootstrap.communication||{},extension=communication.extension||this.getServiceExtension(),controller=communication.controller||this.getServiceController(),action=communication.action,syncActions=communication.syncActions||[],service=urlUtil.route(action||"message",controller,extension,{testDefinition:this.getTestDefinition(),testCompilation:this.getTestCompilation(),serviceCallId:this.getServiceCallId()}),params=_.merge({},communication.params||{},{
service:communication.service||service});return params.timeout?params.timeout*=1e3:params.timeout=storage.timeout,params.interval&&(params.interval*=1e3),{enabled:communication.enabled,type:communication.type,params:params,syncActions:syncActions}}}}}),define("taoQtiTest/runner/helpers/stats",["lodash","taoQtiTest/runner/helpers/map","taoQtiTest/runner/helpers/currentItem"],function(_,mapHelper,currentItemHelper){"use strict";return{getInstantStats:function(scope,runner,sync){var isItemCurrentlyAnswered,map=runner.getTestMap(),context=runner.getTestContext(),stats=_.clone(mapHelper.getScopeStats(map,context.itemPosition,scope));return mapHelper.getItemAt(map
,context.itemPosition).informational||(!(isItemCurrentlyAnswered=currentItemHelper.isAnswered(runner))&&context.itemAnswered?stats.answered--:!isItemCurrentlyAnswered&&!sync||context.itemAnswered?sync&&!isItemCurrentlyAnswered&&context.itemAnswered&&context.isLinear&&stats.answered++:stats.answered++),stats}}}),define("taoQtiTest/runner/helpers/messages",["lodash","i18n","taoQtiTest/runner/helpers/map","taoQtiTest/runner/helpers/stats"],function(_,__,mapHelper,statsHelper){"use strict";return{getExitMessage:function(message,scope,runner,sync){var itemsCountMessage="",testData=runner.getTestData(),testConfig=testData&&testData.config;return(
!testConfig||testConfig.enableUnansweredItemsWarning)&&(itemsCountMessage=function(scope,runner,sync){var stats=statsHelper.getInstantStats(scope,runner,sync),unansweredCount=stats&&stats.questions-stats.answered,flaggedCount=stats&&stats.flagged,itemsCountMessage="";return"section"===scope||"testSection"===scope?(itemsCountMessage=0===unansweredCount?__("You answered all %s question(s) in this section",stats.questions.toString()):__("You answered only %s of the %s question(s) in this section",stats.answered.toString(),stats.questions.toString()),flaggedCount&&(itemsCountMessage+=", "+__("and flagged %s of them",flaggedCount.toString()))):"test"===scope?(
...
itemsCountMessage}(scope,runner,sync)),(itemsCountMessage+" "+message).trim()}}}),define("taoQtiTest/runner/helpers/offlineErrorHelper",["i18n"],function(__){"use strict";return{buildErrorFromContext:function(errorData,context){var err=_.assign(new Error(errorData.message),errorData.data);return _.assign(err,context||{})},getOfflineNavError:function(){return{message:__("We are unable to connect to the server to retrieve the next item."),data:{success:!1,source:"navigator",purpose:"proxy",type:"nav",code:404}}},getOfflineExitError:function(){return{message:__("We are unable to connect the server to submit your results."),data:{success:!1,source:"navigator",
purpose:"proxy",type:"finish",code:404}}},getOfflinePauseError:function(){return{message:__("The test has been paused, we are unable to connect to the server."),data:{success:!1,source:"navigator",purpose:"proxy",type:"pause",code:404}}}}}),define("tpl!taoQtiTest/runner/helpers/templates/offlineSyncModalCountdown",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){return this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{},'<p class="button-subtext"></p>'})}),define("taoQtiTest/runner/helpers/offlineSyncModal",["jquery","lodash","i18n","core/polling","ui/hider",
"ui/waitingDialog/waitingDialog","tpl!taoQtiTest/runner/helpers/templates/offlineSyncModalCountdown"],function($,_,__,polling,hider,waitingDialogFactory,offlineSyncModalCountdownTpl){"use strict";return function(proxy){var $secondaryButton,countdownPolling,waitingConfig={message:__("You are encountering a prolonged connectivity loss."),waitContent:__("Please continue waiting while we try to restore the connection. Alternatively, you may end this test by downloading it as a file which you will have to submit manually."),proceedContent:__("The connection seems to be back, please proceed."),proceedButtonText:__("Proceed & End Test"),showSecondary:!0,
secondaryButtonText:__("Download"),secondaryButtonIcon:"download",buttonSeparatorText:__("or"),width:"600px"},$countdown=$(offlineSyncModalCountdownTpl()),waitingDialog=waitingDialogFactory(waitingConfig).on("render",function(){$secondaryButton=$("div.preview-modal-feedback.modal").find('button[data-control="secondary"]'),$countdown.insertAfter($secondaryButton),proxy.off("reconnect.waiting").after("reconnect.waiting",function(){waitingDialog.endWait()}),waitingDialog.is("waiting")&&waitingDialog.trigger("begincountdown")}).on("wait",function(){waitingDialog.is("rendered")&&waitingDialog.trigger("begincountdown")}).on("begincountdown",function(){
var delaySec=60;$secondaryButton.prop("disabled",!0),countdownPolling=polling({action:function(){delaySec--,$countdown.html(__("The download will be available in <strong>%d</strong> seconds",delaySec)),delaySec<1&&(this.stop(),$secondaryButton.removeProp("disabled"),$countdown.html(""))},interval:1e3,autoStart:!0})}).on("unwait",function(){countdownPolling.stop(),$secondaryButton.prop("disabled",!0),(void 0).remove(),hider.hide(".between-buttons-text")});return waitingDialog}}),define("taoQtiTest/runner/helpers/testContextBuilder",["lodash","util/capitalize","taoQtiTest/runner/helpers/map","taoQtiTest/runner/helpers/navigation"],function(_,capitalize,
mapHelper,navigationHelper){"use strict";function getTestContext(testData,testContext,testMap,item,section,part,position){var isLeavingSection=section.id!==testContext.sectionId,isLeavingPart=part.id!==testContext.testPartId,newTestContext=_.defaults({itemIdentifier:item.id,itemPosition:position,itemAnswered:item.answered||part.isLinear,numberPresented:testMap.stats.viewed,numberCompleted:testMap.stats.answered,hasFeedbacks:!1,remainingAttempts:Math.max(-1,item.remainingAttempts-1),sectionId:section.id,sectionTitle:section.label,testPartId:part.id,isLinear:part.isLinear,isLast:navigationHelper.isLast(testMap,item.id),
canMoveBackward:!part.isLinear&&!navigationHelper.isFirst(testMap,item.id)},testContext);return isLeavingSection&&(newTestContext.numberRubrics=0,newTestContext.rubrics=""),newTestContext.timeConstraints=_.reject(testContext.timeConstraints,function(constraint){return"assessmentItemRef"===constraint.qtiClassName||isLeavingSection&&"assessmentSection"===constraint.qtiClassName||isLeavingPart&&"testPart"===constraint.qtiClassName}),item.timeConstraint&&newTestContext.timeConstraints.push(item.timeConstraint),isLeavingSection&&section.timeConstraint&&newTestContext.timeConstraints.push(section.timeConstraint),
isLeavingPart&&part.timeConstraint&&newTestContext.timeConstraints.push(part.timeConstraint),newTestContext.options=_.defaults(function(categories){return _.isArray(categories)&&categories.length?_.reduce(categories,function(acc,category){var categoryName;return _.isString(category)&&!_.isEmpty(category)&&(categoryName=category.replace("x-tao-option-","").split(/[-_]+/g).map(function(name,index){return 0===index?name:name.length?capitalize(name):""}).join(""))&&(acc[categoryName]=!0),acc},{}):{}}(item.categories||[]),_.pick(testContext.options,["allowComment","allowSkipping","exitButton","logoutButton"])),newTestContext}return{
buildTestContextFromPosition:function(testData,testContext,testMap,position){var updatedMap=mapHelper.updateItemStats(testMap,position),item=mapHelper.getItemAt(updatedMap,position),section=mapHelper.getItemSection(updatedMap,position),part=mapHelper.getItemPart(updatedMap,position);return!!item&&getTestContext(0,testContext,testMap,item,section,part,position)},buildTestContextFromJump:function(testData,testContext,testMap,jump){var part=testMap.parts[jump.part],section=part.sections[jump.section];return getTestContext(0,testContext,testMap,section.items[jump.item],section,part,jump.position)}}}),define("taoQtiTest/runner/navigator/navigator",["lodash",
...
subBranchRuleResult){Array.isArray(subBranchRuleResult)?subBranchRuleResult.forEach(function(value){subBranchRuleResults.push(value)}):subBranchRuleResults.push(subBranchRuleResult)}),resolve(subBranchRuleResults)}).catch(function(err){reject(err)})})}}}),define("taoQtiTest/runner/branchRule/types/match",["lodash","core/promise"],function(_,Promise){"use strict";return function(branchRuleDefinition,item,navigationParams,branchRuleMapper,responseStore){var variableIdentifier=branchRuleDefinition.variable["@attributes"].identifier,correctIdentifier=branchRuleDefinition.correct["@attributes"].identifier;return{validate:function(){return Promise.all([
responseStore.getCorrectResponse(correctIdentifier),responseStore.getResponse(variableIdentifier)]).then(function(result){return _.contains(result[0],result[1])})}}}}),define("taoQtiTest/runner/branchRule/types/or",["core/promise","taoQtiTest/runner/branchRule/helpers/branchRuleHelper"],function(Promise,branchRuleHelper){"use strict";return function(branchRuleDefinition,item,navigationParams,branchRuleMapper,responseStore){return{validate:function(){return branchRuleHelper.evaluateSubBranchRules(branchRuleDefinition,item,navigationParams,branchRuleMapper,responseStore).then(function(subBranchRuleResults){return subBranchRuleResults.some(function(value){
return value})})}}}}),define("taoQtiTest/runner/branchRule/types/and",["core/promise","taoQtiTest/runner/branchRule/helpers/branchRuleHelper"],function(Promise,branchRuleHelper){"use strict";return function(branchRuleDefinition,item,navigationParams,branchRuleMapper,responseStore){return{validate:function(){return branchRuleHelper.evaluateSubBranchRules(branchRuleDefinition,item,navigationParams,branchRuleMapper,responseStore).then(function(subBranchRuleResults){return subBranchRuleResults.every(function(value){return value})})}}}}),define("taoQtiTest/runner/branchRule/types/not",["lodash","core/promise"],function(_,Promise){"use strict";return function(
branchRuleDefinition,item,navigationParams,branchRuleMapper,responseStore){return Array.isArray(branchRuleDefinition)||(branchRuleDefinition=[branchRuleDefinition]),{validate:function(){var promises=branchRuleDefinition.map(function(expression){var subBranchRuleName=_.head(_.keys(expression)),subBranchRuleDefinition=expression[subBranchRuleName];return branchRuleMapper(subBranchRuleName,subBranchRuleDefinition,item,navigationParams,responseStore).validate()});return Promise.all(promises).then(function(results){return _.map(results,function(result){return!result})})}}}}),define("taoQtiTest/runner/branchRule/branchRuleMapper",[
"taoQtiTest/runner/branchRule/types/match","taoQtiTest/runner/branchRule/types/or","taoQtiTest/runner/branchRule/types/and","taoQtiTest/runner/branchRule/types/not"],function(matchBranchRule,orBranchRule,andBranchRule,notBranchRule){"use strict";var branchRuleMap={match:matchBranchRule,or:orBranchRule,and:andBranchRule,not:notBranchRule};return function branchRuleMapperFactory(branchRuleName,branchRuleDefinition,item,navigationParams,responseStore){if(!(branchRuleName in branchRuleMap))throw new Error("Invalid branch rule name: "+branchRuleName);return branchRuleMap[branchRuleName](branchRuleDefinition,item,navigationParams,branchRuleMapperFactory,
responseStore)}}),define("taoQtiTest/runner/branchRule/branchRule",["lodash","core/promise","taoQtiTest/runner/branchRule/helpers/branchRuleHelper","taoQtiTest/runner/branchRule/branchRuleMapper"],function(_,Promise,branchRuleHelper,branchRuleMapper){"use strict";return function(branchRuleDefinition,item,navigationParams,responseStore){return new Promise(function(resolve){if(void 0===branchRuleDefinition["@attributes"]||void 0===branchRuleDefinition["@attributes"].target)return resolve(null);branchRuleHelper.evaluateSubBranchRules(branchRuleDefinition,item,navigationParams,branchRuleMapper,responseStore).then(function(branchRuleResults){if(
branchRuleResults.every(function(branchRuleResult){return branchRuleResult}))return resolve(branchRuleDefinition["@attributes"].target);resolve(null)})})}}),define("taoQtiTest/runner/services/offlineJumpTable",["lodash","i18n","core/promise","taoQtiTest/runner/branchRule/branchRule","taoQtiTest/runner/helpers/map"],function(_,__,Promise,branchRule,mapHelper){"use strict";return function(itemStore,responseStore){var testMap={},jumpTable=[];function getItems(map){return _.uniq(_.map(getSimplifiedTestMap(map),function(row){return row.item}))}function getSimplifiedTestMap(map){var simplifiedTestMap=[];return mapHelper.each(map,function(item,section,part){
simplifiedTestMap.push({item:item.id,itemHasBranchRule:!_.isEmpty(item.branchRule),itemBranchRule:item.branchRule,section:section.id,sectionHasBranchRule:!_.isEmpty(section.branchRule),sectionBranchRule:section.branchRule,part:part.id,partHasBranchRule:!_.isEmpty(part.branchRule),partBranchRule:part.branchRule})}),simplifiedTestMap}return{setTestMap:function(map){return testMap=map,this},init:function(){var firstItem,simplifiedTestMap=getSimplifiedTestMap(testMap);0<simplifiedTestMap.length&&0===jumpTable.length&&(firstItem=simplifiedTestMap[0],this.addJump(firstItem.part,firstItem.section,firstItem.item)),simplifiedTestMap.forEach(function(row){
itemStore.get(row.item).then(function(item){item&&_.forEach(item.itemData.data.responses,function(response){var responseIdentifier=item.itemIdentifier+"."+response.identifier;responseStore.addCorrectResponse(responseIdentifier,response.correctResponses)})}).catch(function(err){return Promise.reject(err)})})},clearJumpTable:function(){return jumpTable=[],this},addJump:function(partIdentifier,sectionIdentifier,itemIdentifier){var self=this;return new Promise(function(resolve){var lastJump=self.getLastJump(),nextPosition=void 0!==lastJump.position?lastJump.position+1:0;jumpTable.push({item:itemIdentifier,part:partIdentifier,section:sectionIdentifier,
position:nextPosition}),resolve()})},jumpTo:function(position){return jumpTable=jumpTable.filter(function(jump){return jump.position<=position}),this},jumpToSkipItem:function(){var self=this;return new Promise(function(resolve){var simplifiedTestMap=getSimplifiedTestMap(testMap),lastJumpItem=self.getLastJump().item||null,items=getItems(testMap),itemSliceIndex=items.indexOf(lastJumpItem),itemIdentifierToAdd=items.slice(itemSliceIndex+1).shift(),itemToAdd=simplifiedTestMap.filter(function(row){return row.item===itemIdentifierToAdd}).shift();return itemToAdd?self.addJump(itemToAdd.part,itemToAdd.section,itemToAdd.item).then(resolve):resolve()})},
jumpToNextItem:function(params){var self=this;return function(params){params.itemResponse&&_.forEach(params.itemResponse,function(response,itemResponseIdentifier){var responseIdentifier=params.itemDefinition+"."+itemResponseIdentifier;_.forEach(response,function(responseEntry){var responseId=responseEntry.identifier;Array.isArray(responseId)?responseId.forEach(function(id){responseStore.addResponse(responseIdentifier,id)}):responseStore.addResponse(responseIdentifier,responseId)})})}(params),new Promise(function(resolve){var simplifiedTestMap=getSimplifiedTestMap(testMap),lastJumpItem=self.getLastJump().item||null,items=getItems(testMap),
itemSliceIndex=items.indexOf(lastJumpItem),itemIdentifierToAdd=items.slice(itemSliceIndex+1).shift(),itemToAdd=simplifiedTestMap.filter(function(row){return row.item===itemIdentifierToAdd}).shift(),lastJumpItemData=simplifiedTestMap.filter(function(row){return row.item===lastJumpItem}).shift();return lastJumpItemData&&lastJumpItemData.itemHasBranchRule?itemStore.get(lastJumpItem).then(function(item){branchRule(lastJumpItemData.itemBranchRule,item,params,responseStore).then(function(itemIdentifierToAddd){null!==itemIdentifierToAddd&&(itemToAdd=simplifiedTestMap.filter(function(row){return row.item===itemIdentifierToAddd}).shift()),self.addJump(itemToAdd.part,
itemToAdd.section,itemToAdd.item).then(resolve)}).catch(function(err){return Promise.reject(err)})}).catch(function(err){return Promise.reject(err)}):itemToAdd?self.addJump(itemToAdd.part,itemToAdd.section,itemToAdd.item).then(resolve):resolve()})},jumpToNextSection:function(params){var self=this;return new Promise(function(resolve){var simplifiedTestMap=getSimplifiedTestMap(testMap),lastJumpSection=self.getLastJump().section||null,sections=function(map){return _.uniq(_.map(getSimplifiedTestMap(map),function(row){return row.section}))}(testMap),sectionSliceIndex=sections.indexOf(lastJumpSection),sectionIdentifierToAdd=sections.slice(sectionSliceIndex+1
).shift(),itemToAdd=simplifiedTestMap.filter(function(row){return row.section===sectionIdentifierToAdd}).shift();return itemToAdd?self.addJump(itemToAdd.part,itemToAdd.section,itemToAdd.item).then(resolve):resolve()})},jumpToPreviousItem:function(params){return new Promise(function(resolve){jumpTable.pop(),resolve()})},getJumpTable:function(){return jumpTable},getLastJump:function(){return 0<jumpTable.length?jumpTable[jumpTable.length-1]:{}}}}}),define("taoQtiTest/runner/navigator/offlineNavigator",["lodash","core/promise","util/capitalize","taoQtiTest/runner/services/offlineJumpTable","taoQtiTest/runner/helpers/testContextBuilder"],function(_,Promise,
capitalize,offlineJumpTableFactory,testContextBuilder){"use strict";return function(itemStore,responseStore){var testData,testContext,testMap,offlineJumpTableHelper=offlineJumpTableFactory(itemStore,responseStore);return{setTestData:function(data){return testData=data,this},setTestContext:function(context){return testContext=context,this},setTestMap:function(map){return testMap=map,this},init:function(){return offlineJumpTableHelper.setTestMap(testMap),offlineJumpTableHelper.init(),this},clearJumpTable:function(){return offlineJumpTableHelper.clearJumpTable(),this},navigate:function(direction,scope,position,params){return new Promise(function(resolve,reject
){var lastJump,navigationActionName="jumpTo"+capitalize(direction)+capitalize(scope);if(void 0===offlineJumpTableHelper[navigationActionName]||"function"!=typeof offlineJumpTableHelper[navigationActionName])throw new Error("Invalid navigation action");offlineJumpTableHelper[navigationActionName](params).then(function(){lastJump=offlineJumpTableHelper.getLastJump(),resolve(testContextBuilder.buildTestContextFromJump(testData,testContext,testMap,lastJump))}).catch(function(err){reject(err)})})}}}}),define("taoQtiTest/runner/proxy/cache/actionStore",["lodash","taoTests/runner/testStore"],function(_,testStore){"use strict";return function(id){var actionQueue=[],
loadStore=function(){return testStore(id).getStore("actions")};if(_.isEmpty(id))throw new TypeError("Please specify the action store id");return{push:function(action,params){return loadStore().then(function(actionStore){return actionQueue.push({action:action,timestamp:Date.now(),parameters:params}),actionStore.setItem("queue",actionQueue)})},flush:function(){return loadStore().then(function(actionStore){return actionStore.getItem("queue").then(function(queue){return actionQueue=[],actionStore.setItem("queue",actionQueue).then(function(){return queue})})})},update:function(params){return _.forEach(actionQueue,function(action){
