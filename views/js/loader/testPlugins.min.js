...
if(testPart&&testPart.sections)return testPart.sections[testContext.sectionId]},progressScopeCounter={test:{total:function(){return Math.max(1,testContext.numberItems)},position:function(){return testContext.itemPosition+1}},testPart:{total:function(){var testPart=getTestPart();return testPart?_.reduce(testMap.parts[testContext.testPartId].sections,function(acc,section){return acc+_.size(section.items)},0):0},position:function(){var testSection=getTestSection();return testSection?testSection.items[testContext.itemIdentifier].positionInPart+1:0}},testSection:{total:function(){var testSection=getTestSection();return testSection?_.size(testSection.items):0},position:function(){
var testSection=getTestSection();return testSection&&testSection.items[testContext.itemIdentifier]?testSection.items[testContext.itemIdentifier].positionInSection+1:0}}},counter=progressScopeCounter[progressConfig.scope]||progressScopeCounter.test,total=counter.total(),currentPosition=counter.position();return{ratio:total>0?Math.floor(currentPosition/total*100):0,label:progressConfig.showTotal?__("Item %d of %d",currentPosition,total):__("Item %d",currentPosition)}}};return updater[progressConfig.indicator]()};return pluginFactory({name:"progressBar",init:function(){var $progressLabel,$progressControl,testRunner=this.getTestRunner(),testData=testRunner.getTestData(),config=testData.config.progressIndicator||{},progressConfig={
indicator:config.type||"percentage",scope:config.scope||"test",showTotal:!!config.showTotal},update=function(){var progressData=progressUpdater(testRunner.getTestContext(),testRunner.getTestMap(),progressConfig);progressData&&$progressLabel&&$progressControl&&($progressLabel.text(progressData.label),$progressControl.progressbar("value",progressData.ratio))};this.$element=$(progressTpl()),$progressLabel=$('[data-control="progress-label"]',this.$element),$progressControl=$('[data-control="progress-bar"]',this.$element),$progressControl.progressbar(),update(),testRunner.on("ready",update).on("loaditem",update)},render:function(){var $container=this.getAreaBroker().getControlArea();
$container.append(this.$element)}})}),define("taoQtiTest/runner/plugins/controls/testState/testState",["lodash","taoTests/runner/plugin"],function(_,pluginFactory){"use strict";return pluginFactory({name:"testState",install:function(){var testRunner=this.getTestRunner();testRunner.getProxy().use(function(req,res,next){var data=res&&res.data;return data&&data.type&&"TestState"===data.type&&!testRunner.getState("closedOrSuspended")?(testRunner.setState("closedOrSuspended",!0),void(testRunner.getState("ready")?!_.isEmpty(data.messages)&&_.find(data.messages,{channel:"teststate"})||testRunner.trigger("leave",data):testRunner.trigger("destroy"))):void next()})},init:function(){
var testRunner=this.getTestRunner(),isLeaving=!1;testRunner.getProxy().channel("teststate",function(data){isLeaving||!data||"close"!==data.type&&"pause"!==data.type||testRunner.getState("closedOrSuspended")||(isLeaving=!0,testRunner.setState("closedOrSuspended",!0),testRunner.trigger("leave",data))})}})}),define("tpl!taoQtiTest/runner/plugins/controls/timer/timer",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){function program1(depth0,data){return" disabled"}this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,buffer="",functionType="function",escapeExpression=this.escapeExpression,self=this;
return buffer+='<span class="qti-timer qti-timer__type-',(helper=helpers.type)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.type,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+" qti-controls ",stack1=helpers.unless.call(depth0,depth0&&depth0.running,{hash:{},inverse:self.noop,fn:self.program(1,program1,data),data:data}),(stack1||0===stack1)&&(buffer+=stack1),buffer+='" data-control="',(helper=helpers.id)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.id,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+'" title="',
(helper=helpers.label)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.label,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+'">\n    <span class="qti-timer_label truncate">',(helper=helpers.label)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.label,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+'</span>\n    <span class="qti-timer_time"></span>\n</span>\n'})}),define("taoQtiTest/runner/plugins/controls/timer/timerComponent",["jquery","lodash","i18n","moment","core/encoder/time","tpl!taoQtiTest/runner/plugins/controls/timer/timer"],function($,_,__,moment,timeEncoder,timerTpl){
"use strict";var hasAnimationFrame="requestAnimationFrame"in window,precision=1e3,timerTypes={test:"assessmentTest",testPart:"testPart",section:"assessmentSection",item:"assessmentItemRef"},timerComponentFactory=function(config){var data;if(!_.isPlainObject(config))throw new TypeError("A timer needs to be configured using a config object");if(_.isEmpty(config.id))throw new TypeError("A timer needs and identifier");if(!_.contains(timerTypes,config.type))throw new TypeError("Invalid type configured");return data=_.defaults(config,{label:"",remaining:0,warnings:{},running:!0}),{init:function(){return this.$element=$(timerTpl(data)),this.$display=$(".qti-timer_time",this.$element),
this},render:function($container){return $container.append(this.$element),this},destroy:function(){return this.$element.remove(),this},refresh:function(){var self=this,update=function(){self.$display.text(timeEncoder.encode(data.remaining/precision))};return hasAnimationFrame?window.requestAnimationFrame(update):update(),this},warn:function(){var remaining,message={},warning=_.findLast(data.warnings,{showed:!1});if(!_.isEmpty(warning)&&_.isFinite(warning.point)&&data.remaining<=warning.point){switch(remaining=moment.duration(data.remaining/precision,"seconds").humanize(),this.$display.removeClass("txt-info txt-warning txt-danger").addClass("txt-"+warning.type),
data.type){case"assessmentItemRef":message.text=__("Warning – You have %s remaining to complete this item.",remaining);break;case"assessmentSection":message.text=__("Warning – You have %s remaining to complete this section.",remaining);break;case"testPart":message.text=__("Warning – You have %s remaining to complete this test part.",remaining);break;case"assessmentTest":message.text=__("Warning – You have %s remaining to complete the test.",remaining)}var showed=_.find(data.warnings,{point:warning.point});showed&&showed.point&&data.warnings[showed.point/precision]&&(data.warnings[showed.point/precision].showed=!0),message.type=warning.type}return message},id:function(){
return data.id},val:function(value){return"undefined"!=typeof value?(data.remaining=value,this):data.remaining},running:function(value){return"undefined"!=typeof value?(data.running=!!value,this):!!data.running}}};return timerComponentFactory}),define("taoQtiTest/runner/helpers/map",["lodash"],function(_){"use strict";function getEmptyStats(){return{questions:0,answered:0,flagged:0,viewed:0,total:0}}return{getJumps:function(map){return map&&map.jumps},getParts:function(map){return map&&map.parts},getSections:function(map){var parts=this.getParts(map),result={};return _.forEach(parts,function(part){var sections=part.sections;sections&&_.forEach(sections,function(section){
result[section.id]=section})}),result},getNextSections:function(map,sectionId){var sections=this.getSections(map),result={},canList=!1;return _.forEach(sections,function(section){canList&&(result[section.id]=section),section.id===sectionId&&(canList=!0)}),result},getJump:function(map,position){var jumps=this.getJumps(map);return jumps&&jumps[position]},getPart:function(map,partName){var parts=this.getParts(map);return parts&&parts[partName]},getSection:function(map,sectionName){var parts=this.getParts(map),section=null;return _.forEach(parts,function(part){var sections=part.sections;if(sections&&sections[sectionName])return section=sections[sectionName],!1}),
section},getItem:function(map,itemName){var jump=_.find(this.getJumps(map),{identifier:itemName});return this.getItemAt(map,jump&&jump.position)},getTestStats:function(map){return map&&map.stats},getPartStats:function(map,partName){var part=this.getPart(map,partName);return part&&part.stats},getSectionStats:function(map,sectionName){var section=this.getSection(map,sectionName);return section&&section.stats},getScopeStats:function(map,position,scope){var jump=this.getJump(map,position);switch(scope){case"section":case"testSection":return this.getSectionStats(map,jump&&jump.section);case"part":case"testPart":return this.getPartStats(map,jump&&jump.part);default:
...
"use strict";function getInstantStats(scope,runner){var isItemCurrentlyAnswered,map=runner.getTestMap(),context=runner.getTestContext(),stats=_.clone(mapHelper.getScopeStats(map,context.itemPosition,scope)),item=mapHelper.getItemAt(map,context.itemPosition);return item.informational||(isItemCurrentlyAnswered=currentItemHelper.isAnswered(runner),!isItemCurrentlyAnswered&&context.itemAnswered?stats.answered--:isItemCurrentlyAnswered&&!context.itemAnswered&&stats.answered++),stats}return{getInstantStats:getInstantStats}}),define("taoQtiTest/runner/helpers/messages",["lodash","i18n","taoQtiTest/runner/helpers/map","taoQtiTest/runner/helpers/stats"],function(_,__,mapHelper,statsHelper){
"use strict";function getExitMessage(message,scope,runner){var itemsCountMessage="",testData=runner.getTestData(),testConfig=testData&&testData.config,messageEnabled=!testConfig||testConfig.enableUnansweredItemsWarning;return messageEnabled&&(itemsCountMessage=getUnansweredItemsWarning(scope,runner)),(itemsCountMessage+" "+message).trim()}function getUnansweredItemsWarning(scope,runner){var stats=statsHelper.getInstantStats(scope,runner),unansweredCount=stats&&stats.questions-stats.answered,flaggedCount=stats&&stats.flagged,itemsCountMessage="";return"section"===scope||"testSection"===scope?(itemsCountMessage=0===unansweredCount?__("You answered all %s question(s) in this section",stats.questions.toString()):__("You answered only %s of the %s question(s) in this section",stats.answered.toString(),stats.questions.toString()),
...
return params=params||{},"move"===action&&"jump"===params.direction&&"item"===params.scope}};return navigationHelper}),define("tpl!taoQtiTest/runner/plugins/controls/timer/timers",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var helper,options,buffer="",helperMissing=helpers.helperMissing,escapeExpression=this.escapeExpression;return buffer+='<div class="timer-box">\n    <a href="#" class="timer-toggler hidden" title="'+escapeExpression((helper=helpers.__||depth0&&depth0.__,options={hash:{},data:data},helper?helper.call(depth0,"Hide timers",options):helperMissing.call(depth0,"__","Hide timers",options)))+'"><span class="icon-clock"></span></a>\n    <div class="timer-wrapper"></div>\n</div>\n';
})}),define("taoQtiTest/runner/plugins/controls/timer/timer",["jquery","lodash","i18n","core/promise","core/polling","core/timer","core/store","ui/hider","taoTests/runner/plugin","taoQtiTest/runner/plugins/controls/timer/timerComponent","taoQtiTest/runner/helpers/messages","taoQtiTest/runner/helpers/navigation","tpl!taoQtiTest/runner/plugins/controls/timer/timers"],function($,_,__,Promise,pollingFactory,timerFactory,store,hider,pluginFactory,timerComponentFactory,messages,navigationHelper,timerBoxTpl){"use strict";var timerRefresh=1e3,precision=1e3,exitMessage=__("Once you close this section, you cannot return to it or change your answers."),timerTypes={test:"assessmentTest",
...
_.contains(["info","warning","danger"],value)&&(timer.warnings[key]={type:value,showed:timer.remaining/precision<=key,point:parseInt(key,10)*precision})}),closestPreviousWarning=_.find(timer.warnings,{showed:!0}),!_.isEmpty(closestPreviousWarning)&&closestPreviousWarning.point&&timer.warnings[closestPreviousWarning.point/precision]&&(timer.warnings[closestPreviousWarning.point/precision].showed=!1))),timer},leaveTimedSection=function(direction,scope,position){var context=testRunner.getTestContext(),map=testRunner.getTestMap();return!(context.isTimeout||context.itemSessionState===itemStates.closed||!isTimedSection(context))&&navigationHelper.isLeavingSection(context,map,direction,scope,position);
},removeTimer=function(type){displayedTimers[type]&&(displayedTimers[type].destroy(),displayedTimers=_.omit(displayedTimers,type),timers=_.omit(timers,type),self.trigger("removetimer",type))},addTimer=function(type,config){config&&(timers[type]=config.regular,displayedTimers[type]=timerComponentFactory(config),displayedTimers[type].init().render(self.$element.find(".timer-wrapper")),self.trigger("addtimer",type,displayedTimers[type]))},updateTimers=function(checkStorage){var timerUpdatePromises=[],context=testRunner.getTestContext();return context.extraTime&&(extraTime=context.extraTime.total,consumedExtraTime=Math.max(consumedExtraTime,context.extraTime.consumed)),
_.forEach(timerTypes,function(type){timerUpdatePromises.push(new Promise(function(resolve){var timerConfig=getTimerConfig(type),timerExists="undefined"!=typeof displayedTimers[type],addAndResolve=function(){return addTimer(type,timerConfig),resolve()};return timerConfig?timerExists&&displayedTimers[type].id()===timerConfig.id?(setRemainingTime(timerConfig,timers[type]),timerConfig.remaining!==timers[type]&&displayedTimers[type].val(timerConfig.remaining).refresh(),resolve()):(removeTimer(type),checkStorage?void self.storage.getItem(timerConfig.id).then(function(savedTime){return _.isNumber(savedTime)&&savedTime>=0&&setRemainingTime(timerConfig,savedTime),addAndResolve();
}).catch(function(){return addAndResolve()}):addAndResolve()):(removeTimer(type),resolve())}))}),Promise.all(timerUpdatePromises).then(function(data){return toggleToggler(),data})};return store("timer-"+testRunner.getConfig().serviceCallId).then(function(timeStore){return self.shouldClearStorage?timeStore.clear().then(function(){return timeStore}):Promise.resolve(timeStore)}).then(function(timeStore){self.storage=timeStore,self.$element=$(timerBoxTpl()),self.$toggler=self.$element.find(".timer-toggler"),self.storage.getItem("zen-mode").then(function(zenMode){zenMode&&!self.$element.hasClass("zen-mode")&&toggleZenMode()}),self.$toggler.on("click",function(e){e.preventDefault(),
toggleZenMode()}),self.stopwatch=timerFactory({autoStart:!1}),self.polling=pollingFactory({action:function(){var timeoutScope,timeoutRef,elapsed=self.stopwatch.tick(),elapsedExtraTime=0,timeout=!1;_.forEach(displayedTimers,function(timer,type){var regularVal,displayedVal,warning,runTimeout=function(){testRunner.timeout(timeoutScope,timeoutRef)};timer.running()&&(regularVal=timers[type]-elapsed,displayedVal=timer.val()-elapsed,timers[type]=Math.max(regularVal,0),timer.val(Math.max(displayedVal,0)).refresh(),regularVal<=0&&(elapsedExtraTime=Math.max(elapsedExtraTime,Math.abs(regularVal))),displayedVal<=0?(timer.running(!1),timeout=!0,timeoutRef=timer.id(),timeoutScope=type,
...
self.enable()}).on("disablenav",function(){self.disable()}).on("nav-previous",function(previousItemWarning){doPrevious(previousItemWarning)})},render:function(){var $container=this.getAreaBroker().getNavigationArea();$container.append(this.$element)},destroy:function(){shortcut.remove("."+this.getName()),this.$element.remove()},enable:function(){this.$element.removeProp("disabled").removeClass("disabled")},disable:function(){this.$element.prop("disabled",!0).addClass("disabled")},show:function(){hider.show(this.$element)},hide:function(){hider.hide(this.$element)}})}),define("tpl!taoQtiTest/runner/plugins/navigation/review/navigator",["handlebars"],function(hb){
...
},disable:function(){this.button.disable()},show:function(){this.button.show()},hide:function(){this.button.hide()}})}),define("tpl!taoQtiTest/runner/plugins/tools/textToSpeech/textToSpeech",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var helper,options,buffer="",helperMissing=helpers.helperMissing,escapeExpression=this.escapeExpression;return buffer+='<div class="text-to-speech hidden">\n    <div class="action click-to-speak" title="'+escapeExpression((helper=helpers.__||depth0&&depth0.__,options={hash:{},data:data
},helper?helper.call(depth0,"Click to speak",options):helperMissing.call(depth0,"__","Click to speak",options)))+'"><span class="icon-click-to-speak"></span></div>\n\n    <div class="action play" title="'+escapeExpression((helper=helpers.__||depth0&&depth0.__,options={hash:{},data:data},helper?helper.call(depth0,"Play",options):helperMissing.call(depth0,"__","Play",options)))+'"><span class="icon-play"></span></div>\n    <div class="action pause" title="'+escapeExpression((helper=helpers.__||depth0&&depth0.__,options={hash:{},data:data},helper?helper.call(depth0,"Pause",options):helperMissing.call(depth0,"__","Pause",options)))+'"><span class="icon-pause"></span></div>\n\n    <div class="action stop" title="'+escapeExpression((helper=helpers.__||depth0&&depth0.__,
options={hash:{},data:data},helper?helper.call(depth0,"Stop",options):helperMissing.call(depth0,"__","Stop",options)))+'"><span class="icon-stop"></span></div>\n\n    <div class="action settings" title="'+escapeExpression((helper=helpers.__||depth0&&depth0.__,options={hash:{},data:data},helper?helper.call(depth0,"Settings",options):helperMissing.call(depth0,"__","Settings",options)))+'">\n        <span class="icon-settings"></span>\n        <div class="settings-menu">\n            <div class="option volume" title="'+escapeExpression((helper=helpers.__||depth0&&depth0.__,options={hash:{},data:data},helper?helper.call(depth0,"Volume",options):helperMissing.call(depth0,"__","Volume",options)))+'">\n                <span class="icon-volume"></span>\n                <div class="slider-container">\n                    <div class="slider"></div>\n                </div>\n                <div class="hover-container"></div>\n            </div>\n            <div class="option speed" title="'+escapeExpression((helper=helpers.__||depth0&&depth0.__,
options={hash:{},data:data},helper?helper.call(depth0,"Speech speed",options):helperMissing.call(depth0,"__","Speech speed",options)))+'">\n                <span class="icon-speed"></span>\n                <div class="slider-container">\n                    <div class="slider"></div>\n                </div>\n                <div class="hover-container"></div>\n            </div>\n        </div>\n    </div>\n</div>\n'})}),define("css!taoQtiTest/runner/plugins/tools/textToSpeech/textToSpeech",[],function(){}),define("taoQtiTest/runner/plugins/tools/textToSpeech/textToSpeech",["jquery","lodash","i18n","ui/component","tpl!taoQtiTest/runner/plugins/tools/textToSpeech/textToSpeech","css!taoQtiTest/runner/plugins/tools/textToSpeech/textToSpeech","nouislider","mathJax"],function($,_,__,componentFactory,tpl){
"use strict";return function(options,config){var component,speed,volume,texthelpMapping={properties:{pageCompleteCallback:"eba_page_complete_callback",texthelpSpeechStream:"TexthelpSpeechStream",volumeValue:"eba_volume_value"},functions:{barDynamicStart:"$rw_barDynamicStart",barCacheInit:"$rw_barCacheInit",barInit:"$rw_barInit",cachePage:"$rw_cachePage",disableSpeech:"$rw_disableSpeech",enableClickToSpeak:"$rw_enableClickToSpeak",enableContinuousReading:"$rw_enableContinuousReading",enableSpeech:"$rw_enableSpeech",event_pause:"$rw_event_pause",event_play:"$rw_event_play",getSpeed:"$rw_getSpeed",getVoice:"$rw_getVoice",hasReachedEnd:"$rw_hasReachedEnd",isPaused:"$rw_isPaused",
isSpeaking:"$rw_isSpeaking",isTextSelectedForPlay:"$rw_isTextSelectedForPlay",isUsingMathjax:"$rw_isUsingMathjax",setSpeedValue:"$rw_setSpeedValue",setUsingMath:"$rw_setUsingMath",setUsingMathjax:"$rw_setUsingMathjax",setUsingMaths:"$rw_setUsingMaths",setVolumeValue:"$rw_setVolumeValue",stopSpeech:"$rw_stopSpeech",tagSentences:"$rw_tagSentences",userParameters:"$rw_userParameters"}};return _.assign(options||{},{contentArea:$("body"),enableClickToSpeak:!1,ignoreEls:["header",".left-bar",".right-bar",".modal","footer",".action-bar",".sts-scope",".media-container"],toolbarUrl:"//taotoolbar.speechstream.net/tao/configQA.js"}),component=componentFactory({_exec:function(action){
var args=[].slice.call(arguments,1),fn=window[texthelpMapping.functions[action]];if(fn&&_.isFunction(fn))return fn.apply(this,args)},_get:function(property){return window[texthelpMapping.properties[property]]},_set:function(property,value){var prop=texthelpMapping.properties[property];if(prop)return window[prop]=value},_init:function(){var self=this;return this._exec("barDynamicStart"),this._exec("barInit"),this._exec("enableClickToSpeak",options.enableClickToSpeak),this._exec("setSpeedValue",speed),this._exec("setVolumeValue",volume),this._set("pageCompleteCallback",function(){self.trigger("stop")}),this},updateTexthelpCache:function(deliveryId,itemId){var tss=this._get("texthelpSpeechStream");
return tss.g_strBookId=deliveryId,tss.g_strPageId=itemId,this._exec("tagSentences",options.$contentArea.selector),this},enable:function(){return this._exec("enableSpeech"),this},disable:function(){return this._exec("disableSpeech"),this},play:function(){return this._exec("isSpeaking")&&!this._exec("isPaused")||(this._exec("event_play"),this.trigger("play")),this},pause:function(){return this._exec("event_pause"),this.trigger("pause"),this},stop:function(){return this._exec("stopSpeech"),this.trigger("stop"),this},setSpeed:function(e,value){return speed=+value,this._exec("setSpeedValue",speed),this.trigger("setSpeed",speed),this},setVolume:function(e,value){return volume=+value,
this._exec("setVolumeValue",volume),this.trigger("setVolume",volume),this},clickToSpeak:function(){var self=this;return options.enableClickToSpeak=!options.enableClickToSpeak,this._exec("enableClickToSpeak",options.enableClickToSpeak),this._exec("enableContinuousReading",!options.enableClickToSpeak),this.trigger("clickToSpeak"),options.$contentArea.find(".qti-item").each(function(){$(this).toggleClass("prevent-click-handler")}),options.$navigationArea.on("click","a",function(){options.$contentArea.find(".qti-item.prevent-click-handler").length>0&&self.clickToSpeak()}),this}},{}).setTemplate(tpl).on("init",function(){var self=this;speed=40,volume=40,$(options.ignoreEls.join(",")).attr("ignore",!0),
require([options.toolbarUrl],function(){var tss=self._get("texthelpSpeechStream");require(["//"+tss.g_strServer+"/SpeechStream/v"+tss.g_strBuild+"/texthelpMain.js"],function(){self._init()})})}).init(config).on("render",function(){var $this=this.getElement();$this.on("mousedown",function(e){return e.preventDefault(),!1}),$this.find(".action").on("click",function(e){var $action=$(this);$action.hasClass("disabled")&&e.stopImmediatePropagation(),$action.closest(".settings").length||($this.find(".settings").removeClass("active"),$this.find(".settings > .settings-menu").hide())}),$this.find(".settings").on("click",function(){$(this).toggleClass("active"),$this.find(".settings-menu").toggle();
}),$this.find(".settings > .settings-menu").on("click",function(e){e.stopPropagation()}).hide(),$this.find(".settings > .settings-menu > .option").on("hover",function(){$(this).find(".slider-container").show()}).on("mouseleave",function(){$(this).find(".slider-container").hide()}),$this.find(".settings > .settings-menu > .option > .slider-container").hide(),$this.find(".click-to-speak").on("click",this.clickToSpeak),$this.find(".play").on("click",this.play),$this.find(".pause").on("click",this.pause).hide(),$this.find(".stop").on("click",this.stop),$this.find(".settings > .settings-menu > .volume .slider").noUiSlider({animate:!0,connected:!0,range:{min:0,max:100
},start:volume,step:10}).on("change",this.setVolume),$this.find(".settings > .settings-menu > .speed .slider").noUiSlider({animate:!0,connected:!0,range:{min:0,max:100},start:speed,step:15}).on("change",this.setSpeed)}).on("clickToSpeak",function(){var $el=this.getElement();options.enableClickToSpeak?($el.find(".click-to-speak").addClass("active"),$el.find(".play").addClass("disabled").show(),$el.find(".pause").addClass("disabled").hide(),options.$contentArea.css("cursor","pointer")):($el.find(".click-to-speak").removeClass("active"),$el.find(".play").removeClass("disabled").show(),$el.find(".pause").removeClass("disabled").hide(),options.$contentArea.css("cursor","default"));
