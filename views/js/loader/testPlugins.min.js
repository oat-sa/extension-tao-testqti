...
"use strict";function getEmptyDetails(){return{position:0,reached:0,viewed:0,completed:0,total:0}}function updateDetails(stats,element,position){element.position<=position&&stats.position++,element.stats.viewed&&(stats.reached++,element.stats.viewed===element.stats.total&&stats.viewed++),element.stats.answered&&element.stats.answered===element.stats.questions&&stats.completed++,stats.total++}function getDetailedStats(testMap,currentItem){var stats=_.clone(testMap.stats);return stats.parts=getEmptyDetails(),stats.sections=getEmptyDetails(),stats.answerableParts=getEmptyDetails(),stats.answerableSections=getEmptyDetails(),_.forEach(testMap.parts,function(part){
updateDetails(stats.parts,part,currentItem.position),part.stats.questions>0&&updateDetails(stats.answerableParts,part,currentItem.position),_.forEach(part.sections,function(section){updateDetails(stats.sections,section,currentItem.position),section.stats.questions>0&&updateDetails(stats.answerableSections,section,currentItem.position)})}),stats}function getRatio(position,total){return position&&total>0?Math.floor(position/total*100):0}function getProgressionLabel(position,total,type,config){var patterns=labels[type]||labels.item,pattern=config.showTotal?patterns.long:patterns.short;return format(pattern,position||"0",total||"0")}function getPositionProgression(position,total,type,config){
return{position:position||0,total:total||0,ratio:getRatio(position,total),label:getProgressionLabel(position,total,type,config)}}function getRatioProgression(position,total){var ratio=getRatio(position,total);return{position:position||0,total:total||0,ratio:ratio,label:ratio+"%"}}var defaultConfig={scope:"test",indicator:"percentage",showTotal:!0},labels={item:{long:__("Item %d of %d"),short:__("Item %d")},section:{long:__("Section %d of %d"),short:__("Section %d")}},scopes={test:function(testMap,testContext){var scopedMap=mapHelper.getScopeMap(testMap,testContext.itemPosition,"test"),item=mapHelper.getItemAt(scopedMap,testContext.itemPosition),stats=getDetailedStats(scopedMap,item);
return stats.position=item.position+1,stats.completed=testContext.numberCompleted,stats.overall=testContext.numberItems,stats},testPart:function(testMap,testContext){var scopedMap=mapHelper.getScopeMap(testMap,testContext.itemPosition,"testPart"),item=mapHelper.getItemAt(scopedMap,testContext.itemPosition),stats=getDetailedStats(scopedMap,item);return stats.position=item.positionInPart+1,stats.completed=testContext.numberCompleted,stats.overall=testContext.numberItems,stats},testSection:function(testMap,testContext){var scopedMap=mapHelper.getScopeMap(testMap,testContext.itemPosition,"testSection"),item=mapHelper.getItemAt(scopedMap,testContext.itemPosition),stats=getDetailedStats(scopedMap,item);
return stats.position=item.positionInSection+1,stats.completed=testContext.numberCompleted,stats.overall=testContext.numberItems,stats}},indicators={percentage:function(stats){return getRatioProgression(stats.completed,stats.overall)},position:function(stats,config){return getPositionProgression(stats.position,stats.total,"item",config)},questions:function(stats,config){return getPositionProgression(stats.questionsViewed,stats.questions,"item",config)},sections:function(stats,config){return getPositionProgression(stats.answerableSections.reached,stats.answerableSections.total,"section",config)}};return{computeStats:function(testMap,testContext,scope){var statsComputer=scope&&scopes[scope]||scopes.test;
return statsComputer(testMap,testContext)},computeIndicator:function(stats,type,config){var indicatorComputer=type&&indicators[type]||indicators.percentage;return indicatorComputer(stats||{},config||defaultConfig)},computeProgress:function(testMap,testContext,config){var progressData;return config=_.defaults(config||{},defaultConfig),progressData=this.computeStats(testMap,testContext,config.scope),this.computeIndicator(progressData,config.indicator,config)}}}),define("tpl!taoQtiTest/runner/plugins/controls/progressbar/renderer/percentage",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){return this.compilerInfo=[4,">= 1.0.0"],
...
testRunner.setState("closedOrSuspended",!0),testRunner.trigger("leave",data))})}})}),define("tpl!taoQtiTest/runner/plugins/controls/timer/component/tpl/countdown",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,buffer="",functionType="function",escapeExpression=this.escapeExpression;return buffer+='<div class="countdown" data-control="',(helper=helpers.id)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.id,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),
buffer+=escapeExpression(stack1)+'" data-type="',(helper=helpers.type)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.type,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+'" data-scope="',(helper=helpers.scope)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.scope,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+'" title="',(helper=helpers.label)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.label,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},
data:data}):helper),buffer+=escapeExpression(stack1)+'" disabled>\n    <span class="label truncate">',(helper=helpers.label)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.label,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+'</span>\n    <span class="time"></span>\n</div>\n'})}),define("css!taoQtiTest/runner/plugins/controls/timer/component/css/countdown",[],function(){}),define("taoQtiTest/runner/plugins/controls/timer/component/countdown",["jquery","lodash","core/encoder/time","core/polling","core/timer","ui/component","tpl!taoQtiTest/runner/plugins/controls/timer/component/tpl/countdown","ui/tooltip","css!taoQtiTest/runner/plugins/controls/timer/component/css/countdown.css"],function($,_,timeEncoder,pollingFactory,timerFactory,component,countdownTpl){
"use strict";var precision=1e3,defaults={showBeforeStart:!0,displayWarning:!0,polling:!0,frequency:500},warningTimeout={info:2e3,success:2e3,warning:4e3,danger:4e3,error:8e3};return function($container,config){var $time,countdown=component({update:function(remainingTime){var encodedTime,warningId,warningMessage,self=this;return this.is("completed")||(remainingTime<=0?this.remainingTime=0:this.remainingTime=parseInt(remainingTime,10),this.is("rendered")&&this.is("running")&&(encodedTime=timeEncoder.encode(this.remainingTime/precision),encodedTime!==this.encodedTime&&(this.encodedTime=encodedTime,$time.text(this.encodedTime)),this.warnings&&(warningId=_.findLastKey(this.warnings,function(warning){
return warning&&!warning.shown&&warning.threshold>0&&warning.threshold>=self.remainingTime}),warningId&&(this.warnings[warningId].shown=!0,warningMessage=_.isFunction(this.warnings[warningId].message)?this.warnings[warningId].message(this.remainingTime):this.warnings[warningId].message,this.trigger("warn",warningMessage,this.warnings[warningId].level))),this.trigger("change",this.remainingTime,encodedTime)),0===this.remainingTime&&this.complete()),this},start:function(){return!this.is("rendered")||this.is("running")||this.is("completed")||(this.enable(),this.setState("running",!0),this.polling&&(this.polling.start(),this.is("started")?this.timer.resume():(this.setState("started",!0),
this.timer.start())),this.trigger("start")),this},stop:function(){return this.is("rendered")&&this.is("running")&&(this.setState("running",!1),this.polling&&(this.timer.pause(),this.polling.stop()),this.trigger("stop")),this},complete:function(){return this.is("rendered")&&this.is("running")&&!this.is("completed")&&(this.stop(),this.setState("completed",!0),this.trigger("complete end")),this}},defaults).on("init",function(){var self=this;this.remainingTime=this.config.remainingTime,this.config.warnings&&(this.warnings=_.sortBy(this.config.warnings,"threshold")),this.config.polling===!0&&this.config.frequency>0&&(this.timer=timerFactory({autoStart:!1}),this.polling=pollingFactory({
action:function(){var elapsed=self.timer.tick();self.update(self.remainingTime-elapsed)},interval:this.config.frequency,autoStart:!1})),this.render($container)}).on("render",function(){$time=$(".time",this.getElement()),this.config.showBeforeStart===!0&&$time.text(timeEncoder.encode(this.remainingTime/precision))}).on("warn",function(message,level){level=level||"warning",this.is("rendered")&&this.is("running")&&_.isString(message)&&!_.isEmpty(message)&&($time.removeClass("txt-success txt-info txt-warning txt-danger txt-error").addClass("txt-"+level),this.config.displayWarning===!0&&this.getElement().qtip({show:{ready:!0},hide:{event:!1,inactive:warningTimeout[level]||2e3
},suppress:!1,theme:level,content:{text:message},position:{my:"top center",at:"bottom center"}}))});return countdown.setTemplate(countdownTpl),_.defer(function(){countdown.init(config)}),countdown}}),define("tpl!taoQtiTest/runner/plugins/controls/timer/component/tpl/timerbox",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){function program1(depth0,data){var helper,options,buffer="";return buffer+='\n    <a href="#" class="timer-toggler hidden" title="'+escapeExpression((helper=helpers.__||depth0&&depth0.__,options={hash:{},data:data},helper?helper.call(depth0,"Hide timers",options):helperMissing.call(depth0,"__","Hide timers",options)))+'"><span class="icon-clock"></span></a>\n    ';
...
"use strict";function getInstantStats(scope,runner,sync){var isItemCurrentlyAnswered,map=runner.getTestMap(),context=runner.getTestContext(),stats=_.clone(mapHelper.getScopeStats(map,context.itemPosition,scope)),item=mapHelper.getItemAt(map,context.itemPosition);return item.informational||(isItemCurrentlyAnswered=currentItemHelper.isAnswered(runner),!isItemCurrentlyAnswered&&context.itemAnswered?stats.answered--:!isItemCurrentlyAnswered&&!sync||context.itemAnswered||stats.answered++),stats}return{getInstantStats:getInstantStats}}),define("taoQtiTest/runner/helpers/messages",["lodash","i18n","taoQtiTest/runner/helpers/map","taoQtiTest/runner/helpers/stats"],function(_,__,mapHelper,statsHelper){
