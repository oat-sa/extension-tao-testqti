...
buttons:{labels:{ok:__("Close this Section"),cancel:__("Review my Answers")}}})});return movePromise.then(function(){doDisable(),removeTimer(timerTypes.item)}).catch(function(){_.defer(function(){testRunner.trigger("enableitem enablenav")})}),movePromise}).before("move skip exit timeout",function(){testRunner.getProxy().addCallActionParams({consumedExtraTime:lastConsumedExtraTime/precision}),lastConsumedExtraTime=0}).before("finish",function(){return new Promise(function(resolve){self.storage.removeStore().then(resolve).catch(resolve)})})})},render:function(){var $container=this.getAreaBroker().getControlArea();$container.append(this.$element)},destroy:function(){
