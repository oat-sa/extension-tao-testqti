...
$container.append(this.$element)}})}),define("taoQtiTest/runner/plugins/controls/testState/testState",["lodash","taoTests/runner/plugin"],function(_,pluginFactory){"use strict";return pluginFactory({name:"testState",install:function(){var testRunner=this.getTestRunner();testRunner.getProxy().use(function(req,res,next){var data=res&&res.data;return data&&data.type&&"TestState"===data.type?(testRunner.setState("closedOrSuspended",!0),void(testRunner.getState("ready")?!_.isEmpty(data.messages)&&_.find(data.messages,{channel:"teststate"})||testRunner.trigger("leave",data):testRunner.trigger("destroy"))):void next()})},init:function(){var testRunner=this.getTestRunner(),isLeaving=!1;
testRunner.getProxy().channel("teststate",function(data){isLeaving||!data||"close"!==data.type&&"pause"!==data.type||testRunner.getState("closedOrSuspended")||(isLeaving=!0,testRunner.setState("closedOrSuspended",!0),testRunner.trigger("leave",data))})}})}),define("tpl!taoQtiTest/runner/plugins/controls/timer/timer",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){function program1(depth0,data){return" disabled"}this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,buffer="",functionType="function",escapeExpression=this.escapeExpression,self=this;return buffer+='<span class="qti-timer qti-timer__type-',
...
return _.isString(value)&&(value=[value]),value=_.map(value||[],function(v){return"boolean"===baseType?v===!0||"true"===v:v}),mappedCardinality&&("base"===mappedCardinality?0===value.length?response.base=null:(response.base={},response.base[baseType]=value[0]):(response[mappedCardinality]={},response[mappedCardinality][baseType]=value)),response},isQtiValueNull:function(value,baseType,cardinality){var mappedCardinality=responseCardinalities[cardinality];return _.isObject(value)&&null===value[mappedCardinality]&&(value=null),_.isObject(value)&&value[mappedCardinality]&&"undefined"!=typeof value[mappedCardinality][baseType]&&(value=value[mappedCardinality][baseType]),
null===value||"string"===baseType&&_.isEmpty(value)||"single"!==cardinality&&_.isEmpty(value)},isQuestionAnswered:function(response,baseType,cardinality,defaultValue){var answered;return defaultValue=defaultValue||null,answered=!currentItemHelper.isQtiValueNull(response,baseType,cardinality)&&!_.isEqual(response,currentItemHelper.toResponse(defaultValue,baseType,cardinality))},isAnswered:function(runner,partially){var itemRunner=runner.itemRunner,responses=itemRunner&&itemRunner.getResponses(),count=0,empty=0;return itemRunner&&_.forEach(currentItemHelper.getDeclarations(runner),function(declaration){var attributes=declaration.attributes||{},response=responses[attributes.identifier],baseType=attributes.baseType,cardinality=attributes.cardinality;
count++,currentItemHelper.isQuestionAnswered(response,baseType,cardinality,declaration.defaultValue)||empty++}),partially===!1?count>0&&0===empty:count>0&&empty<count}};return currentItemHelper}),define("taoQtiTest/runner/helpers/stats",["lodash","taoQtiTest/runner/helpers/map","taoQtiTest/runner/helpers/currentItem"],function(_,mapHelper,currentItemHelper){"use strict";function getInstantStats(scope,runner){var map=runner.getTestMap(),context=runner.getTestContext(),stats=_.clone(mapHelper.getScopeStats(map,context.itemPosition,scope)),isItemCurrentlyAnswered=currentItemHelper.isAnswered(runner);return!isItemCurrentlyAnswered&&context.itemAnswered?stats.answered--:isItemCurrentlyAnswered&&!context.itemAnswered&&stats.answered++,
stats}return{getInstantStats:getInstantStats}}),define("taoQtiTest/runner/helpers/messages",["lodash","i18n","taoQtiTest/runner/helpers/map","taoQtiTest/runner/helpers/stats","taoQtiTest/runner/helpers/currentItem"],function(_,__,mapHelper,statsHelper,currentItemHelper){"use strict";function getExitMessage(message,scope,runner){var itemsCountMessage="",testData=runner.getTestData(),testConfig=testData&&testData.config,messageEnabled=!testConfig||testConfig.enableUnansweredItemsWarning;return messageEnabled&&(itemsCountMessage=getUnansweredItemsWarning(scope,runner)),(itemsCountMessage+" "+message).trim()}function getUnansweredItemsWarning(scope,runner){var belowSections,stats=statsHelper.getInstantStats(scope,runner),unansweredCount=stats&&stats.questions-stats.answered,flaggedCount=stats&&stats.flagged,itemsCountMessage="",map=runner.getTestMap(),jump=mapHelper.getJump(map,runner.getTestContext().itemPosition);
return"section"===scope||"testSection"===scope?(0===unansweredCount?itemsCountMessage+=__("You answered all %s question(s) in this section",stats.questions.toString()):itemsCountMessage=__("You answered only %s of the %s question(s) in this section",stats.answered.toString(),stats.questions.toString()),flaggedCount&&(itemsCountMessage+=", ",itemsCountMessage+=__("and flagged %s of them",flaggedCount.toString()))):"test"===scope?(belowSections=getNextSections(map,jump.section),_.forEach(belowSections,function(section){_.forEach(mapHelper.getSectionStats(runner.getTestMap(),section.id),function(sectionStats,statsKey){stats[statsKey]+=sectionStats})}),unansweredCount=stats&&stats.questions-stats.answered,
flaggedCount=stats&&stats.flagged,itemsCountMessage=0===unansweredCount?__("You answered all %s question(s) in this test",stats.questions.toString()):__("You have %s unanswered question(s)",unansweredCount.toString()),flaggedCount&&(itemsCountMessage+=" "+__("and you flagged %s item(s) that you can review now",flaggedCount.toString()))):"part"===scope&&(itemsCountMessage=0===unansweredCount?__("You answered all %s question(s)",stats.questions.toString()):__("You have %s unanswered question(s)",unansweredCount.toString()),flaggedCount&&(itemsCountMessage+=" "+__("and you flagged %s item(s) that you can review now",flaggedCount.toString()))),itemsCountMessage&&(itemsCountMessage+="."),
itemsCountMessage}function getNextSections(map,currentSectionId){var sections=mapHelper.getSections(map),result={},below=!1;return _.forEach(sections,function(section){below&&(result[section.id]=section),section.id===currentSectionId&&(below=!0)}),result}return{getExitMessage:getExitMessage}}),define("tpl!taoQtiTest/runner/plugins/controls/timer/timers",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var helper,options,buffer="",helperMissing=helpers.helperMissing,escapeExpression=this.escapeExpression;return buffer+='<div class="timer-box">\n    <a href="#" class="timer-toggler hidden" title="'+escapeExpression((helper=helpers.__||depth0&&depth0.__,
options={hash:{},data:data},helper?helper.call(depth0,"Hide timers",options):helperMissing.call(depth0,"__","Hide timers",options)))+'"><span class="icon-clock"></span></a>\n    <div class="timer-wrapper"></div>\n</div>\n'})}),define("taoQtiTest/runner/plugins/controls/timer/timer",["jquery","lodash","i18n","core/promise","core/polling","core/timer","core/store","ui/hider","taoTests/runner/plugin","taoQtiTest/runner/plugins/controls/timer/timerComponent","taoQtiTest/runner/helpers/messages","tpl!taoQtiTest/runner/plugins/controls/timer/timers"],function($,_,__,Promise,pollingFactory,timerFactory,store,hider,pluginFactory,timerComponentFactory,messages,timerBoxTpl){
...
running:!0,warnings:{}},timeConstraint.seconds*precision),_(timerWarning[timeConstraint.qtiClassName]).forEach(function(value,key){_.contains(["info","warning","danger"],value)&&(timer.warnings[key]={type:value,showed:timer.remaining/precision<=key,point:parseInt(key,10)*precision})}),closestPreviousWarning=_.find(timer.warnings,{showed:!0}),!_.isEmpty(closestPreviousWarning)&&closestPreviousWarning.point&&timer.warnings[closestPreviousWarning.point/precision]&&(timer.warnings[closestPreviousWarning.point/precision].showed=!1))),timer},leaveTimedSection=function(type,scope,position){var context=testRunner.getTestContext(),map=testRunner.getTestMap(),section=map.parts[context.testPartId].sections[context.sectionId],nbItems=_.size(section.items),item=_.find(section.items,{
position:context.itemPosition});return!(context.isTimeout||context.itemSessionState===itemStates.closed||!isTimedSection(context)||!item)&&!!("next"===type&&("assessmentSection"===scope||item.positionInSection+1===nbItems)||"previous"===type&&0===item.positionInSection||"jump"===type&&position>0&&(position<section.position||position>=section.position+nbItems))},removeTimer=function(type){displayedTimers[type]&&(self.storage.removeItem(displayedTimers[type].id()),displayedTimers[type].destroy(),displayedTimers=_.omit(displayedTimers,type),timers=_.omit(timers,type),self.trigger("removetimer",type))},addTimer=function(type,config){config&&(timers[type]=config.regular,
displayedTimers[type]=timerComponentFactory(config),displayedTimers[type].init().render(self.$element.find(".timer-wrapper")),self.trigger("addtimer",type,displayedTimers[type]))},updateTimers=function(checkStorage){var timerUpdatePromises=[],context=testRunner.getTestContext();return context.extraTime&&(extraTime=context.extraTime.total,consumedExtraTime=Math.max(consumedExtraTime,context.extraTime.consumed)),_.forEach(timerTypes,function(type){timerUpdatePromises.push(new Promise(function(resolve){var timerConfig=getTimerConfig(type);return displayedTimers[type]?(timerConfig?displayedTimers[type].id()!==timerConfig.id?(removeTimer(type),addTimer(type,timerConfig)):(setRemainingTime(timerConfig,timers[type]),
...
self.enable()}).on("disablenav",function(){self.disable()})},render:function(){var $container=this.getAreaBroker().getNavigationArea();$container.append(this.$element)},destroy:function(){this.$element.remove()},enable:function(){this.$element.removeProp("disabled").removeClass("disabled")},disable:function(){this.$element.prop("disabled",!0).addClass("disabled")},show:function(){hider.show(this.$element)},hide:function(){hider.hide(this.$element)}})}),define("taoQtiTest/runner/helpers/navigation",["lodash","taoQtiTest/runner/helpers/map"],function(_,mapHelper){"use strict";var navigationHelper={isLeavingSection:function(testContext,testMap,direction,scope,position){
...
visualGuides.$maskBg.remove(),visualGuides.$innerWindow.remove()}};return overlay=componentFactory(overlayAPI,overlayConfig),makeDraggable(overlay),makeStackable(overlay,stackingOptions),overlay.on("render",function(){var self=this,$element=this.getElement();$element.addClass("line-reader-overlay "+overlayConfig.id).on("mousedown",function(){bringAllToFront(),self.transformOverlay()}).on("mouseup",function(){self.restoreOverlay()})}).on("dragstart",function(){innerDrag.hide(),closer.hide(),invokeOnMasks("hide"),this.appendVisualGuides()}).on("dragmove",function(xOffsetRelative,yOffsetRelative){position.outerX+=xOffsetRelative,position.outerY+=yOffsetRelative,
...
var args=[].slice.call(arguments,1),fn=window[texthelpMapping.functions[action]];if(fn&&_.isFunction(fn))return fn.apply(this,args)},_get:function(property){return window[texthelpMapping.properties[property]]},_set:function(property,value){var prop=texthelpMapping.properties[property];if(prop)return window[prop]=value},_init:function(){var self=this;this._exec("barDynamicStart"),this._exec("barInit"),this._exec("enableClickToSpeak",options.enableClickToSpeak),this._exec("setSpeedValue",speed),this._exec("setVolumeValue",volume),this._set("pageCompleteCallback",function(){self.trigger("stop")})},updateTexthelpCache:function(deliveryId,itemId){var tss=this._get("texthelpSpeechStream");
tss.g_strBookId=deliveryId,tss.g_strPageId=itemId,this._exec("tagSentences",options.$contentArea.selector)},enable:function(){this._exec("enableSpeech")},disable:function(){this._exec("disableSpeech")},play:function(){this._exec("isSpeaking")&&!this._exec("isPaused")||(this._exec("event_play"),this.trigger("play"))},pause:function(){this._exec("event_pause"),this.trigger("pause")},stop:function(){this._exec("stopSpeech"),this.trigger("stop")},setSpeed:function(e,value){speed=+value,this._exec("setSpeedValue",speed),this.trigger("setSpeed",speed)},setVolume:function(e,value){volume=+value,this._exec("setVolumeValue",volume),this.trigger("setVolume",volume)},clickToSpeak:function(){
options.enableClickToSpeak=!options.enableClickToSpeak,this._exec("enableClickToSpeak",options.enableClickToSpeak),this._exec("enableContinuousReading",!options.enableClickToSpeak),this.trigger("clickToSpeak")}},{}).setTemplate(tpl).on("init",function(){var self=this;speed=40,volume=40,$(options.ignoreEls.join(",")).attr("ignore",!0),require([options.toolbarUrl],function(){var tss=self._get("texthelpSpeechStream");require(["//"+tss.g_strServer+"/SpeechStream/v"+tss.g_strBuild+"/texthelpMain.js"],function(){self._init()})})}).init(config).on("render",function(){var $this=this.getElement();$this.on("mousedown",function(e){return e.preventDefault(),!1}),$this.find(".action").on("click",function(e){
var $action=$(this);$action.hasClass("disabled")&&e.stopImmediatePropagation(),$action.closest(".settings").length||($this.find(".settings").removeClass("active"),$this.find(".settings > .settings-menu").hide())}),$this.find(".settings").on("click",function(){$(this).toggleClass("active"),$this.find(".settings-menu").toggle()}),$this.find(".settings > .settings-menu").on("click",function(e){e.stopPropagation()}).hide(),$this.find(".settings > .settings-menu > .option").on("hover",function(){$(this).find(".slider-container").show()}).on("mouseleave",function(){$(this).find(".slider-container").hide()}),$this.find(".settings > .settings-menu > .option > .slider-container").hide(),
$this.find(".click-to-speak").on("click",this.clickToSpeak),$this.find(".play").on("click",this.play),$this.find(".pause").on("click",this.pause).hide(),$this.find(".stop").on("click",this.stop),$this.find(".settings > .settings-menu > .volume .slider").noUiSlider({animate:!0,connected:!0,range:{min:0,max:100},start:volume,step:10}).on("change",this.setVolume),$this.find(".settings > .settings-menu > .speed .slider").noUiSlider({animate:!0,connected:!0,range:{min:0,max:100},start:speed,step:10}).on("change",this.setSpeed)}).on("clickToSpeak",function(){var $el=this.getElement();options.enableClickToSpeak?($el.find(".click-to-speak").addClass("active"),$el.find(".play").addClass("disabled").show(),
$el.find(".pause").addClass("disabled").hide(),options.$contentArea.css("cursor","pointer")):($el.find(".click-to-speak").removeClass("active"),$el.find(".play").removeClass("disabled").show(),$el.find(".pause").removeClass("disabled").hide(),options.$contentArea.css("cursor","default"))}).on("play",function(){var $el=this.getElement();$el.find(".play").hide(),$el.find(".pause").show()}).on("pause",function(){var $el=this.getElement();$el.find(".play").show(),$el.find(".pause").hide()}).on("stop",function(){var $el=this.getElement();$el.find(".play").show(),$el.find(".pause").hide()})}}),define("taoQtiTest/runner/plugins/tools/textToSpeech/plugin",["jquery","lodash","i18n","taoTests/runner/plugin","ui/hider","ui/stacker","taoQtiTest/runner/plugins/tools/textToSpeech/textToSpeech"],function($,_,__,pluginFactory,hider,stackerFactory,ttsFactory){
"use strict";return pluginFactory({name:"textToSpeech",init:function(){var self=this,testRunner=this.getTestRunner();return this.ttsButton=this.getAreaBroker().getToolbox().createEntry({control:"tts",icon:"headphones",text:__("Text to Speech"),title:__("Text to Speech")}).on("render",function(){var stacker=stackerFactory("test-runner");self.tts=ttsFactory({$contentArea:testRunner.getAreaBroker().getContentArea()}).render(self.ttsButton.getElement()),stacker.autoBringToFront(self.tts.getElement())}).on("click",function(e){var ttsEl=self.tts.getElement();$(e.target).closest(ttsEl).length||(hider.toggle(ttsEl),ttsEl.hasClass("hidden")?self.tts.disable():self.tts.enable());
}),testRunner.on("loaditem",function(){self.tts.disable(),self.ttsButton.disable(),self.ttsButton.hide()}).on("enabletools",function(){self.tts.enable(),self.ttsButton.enable()}).on("renderitem",function(){var context=testRunner.getTestContext(),config=testRunner.getConfig();context.options.textToSpeech&&(self.tts.enable(),self.ttsButton.enable(),self.ttsButton.show(),self.tts.updateTexthelpCache(config.serviceCallId,this.itemRunner._item.attributes.identifier))}).on("disabletools unloaditem",function(){self.tts.disable(),self.ttsButton.disable()}),this}})}),define("taoQtiTest/runner/plugins/tools/zoom",["jquery","i18n","ui/hider","ui/transformer","util/shortcut","util/namespace","taoTests/runner/plugin"],function($,__,hider,transformer,shortcut,namespaceHelper,pluginFactory){
