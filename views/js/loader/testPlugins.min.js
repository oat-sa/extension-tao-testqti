...
"use strict";function getInstantStats(scope,runner){var map=runner.getTestMap(),context=runner.getTestContext(),stats=_.clone(mapHelper.getScopeStats(map,context.itemPosition,scope)),isItemCurrentlyAnswered=currentItemHelper.isAnswered(runner);return isItemCurrentlyAnswered&&(!isItemCurrentlyAnswered&&context.itemAnswered&&stats.answered--,isItemCurrentlyAnswered&&!context.itemAnswered&&stats.answered++),stats}return{getInstantStats:getInstantStats}}),define("taoQtiTest/runner/helpers/messages",["lodash","i18n","taoQtiTest/runner/helpers/map","taoQtiTest/runner/helpers/stats","taoQtiTest/runner/helpers/currentItem"],function(_,__,mapHelper,statsHelper,currentItemHelper){
"use strict";function getExitMessage(message,scope,runner){var itemsCountMessage="",testData=runner.getTestData(),testConfig=testData&&testData.config,messageEnabled=!testConfig||testConfig.enableUnansweredItemsWarning;return messageEnabled&&(itemsCountMessage=getUnansweredItemsWarning(scope,runner)),(itemsCountMessage+" "+message).trim()}function getUnansweredItemsWarning(scope,runner){var belowSections,stats=statsHelper.getInstantStats(scope,runner),unansweredCount=stats&&stats.questions-stats.answered,flaggedCount=stats&&stats.flagged,itemsCountMessage="",map=runner.getTestMap(),jump=mapHelper.getJump(map,runner.getTestContext().itemPosition);return"section"===scope||"testSection"===scope?(0===unansweredCount?itemsCountMessage+=__("You answered all %s question(s) in this section",stats.questions.toString()):itemsCountMessage=__("You answered only %s of the %s question(s) in this section",stats.answered.toString(),stats.questions.toString()),
flaggedCount&&(itemsCountMessage+=", ",itemsCountMessage+=__("and flagged %s of them",flaggedCount.toString()))):"test"===scope?(belowSections=getNextSections(map,jump.section),stats=_.clone(mapHelper.getSectionStats(runner.getTestMap(),jump.section)),_.forEach(belowSections,function(section){_.forEach(mapHelper.getSectionStats(runner.getTestMap(),section.id),function(sectionStats,statsKey){stats[statsKey]+=sectionStats})}),unansweredCount=stats&&stats.questions-stats.answered,flaggedCount=stats&&stats.flagged,currentItemHelper.isAnswered(runner)&&unansweredCount--,itemsCountMessage=0===unansweredCount?__("You answered all %s question(s) in this test",stats.questions.toString()):__("You have %s unanswered question(s)",unansweredCount.toString()),
flaggedCount&&(itemsCountMessage+=" "+__("and you flagged %s item(s) that you can review now",flaggedCount.toString()))):"part"===scope&&(itemsCountMessage=0===unansweredCount?__("You answered all %s question(s)",stats.questions.toString()):__("You have %s unanswered question(s)",unansweredCount.toString()),flaggedCount&&(itemsCountMessage+=" "+__("and you flagged %s item(s) that you can review now",flaggedCount.toString()))),itemsCountMessage&&(itemsCountMessage+="."),itemsCountMessage}function getNextSections(map,currentSectionId){var sections=mapHelper.getSections(map),result={},below=!1;return _.forEach(sections,function(section){below&&(result[section.id]=section),
section.id===currentSectionId&&(below=!0)}),result}return{getExitMessage:getExitMessage}}),define("tpl!taoQtiTest/runner/plugins/controls/timer/timers",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var helper,options,buffer="",helperMissing=helpers.helperMissing,escapeExpression=this.escapeExpression;return buffer+='<div class="timer-box">\n    <a href="#" class="timer-toggler hidden" title="'+escapeExpression((helper=helpers.__||depth0&&depth0.__,options={hash:{},data:data},helper?helper.call(depth0,"Hide timers",options):helperMissing.call(depth0,"__","Hide timers",options)))+'"><span class="icon-clock"></span></a>\n    <div class="timer-wrapper"></div>\n</div>\n';
})}),define("taoQtiTest/runner/plugins/controls/timer/timer",["jquery","lodash","i18n","core/promise","core/polling","core/timer","core/store","ui/hider","taoTests/runner/plugin","taoQtiTest/runner/plugins/controls/timer/timerComponent","taoQtiTest/runner/helpers/messages","tpl!taoQtiTest/runner/plugins/controls/timer/timers"],function($,_,__,Promise,pollingFactory,timerFactory,store,hider,pluginFactory,timerComponentFactory,messages,timerBoxTpl){"use strict";var timerRefresh=1e3,precision=1e3,exitMessage=__("Once you close this section, you cannot return to it or change your answers."),timerTypes={test:"assessmentTest",testPart:"testPart",section:"assessmentSection",
item:"assessmentItemRef"},isTimedSection=function(context){var timeConstraints=context.timeConstraints||[];return _.some(timeConstraints,function(constraint){return"assessmentSection"===constraint.qtiClassName})};return pluginFactory({name:"timer",install:function(){var self=this,testRunner=this.getTestRunner();testRunner.on("storechange",function(){self.shouldClearStorage=!0})},init:function(){function doEnable(){self.getState("enabled")||self.enable()}function doDisable(){self.getState("enabled")&&self.disable()}function toggleZenMode(){self.$element.hasClass("zen-mode")?(self.$element.removeClass("zen-mode"),self.$toggler.attr("title",__("Hide timers")),self.storage.removeItem("zen-mode")):(self.$element.addClass("zen-mode"),
...
showed:timer.remaining/precision<=key,point:parseInt(key,10)*precision})}),closestPreviousWarning=_.find(timer.warnings,{showed:!0}),!_.isEmpty(closestPreviousWarning)&&closestPreviousWarning.point&&timer.warnings[closestPreviousWarning.point/precision]&&(timer.warnings[closestPreviousWarning.point/precision].showed=!1))),timer},leaveTimedSection=function(type,scope,position){var context=testRunner.getTestContext(),map=testRunner.getTestMap(),section=map.parts[context.testPartId].sections[context.sectionId],nbItems=_.size(section.items),item=_.find(section.items,{position:context.itemPosition});return!(context.isTimeout||context.itemSessionState===itemStates.closed||!isTimedSection(context)||!item)&&!!("next"===type&&("assessmentSection"===scope||item.positionInSection+1===nbItems)||"previous"===type&&0===item.positionInSection||"jump"===type&&position>0&&(position<section.position||position>=section.position+nbItems));
