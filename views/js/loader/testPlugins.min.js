...
"use strict";function getInstantStats(scope,runner){var map=runner.getTestMap(),context=runner.getTestContext(),stats=_.clone(mapHelper.getScopeStats(map,context.itemPosition,scope)),isItemCurrentlyAnswered=currentItemHelper.isAnswered(runner);return!isItemCurrentlyAnswered&&context.itemAnswered?stats.answered--:isItemCurrentlyAnswered&&!context.itemAnswered&&stats.answered++,stats}return{getInstantStats:getInstantStats}}),define("taoQtiTest/runner/helpers/messages",["lodash","i18n","taoQtiTest/runner/helpers/map","taoQtiTest/runner/helpers/stats"],function(_,__,mapHelper,statsHelper){"use strict";function getExitMessage(message,scope,runner){var itemsCountMessage="",testData=runner.getTestData(),testConfig=testData&&testData.config,messageEnabled=!testConfig||testConfig.enableUnansweredItemsWarning;
return messageEnabled&&(itemsCountMessage=getUnansweredItemsWarning(scope,runner)),(itemsCountMessage+" "+message).trim()}function getUnansweredItemsWarning(scope,runner){var stats=statsHelper.getInstantStats(scope,runner),unansweredCount=stats&&stats.questions-stats.answered,flaggedCount=stats&&stats.flagged,itemsCountMessage="";return"section"===scope||"testSection"===scope?(itemsCountMessage=0===unansweredCount?__("You answered all %s question(s) in this section",stats.questions.toString()):__("You answered only %s of the %s question(s) in this section",stats.answered.toString(),stats.questions.toString()),flaggedCount&&(itemsCountMessage+=", "+__("and flagged %s of them",flaggedCount.toString()))):"test"===scope?(itemsCountMessage=0===unansweredCount?__("You answered all %s question(s) in this test",stats.questions.toString()):__("You have %s unanswered question(s)",unansweredCount.toString()),
flaggedCount&&(itemsCountMessage+=" "+__("and you flagged %s item(s) that you can review now",flaggedCount.toString()))):"part"===scope&&(itemsCountMessage=0===unansweredCount?__("You answered all %s question(s)",stats.questions.toString()):__("You have %s unanswered question(s)",unansweredCount.toString()),flaggedCount&&(itemsCountMessage+=" "+__("and you flagged %s item(s) that you can review now",flaggedCount.toString()))),itemsCountMessage&&(itemsCountMessage+="."),itemsCountMessage}return{getExitMessage:getExitMessage}}),define("taoQtiTest/runner/helpers/navigation",["lodash","taoQtiTest/runner/helpers/map"],function(_,mapHelper){"use strict";var navigationHelper={
...
this._exec("setVolumeValue",volume),this.trigger("setVolume",volume),this},clickToSpeak:function(){return options.enableClickToSpeak=!options.enableClickToSpeak,this._exec("enableClickToSpeak",options.enableClickToSpeak),this._exec("enableContinuousReading",!options.enableClickToSpeak),this.trigger("clickToSpeak"),this}},{}).setTemplate(tpl).on("init",function(){var self=this;speed=40,volume=40,$(options.ignoreEls.join(",")).attr("ignore",!0),require([options.toolbarUrl],function(){var tss=self._get("texthelpSpeechStream");require(["//"+tss.g_strServer+"/SpeechStream/v"+tss.g_strBuild+"/texthelpMain.js"],function(){self._init()})})}).init(config).on("render",function(){
var $this=this.getElement();$this.on("mousedown",function(e){return e.preventDefault(),!1}),$this.find(".action").on("click",function(e){var $action=$(this);$action.hasClass("disabled")&&e.stopImmediatePropagation(),$action.closest(".settings").length||($this.find(".settings").removeClass("active"),$this.find(".settings > .settings-menu").hide())}),$this.find(".settings").on("click",function(){$(this).toggleClass("active"),$this.find(".settings-menu").toggle()}),$this.find(".settings > .settings-menu").on("click",function(e){e.stopPropagation()}).hide(),$this.find(".settings > .settings-menu > .option").on("hover",function(){$(this).find(".slider-container").show();
}).on("mouseleave",function(){$(this).find(".slider-container").hide()}),$this.find(".settings > .settings-menu > .option > .slider-container").hide(),$this.find(".click-to-speak").on("click",this.clickToSpeak),$this.find(".play").on("click",this.play),$this.find(".pause").on("click",this.pause).hide(),$this.find(".stop").on("click",this.stop),$this.find(".settings > .settings-menu > .volume .slider").noUiSlider({animate:!0,connected:!0,range:{min:0,max:100},start:volume,step:10}).on("change",this.setVolume),$this.find(".settings > .settings-menu > .speed .slider").noUiSlider({animate:!0,connected:!0,range:{min:0,max:100},start:speed,step:10}).on("change",this.setSpeed);
}).on("clickToSpeak",function(){var $el=this.getElement();options.enableClickToSpeak?($el.find(".click-to-speak").addClass("active"),$el.find(".play").addClass("disabled").show(),$el.find(".pause").addClass("disabled").hide(),options.$contentArea.css("cursor","pointer")):($el.find(".click-to-speak").removeClass("active"),$el.find(".play").removeClass("disabled").show(),$el.find(".pause").removeClass("disabled").hide(),options.$contentArea.css("cursor","default"))}).on("play",function(){var $el=this.getElement();$el.find(".play").hide(),$el.find(".pause").show()}).on("pause",function(){var $el=this.getElement();$el.find(".play").show(),$el.find(".pause").hide();
}).on("stop",function(){var $el=this.getElement();$el.find(".play").show(),$el.find(".pause").hide()})}}),define("taoQtiTest/runner/plugins/tools/textToSpeech/plugin",["jquery","lodash","i18n","taoTests/runner/plugin","ui/hider","ui/stacker","taoQtiTest/runner/plugins/tools/textToSpeech/textToSpeech"],function($,_,__,pluginFactory,hider,stackerFactory,ttsFactory){"use strict";return pluginFactory({name:"textToSpeech",init:function(){var self=this,testRunner=this.getTestRunner();return this.ttsButton=this.getAreaBroker().getToolbox().createEntry({control:"tts",icon:"headphones",text:__("Text to Speech"),title:__("Text to Speech")}).on("render",function(){var stacker=stackerFactory("test-runner");
self.tts=ttsFactory({$contentArea:testRunner.getAreaBroker().getContentArea()}).render(self.ttsButton.getElement()).disable().hide(),stacker.autoBringToFront(self.tts.getElement())}).on("click",function(e){var ttsEl=self.tts.getElement();$(e.target).closest(ttsEl).length||(hider.toggle(ttsEl),ttsEl.hasClass("hidden")?self.tts.disable():self.tts.enable())}).disable().hide(),testRunner.on("loaditem",function(){self.disable(),self.ttsButton.hide()}).on("enabletools",function(){self.enable()}).on("renderitem",function(){var context=testRunner.getTestContext(),config=testRunner.getConfig();context.options.textToSpeech&&(self.enable(),self.ttsButton.show(),self.tts.updateTexthelpCache(config.serviceCallId,this.itemRunner._item.attributes.identifier));
}).on("disabletools unloaditem",function(){self.disable()}),this},enable:function(){this.tts&&this.tts.enable(),this.ttsButton&&this.ttsButton.enable()},disable:function(){this.tts&&this.tts.disable(),this.ttsButton&&this.ttsButton.disable()}})}),define("taoQtiTest/runner/plugins/tools/zoom",["jquery","i18n","ui/hider","ui/transformer","util/shortcut","util/namespace","taoTests/runner/plugin"],function($,__,hider,transformer,shortcut,namespaceHelper,pluginFactory){"use strict";var standard=100,increment=10,threshold={lower:10,upper:200},setZoomLevel=function($target,level){transformer.setTransformOrigin($target,"0 0"),transformer.scale($target,level/100)},resetZoom=function($target){
transformer.reset($target)},forceRepaint=function($target){var sel=$target[0];sel&&(sel.style.display="none",sel.offsetHeight,sel.style.display="")};return pluginFactory({name:"zoom",init:function(){function isConfigured(){var context=testRunner.getTestContext()||{},options=context.options||{};return!!options.zoom}function togglePlugin(){isConfigured()?self.show():self.hide()}function zoomAction(dir){var el,sx,sy,before,after,inc=increment*dir;self.$zoomTarget&&(el=self.$zoomTarget[0],before=el.getBoundingClientRect(),sx=self.$container.scrollLeft(),sy=self.$container.scrollTop(),self.zoom=Math.max(threshold.lower,Math.min(threshold.upper,self.zoom+inc)),self.zoom===standard?resetZoom(self.$zoomTarget):setZoomLevel(self.$zoomTarget,self.zoom),
forceRepaint(self.$zoomTarget),after=el.getBoundingClientRect(),sx=Math.max(0,sx+(after.width-before.width)/2),sy=Math.max(0,sy+(after.height-before.height)/2),self.$container.scrollLeft(sx).scrollTop(sy))}function zoomIn(){self.getState("enabled")!==!1&&zoomAction(1)}function zoomOut(){self.getState("enabled")!==!1&&zoomAction(-1)}var self=this,testRunner=this.getTestRunner(),testData=testRunner.getTestData()||{},testConfig=testData.config||{},pluginShortcuts=(testConfig.shortcuts||{})[this.getName()]||{};this.buttonZoomOut=this.getAreaBroker().getToolbox().createEntry({control:"zoomOut",title:__("Zoom out"),icon:"remove"}),this.buttonZoomIn=this.getAreaBroker().getToolbox().createEntry({
