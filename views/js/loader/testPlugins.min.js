...
"use strict";var checkInterval=3e4;return pluginFactory({name:"connectivity",init:function(){},install:function(){var self=this,waiting=!1,testRunner=this.getTestRunner(),proxy=testRunner.getProxy(),displayWaitingDialog=function(message){var dialog;return new Promise(function(resolve){waiting||(waiting=!0,testRunner.before("pause.waiting",function(){return new Promise(function(pauseResolve){proxy.off("reconnect.pausing").after("reconnect.pausing",pauseResolve)})}),dialog=waitingDialog({message:__("You are encountering a prolonged connectivity loss. ")+message,waitContent:__("Please wait while we try to restore the connection."),proceedContent:__("The connection seems to be back, please proceed")
}).on("proceed",function(){resolve()}).on("render",function(){proxy.off("reconnect.waiting").after("reconnect.waiting",function(){testRunner.off("pause.waiting"),waiting=!1,dialog.endWait()})}))})};this.polling=pollingFactory({action:function(){testRunner.getProxy().telemetry(testRunner.getTestContext().itemIdentifier,"up").catch(_.noop)},interval:checkInterval,autoStart:!1}),this.$element=$(connectivityTpl({state:proxy.isOnline()?"connected":"disconnected"})),proxy.on("disconnect",function(source){testRunner.getState("disconnected")||(testRunner.setState("disconnected",!0),testRunner.trigger("disconnect",source),self.$element.removeClass("connected").addClass("disconnected"),
self.polling.start())}).on("reconnect",function(){testRunner.getState("disconnected")&&(testRunner.setState("disconnected",!1),testRunner.trigger("reconnect"),self.$element.removeClass("disconnected").addClass("connected"),self.polling.stop())}),testRunner.before("leave",function(e,data){if(proxy.isOffline())return displayWaitingDialog(data.message).then(function(){testRunner.trigger("leave",data)}).catch(function(generalErr){testRunner.trigger("error",generalErr)}),!1}),testRunner.before("error",function(e,err){return!proxy.isConnectivityError(err)&&(proxy.isOffline()?(displayWaitingDialog(err.message).then(function(){"nav"===err.type&&testRunner.loadItem(testRunner.getTestContext().itemIdentifier),
"finish"===err.type&&testRunner.finish(),"pause"===err.type&&testRunner.trigger("pause",{reasons:err.data&&err.data.reasons,message:err.data&&err.data.comment})}).catch(function(generalErr){testRunner.trigger("error",generalErr)}),!1):void 0)})},render:function(){var $container=this.getAreaBroker().getControlArea();$container.append(this.$element)}})}),define("taoQtiTest/runner/plugins/controls/duration/duration",["lodash","core/polling","core/timer","core/store","core/promise","taoTests/runner/plugin"],function(_,pollingFactory,timerFactory,store,Promise,pluginFactory){"use strict";var refresh=1e3;return pluginFactory({name:"duration",install:function(){var self=this;
this.getTestRunner().on("storechange",function(){self.shouldClearStorage=!0})},init:function(){var self=this,testRunner=this.getTestRunner();return store("duration-"+testRunner.getConfig().serviceCallId).then(function(durationStore){return self.shouldClearStorage?durationStore.clear().then(function(){return durationStore}):durationStore}).then(function(durationStore){function getItemDuration(attemptId){return/^(.*)+#+\d+$/.test(attemptId)?durationStore.getItem(attemptId):Promise.reject(new Error('Is it really an attempt id, like "itemid#attempt"'))}self.stopwatch=timerFactory({autoStart:!1}),self.polling=pollingFactory({action:function(){var context=testRunner.getTestContext(),itemAttemptId=context.itemIdentifier+"#"+context.attempt;
durationStore.getItem(itemAttemptId).then(function(duration){var elapsed=self.stopwatch.tick();duration=_.isNumber(duration)?duration:0,elapsed=_.isNumber(elapsed)&&elapsed>0?elapsed/1e3:0,durationStore.setItem(itemAttemptId,duration+elapsed)})},interval:refresh,autoStart:!1}),testRunner.after("renderitem",function(){self.enable()}).on("enableitem",function(){self.enable()}).before("move skip exit timeout",function(){var context=testRunner.getTestContext(),itemAttemptId=context.itemIdentifier+"#"+context.attempt;return getItemDuration(itemAttemptId).then(function(duration){var params={itemDuration:0};_.isNumber(duration)&&duration>0&&(params.itemDuration=duration),
...
var answered;return defaultValue=defaultValue||null,answered=!currentItemHelper.isQtiValueNull(response,baseType,cardinality)&&!_.isEqual(response,currentItemHelper.toResponse(defaultValue,baseType,cardinality))},isAnswered:function(runner,partially){var itemRunner=runner.itemRunner,responses=itemRunner&&itemRunner.getResponses(),count=0,empty=0;return itemRunner&&_.forEach(currentItemHelper.getDeclarations(runner),function(declaration){var attributes=declaration.attributes||{},response=responses[attributes.identifier],baseType=attributes.baseType,cardinality=attributes.cardinality;count++,currentItemHelper.isQuestionAnswered(response,baseType,cardinality,declaration.defaultValue)||empty++;
}),partially===!1?count>0&&0===empty:count>0&&empty<count}};return currentItemHelper}),define("taoQtiTest/runner/helpers/stats",["lodash","taoQtiTest/runner/helpers/map","taoQtiTest/runner/helpers/currentItem"],function(_,mapHelper,currentItemHelper){"use strict";function getInstantStats(scope,runner){var isItemCurrentlyAnswered,map=runner.getTestMap(),context=runner.getTestContext(),stats=_.clone(mapHelper.getScopeStats(map,context.itemPosition,scope)),item=mapHelper.getItemAt(map,context.itemPosition);return item.informational||(isItemCurrentlyAnswered=currentItemHelper.isAnswered(runner),!isItemCurrentlyAnswered&&context.itemAnswered?stats.answered--:isItemCurrentlyAnswered&&!context.itemAnswered&&stats.answered++),
stats}return{getInstantStats:getInstantStats}}),define("taoQtiTest/runner/helpers/messages",["lodash","i18n","taoQtiTest/runner/helpers/map","taoQtiTest/runner/helpers/stats"],function(_,__,mapHelper,statsHelper){"use strict";function getExitMessage(message,scope,runner){var itemsCountMessage="",testData=runner.getTestData(),testConfig=testData&&testData.config,messageEnabled=!testConfig||testConfig.enableUnansweredItemsWarning;return messageEnabled&&(itemsCountMessage=getUnansweredItemsWarning(scope,runner)),(itemsCountMessage+" "+message).trim()}function getUnansweredItemsWarning(scope,runner){var stats=statsHelper.getInstantStats(scope,runner),unansweredCount=stats&&stats.questions-stats.answered,flaggedCount=stats&&stats.flagged,itemsCountMessage="";
