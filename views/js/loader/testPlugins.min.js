...
closeDialogs(".@"),dialogShortcut.clear(),dialogShortcut=null})}})}),define("taoQtiTest/runner/plugins/content/dialog/exitMessages",["lodash","core/promise","taoTests/runner/plugin"],function(_,Promise,pluginFactory){"use strict";return pluginFactory({name:"exitMessages",init:function(){},install:function(){var testRunner=this.getTestRunner();testRunner.before("leave",function(e,data){if(_.isObject(data)&&data.message)return new Promise(function(resolve){var context=testRunner.getTestContext();context&&context.itemUri&&testRunner.disableItem(context.itemUri),testRunner.trigger("alert.leave",data.message,resolve)})})}})}),define("taoQtiTest/runner/plugins/content/dialog/itemAlertMessage",["jquery","i18n","ui/hider","taoTests/runner/plugin"],function($,__,hider,pluginFactory){
...
this.$element.addClass("overlay"),overlayConfig.full&&this.$element.addClass("overlay-full")},disable:function(){this.$element.removeClass("overlay overlay-full")},show:function(){this.enable()},hide:function(){this.disable()}})}),define("taoQtiTest/runner/plugins/content/responsiveness/collapser",["lodash","taoTests/runner/plugin"],function(_,pluginFactory){"use strict";var noLabelCls="no-tool-label",noLabelHoverCls="no-tool-label-hover",defaults={collapseTools:!0,collapseNavigation:!1,collapseInOrder:!1,hover:!1,collaspeOrder:[]};return pluginFactory({name:"collapser",init:function(){function getAvailableWidth(){return $actionsBar.width()}function getToolbarWidth(){
return $toolbox.width()+$navigation.width()}function collapseNeeded(){return getToolbarWidth()>getAvailableWidth()}function collapseAll(yes){config.collapseTools&&$toolbox.toggleClass(collapseCls,yes),config.collapseNavigation&&$navigation.toggleClass(collapseCls,yes)}function shouldCollapseInOrder(){return config.collapseInOrder&&_.isArray(config.collapseOrder)&&config.collapseOrder.length}function collapseInOrder(){for(var toCollapse,collapseOrderCopy=_.clone(config.collapseOrder);collapseNeeded()&&collapseOrderCopy.length;)toCollapse=collapseOrderCopy.shift(),$actionsBar.find(toCollapse).toggleClass(collapseCls,!0)}var testRunner=this.getTestRunner(),testData=testRunner.getTestData()||{},testConfig=testData.config||{},pluginsConfig=testConfig.plugins||{},config=_.defaults(pluginsConfig.collapser||{},defaults),areaBroker=testRunner.getAreaBroker(),$actionsBar=areaBroker.getArea("actionsBar"),$toolbox=areaBroker.getToolboxArea(),$navigation=areaBroker.getNavigationArea(),collapseCls=config.hover?noLabelHoverCls:noLabelCls;
testRunner.on("renderitem loaditem",function(){collapseAll(!1),shouldCollapseInOrder()?collapseInOrder():collapseAll(collapseNeeded())})}})}),define("tpl!taoQtiTest/runner/plugins/content/rubricBlock/rubricBlock",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){return this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{},'<div id="qti-rubrics"></div>\n'})}),define("taoQtiTest/runner/plugins/content/rubricBlock/rubricBlock",["jquery","i18n","ui/hider","taoTests/runner/plugin","tpl!taoQtiTest/runner/plugins/content/rubricBlock/rubricBlock"],function($,__,hider,pluginFactory,containerTpl){
"use strict";var blankifyLinks=function($container){$("a",$container).attr("target","_blank")};return pluginFactory({name:"rubricBlock",init:function(){var self=this,testRunner=this.getTestRunner();this.$element=$(containerTpl()),this.hide(),testRunner.on("ready",function(){self.hide()}).on("loadrubricblock",function(rubrics){rubrics&&(self.$element.html(rubrics),blankifyLinks(self.$element),testRunner.trigger("rubricblock"))}).on("renderitem",function(){self.show()}).on("unloaditem",function(){self.hide(),self.$element.empty()})},render:function(){var $container=this.getAreaBroker().getContentArea();$container.before(this.$element)},destroy:function(){this.$element.remove();
},enable:function(){this.$element.removeProp("disabled").removeClass("disabled")},disable:function(){this.$element.prop("disabled",!0).addClass("disabled")},show:function(){hider.show(this.$element)},hide:function(){hider.hide(this.$element)}})}),define("taoQtiTest/runner/plugins/controls/connectivity/connectivity",["jquery","lodash","i18n","taoTests/runner/plugin"],function($,_,__,pluginFactory){"use strict";return pluginFactory({name:"connectivity",init:function(){},install:function(){function disconnect(){testRunner.getState("disconnected")||testRunner.trigger("disconnect")}function reconnect(){testRunner.getState("disconnected")&&testRunner.trigger("reconnect");
}var testRunner=this.getTestRunner();$(window).on("offline.connectivity",function(){disconnect()}),$(window).on("online.connectivity",function(){reconnect()}),testRunner.on("disconnect",function(){testRunner.setState("disconnected",!0)}).on("reconnect",function(){testRunner.setState("disconnected",!1)}).before("error",function(e,error){if(_.isObject(error)&&"network"===error.source&&!error.code)return disconnect(),!1}),testRunner.getProxy().on("receive",function(){reconnect()})},destroy:function(){$(window).off("offline.connectivity"),$(window).off("online.connectivity")}})}),define("taoQtiTest/runner/plugins/controls/duration/duration",["lodash","core/polling","core/timer","core/store","core/promise","taoTests/runner/plugin"],function(_,pollingFactory,timerFactory,store,Promise,pluginFactory){
"use strict";var refresh=1e3;return pluginFactory({name:"duration",init:function(){var self=this,testRunner=this.getTestRunner(),initContext=testRunner.getTestContext(),initItemAttemptId=initContext.itemIdentifier+"#"+initContext.attempt;return store("duration-"+testRunner.getConfig().serviceCallId).then(function(durationStore){function getItemDuration(attemptId){return/^(.*)+#+\d+$/.test(attemptId)?durationStore.getItem(attemptId):Promise.reject(new Error('Is it really an attempt id, like "itemid#attempt"'))}return self.stopwatch=timerFactory({autoStart:!1}),durationStore.getItem(initItemAttemptId).then(function(initDuration){if(initDuration=_.isNumber(initDuration)?initDuration:0,
!initDuration&&initContext.attemptDuration)return durationStore.setItem(initItemAttemptId,initContext.attemptDuration||0)}).then(function(){self.polling=pollingFactory({action:function(){var elapsed=self.stopwatch.tick(),context=testRunner.getTestContext(),itemAttemptId=context.itemIdentifier+"#"+context.attempt;durationStore.getItem(itemAttemptId).then(function(duration){duration=_.isNumber(duration)?duration:0,elapsed=_.isNumber(elapsed)&&elapsed>0?elapsed/1e3:0,durationStore.setItem(itemAttemptId,duration+elapsed)})},interval:refresh,autoStart:!1}),testRunner.after("renderitem enableitem",function(){self.enable()}).before("move disableitem skip error",function(){
self.getState("enabled")&&self.disable()}).before("move skip exit timeout",function(){var context=testRunner.getTestContext(),itemAttemptId=context.itemIdentifier+"#"+context.attempt;return getItemDuration(itemAttemptId).then(function(duration){var params={itemDuration:0};_.isNumber(duration)&&duration>0&&(params.itemDuration=duration),testRunner.getProxy().addCallActionParams(params)})}).on("plugin-get.duration",function(e,attemptId,getDuration){_.isFunction(getDuration)&&getDuration(getItemDuration(attemptId))}).before("finish",function(){return new Promise(function(resolve){durationStore.removeStore().then(resolve).catch(resolve)})})})})},destroy:function(){
this.polling.stop(),this.stopwatch.stop()},enable:function(){this.polling.start(),this.stopwatch.resume()},disable:function(){this.polling.stop(),this.stopwatch.pause()}})}),define("tpl!taoQtiTest/runner/plugins/controls/progressbar/progressbar",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){return this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{},'<div class="progress-box">\n    <div data-control="progress-bar" class="qti-controls progressbar info"></div>\n    <div data-control="progress-label" class="qti-controls"></div>\n</div>\n'})}),define("taoQtiTest/runner/plugins/controls/progressbar/progressbar",["jquery","lodash","i18n","taoTests/runner/plugin","tpl!taoQtiTest/runner/plugins/controls/progressbar/progressbar","ui/progressbar"],function($,_,__,pluginFactory,progressTpl){
"use strict";var progressUpdater=function(testContext,testMap,progressIndicator,progressScope){var updater={percentage:function(){var total=Math.max(1,testContext.numberItems),ratio=Math.floor(testContext.numberCompleted/total*100);return{ratio:ratio,label:ratio+"%"}},position:function(){var getTestPart=function(){if(testMap&&testMap.parts)return testMap.parts[testContext.testPartId]},getTestSection=function(){var testPart=getTestPart();if(testPart&&testPart.sections)return testPart.sections[testContext.sectionId]},progressScopeCounter={test:{total:function(){return Math.max(1,testContext.numberItems)},position:function(){return testContext.itemPosition+1}},testPart:{
total:function(){var testPart=getTestPart();return testPart?_.reduce(testMap.parts[testContext.testPartId].sections,function(acc,section){return acc+_.size(section.items)},0):0},position:function(){var testSection=getTestSection();return testSection?testSection.items[testContext.itemIdentifier].positionInPart+1:0}},testSection:{total:function(){var testSection=getTestSection();return testSection?_.size(testSection.items):0},position:function(){var testSection=getTestSection();return testSection?testSection.items[testContext.itemIdentifier].positionInSection+1:0}}},counter=progressScopeCounter[progressScope]||progressScopeCounter.test,total=counter.total(),currentPosition=counter.position();
return{ratio:total>0?Math.floor(currentPosition/total*100):0,label:__("Item %d of %d",currentPosition,total)}}};return updater[progressIndicator]()};return pluginFactory({name:"progressBar",init:function(){var $progressLabel,$progressControl,testRunner=this.getTestRunner(),testData=testRunner.getTestData(),config=testData.config.progressIndicator||{},progressIndicator=config.type||"percentage",progressScope=config.scope||"test",update=function(){var progressData=progressUpdater(testRunner.getTestContext(),testRunner.getTestMap(),progressIndicator,progressScope);progressData&&$progressLabel&&$progressControl&&($progressLabel.text(progressData.label),$progressControl.progressbar("value",progressData.ratio));
};this.$element=$(progressTpl()),$progressLabel=$('[data-control="progress-label"]',this.$element),$progressControl=$('[data-control="progress-bar"]',this.$element),$progressControl.progressbar(),update(),testRunner.on("ready",update).on("loaditem",update)},render:function(){var $container=this.getAreaBroker().getControlArea();$container.append(this.$element)}})}),define("taoQtiTest/runner/plugins/controls/testState/testState",["lodash","taoTests/runner/plugin"],function(_,pluginFactory){"use strict";return pluginFactory({name:"testState",install:function(){var testRunner=this.getTestRunner();testRunner.getProxy().use(function(req,res,next){var data=res&&res.data;
return data&&data.type&&"TestState"===data.type?(testRunner.setState("closedOrSuspended",!0),void(testRunner.getState("ready")?!_.isEmpty(data.messages)&&_.find(data.messages,{channel:"teststate"})||testRunner.trigger("leave",data):testRunner.trigger("destroy"))):void next()})},init:function(){var testRunner=this.getTestRunner(),isLeaving=!1;testRunner.getProxy().channel("teststate",function(data){isLeaving||!data||"close"!==data.type&&"pause"!==data.type||testRunner.getState("closedOrSuspended")||(isLeaving=!0,testRunner.setState("closedOrSuspended",!0),testRunner.trigger("leave",data))})}})}),define("tpl!taoQtiTest/runner/plugins/controls/timer/timer",["handlebars"],function(hb){
return hb.template(function(Handlebars,depth0,helpers,partials,data){function program1(depth0,data){return" disabled"}this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,buffer="",functionType="function",escapeExpression=this.escapeExpression,self=this;return buffer+='<span class="qti-timer qti-timer__type-',(helper=helpers.type)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.type,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+" qti-controls ",stack1=helpers.unless.call(depth0,depth0&&depth0.running,{hash:{},
inverse:self.noop,fn:self.program(1,program1,data),data:data}),(stack1||0===stack1)&&(buffer+=stack1),buffer+='" data-control="',(helper=helpers.id)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.id,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+'" title="',(helper=helpers.label)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.label,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+'">\n    <span class="qti-timer_label truncate">',(helper=helpers.label)?stack1=helper.call(depth0,{
hash:{},data:data}):(helper=depth0&&depth0.label,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+'</span>\n    <span class="qti-timer_time"></span>\n</span>\n'})}),define("taoQtiTest/runner/plugins/controls/timer/timerComponent",["jquery","lodash","i18n","moment","core/encoder/time","tpl!taoQtiTest/runner/plugins/controls/timer/timer"],function($,_,__,moment,timeEncoder,timerTpl){"use strict";var hasAnimationFrame="requestAnimationFrame"in window,precision=1e3,timerTypes={test:"assessmentTest",testPart:"testPart",section:"assessmentSection",item:"assessmentItemRef"},timerComponentFactory=function(config){
var data;if(!_.isPlainObject(config))throw new TypeError("A timer needs to be configured using a config object");if(_.isEmpty(config.id))throw new TypeError("A timer needs and identifier");if(!_.contains(timerTypes,config.type))throw new TypeError("Invalid type configured");return data=_.defaults(config,{label:"",remaining:0,warnings:{},running:!0}),{init:function(){return this.$element=$(timerTpl(data)),this.$display=$(".qti-timer_time",this.$element),this},render:function($container){return $container.append(this.$element),this},destroy:function(){return this.$element.remove(),this},refresh:function(){var self=this,update=function(){self.$display.text(timeEncoder.encode(data.remaining/precision));
};return hasAnimationFrame?window.requestAnimationFrame(update):update(),this},warn:function(){var remaining,message={},warning=_.findLast(data.warnings,{showed:!1});if(!_.isEmpty(warning)&&_.isFinite(warning.point)&&data.remaining<=warning.point){switch(remaining=moment.duration(data.remaining/precision,"seconds").humanize(),this.$display.removeClass("txt-info txt-warning txt-danger").addClass("txt-"+warning.type),data.type){case"assessmentItemRef":message.text=__("Warning – You have %s remaining to complete this item.",remaining);break;case"assessmentSection":message.text=__("Warning – You have %s remaining to complete this section.",remaining);break;case"testPart":
message.text=__("Warning – You have %s remaining to complete this test part.",remaining);break;case"assessmentTest":message.text=__("Warning – You have %s remaining to complete the test.",remaining)}var showed=_.find(data.warnings,{point:warning.point});showed&&showed.point&&data.warnings[showed.point/precision]&&(data.warnings[showed.point/precision].showed=!0),message.type=warning.type}return message},id:function(){return data.id},val:function(value){return"undefined"!=typeof value?(data.remaining=value,this):data.remaining},running:function(value){return"undefined"!=typeof value?(data.running=!!value,this):!!data.running}}};return timerComponentFactory}),define("taoQtiTest/runner/helpers/map",["lodash"],function(_){
"use strict";function getEmptyStats(){return{questions:0,answered:0,flagged:0,viewed:0,total:0}}return{getJumps:function(map){return map&&map.jumps},getParts:function(map){return map&&map.parts},getJump:function(map,position){var jumps=this.getJumps(map);return jumps&&jumps[position]},getPart:function(map,partName){var parts=this.getParts(map);return parts&&parts[partName]},getSection:function(map,sectionName){var parts=this.getParts(map),section=null;return _.forEach(parts,function(part){var sections=part.sections;if(sections&&sections[sectionName])return section=sections[sectionName],!1}),section},getItem:function(map,itemName){var jump=_.find(this.getJumps(map),{
...
(itemsCountMessage+" "+message).trim()}return{getExitMessage:getExitMessage}}),define("tpl!taoQtiTest/runner/plugins/controls/timer/timers",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){return this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{},'<div class="timer-box"></div>\n'})}),define("taoQtiTest/runner/plugins/controls/timer/timer",["jquery","lodash","i18n","core/promise","core/polling","core/timer","core/store","ui/hider","taoTests/runner/plugin","taoQtiTest/runner/plugins/controls/timer/timerComponent","taoQtiTest/runner/helpers/messages","tpl!taoQtiTest/runner/plugins/controls/timer/timers"],function($,_,__,Promise,pollingFactory,timerFactory,store,hider,pluginFactory,timerComponentFactory,messages,timerBoxTpl){
"use strict";var timerRefresh=1e3,precision=1e3,exitMessage=__("After you complete the section it would be impossible to return to this section to make changes. Are you sure you want to end the section?"),timerTypes={test:"assessmentTest",testPart:"testPart",section:"assessmentSection",item:"assessmentItemRef"},isTimedSection=function(context){var timeConstraints=context.timeConstraints||[];return _.some(timeConstraints,function(constraint){return"assessmentSection"===constraint.qtiClassName})};return pluginFactory({name:"timer",install:function(){var self=this,testRunner=this.getTestRunner();testRunner.on("storechange",function(){self.shouldClearStorage=!0});
},init:function(){function doEnable(){self.getState("enabled")||self.enable()}function doDisable(){self.getState("enabled")&&self.disable()}var self=this,testRunner=this.getTestRunner(),testData=testRunner.getTestData()||{},itemStates=testData.itemStates||{},timerWarning=testData.config&&testData.config.timerWarning||{},displayedTimers={},timers={},extraTime=0,consumedExtraTime=0,lastConsumedExtraTime=0,getRemainingExtraTime=function(){return Math.max(0,extraTime-consumedExtraTime)*precision},setRemainingTime=function(timerConfig,remaining){return timerConfig.remaining=remaining+getRemainingExtraTime(),timerConfig.regular=remaining,timerConfig},getTimerConfig=function(type){
var timeConstraint,timer,closestPreviousWarning,context=testRunner.getTestContext();return context.isTimeout||context.itemSessionState!==itemStates.interacting||(timeConstraint=_.findLast(context.timeConstraints,{qtiClassName:type}),timeConstraint&&(timer=setRemainingTime({label:timeConstraint.label,type:timeConstraint.qtiClassName,id:timeConstraint.source,running:!0,warnings:{}},timeConstraint.seconds*precision),_(timerWarning[timeConstraint.qtiClassName]).forEach(function(value,key){_.contains(["info","warning","danger"],value)&&(timer.warnings[key]={type:value,showed:timer.remaining/precision<=key,point:parseInt(key,10)*precision})}),closestPreviousWarning=_.find(timer.warnings,{
showed:!0}),!_.isEmpty(closestPreviousWarning)&&closestPreviousWarning.point&&timer.warnings[closestPreviousWarning.point/precision]&&(timer.warnings[closestPreviousWarning.point/precision].showed=!1))),timer},leaveTimedSection=function(type,scope,position){var context=testRunner.getTestContext(),map=testRunner.getTestMap(),section=map.parts[context.testPartId].sections[context.sectionId],nbItems=_.size(section.items),item=_.find(section.items,{position:context.itemPosition});return!(context.isTimeout||context.itemSessionState===itemStates.closed||!isTimedSection(context)||!item)&&!!("next"===type&&("assessmentSection"===scope||item.positionInSection+1===nbItems)||"previous"===type&&0===item.positionInSection||"jump"===type&&position>0&&(position<section.position||position>=section.position+nbItems));
},removeTimer=function(type){displayedTimers[type]&&(self.storage.removeItem(displayedTimers[type].id()),displayedTimers[type].destroy(),displayedTimers=_.omit(displayedTimers,type),timers=_.omit(timers,type),self.trigger("removetimer",type))},addTimer=function(type,config){config&&(timers[type]=config.regular,displayedTimers[type]=timerComponentFactory(config),displayedTimers[type].init().render(self.$element),self.trigger("addtimer",type,displayedTimers[type]))},updateTimers=function(checkStorage){var timerUpdatePromises=[],context=testRunner.getTestContext();return context.extraTime&&(extraTime=context.extraTime.total,consumedExtraTime=Math.max(consumedExtraTime,context.extraTime.consumed)),
...
resolve()):resolve()}))}),Promise.all(timerUpdatePromises)};return store("timer-"+testRunner.getConfig().serviceCallId).then(function(timeStore){return self.shouldClearStorage?timeStore.clear().then(function(){return timeStore}):Promise.resolve(timeStore)}).then(function(timeStore){self.storage=timeStore,self.$element=$(timerBoxTpl()),self.stopwatch=timerFactory({autoStart:!1}),self.polling=pollingFactory({action:function(){var timeoutScope,timeoutRef,elapsed=self.stopwatch.tick(),elapsedExtraTime=0,timeout=!1;_.forEach(displayedTimers,function(timer,type){var regularVal,displayedVal,warning,runTimeout=function(){testRunner.timeout(timeoutScope,timeoutRef)};timer.running()&&(regularVal=timers[type]-elapsed,
...
return pluginFactory({name:pluginName,init:function(){function isPluginEnabled(){var context=testRunner.getTestContext()||{},options=context.options||{};return!!options.eliminator}function togglePluginButton(){isPluginEnabled()?self.show():self.hide()}function togglePlugin(){self.$choiceInteractions.toggleClass("eliminable"),isEliminable()?enableEliminator():disableEliminator()}function isEliminable(){return self.$choiceInteractions.hasClass("eliminable")}function enableEliminator(){var $choices=self.$choiceInteractions.find(".qti-choice");self.button.turnOn(),self.trigger("start"),config.restoreEliminationsOnOpen&&$choices.each(function(){var input=this.querySelector(".real-label input");
this.dataset.wasEliminated&&(this.dataset.wasEliminated=null,this.classList.add("eliminated"),input.setAttribute("disabled","disabled"),input.checked=!1)})}function disableEliminator(){var $choices=self.$choiceInteractions.find(".qti-choice");self.$choiceInteractions.removeClass("eliminable"),self.button.turnOff(),self.trigger("end"),$choices.each(function(){this.classList.contains("eliminated")&&(this.dataset.wasEliminated=!0,this.classList.remove("eliminated"),this.querySelector(".real-label input").removeAttribute("disabled"))})}var self=this,testRunner=this.getTestRunner(),$container=testRunner.getAreaBroker().getContentArea().parent(),testConfig=testRunner.getTestData().config||{},pluginShortcuts=(testConfig.shortcuts||{})[pluginName]||{},config=_.defaults(_.clone((testConfig.plugins||{})[pluginName])||{},defaultConfig);
this.button=this.getAreaBroker().getToolbox().createEntry({control:"eliminator",title:__("Eliminate choices"),icon:"eliminate",text:__("Answer Eliminator")}),this.button.on("click",function(e){e.preventDefault(),testRunner.trigger(actionPrefix+"toggle")}),testConfig.allowShortcuts&&_.forEach(pluginShortcuts,function(command,key){shortcut.add(namespaceHelper.namespaceAll(command,pluginName,!0),function(){testRunner.trigger(actionPrefix+key)},{avoidInput:!0})}),this.disable(),testRunner.on("loaditem",togglePluginButton).on("renderitem",function(){return self.$choiceInteractions=$container.find(".qti-choiceInteraction"),self.$choiceInteractions.length?void(isPluginEnabled()&&self.show()):void self.hide();
}).on("enabletools renderitem",function(){self.enable()}).on("disabletools unloaditem",function(){self.disable(),disableEliminator()}).on(actionPrefix+"toggle",function(){isPluginEnabled()&&togglePlugin()}).on("tool-answer-masking-toggle",function(){isEliminable()&&disableEliminator()})},destroy:function(){shortcut.remove("."+pluginName)},enable:function(){this.button.enable()},disable:function(){this.button.disable()},show:function(){this.button.show()},hide:function(){this.button.hide()}})}),define("tpl!taoQtiTest/runner/plugins/tools/answerMasking/tpl/mask",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){return this.compilerInfo=[4,">= 1.0.0"],
helpers=this.merge(helpers,Handlebars.helpers),data=data||{},'<div class="answer-mask">\n    <span class="answer-mask-toggle"></span>\n</div>'})}),define("taoQtiTest/runner/plugins/tools/answerMasking/answerMasking",["lodash","jquery","core/statifier","ui/component","tpl!taoQtiTest/runner/plugins/tools/answerMasking/tpl/mask"],function(_,$,statifier,componentFactory,maskTpl){"use strict";var ns=".answerMasking";return function($contentArea){function createMask($container){return componentFactory(maskApi).setTemplate(maskTpl).on("render",function(){var self=this,$component=this.getElement();$component.on("click"+ns,function(e){e.stopPropagation(),e.preventDefault(),
self.toggle()})}).on("destroy",function(){var $component=this.getElement();$component.off(ns)}).init().render($container).mask()}var answerMasking,allMasks=[],maskApi={toggle:function(){return this.is("masked")?this.reveal():this.mask()},reveal:function(){var $container=this.getContainer();return $container.removeClass("masked"),this.setState("masked",!1),this},mask:function(){var $container=this.getContainer();return $container.addClass("masked"),this.setState("masked",!0),this}};return answerMasking={enable:function(){var $choiceInteractions=$contentArea.find(".qti-choiceInteraction"),$qtiChoices=$contentArea.find(".qti-choice");allMasks=[],$choiceInteractions.addClass("maskable"),
$qtiChoices.each(function(){var $choice=$(this);allMasks.push(createMask($choice))}),this.setState("enabled",!0)},disable:function(){var $choiceInteractions=$contentArea.find(".qti-choiceInteraction");$choiceInteractions.removeClass("maskable"),allMasks.forEach(function(mask){mask.reveal(),mask.destroy()}),allMasks=[],this.setState("enabled",!1)},getMasksState:function(){var state=allMasks.map(function(mask){return mask.is("masked")});return state},setMasksState:function(state){state=state||[],state.forEach(function(masked,index){var mask=allMasks[index];_.isObject(mask)&&_.isFunction(mask.reveal)&&!masked&&mask.reveal()})}},statifier(answerMasking),answerMasking;
...
})}),define("taoQtiTest/runner/plugins/tools/areaMasking/mask",["lodash","ui/movableComponent","tpl!taoQtiTest/runner/plugins/tools/areaMasking/mask"],function(_,movableComponent,areaMaskingTpl){"use strict";function maskingComponentFactory(){var maskComponent=movableComponent({preview:function(){var self=this,delay=this.config.previewDelay||1e3;return!this.is("rendered")||this.is("disabled")||this.is("previewing")||(this.setState("previewing",!0).trigger("preview"),_.delay(function(){self.setState("previewing",!1)},delay)),this}},defaultConfig);return maskComponent.setTemplate(areaMaskingTpl).on("render",function(){var self=this,$element=this.getElement();$element.on("click",".view",function(e){
e.preventDefault(),self.preview()}).on("click",".close",function(e){e.preventDefault(),self.destroy()})}),maskComponent}var defaultConfig={previewDelay:3e3,stackingScope:"test-runner"};return maskingComponentFactory}),define("taoQtiTest/runner/plugins/tools/areaMasking/areaMasking",["jquery","lodash","i18n","ui/hider","util/shortcut","util/namespace","taoTests/runner/plugin","taoQtiTest/runner/plugins/tools/areaMasking/mask"],function($,_,__,hider,shortcut,namespaceHelper,pluginFactory,maskComponent){"use strict";var pluginName="area-masking",actionPrefix="tool-"+pluginName+"-",defaultConfig={max:5,foo:!0};return pluginFactory({name:pluginName,init:function(){
function addMask(){maskComponent().on("render",function(){self.masks.push(this),self.button.turnOn(),self.trigger("maskadd")}).on("destroy",function(){self.masks=_.without(self.masks,this),self.masks.length<config.max&&self.enable(),0===self.masks.length&&(self.button.turnOff(),self.trigger("close")),self.trigger("maskclose")}).init({x:10*self.masks.length,y:10*self.masks.length}).render($container)}function isEnabled(){var context=testRunner.getTestContext(),options=context.options||{};return!!options.areaMasking}function togglePlugin(){isEnabled()?self.show():self.hide()}var self=this,testRunner=this.getTestRunner(),$container=testRunner.getAreaBroker().getContentArea().parent(),testConfig=testRunner.getTestData().config||{},config=_.defaults(_.clone((testConfig.plugins||{})[pluginName])||{},defaultConfig),pluginShortcuts=(testConfig.shortcuts||{})[pluginName]||{};
this.masks=[],this.button=this.getAreaBroker().getToolbox().createEntry({control:"area-masking",text:__("Masking"),title:__("Covers parts of the item"),icon:"eye-slash"}),this.button.on("click",function(e){e.preventDefault(),testRunner.trigger(actionPrefix+"toggle")}),testConfig.allowShortcuts&&_.forEach(pluginShortcuts,function(command,key){shortcut.add(namespaceHelper.namespaceAll(command,pluginName,!0),function(){testRunner.trigger(actionPrefix+key)},{avoidInput:!0})}),this.disable(),testRunner.on("loaditem",togglePlugin).on("enabletools renderitem",function(){self.enable()}).on("disabletools unloaditem",function(){self.disable(),_.invoke(self.masks,"destroy");
}).on(actionPrefix+"toggle",function(){isEnabled()&&(0===self.masks.length&&self.trigger("open"),self.masks.length<config.max?addMask():1===config.max&&_.invoke(self.masks,"destroy"))})},destroy:function(){shortcut.remove("."+pluginName)},enable:function(){this.button.enable()},disable:function(){this.button.disable()},show:function(){this.button.show()},hide:function(){this.button.hide()}})}),define("taoQtiTest/runner/plugins/tools/calculator",["jquery","lodash","i18n","ui/hider","ui/calculator","util/shortcut","util/namespace","taoTests/runner/plugin"],function($,_,__,hider,calculatorFactory,shortcut,namespaceHelper,pluginFactory){"use strict";var _default={
height:360,width:240,top:50,left:10,stackingScope:"test-runner"};return pluginFactory({name:"calculator",init:function(){function isEnabled(){var context=testRunner.getTestContext()||{},options=context.options||{};return!!options.calculator}function togglePlugin(){isEnabled()?self.show():self.hide()}function buildCalculator(calcTpl){self.calculator=calculatorFactory(_.defaults({renderTo:self.$calculatorContainer,replace:!0,draggableContainer:areaBroker.getContainer(),alternativeTemplate:calcTpl||null},_default)).on("show",function(){self.trigger("open"),self.button.turnOn()}).on("hide",function(){self.trigger("close"),self.button.turnOff()}).show()}function toggleCalculator(){
self.getState("enabled")!==!1&&(self.calculator?self.calculator.is("hidden")?self.calculator.show():self.calculator.hide():config.template?require(["tpl!"+config.template.replace(/\.tpl$/,"")],function(calcTpl){buildCalculator(calcTpl)},function(){buildCalculator()}):buildCalculator())}var self=this,testRunner=this.getTestRunner(),areaBroker=this.getAreaBroker(),testData=testRunner.getTestData()||{},testConfig=testData.config||{},pluginsConfig=testConfig.plugins||{},config=pluginsConfig.calculator||{},pluginShortcuts=(testConfig.shortcuts||{})[this.getName()]||{};this.button=this.getAreaBroker().getToolbox().createEntry({control:"calculator",title:__("Open Calculator"),
icon:"table",text:__("Calculator")}),this.$calculatorContainer=$('<div class="widget-calculator">'),this.calculator=null,this.button.on("click",function(e){$(e.target).closest(".widget-calculator").length||(e.preventDefault(),testRunner.trigger("tool-calculator"))}),testConfig.allowShortcuts&&pluginShortcuts.toggle&&shortcut.add(namespaceHelper.namespaceAll(pluginShortcuts.toggle,this.getName(),!0),function(){testRunner.trigger("tool-calculator")},{avoidInput:!0,allowIn:".widget-calculator"}),togglePlugin(),this.disable(),testRunner.on("loaditem",togglePlugin).on("enabletools renderitem",function(){self.enable()}).on("disabletools unloaditem",function(){self.disable(),
self.calculator&&(self.calculator.destroy(),self.calculator=null)}).on("tool-calculator",function(){isEnabled()&&toggleCalculator()})},render:function(){var areaBroker=this.getAreaBroker();areaBroker.getContainer().append(this.$calculatorContainer)},destroy:function(){shortcut.remove("."+this.getName()),this.$calculatorContainer.remove(),this.calculator&&this.calculator.destroy()},enable:function(){this.button.enable()},disable:function(){this.button.disable(),this.calculator&&this.calculator.hide()},show:function(){this.button.show()},hide:function(){this.button.hide(),this.calculator&&this.calculator.hide()}})}),define("tpl!taoQtiTest/runner/plugins/tools/comment/comment",["handlebars"],function(hb){
return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var helper,options,buffer="",helperMissing=helpers.helperMissing,escapeExpression=this.escapeExpression;return buffer+='<div data-control="qti-comment" class="hidden">\n    <textarea data-control="qti-comment-text" placeholder="'+escapeExpression((helper=helpers.__||depth0&&depth0.__,options={hash:{},data:data},helper?helper.call(depth0,"Your comment…",options):helperMissing.call(depth0,"__","Your comment…",options)))+'"></textarea>\n    <button data-control="qti-comment-cancel" class="btn-info small"></span>'+escapeExpression((helper=helpers.__||depth0&&depth0.__,
...
data=data||{};var stack1,helper,options,buffer="",functionType="function",escapeExpression=this.escapeExpression,helperMissing=helpers.helperMissing;return buffer+='<div class="magnifier">\n    <div class="level">',(helper=helpers.level)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.level,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+'</div>\n    <div class="controls close">\n        <a href="#" class="closeMagnifier" data-control="closeMagnifier" title="'+escapeExpression((helper=helpers.__||depth0&&depth0.__,options={hash:{},data:data},helper?helper.call(depth0,"Close Magnifier",options):helperMissing.call(depth0,"__","Close Magnifier",options)))+'"><span class="icon-result-nok"></span></a>\n    </div>\n    <div class="controls zoom">\n        <a href="#" class="control"  data-control="zoomOut" title="'+escapeExpression((helper=helpers.__||depth0&&depth0.__,
options={hash:{},data:data},helper?helper.call(depth0,"Magnify less",options):helperMissing.call(depth0,"__","Magnify less",options)))+'"><span class="icon-remove"></span></a>\n        <a href="#" class="control" data-control="zoomIn" title="'+escapeExpression((helper=helpers.__||depth0&&depth0.__,options={hash:{},data:data},helper?helper.call(depth0,"Magnify more",options):helperMissing.call(depth0,"__","Magnify more",options)))+'"><span class="icon-add"></span></a>\n    </div>\n    <div class="overlay"></div>\n    <div class="inner"></div>\n</div>\n'})}),define("taoQtiTest/runner/plugins/tools/magnifier/magnifierPanel",["jquery","lodash","ui/movableComponent","tpl!taoQtiTest/runner/plugins/tools/magnifier/magnifierPanel"],function($,_,movableComponent,magnifierPanelTpl){
"use strict";function magnifierPanelFactory(config){function scrollInClone(scrollData){var $clonedTarget;controls&&controls.$clone&&scrollData&&scrollData.id&&($clonedTarget=controls.$clone.find("[data-magnifier-scroll="+scrollData.id+"]"),$clonedTarget.length&&(_.isNumber(scrollData.scrollTop)&&($clonedTarget[0].scrollTop=scrollData.scrollTop),_.isNumber(scrollData.scrollLeft)&&($clonedTarget[0].scrollLeft=scrollData.scrollLeft)))}function setScrollingListener(){window.addEventListener("scroll",scrollingListenerCallback,!0)}function removeScrollingListener(){window.removeEventListener("scroll",scrollingListenerCallback,!0)}function applyScrolling(){_.forEach(scrolling,scrollInClone);
}function adjustZoomLevel(level){return Math.max(zoomLevelMin,Math.min(parseFloat(level),zoomLevelMax))}function applyZoomLevel(){controls&&controls.$inner.css({transform:"scale("+zoomLevel+")"})}function showZoomLevel(){var $newZoomLevel;controls&&($newZoomLevel=controls.$zoomLevel.clone(!0).html(zoomLevel),controls.$zoomLevel.before($newZoomLevel).remove(),controls.$zoomLevel=$newZoomLevel)}function updateMaxSize(){var $window=$(window);magnifierPanel.config.maxWidth=$window.width()*maxRatio,magnifierPanel.config.maxHeight=$window.height()*maxRatio}function applySize(){controls&&controls.$clone&&(targetWidth=controls.$target.width(),targetHeight=controls.$target.height(),
controls.$clone.width(targetWidth).height(targetHeight))}function updateZoom(){var position;controls&&controls.$target&&(position=magnifierPanel.getPosition(),position.x+=dx+controls.$target.scrollLeft(),position.y+=dy+controls.$target.scrollTop(),magnifierPanel.zoomAt(position.x,position.y))}function createObserver(){observer=new window.MutationObserver(updateMagnifier)}function startObserver(){controls&&controls.$target&&observer.observe(controls.$target.get(0),{childList:!0,attributes:!0,characterData:!0,subtree:!0}),setScrollingListener()}function stopObserver(){observer.disconnect(),removeScrollingListener()}function translateMagnifier(coordinate,actualSize,magnifierSize){
var delta=0,ratio=zoomLevel;return actualSize&&(delta=actualSize*(zoomLevel-1)/2,ratio=(actualSize*zoomLevel-magnifierSize)/(actualSize-magnifierSize)),coordinate*ratio-delta}function getElementFromPoint(x,y){var el;return controls&&controls.$overlay.addClass("hidden"),el=document.elementFromPoint(x,y),controls&&controls.$overlay.removeClass("hidden"),el}function findSourceNode(node,root,target){var $node=$(node),$root=$(root),$target=$(target),indexes=[$node.index()];return $node.parents().each(function(){var $this=$(this);return!$this.is($root)&&void indexes.push($this.index())}),indexes.pop(),indexes.length?($node=$target,_.forEachRight(indexes,function(index){
if($node=$node.children().eq(index),!$node.length)return!1})):$node=$(),$node}var targetWidth,targetHeight,dx,dy,initConfig=_.defaults(config||{},defaultConfig),zoomLevelMin=parseFloat(initConfig.levelMin),zoomLevelMax=parseFloat(initConfig.levelMax),zoomLevelStep=parseFloat(initConfig.levelStep),zoomLevel=adjustZoomLevel(initConfig.level),maxRatio=parseFloat(initConfig.maxRatio),baseSize=parseInt(initConfig.baseSize,10),zoomSize=baseSize*zoomLevel,$initTarget=null,controls=null,observer=null,scrolling=[],magnifierPanel=movableComponent({getZoomLevel:function(){return zoomLevel},getTarget:function(){return controls&&controls.$target},setTarget:function($newTarget){
return controls?(controls.$target=$newTarget,controls.$viewTarget=null,setScrollingListener(),this.trigger("targetchange",controls.$target),this.update()):$initTarget=$newTarget,this},zoomTo:function(level){return level&&_.isFinite(level)&&(zoomLevel=adjustZoomLevel(level)),applyZoomLevel(),showZoomLevel(),updateMaxSize(),updateZoom(),this.trigger("zoom",zoomLevel),this},zoomBy:function(step){return step&&_.isFinite(step)&&this.zoomTo(zoomLevel+parseFloat(step)),this},zoomIn:function(){return this.zoomBy(zoomLevelStep)},zoomOut:function(){return this.zoomBy(-zoomLevelStep)},zoomAt:function(x,y){var position;controls&&(position=this.translate(x,y),controls.$inner.css({
top:-position.top,left:-position.left}))},translate:function(x,y){return{top:translateMagnifier(y,targetHeight,this.config.height),left:translateMagnifier(x,targetWidth,this.config.width)}},update:function(){return controls&&controls.$target&&(controls.$clone=controls.$target.clone().removeAttr("id"),controls.$clone.find("iframe").remove(),controls.$clone.find("[name],[id],[data-serial]").removeAttr("name id data-serial"),controls.$inner.empty().append(controls.$clone),applySize(),applyZoomLevel(),updateZoom(),updateMaxSize(),applyScrolling(),this.trigger("update")),this}},defaultConfig),updateMagnifier=_.debounce(_.bind(magnifierPanel.update,magnifierPanel),debounceDelay),scrollingListenerCallback=_.throttle(function(event){
var scrollId,scrollData,$target=$(event.target),scrollingTop=event.target.scrollTop,scrollLeft=event.target.scrollLeft;controls&&controls.$clone&&$target.data("magnifier-scroll")?(scrollId=$target.data("magnifier-scroll"),scrollData=_.find(scrolling,{id:scrollId}),scrollData.scrollTop=scrollingTop,scrollData.scrollLeft=scrollLeft,scrollInClone(scrollData)):(scrollId=_.uniqueId("scrolling_"),$target.attr("data-magnifier-scroll",scrollId),scrolling.push({id:scrollId,scrollTop:scrollingTop,scrollLeft:scrollLeft}),magnifierPanel.update())},scrollingDelay);return initConfig.width=zoomSize,initConfig.height=zoomSize/screenRatio,initConfig.minWidth=baseSize*zoomLevelMin,
initConfig.minHeight=baseSize*zoomLevelMin/screenRatio,magnifierPanel.setTemplate(magnifierPanelTpl).on("render",function(){var self=this,$component=this.getElement();this.setState("hidden",!0),dx=($component.outerWidth()-$component.width())/2,dy=($component.outerHeight()-$component.height())/2,controls={$target:$initTarget,$inner:$(".inner",$component),$zoomLevel:$(".level",$component),$overlay:$(".overlay",$component)},$initTarget=null,$component.on("click",".zoom",function(event){var $button=$(event.target).closest(".control"),action=$button.data("control");event.preventDefault(),action&&self[action]&&self[action]()}),$component.on("click",".closeMagnifier",function(event){
event.preventDefault(),self.hide(),self.trigger("close")}),$component.on("click",".overlay",function(event){self.is("noclick")?self.setState("noclick",!1):findSourceNode(getElementFromPoint(event.pageX,event.pageY),controls.$inner,controls.$target).click().focus()}),createObserver(),updateMaxSize(),applyZoomLevel()}).on("dragstart resizestart",function(){this.setState("noclick",!0)}).on("move resize",function(){updateZoom()}).on("show",function(){updateMagnifier(),startObserver()}).on("hide",function(){stopObserver()}).on("resize",function(){updateMaxSize()}).on("destroy",function(){stopObserver(),$initTarget=null,controls=null,observer=null}).init(initConfig),
magnifierPanel}var screenRatio=window.screen.width/window.screen.height,debounceDelay=50,scrollingDelay=20,defaultBaseSize=116,defaultLevelMin=2,defaultLevelMax=8,defaultLevel=defaultLevelMin,defaultConfig={level:defaultLevel,levelMin:defaultLevelMin,levelMax:defaultLevelMax,levelStep:.5,baseSize:defaultBaseSize,width:defaultBaseSize*defaultLevel,height:defaultBaseSize*defaultLevel/screenRatio,minWidth:defaultBaseSize*defaultLevelMin,minHeight:defaultBaseSize*defaultLevelMin/screenRatio,maxRatio:.5,stackingScope:"test-runner"};return magnifierPanelFactory}),define("taoQtiTest/runner/plugins/tools/magnifier/magnifier",["jquery","lodash","i18n","taoTests/runner/plugin","ui/hider","util/shortcut","util/namespace","taoQtiTest/runner/plugins/tools/magnifier/magnifierPanel"],function($,_,__,pluginFactory,hider,shortcut,namespaceHelper,magnifierPanelFactory){
"use strict";var pluginName="magnifier",actionPrefix="tool-"+pluginName+"-",defaultOptions={zoomMin:2,zoomMax:8,zoomStep:.5};return pluginFactory({name:pluginName,init:function(){function getMagnifierPanel(){var $container;return magnifierPanel||($container=testRunner.getAreaBroker().getContainer(),magnifierPanel=magnifierPanelFactory({levelMin:pluginConfig.zoomMin,levelMax:pluginConfig.zoomMax,levelStep:pluginConfig.zoomStep}).on("show",function(){self.trigger("magnifier-show")}).on("hide",function(){self.trigger("magnifier-hide")}).on("zoom",function(level){self.trigger("magnifier-zoom",level)}).on("close",function(){hideMagnifier()}).setTarget($container).render($container.parent()),
self.trigger("magnifier-create")),magnifierPanel}function isEnabled(){var context=testRunner.getTestContext()||{},options=context.options||{};return!!options.magnifier}function togglePlugin(){isEnabled()?self.show():self.hide()}function toggleMagnifier(){self.getState("enabled")&&(self.getState("active")?hideMagnifier():showMagnifier())}function showMagnifier(){getMagnifierPanel(),magnifierPanel.is("hidden")&&magnifierPanel.show(),self.button.turnOn(),testRunner.trigger("plugin-open."+pluginName),self.setState("active",!0)}function hideMagnifier(){self.setState("active",!1),self.button.turnOff(),testRunner.trigger("plugin-close."+pluginName),magnifierPanel&&!magnifierPanel.is("hidden")&&magnifierPanel.hide();
}var self=this,testRunner=this.getTestRunner(),testData=testRunner.getTestData()||{},testConfig=testData.config||{},pluginConfig=_.defaults((testConfig.plugins||{})[pluginName]||{},defaultOptions),pluginShortcuts=(testConfig.shortcuts||{})[pluginName]||{},magnifierPanel=null;this.button=this.getAreaBroker().getToolbox().createEntry({control:"magnify",title:__("Displays a customisable magnifier"),text:__("Magnifying Glass"),icon:"find"}),this.button.on("click",function(event){event.preventDefault(),testRunner.trigger(actionPrefix+"toggle")}),testConfig.allowShortcuts&&_.forEach(pluginShortcuts,function(command,key){shortcut.add(namespaceHelper.namespaceAll(command,pluginName,!0),function(){
testRunner.trigger(actionPrefix+key)},{avoidInput:!0})}),togglePlugin(),this.disable(),testRunner.on("loaditem",function(){togglePlugin(),self.disable()}).on("renderitem",function(){magnifierPanel&&magnifierPanel.update().zoomTo(pluginConfig.zoomMin)}).on("enabletools renderitem",function(){self.enable()}).on("disabletools unloaditem",function(){self.getState("active")&&hideMagnifier(),self.disable()}).on("destroy",function(){magnifierPanel&&magnifierPanel.destroy(),magnifierPanel=null}).on(actionPrefix+"toggle",function(){isEnabled()&&toggleMagnifier()}).on(actionPrefix+"in",function(){isEnabled()&&self.getState("enabled")&&self.getState("active")&&getMagnifierPanel().zoomIn();
