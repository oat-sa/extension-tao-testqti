...
navigableTrees=navigableDomElement.createFromDoms($trees),navigableTrees.length&&(itemsNavigator=keyNavigator({id:"navigator-items",replace:!0,elements:navigableTrees,defaultPosition:function(navigables){var pos=0;return filterCursor&&"flagged"!==filterCursor.navigable.getElement().data("mode")&&_.forIn(navigables,function(navigable,i){var $parent=navigable.getElement().parent(".qti-navigator-item");return $parent.hasClass("active")&&$parent.is(":visible")?(pos=i,!1):void 0}),pos}}).on("down",function(){this.next()}).on("up",function(){this.previous()}).on("right",function(){filtersNavigator&&filtersNavigator.focus().next()}).on("left",function(){filtersNavigator&&filtersNavigator.focus().previous();
...
closeDialogs(".@"),dialogShortcut.clear(),dialogShortcut=null})}})}),define("taoQtiTest/runner/plugins/content/dialog/exitMessages",["lodash","core/promise","taoTests/runner/plugin"],function(_,Promise,pluginFactory){"use strict";return pluginFactory({name:"exitMessages",init:function(){},install:function(){var testRunner=this.getTestRunner();testRunner.before("leave",function(e,data){return _.isObject(data)&&data.message?new Promise(function(resolve){var context=testRunner.getTestContext();context&&context.itemUri&&testRunner.disableItem(context.itemUri),testRunner.trigger("alert.leave",data.message,resolve)}):void 0})}})}),define("taoQtiTest/runner/plugins/content/dialog/itemAlertMessage",["jquery","i18n","ui/hider","taoTests/runner/plugin"],function($,__,hider,pluginFactory){
...
}var testRunner=this.getTestRunner();$(window).on("offline.connectivity",function(){disconnect()}),$(window).on("online.connectivity",function(){reconnect()}),testRunner.on("disconnect",function(){testRunner.setState("disconnected",!0)}).on("reconnect",function(){testRunner.setState("disconnected",!1)}).before("error",function(e,error){return _.isObject(error)&&"network"===error.source&&!error.code?(disconnect(),!1):void 0}),testRunner.getProxy().on("receive",function(){reconnect()})},destroy:function(){$(window).off("offline.connectivity"),$(window).off("online.connectivity")}})}),define("taoQtiTest/runner/plugins/controls/duration/duration",["lodash","core/polling","core/timer","core/store","core/promise","taoTests/runner/plugin"],function(_,pollingFactory,timerFactory,store,Promise,pluginFactory){
"use strict";var refresh=1e3;return pluginFactory({name:"duration",init:function(){var self=this,testRunner=this.getTestRunner(),initContext=testRunner.getTestContext(),initItemAttemptId=initContext.itemIdentifier+"#"+initContext.attempt;return store("duration-"+testRunner.getConfig().serviceCallId).then(function(durationStore){function getItemDuration(attemptId){return/^(.*)+#+\d+$/.test(attemptId)?durationStore.getItem(attemptId):Promise.reject(new Error('Is it really an attempt id, like "itemid#attempt"'))}return self.stopwatch=timerFactory({autoStart:!1}),durationStore.getItem(initItemAttemptId).then(function(initDuration){return initDuration=_.isNumber(initDuration)?initDuration:0,
!initDuration&&initContext.attemptDuration?durationStore.setItem(initItemAttemptId,initContext.attemptDuration||0):void 0}).then(function(){self.polling=pollingFactory({action:function(){var elapsed=self.stopwatch.tick(),context=testRunner.getTestContext(),itemAttemptId=context.itemIdentifier+"#"+context.attempt;durationStore.getItem(itemAttemptId).then(function(duration){duration=_.isNumber(duration)?duration:0,elapsed=_.isNumber(elapsed)&&elapsed>0?elapsed/1e3:0,durationStore.setItem(itemAttemptId,duration+elapsed)})},interval:refresh,autoStart:!1}),testRunner.after("renderitem enableitem",function(){self.enable()}).before("move disableitem skip error",function(){
...
isItemCurrentlyAnswered&&!context.itemAnswered&&stats.answered++),stats}return{getInstantStats:getInstantStats}}),define("taoQtiTest/runner/helpers/messages",["i18n","taoQtiTest/runner/helpers/map","taoQtiTest/runner/helpers/stats"],function(__,mapHelper,statsHelper){"use strict";function getExitMessage(message,scope,runner){var stats=statsHelper.getInstantStats(scope,runner),unansweredCount=stats&&stats.questions-stats.answered,flaggedCount=stats&&stats.flagged,itemsCountMessage="",testData=runner.getTestData(),testConfig=testData&&testData.config,messageEnabled=testConfig?testConfig.enableUnansweredItemsWarning:!0;return messageEnabled&&(flaggedCount&&unansweredCount?itemsCountMessage=__("You have %s unanswered question(s) and have %s item(s) marked for review.",unansweredCount.toString(),flaggedCount.toString()):flaggedCount?itemsCountMessage=__("You have %s item(s) marked for review.",flaggedCount.toString()):unansweredCount&&(itemsCountMessage=__("You have %s unanswered question(s).",unansweredCount.toString()))),
...
displayedVal=timer.val()-elapsed,timers[type]=Math.max(regularVal,0),timer.val(Math.max(displayedVal,0)).refresh(),0>=regularVal&&(elapsedExtraTime=Math.max(elapsedExtraTime,Math.abs(regularVal))),0>=displayedVal?(timer.running(!1),timeout=!0,timeoutRef=timer.id(),timeoutScope=type,self.storage.setItem(timer.id(),0).then(runTimeout)["catch"](runTimeout)):(self.storage.setItem(timer.id(),timers[type]),warning=timer.warn(),_.isEmpty(warning)||testRunner.trigger(warning.type,warning.text)))}),elapsedExtraTime&&(consumedExtraTime+=elapsedExtraTime/precision,lastConsumedExtraTime+=elapsedExtraTime),timeout&&self.disable()},interval:timerRefresh,autoStart:!1}),testRunner.before("renderitem resumeitem",function(){
...
"use strict";return pluginFactory({name:"previous",init:function(){function doPrevious(previousItemWarning){function enable(){testRunner.trigger("enablenav enabletools")}var context=testRunner.getTestContext();self.getState("enabled")!==!1&&(testRunner.trigger("disablenav disabletools"),previousItemWarning&&-1!==context.remainingAttempts?testRunner.trigger("confirm.previous",__("You are about to go to the previous item. Click OK to continue and go to the previous item."),testRunner.previous,enable):testRunner.previous())}var self=this,testRunner=this.getTestRunner(),testData=testRunner.getTestData(),testConfig=testData.config||{},pluginShortcuts=(testConfig.shortcuts||{})[this.getName()]||{},canDoPrevious=function(){
...
text:__("Hide Review")}};return pluginFactory({name:"review",init:function(){function isPluginAllowed(){var context=testRunner.getTestContext();return navigatorConfig.enabled&&context&&context.options&&context.options.reviewScreen}function flagItem(position,flag){return self.disable(),testRunner.getProxy().callTestAction("flagItem",{position:position,flag:flag}).then(function(){var context=testRunner.getTestContext();context.itemPosition===position&&(context.itemFlagged=flag),updateButton(self.flagItemButton,getFlagItemButtonData(context)),self.navigator.setItemFlag(position,flag),self.enable()})["catch"](function(){self.navigator.setItemFlag(position,!flag),
self.enable()})}function flagCurrentItem(){var context=testRunner.getTestContext();self.getState("enabled")!==!1&&flagItem(context.itemPosition,!context.itemFlagged)}function togglePanel(forcedState){var isHidden=_.isUndefined(forcedState)?self.navigator.is("hidden"):forcedState;isHidden?(self.explicitlyHidden=!1,self.navigator.show()):(self.explicitlyHidden=!0,self.navigator.hide()),updateButton(self.toggleButton,getToggleButtonData(self.navigator))}var previousItemPosition,self=this,testRunner=this.getTestRunner(),testData=testRunner.getTestData(),testContext=testRunner.getTestContext(),testMap=testRunner.getTestMap(),testConfig=testData.config||{},pluginShortcuts=(testConfig.shortcuts||{})[this.getName()]||{},navigatorConfig=testConfig.review||{};
...
_.delay(function(){self.setState("previewing",!1)},delay)),this}},defaultConfig);return maskComponent.setTemplate(areaMaskingTpl).on("render",function(){var self=this,$element=this.getElement();$element.on("click",".view",function(e){e.preventDefault(),self.preview()}).on("click",".close",function(e){e.preventDefault(),self.destroy()})}),maskComponent}var defaultConfig={previewDelay:3e3,stackingScope:"test-runner"};return maskingComponentFactory}),define("taoQtiTest/runner/plugins/tools/areaMasking/areaMasking",["jquery","lodash","i18n","ui/hider","util/shortcut","util/namespace","taoTests/runner/plugin","taoQtiTest/runner/plugins/tools/areaMasking/mask"],function($,_,__,hider,shortcut,namespaceHelper,pluginFactory,maskComponent){
