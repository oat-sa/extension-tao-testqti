...
"use strict";function getEmptyDetails(){return{position:0,reached:0,viewed:0,completed:0,total:0}}function updateDetails(stats,element,position){element.position<=position&&stats.position++,element.stats.viewed&&(stats.reached++,element.stats.viewed===element.stats.total&&stats.viewed++),element.stats.answered&&element.stats.answered===element.stats.questions&&stats.completed++,stats.total++}function updateItemDetails(stats,element,position){element.position<=position&&stats.position++,element.viewed&&(stats.reached++,stats.viewed++),element.answered&&stats.completed++,stats.total++}function getCategoriesToMatch(categories){var matchSize=categories&&categories.length;
return matchSize&&_.reduce(categories,function(map,category){return map[category]=!0,map},{})}function getDetailedStats(testMap,currentItem,config){var categoriesToMatch,matchSize,stats=_.clone(testMap.stats);return"categories"===config.indicator&&(categoriesToMatch=getCategoriesToMatch(config.categories),matchSize=config.categories&&config.categories.length,stats.matchedCategories=getEmptyDetails()),stats.parts=getEmptyDetails(),stats.sections=getEmptyDetails(),stats.answerableParts=getEmptyDetails(),stats.answerableSections=getEmptyDetails(),_.forEach(testMap.parts,function(part){updateDetails(stats.parts,part,currentItem.position),part.stats.questions>0&&updateDetails(stats.answerableParts,part,currentItem.position),
_.forEach(part.sections,function(section){updateDetails(stats.sections,section,currentItem.position),section.stats.questions>0&&updateDetails(stats.answerableSections,section,currentItem.position),"categories"===config.indicator&&_.forEach(section.items,function(item){matchCategories(item.categories,categoriesToMatch,matchSize)&&updateItemDetails(stats.matchedCategories,item,currentItem.position)})})}),stats}function matchCategories(categories,expectedCategories,minWanted){var matched=0;return expectedCategories&&_.forEach(categories,function(category){if(expectedCategories[category]&&(matched++,matched>=minWanted))return!1}),matched===minWanted}function getRatio(position,total){
return position&&total>0?Math.floor(position/total*100):0}function getProgressionLabel(position,total,type,config){var patterns=labels[type]||labels.item,pattern=config.showTotal?patterns.long:patterns.short;return format(pattern,position||"0",total||"0")}function getPositionProgression(position,total,type,config){return{position:position||0,total:total||0,ratio:getRatio(position,total),label:getProgressionLabel(position,total,type,config)}}function getRatioProgression(position,total){var ratio=getRatio(position,total);return{position:position||0,total:total||0,ratio:ratio,label:ratio+"%"}}var defaultConfig={scope:"test",indicator:"percentage",showTotal:!0,categories:[]
},labels={item:{long:__("Item %d of %d"),short:__("Item %d")},section:{long:__("Section %d of %d"),short:__("Section %d")}},scopes={test:function(testMap,testContext,config){var scopedMap=mapHelper.getScopeMap(testMap,testContext.itemPosition,"test"),item=mapHelper.getItemAt(scopedMap,testContext.itemPosition),stats=getDetailedStats(scopedMap,item,config);return stats.position=item.position+1,stats.completed=testContext.numberCompleted,stats.overall=testContext.numberItems,stats},testPart:function(testMap,testContext,config){var scopedMap=mapHelper.getScopeMap(testMap,testContext.itemPosition,"testPart"),item=mapHelper.getItemAt(scopedMap,testContext.itemPosition),stats=getDetailedStats(scopedMap,item,config);
return stats.position=item.positionInPart+1,stats.completed=testContext.numberCompleted,stats.overall=testContext.numberItems,stats},testSection:function(testMap,testContext,config){var scopedMap=mapHelper.getScopeMap(testMap,testContext.itemPosition,"testSection"),item=mapHelper.getItemAt(scopedMap,testContext.itemPosition),stats=getDetailedStats(scopedMap,item,config);return stats.position=item.positionInSection+1,stats.completed=testContext.numberCompleted,stats.overall=testContext.numberItems,stats}},indicators={percentage:function(stats){return getRatioProgression(stats.completed,stats.overall)},position:function(stats,config){return getPositionProgression(stats.position,stats.total,"item",config);
},questions:function(stats,config){return getPositionProgression(stats.questionsViewed,stats.questions,"item",config)},sections:function(stats,config){return getPositionProgression(stats.answerableSections.reached,stats.answerableSections.total,"section",config)},categories:function(stats,config){return getPositionProgression(stats.matchedCategories.position,stats.matchedCategories.total,"item",config)}};return{isMatchedCategories:function(categories,expectedCategories){var categoriesToMatch=getCategoriesToMatch(expectedCategories),matchSize=expectedCategories&&expectedCategories.length;return matchCategories(categories,categoriesToMatch,matchSize)},computeStats:function(testMap,testContext,config){
var statsComputer=config.scope&&scopes[config.scope]||scopes.test;return statsComputer(testMap,testContext,config||defaultConfig)},computeIndicator:function(stats,type,config){var indicatorComputer=type&&indicators[type]||indicators.percentage;return indicatorComputer(stats||{},config||defaultConfig)},computeProgress:function(testMap,testContext,config){var progressData;return config=_.defaults(config||{},defaultConfig),progressData=this.computeStats(testMap,testContext,config),this.computeIndicator(progressData,config.indicator,config)}}}),define("tpl!taoQtiTest/runner/plugins/controls/progressbar/renderer/percentage",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){
...
},show:function(){this.renderer&&this.renderer.show()},hide:function(){this.renderer&&this.renderer.hide()}})}),define("taoQtiTest/runner/plugins/controls/progressbar/progressbar2",["lodash","taoTests/runner/plugin","taoQtiTest/runner/helpers/map","taoQtiTest/runner/plugins/controls/progressbar/progress","taoQtiTest/runner/plugins/controls/progressbar/renderer/percentage","taoQtiTest/runner/plugins/controls/progressbar/renderer/position"],function(_,pluginFactory,mapHelper,progressHelper,percentageRendererFactory,positionRendererFactory){"use strict";var renderers={percentage:percentageRendererFactory,position:positionRendererFactory};return pluginFactory({name:"progressBar2",
init:function(){var testRunner=this.getTestRunner(),testData=testRunner.getTestData(),config=_.defaults(this.getConfig(),testData.config.progressIndicator||{}),self=this,rendererFactory=renderers[config.renderer]||renderers.percentage,progressConfig={indicator:"percentage2",scope:config.scope||"test",showLabel:config.showLabel,showTotal:config.showTotal,categories:config.categories},hiddenByQuestions=function(item){return item&&item.informational&&"questions"===progressConfig.indicator},hiddenByCategories=function(item){return item&&"categories"===progressConfig.indicator&&!progressHelper.isMatchedCategories(item.categories,progressConfig.categories)},isProgressbarHidden=function(item){
return hiddenByQuestions(item)||hiddenByCategories(item)},update=function(){var data,testContext=testRunner.getTestContext(),testMap=testRunner.getTestMap(),item=mapHelper.getItemAt(testMap,testContext.itemPosition);isProgressbarHidden(item)?self.renderer.hide():(self.renderer.show(),data=progressHelper.computeProgress(testMap,testContext,progressConfig),data.label+=" (old)",self.renderer.update(data))};this.renderer=rendererFactory(progressConfig),update(),testRunner.on("ready loaditem",update)},render:function(){var $container=this.getAreaBroker().getControlArea();this.renderer.render($container)},destroy:function(){this.renderer&&this.renderer.destroy(),this.renderer=null;
...
name:"guidedNavigation",setUp:function(){testRunner.trigger("hidenav")},complete:function(){testRunner.trigger("disableitem disablenav shownav"),setTimeout(function(){testRunner.trigger("move","next","item")},500)}}}}),define("taoQtiTest/runner/plugins/controls/timer/strategy/timeout",[],function(){"use strict";return function(testRunner,timer){return!(!timer||"max"!==timer.type)&&{name:"timeout",complete:function(){if(timer.qtiClassName&&timer.source)return testRunner.timeout(timer.qtiClassName,timer.source)}}}}),define("taoQtiTest/runner/helpers/currentItem",["lodash"],function(_){"use strict";var responseCardinalities={single:"base",multiple:"list",ordered:"list",
record:"record"},currentItemHelper={getDeclarations:function(runner){var itemRunner=runner.itemRunner;return itemRunner._item&&itemRunner._item.responses},getResponseDeclaration:function(runner,identifier){var found=null;return _.forEach(currentItemHelper.getDeclarations(runner),function(declaration){var attributes=declaration.attributes||{};if(attributes.identifier===identifier)return found=declaration,!1}),found},toResponse:function(value,baseType,cardinality){var mappedCardinality=responseCardinalities[cardinality],response={};return _.isString(value)&&(value=[value]),value=_.map(value||[],function(v){return"boolean"===baseType?v===!0||"true"===v:v}),mappedCardinality&&("base"===mappedCardinality?0===value.length?response.base=null:(response.base={},
response.base[baseType]=value[0]):(response[mappedCardinality]={},response[mappedCardinality][baseType]=value)),response},isQtiValueNull:function(value,baseType,cardinality){var mappedCardinality=responseCardinalities[cardinality];return _.isObject(value)&&null===value[mappedCardinality]&&(value=null),_.isObject(value)&&value[mappedCardinality]&&"undefined"!=typeof value[mappedCardinality][baseType]&&(value=value[mappedCardinality][baseType]),null===value||"string"===baseType&&_.isEmpty(value)||"single"!==cardinality&&_.isEmpty(value)},isQuestionAnswered:function(response,baseType,cardinality,defaultValue){var answered;return defaultValue=defaultValue||null,answered=!currentItemHelper.isQtiValueNull(response,baseType,cardinality)&&!_.isEqual(response,currentItemHelper.toResponse(defaultValue,baseType,cardinality));
},isAnswered:function(runner,partially){var itemRunner=runner.itemRunner,responses=itemRunner&&itemRunner.getResponses(),count=0,empty=0;return itemRunner&&_.forEach(currentItemHelper.getDeclarations(runner),function(declaration){var attributes=declaration.attributes||{},response=responses[attributes.identifier],baseType=attributes.baseType,cardinality=attributes.cardinality;count++,currentItemHelper.isQuestionAnswered(response,baseType,cardinality,declaration.defaultValue)||empty++}),partially===!1?count>0&&0===empty:count>0&&empty<count}};return currentItemHelper}),define("taoQtiTest/runner/helpers/stats",["lodash","taoQtiTest/runner/helpers/map","taoQtiTest/runner/helpers/currentItem"],function(_,mapHelper,currentItemHelper){
"use strict";function getInstantStats(scope,runner,sync){var isItemCurrentlyAnswered,map=runner.getTestMap(),context=runner.getTestContext(),stats=_.clone(mapHelper.getScopeStats(map,context.itemPosition,scope)),item=mapHelper.getItemAt(map,context.itemPosition);return item.informational||(isItemCurrentlyAnswered=currentItemHelper.isAnswered(runner),!isItemCurrentlyAnswered&&context.itemAnswered?stats.answered--:!isItemCurrentlyAnswered&&!sync||context.itemAnswered?sync&&!isItemCurrentlyAnswered&&context.itemAnswered&&context.isLinear&&stats.answered++:stats.answered++),stats}return{getInstantStats:getInstantStats}}),define("taoQtiTest/runner/helpers/messages",["lodash","i18n","taoQtiTest/runner/helpers/map","taoQtiTest/runner/helpers/stats"],function(_,__,mapHelper,statsHelper){
"use strict";function getExitMessage(message,scope,runner,sync){var itemsCountMessage="",testData=runner.getTestData(),testConfig=testData&&testData.config,messageEnabled=!testConfig||testConfig.enableUnansweredItemsWarning;return messageEnabled&&(itemsCountMessage=getUnansweredItemsWarning(scope,runner,sync)),(itemsCountMessage+" "+message).trim()}function getUnansweredItemsWarning(scope,runner,sync){var stats=statsHelper.getInstantStats(scope,runner,sync),unansweredCount=stats&&stats.questions-stats.answered,flaggedCount=stats&&stats.flagged,itemsCountMessage="";return"section"===scope||"testSection"===scope?(itemsCountMessage=0===unansweredCount?__("You answered all %s question(s) in this section",stats.questions.toString()):__("You answered only %s of the %s question(s) in this section",stats.answered.toString(),stats.questions.toString()),
flaggedCount&&(itemsCountMessage+=", "+__("and flagged %s of them",flaggedCount.toString()))):"test"===scope?(itemsCountMessage=0===unansweredCount?__("You answered all %s question(s) in this test",stats.questions.toString()):__("You have %s unanswered question(s)",unansweredCount.toString()),flaggedCount&&(itemsCountMessage+=" "+__("and you flagged %s item(s) that you can review now",flaggedCount.toString()))):"part"===scope&&(itemsCountMessage=0===unansweredCount?__("You answered all %s question(s)",stats.questions.toString()):__("You have %s unanswered question(s)",unansweredCount.toString()),flaggedCount&&(itemsCountMessage+=" "+__("and you flagged %s item(s) that you can review now",flaggedCount.toString()))),
itemsCountMessage&&(itemsCountMessage+="."),itemsCountMessage}return{getExitMessage:getExitMessage}}),define("taoQtiTest/runner/helpers/navigation",["lodash","taoQtiTest/runner/helpers/map"],function(_,mapHelper){"use strict";var navigationHelper={isLeavingSection:function(testContext,testMap,direction,scope,position){var section,sectionStats,nbItems,item;if(_.isPlainObject(testContext)&&_.isPlainObject(testMap)&&!_.isEmpty(testContext.sectionId)&&!_.isEmpty(testContext.itemIdentifier))return section=mapHelper.getSection(testMap,testContext.sectionId),sectionStats=mapHelper.getSectionStats(testMap,testContext.sectionId),nbItems=sectionStats&&sectionStats.total,
...
return timer.type=type,"max"===type?timer.id=constraintData.source:timer.id=type+"-"+constraintData.scope+"-"+constraintData.source,"min"===type?(timer.originalTime=constraintData.minTime*precision,timer.remainingTime=constraintData.minTimeRemaining*precision):(timer.originalTime=constraintData.maxTime*precision,timer.remainingTime=constraintData.maxTimeRemaining*precision),timer.extraTime>0&&(timer.extraTime=timer.extraTime*precision),"max"===type&&_.isArray(constraintsWarnings[timer.scope])&&(timer.warnings=constraintsWarnings[timer.scope]),timer};return _.forEach(timeConstraints,function(timeConstraint){var newTimer,constraintData=_.clone(timeConstraint);constraintData.scope=getScope(timeConstraint.scope||timeConstraint.qtiClassName),
constraintData.scope?constraintData.minTime===!1&&constraintData.maxTime===!1?logger.warning("Time constraint defined with no time, skipping"):config.guidedNavigation&&isLinear&&constraintData.maxTime&&constraintData.minTime&&constraintData.minTime===constraintData.maxTime&&constraintData.maxTime>0?(newTimer=buildTimer("locked",constraintData),timers[newTimer.id]=newTimer):(isLinear&&constraintData.minTime&&constraintData.minTime>0&&(newTimer=buildTimer("min",constraintData),timers[newTimer.id]=newTimer),constraintData.maxTime&&constraintData.maxTime>0&&(newTimer=buildTimer("max",constraintData),timers[newTimer.id]=newTimer)):logger.warning("Wrong data, a time constraint should apply to a valid scope, skipping");
}),logger.debug("Timers built from timeConstraints",timers),timers}}),define("taoQtiTest/runner/plugins/controls/timer/plugin",["jquery","lodash","core/promise","taoTests/runner/plugin","taoQtiTest/runner/plugins/controls/timer/strategy/strategyHandler","taoQtiTest/runner/plugins/controls/timer/component/timerbox","taoQtiTest/runner/plugins/controls/timer/timers"],function($,_,Promise,pluginFactory,getStrategyHandler,timerboxFactory,timersFactory){"use strict";return pluginFactory({name:"timer",install:function(){this.loadTimers=function(timeStore,timeConstraints,isLinear,config){var timers=timersFactory(timeConstraints,isLinear,config);return Promise.all(_.map(timers,function(timer){
return timeStore.getItem(timer.id).then(function(savedTime){_.isNumber(savedTime)&&savedTime>=0&&(timer.remainingTime=savedTime),_.isNumber(timer.extraTime)&&timer.extraTime>0&&(timer.remainingWithoutExtraTime=timer.remainingTime,timer.remainingTime=timer.remainingTime+timer.extraTime)})})).then(function(){return timers})},this.saveTimers=function(timeStore,timers){return Promise.all(_.map(timers,function(timer){return _.isNumber(timer.extraTime)&&timer.extraTime>0&&timer.remainingWithoutExtraTime?(timer.remainingWithoutExtraTime=Math.max(0,timer.remainingTime-timer.extraTime),timeStore.setItem(timer.id,timer.remainingWithoutExtraTime)):timeStore.setItem(timer.id,timer.remainingTime);
}))},this.getTestRunner().getTestStore().setVolatile(this.getName())},init:function(){var self=this,testRunner=this.getTestRunner(),testData=testRunner.getTestData(),config=_.merge({},this.getConfig(),{contextualWarnings:!1,warnings:testData&&testData.config&&testData.config.timerWarning||{},guidedNavigation:testData&&testData.config&&testData.config.guidedNavigation}),strategyHandler=getStrategyHandler(testRunner),handleError=function(err){testRunner.trigger("error",err)};return new Promise(function(resolve){return testRunner.getPluginStore(self.getName()).then(function(timeStore){testRunner.before("renderitem resumeitem",function(){var testContext=testRunner.getTestContext();
if(self.timerbox&&testContext.timeConstraints)return self.loadTimers(timeStore,testContext.timeConstraints,!!testContext.isLinear,config).then(function(timers){return self.timerbox.update(timers)}).catch(handleError)}).on("enableitem renderitem",function(){self.timerbox&&self.timerbox.start()}).on("disableitem move skip",function(){self.timerbox&&self.timerbox.stop()}),timeStore.getItem("zen-mode").then(function(startZen){self.timerbox=timerboxFactory({zenMode:{enabled:!0,startHidden:!!startZen},displayWarning:config.contextualWarnings}).on("change",_.throttle(function(){self.saveTimers(timeStore,this.getTimers())},1e3)).on("timeradd",function(timer){strategyHandler.setUp(timer).catch(handleError);
}).on("timerremove",function(timer){strategyHandler.tearDown(timer).catch(handleError)}).on("timerstart",function(timer){strategyHandler.start(timer).catch(handleError)}).on("timerstop",function(timer){strategyHandler.stop(timer).catch(handleError)}).on("timerend",function(timer){strategyHandler.complete(timer).catch(handleError)}).on("timerchange",function(action,timer){self.trigger(action+"timer",timer.qtiClassName,timer)}).on("zenchange",function(isZen){timeStore.setItem("zen-mode",!!isZen)}).on("init",resolve).on("error",handleError),config.contextualWarnings||self.timerbox.on("warn",function(message,level){level&&message&&testRunner.trigger(level,message);
})}).catch(handleError)})})},render:function(){this.timerbox.render(this.getAreaBroker().getControlArea())},destroy:function(){this.timerbox&&this.timerbox.stop().destroy()},show:function(){this.timerbox&&this.timerbox.show()},hide:function(){this.timerbox&&this.timerbox.hide()}})}),define("tpl!taoQtiTest/runner/plugins/controls/title/title",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){function program1(depth0,data){var stack1,helper,buffer="";return buffer+='\n        <span data-control="',(helper=helpers.control)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.control,stack1=typeof helper===functionType?helper.call(depth0,{
hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+'" class="qti-controls" title="',(helper=helpers.text)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.text,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+'">',stack1=helpers.unless.call(depth0,null==data||data===!1?data:data.first,{hash:{},inverse:self.noop,fn:self.program(2,program2,data),data:data}),(stack1||0===stack1)&&(buffer+=stack1),(helper=helpers.text)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.text,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),
...
},init:function(){function changeTheme(themeId){var $qtiItem=$(".qti-item"),previousTheme=state.selectedTheme;state.selectedTheme=themeId,$qtiItem&&_.defer(function(){$qtiItem.trigger("themechange",[state.selectedTheme])}),self.storage&&self.storage.setItem("itemThemeId",themeId),previousTheme!==state.selectedTheme&&testRunner.trigger("themechange",state.selectedTheme,previousTheme),allMenuEntries.forEach(function(menuEntry){menuEntry.getId()===themeId?menuEntry.turnOn():menuEntry.turnOff()})}var self=this,pluginName=this.getName(),testRunner=this.getTestRunner(),testData=testRunner.getTestData()||{},testConfig=testData.config||{},pluginShortcuts=(testConfig.shortcuts||{})[this.getName()]||{},themesConfig=themeHandler.get("items")||{},state={
...
testRunner.trigger("tool-themeswitcher-toggle")},{avoidInput:!0}),this.disable(),testRunner.on("loaditem",function(){self.show()}).on("renderitem",function(){self.enable(),changeTheme(state.selectedTheme)}).on("enabletools",function(){self.enable()}).on("disabletools unloaditem",function(){self.disable()}).on("tool-themeswitcher-toggle",function(){self.getState("enabled")!==!1&&self.menuButton.toggleMenu()}),testRunner.getPluginStore(this.getName()).then(function(itemThemesStore){self.storage=itemThemesStore,self.storage.getItem("itemThemeId").then(function(itemThemeId){itemThemeId&&state.selectedTheme!==itemThemeId&&changeTheme(itemThemeId)})})},destroy:function(){
