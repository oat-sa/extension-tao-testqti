...
;this.$element.addClass("overlay"),overlayConfig.full&&this.$element.addClass("overlay-full")},disable:function(){this.$element.removeClass("overlay overlay-full")},show:function(){this.enable()},hide:function(){this.disable()}})}),define("taoQtiTest/runner/plugins/content/responsiveness/collapser",["lodash","jquery","taoTests/runner/plugin"],function(_,$,pluginFactory){"use strict";var ns=".collapser",labelHiddenCls="no-tool-label",separatorCls="separator",defaults={collapseTools:!0,collapseNavigation:!1,collapseInOrder:!1,hover:!1,collapseOrder:[]},$window=$(window);return pluginFactory({name:"collapser",init:function(){function buildCollapsiblesList(){
allCollapsibles=config.collapseInOrder&&config.collapseOrder.length?getCollapsiblesFromConfig():config.collapseInOrder?getSortedCollapsiblesFromDom():getUnsortedCollapsiblesFromDom()}function getExtraWidth($element){var expandedWidth,collapsedWidth;return $element.removeClass(collapseCls),expandedWidth=$element.outerWidth(!0),$element.addClass(collapseCls),collapsedWidth=$element.outerWidth(!0),$element.removeClass(collapseCls),expandedWidth-collapsedWidth}function toggleCollapsibles(){availableWidth=getAvailableWidth(),availableWidth<previousAvailableWidth?collapseInOrder():expandInOrder(),previousAvailableWidth=availableWidth}function collapseInOrder(){
for(var toCollapse,collapsiblesCopy=_.clone(allCollapsibles);collapseNeeded()&&collapsiblesCopy.length;)toCollapse=collapsiblesCopy.shift(),toCollapse.$elements.addClass(collapseCls)}function collapseNeeded(){return getToolbarWidth()>getAvailableWidth()}function expandInOrder(){_.forEachRight(allCollapsibles,function(toExpand){if(toExpand.$elements.hasClass(collapseCls)){if(!expandPossible(toExpand.extraWidth))return!1;toExpand.$elements.removeClass(collapseCls)}})}function expandPossible(extraWidth){return getToolbarWidth()+extraWidth<getAvailableWidth()}function getAvailableWidth(){return $actionsBar.width()-20}function getToolbarWidth(){
return $toolbox.outerWidth(!0)+$navigation.outerWidth(!0)}function getControlsFromDom(){var $controls=$();return config.collapseTools&&($controls=$controls.add($toolbox.find(">ul>[data-control]").not("."+labelHiddenCls).not("."+separatorCls))),config.collapseNavigation&&($controls=$controls.add($navigation.find(">ul>[data-control]").not("."+labelHiddenCls).not("."+separatorCls))),$controls}function getCollapsiblesFromConfig(){return _.compact(config.collapseOrder.map(function(selector){var $elements=$(selector).not("."+labelHiddenCls).not("."+separatorCls),extraWidth=0;return!!$elements.length&&($elements.each(function(){extraWidth+=getExtraWidth($(this))}),
{$elements:$elements,extraWidth:extraWidth})}))}function getSortedCollapsiblesFromDom(){var $elements=getControlsFromDom(),_allCollapsibles=[],order={};return $elements.each(function(){var ctrl=this.dataset.control,key=ctrl.substring(0,ctrl.search(/[A-Z-_]/))||ctrl;order[key]=order[key]||$(),order[key]=order[key].add($(this))}),_.forOwn(order,function($elements){var extraWidth=0;$elements.each(function(){extraWidth+=getExtraWidth($(this))}),_allCollapsibles.push({$elements:$elements,extraWidth:extraWidth})}),_.compact(_allCollapsibles)}function getUnsortedCollapsiblesFromDom(){var $elements=getControlsFromDom(),_allCollapsibles=[],extraWidth=0
;return $elements.each(function(){extraWidth+=getExtraWidth($(this))}),_allCollapsibles.push({$elements:$elements,extraWidth:extraWidth}),_.compact(_allCollapsibles)}var allCollapsibles,availableWidth,previousAvailableWidth,testRunner=this.getTestRunner(),testData=testRunner.getTestData()||{},testConfig=testData.config||{},pluginsConfig=testConfig.plugins||{},config=_.defaults(pluginsConfig.collapser||{},defaults),collapseCls=config.hover?"tool-label-collapsed-hover":"tool-label-collapsed",areaBroker=testRunner.getAreaBroker(),$actionsBar=areaBroker.getArea("actionsBar"),$toolbox=areaBroker.getToolboxArea(),$navigation=areaBroker.getNavigationArea()
;$window.on("resize"+ns,_.throttle(function(){testRunner.trigger("collapseTools")},40)),testRunner.after("renderitem loaditem",function(){previousAvailableWidth=1/0,buildCollapsiblesList(),testRunner.trigger("collapseTools")}).on("collapseTools"+ns,function(){toggleCollapsibles()})},destroy:function(){$window.off(ns)}})}),define("tpl!taoQtiTest/runner/plugins/content/rubricBlock/rubricBlock",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){return this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{},'<div id="qti-rubrics"></div>\n'})}),
...
define("taoQtiTest/runner/plugins/controls/connectivity/connectivity",["jquery","lodash","i18n","core/promise","core/polling","ui/waitingDialog/waitingDialog","taoTests/runner/plugin","tpl!taoQtiTest/runner/plugins/controls/connectivity/connectivity"],function($,_,__,Promise,pollingFactory,waitingDialog,pluginFactory,connectivityTpl){"use strict";return pluginFactory({name:"connectivity",init:function(){},install:function(){var self=this,waiting=!1,testRunner=this.getTestRunner(),proxy=testRunner.getProxy(),displayWaitingDialog=function(message){var dialog;return new Promise(function(resolve){waiting||(waiting=!0,testRunner.before("pause.waiting",function(){
return new Promise(function(pauseResolve){proxy.off("reconnect.pausing").after("reconnect.pausing",pauseResolve)})}),dialog=waitingDialog({message:__("You are encountering a prolonged connectivity loss. ")+message,waitContent:__("Please wait while we try to restore the connection."),proceedContent:__("The connection seems to be back, please proceed")}).on("proceed",function(){resolve()}).on("render",function(){proxy.off("reconnect.waiting").after("reconnect.waiting",function(){testRunner.off("pause.waiting"),waiting=!1,dialog.endWait()})}))})};this.polling=pollingFactory({action:function(){
testRunner.getProxy().telemetry(testRunner.getTestContext().itemIdentifier,"up").catch(_.noop)},interval:3e4,autoStart:!1}),this.$element=$(connectivityTpl({state:proxy.isOnline()?"connected":"disconnected"})),proxy.on("disconnect",function(source){testRunner.getState("disconnected")||(testRunner.setState("disconnected",!0),testRunner.trigger("disconnect",source),self.$element.removeClass("connected").addClass("disconnected"),self.polling.start())}).on("reconnect",function(){testRunner.getState("disconnected")&&(testRunner.setState("disconnected",!1),testRunner.trigger("reconnect"),self.$element.removeClass("disconnected").addClass("connected"),
self.polling.stop())}),testRunner.before("leave",function(e,data){if(proxy.isOffline())return displayWaitingDialog(data.message).then(function(){testRunner.trigger("leave",data)}).catch(function(generalErr){testRunner.trigger("error",generalErr)}),!1}),testRunner.before("error",function(e,err){return!proxy.isConnectivityError(err)&&(proxy.isOffline()?(displayWaitingDialog(err.message).then(function(){"nav"===err.type&&testRunner.loadItem(testRunner.getTestContext().itemIdentifier),"finish"===err.type&&testRunner.finish(),"pause"===err.type&&testRunner.trigger("pause",{reasons:err.data&&err.data.reasons,message:err.data&&err.data.comment})
}).catch(function(generalErr){testRunner.trigger("error",generalErr)}),!1):void 0)})},render:function(){this.getAreaBroker().getControlArea().append(this.$element)}})}),define("taoQtiTest/runner/plugins/controls/duration/duration",["lodash","core/polling","core/timer","core/store","core/promise","taoTests/runner/plugin"],function(_,pollingFactory,timerFactory,store,Promise,pluginFactory){"use strict";return pluginFactory({name:"duration",install:function(){var self=this;this.getTestRunner().on("storechange",function(){self.shouldClearStorage=!0})},init:function(){var self=this,testRunner=this.getTestRunner()
;return store("duration-"+testRunner.getConfig().serviceCallId).then(function(durationStore){return self.shouldClearStorage?durationStore.clear().then(function(){return durationStore}):durationStore}).then(function(durationStore){function getItemDuration(attemptId){return/^(.*)+#+\d+$/.test(attemptId)?durationStore.getItem(attemptId):Promise.reject(new Error('Is it really an attempt id, like "itemid#attempt"'))}self.stopwatch=timerFactory({autoStart:!1}),self.polling=pollingFactory({action:function(){var context=testRunner.getTestContext(),itemAttemptId=context.itemIdentifier+"#"+context.attempt;durationStore.getItem(itemAttemptId).then(function(duration){
var elapsed=self.stopwatch.tick();duration=_.isNumber(duration)?duration:0,elapsed=_.isNumber(elapsed)&&elapsed>0?elapsed/1e3:0,durationStore.setItem(itemAttemptId,duration+elapsed)})},interval:1e3,autoStart:!1}),testRunner.after("renderitem",function(){self.enable()}).on("enableitem",function(){self.enable()}).before("move skip exit timeout",function(){var context=testRunner.getTestContext();return getItemDuration(context.itemIdentifier+"#"+context.attempt).then(function(duration){var params={itemDuration:0};_.isNumber(duration)&&duration>0&&(params.itemDuration=duration),testRunner.getProxy().addCallActionParams(params)})
}).on("move skip exit timeout error disableitem",function(){self.disable()}).on("plugin-get.duration",function(e,attemptId,getDuration){_.isFunction(getDuration)&&getDuration(getItemDuration(attemptId))}).before("finish",function(){return new Promise(function(resolve){durationStore.removeStore().then(resolve).catch(resolve)})})})},destroy:function(){this.polling.stop(),this.stopwatch.stop()},enable:function(){this.getState("enabled")||(this.polling.start(),this.stopwatch.resume())},disable:function(){this.getState("enabled")&&(this.polling.stop(),this.stopwatch.pause())}})}),
define("tpl!taoQtiTest/runner/plugins/controls/progressbar/progressbar",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){return this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{},'<div class="progress-box">\n    <div data-control="progress-bar" class="qti-controls progressbar info"></div>\n    <div data-control="progress-label" class="qti-controls"></div>\n</div>\n'})}),
...
flaggedCount&&(itemsCountMessage+=" "+__("and you flagged %s item(s) that you can review now",flaggedCount.toString()))):"part"===scope&&(itemsCountMessage=0===unansweredCount?__("You answered all %s question(s)",stats.questions.toString()):__("You have %s unanswered question(s)",unansweredCount.toString()),flaggedCount&&(itemsCountMessage+=" "+__("and you flagged %s item(s) that you can review now",flaggedCount.toString()))),itemsCountMessage&&(itemsCountMessage+="."),itemsCountMessage}return{getExitMessage:getExitMessage}}),define("taoQtiTest/runner/helpers/navigation",["lodash","taoQtiTest/runner/helpers/map"],function(_,mapHelper){"use strict"
;var navigationHelper={isLeavingSection:function(testContext,testMap,direction,scope,position){var section,sectionStats,nbItems,item;if(_.isPlainObject(testContext)&&_.isPlainObject(testMap)&&!_.isEmpty(testContext.sectionId)&&!_.isEmpty(testContext.itemIdentifier))return section=mapHelper.getSection(testMap,testContext.sectionId),sectionStats=mapHelper.getSectionStats(testMap,testContext.sectionId),nbItems=sectionStats&&sectionStats.total,item=mapHelper.getItem(testMap,testContext.itemIdentifier),
...
var timerUpdatePromises=[],context=testRunner.getTestContext();return context.extraTime&&(extraTime=context.extraTime.total,consumedExtraTime=Math.max(consumedExtraTime,context.extraTime.consumed)),_.forEach(timerTypes,function(type){timerUpdatePromises.push(new Promise(function(resolve){var timerConfig=getTimerConfig(type),timerExists=void 0!==displayedTimers[type],addAndResolve=function(){return addTimer(type,timerConfig),resolve()};return timerConfig?timerExists&&displayedTimers[type].id()===timerConfig.id?(setRemainingTime(timerConfig,timers[type]),timerConfig.remaining!==timers[type]&&displayedTimers[type].val(timerConfig.remaining).refresh(),
resolve()):(removeTimer(type),checkStorage?void self.storage.getItem(timerConfig.id).then(function(savedTime){return _.isNumber(savedTime)&&savedTime>=0&&setRemainingTime(timerConfig,savedTime),addAndResolve()}).catch(function(){return addAndResolve()}):addAndResolve()):(removeTimer(type),resolve())}))}),Promise.all(timerUpdatePromises).then(function(data){return toggleToggler(),data})};return store("timer-"+testRunner.getConfig().serviceCallId).then(function(timeStore){return self.shouldClearStorage?timeStore.clear().then(function(){return timeStore}):Promise.resolve(timeStore)}).then(function(timeStore){self.storage=timeStore,
self.$element=$(timerBoxTpl()),self.$toggler=self.$element.find(".timer-toggler"),self.storage.getItem("zen-mode").then(function(zenMode){zenMode&&!self.$element.hasClass("zen-mode")&&toggleZenMode()}),self.$toggler.on("click",function(e){e.preventDefault(),toggleZenMode()}),self.stopwatch=timerFactory({autoStart:!1}),self.polling=pollingFactory({action:function(){var timeoutScope,timeoutRef,elapsed=self.stopwatch.tick(),elapsedExtraTime=0,timeout=!1;_.forEach(displayedTimers,function(timer,type){var regularVal,displayedVal,warning,runTimeout=function(){testRunner.timeout(timeoutScope,timeoutRef)};timer.running()&&(regularVal=timers[type]-elapsed,
displayedVal=timer.val()-elapsed,timers[type]=Math.max(regularVal,0),timer.val(Math.max(displayedVal,0)).refresh(),regularVal<=0&&(elapsedExtraTime=Math.max(elapsedExtraTime,Math.abs(regularVal))),displayedVal<=0?(timer.running(!1),timeout=!0,timeoutRef=timer.id(),timeoutScope=type,self.storage.setItem(timer.id(),0).then(runTimeout).catch(runTimeout)):(self.storage.setItem(timer.id(),timers[type]),warning=timer.warn(),_.isEmpty(warning)||testRunner.trigger(warning.type,warning.text)))}),elapsedExtraTime&&(consumedExtraTime+=elapsedExtraTime/1e3,lastConsumedExtraTime+=elapsedExtraTime),timeout&&self.disable()},interval:1e3,autoStart:!1}),
testRunner.before("renderitem resumeitem",function(){return updateTimers(!0)}).on("enableitem",doEnable).on("disableitem",doDisable).after("renderitem",doEnable).before("move skip",function(e,type,scope,position){var context=testRunner.getTestContext(),testDataBeforeMove=testRunner.getTestData(),config=testDataBeforeMove&&testDataBeforeMove.config,timerConfig=config&&config.timer||{},options=context&&context.options||{},movePromise=new Promise(function(resolve,reject){
...
}).on("renderitem",function(){var testContext=testRunner.getTestContext();isEnabled()&&highlighter.restoreHighlight(testContext.itemIdentifier)}).on("beforeunloaditem",function(){var testContext=testRunner.getTestContext();isEnabled()&&highlighter.saveHighlight(testContext.itemIdentifier)}).on("disabletools unloaditem",function(){self.disable(),highlighter.toggleHighlighting(!1)}).on("tool-highlight",function(){isEnabled()&&highlighter.trigger()}).on("tool-highlight-remove",function(){highlighter.clearHighlights()}).on("plugin-start.highlighter",function(){self.buttonMain.turnOn()}).on("plugin-end.highlighter",function(){self.buttonMain.turnOff()})},
destroy:function(){shortcut.remove("."+this.getName()),$(document).off(".highlighter")},enable:function(){this.buttonMain.enable(),this.buttonRemove.enable()},disable:function(){this.buttonMain.disable(),this.buttonRemove.disable()},show:function(){this.buttonMain.show(),this.buttonRemove.show()},hide:function(){this.buttonMain.hide(),this.buttonRemove.hide()}})}),define("taoQtiTest/runner/plugins/tools/itemThemeSwitcher/itemThemeSwitcher",["jquery","lodash","i18n","core/store","taoTests/runner/plugin","ui/hider","ui/themes","util/shortcut","util/namespace"],function($,_,__,storeFactory,pluginFactory,hider,themeHandler,shortcut,namespaceHelper){"use strict"
;var pluginName="itemThemeSwitcher";return pluginFactory({name:pluginName,install:function(){var self=this;this.getTestRunner().on("storechange",function(){self.shouldClearStorage=!0})},init:function(){function changeTheme(themeId){var $qtiItem=$(".qti-item"),previousTheme=state.selectedTheme;state.selectedTheme=themeId,$qtiItem&&_.defer(function(){$qtiItem.trigger("themechange",[state.selectedTheme])}),self.storage&&self.storage.setItem("itemThemeId",themeId),previousTheme!==state.selectedTheme&&testRunner.trigger("themechange",state.selectedTheme,previousTheme)}
var self=this,testRunner=this.getTestRunner(),testData=testRunner.getTestData()||{},testConfig=testData.config||{},pluginShortcuts=(testConfig.shortcuts||{})[this.getName()]||{},themesConfig=themeHandler.get("items")||{},state={availableThemes:[],defaultTheme:"",selectedTheme:""},allMenuEntries=[];return themesConfig&&(themesConfig.default&&(state.defaultTheme=themesConfig.default,state.selectedTheme=themesConfig.default),themesConfig.available&&_.forEach(themesConfig.available,function(theme){state.availableThemes.push({id:theme.id,label:theme.name})})),this.menuButton=this.getAreaBroker().getToolbox().createMenu({control:"color-contrast",
title:__("Change the current color preset"),icon:"contrast",text:__("Contrast")}).on("click",function(e){e.preventDefault(),testRunner.trigger("tool-themeswitcher-toggle")}).on("openmenu",function(){testRunner.trigger("plugin-open."+pluginName,state.selectedTheme)}).on("closemenu",function(){testRunner.trigger("plugin-close."+pluginName,state.selectedTheme)}),state.availableThemes.forEach(function(theme){var themeEntry=self.getAreaBroker().getToolbox().createEntry({control:theme.id,title:theme.label,icon:"preview",text:__(theme.label)});themeEntry.setMenuId("color-contrast"),themeEntry.on("click",function(e){var themeId=this.config.control
;e.preventDefault(),self.menuButton.turnOffAll(),this.turnOn(),changeTheme(themeId)}),state.defaultTheme===theme.id&&themeEntry.on("render",function(){this.turnOn()}),allMenuEntries.push(themeEntry)}),testConfig.allowShortcuts&&pluginShortcuts.toggle&&shortcut.add(namespaceHelper.namespaceAll(pluginShortcuts.toggle,this.getName(),!0),function(){testRunner.trigger("tool-themeswitcher-toggle")},{avoidInput:!0}),this.disable(),testRunner.on("loaditem",function(){self.show()}).on("renderitem",function(){self.enable(),changeTheme(state.selectedTheme)}).on("enabletools",function(){self.enable()}).on("disabletools unloaditem",function(){self.disable()
}).on("tool-themeswitcher-toggle",function(){!1!==self.getState("enabled")&&self.menuButton.toggleMenu()}),storeFactory(pluginName+"-"+testRunner.getConfig().serviceCallId).then(function(itemThemesStore){return self.shouldClearStorage?itemThemesStore.clear().then(function(){return itemThemesStore}):itemThemesStore}).then(function(itemThemesStore){testRunner.after("renderitem enableitem",function(){self.storage=itemThemesStore,self.storage.getItem("itemThemeId").then(function(itemThemeId){itemThemeId&&state.selectedTheme!==itemThemeId&&(changeTheme(itemThemeId),allMenuEntries.forEach(function(menuEntry){
menuEntry.getId()===itemThemeId?menuEntry.turnOn():menuEntry.turnOff()}))})}).before("finish",function(){return new Promise(function(resolve){self.storage.removeStore().then(resolve).catch(resolve)})})})},destroy:function(){shortcut.remove("."+this.getName())},enable:function(){this.menuButton.enable()},disable:function(){this.menuButton.disable()},show:function(){this.menuButton.show()},hide:function(){this.menuButton.hide()}})}),
...
play:function(){return this._exec("isSpeaking")&&!this._exec("isPaused")||(this._exec("event_play"),this.trigger("play")),this},pause:function(){return this._exec("event_pause"),this.trigger("pause"),this},stop:function(){return this._exec("stopSpeech"),this.trigger("stop"),this},setSpeed:function(e,value){return speed=+value,this._exec("setSpeedValue",speed),this.trigger("setSpeed",speed),this},setVolume:function(e,value){return volume=+value,this._exec("setVolumeValue",volume),this.trigger("setVolume",volume),this},clickToSpeak:function(){var self=this;return options.enableClickToSpeak=!options.enableClickToSpeak,
this._exec("enableClickToSpeak",options.enableClickToSpeak),this._exec("enableContinuousReading",!options.enableClickToSpeak),this.trigger("clickToSpeak"),options.$contentArea.find(".qti-item").each(function(){$(this).toggleClass("prevent-click-handler")}),options.$navigationArea.on("click","a",function(){options.$contentArea.find(".qti-item.prevent-click-handler").length>0&&self.clickToSpeak()}),this}},{}).setTemplate(tpl).on("init",function(){var self=this;speed=40,volume=40,$(options.ignoreEls.join(",")).attr("ignore",!0),require([options.toolbarUrl],function(){var tss=self._get("texthelpSpeechStream")
;require(["//"+tss.g_strServer+"/SpeechStream/v"+tss.g_strBuild+"/texthelpMain.js"],function(){self._init()})})}).init(config).on("render",function(){var $this=this.getElement();$this.on("mousedown",function(e){return e.preventDefault(),!1}),$this.find(".action").on("click",function(e){var $action=$(this);$action.hasClass("disabled")&&e.stopImmediatePropagation(),$action.closest(".settings").length||($this.find(".settings").removeClass("active"),$this.find(".settings > .settings-menu").hide())}),$this.find(".settings").on("click",function(){$(this).toggleClass("active"),$this.find(".settings-menu").toggle()}),
$this.find(".settings > .settings-menu").on("click",function(e){e.stopPropagation()}).hide(),$this.find(".settings > .settings-menu > .option").on("hover",function(){$(this).find(".slider-container").show()}).on("mouseleave",function(){$(this).find(".slider-container").hide()}),$this.find(".settings > .settings-menu > .option > .slider-container").hide(),$this.find(".click-to-speak").on("click",this.clickToSpeak),$this.find(".play").on("click",this.play),$this.find(".pause").on("click",this.pause).hide(),$this.find(".stop").on("click",this.stop),$this.find(".settings > .settings-menu > .volume .slider").noUiSlider({animate:!0,connected:!0,range:{min:0,
max:100},start:volume,step:10}).on("change",this.setVolume),$this.find(".settings > .settings-menu > .speed .slider").noUiSlider({animate:!0,connected:!0,range:{min:0,max:100},start:speed,step:15}).on("change",this.setSpeed)}).on("clickToSpeak",function(){var $el=this.getElement();options.enableClickToSpeak?($el.find(".click-to-speak").addClass("active"),$el.find(".play").addClass("disabled").show(),$el.find(".pause").addClass("disabled").hide(),options.$contentArea.css("cursor","pointer")):($el.find(".click-to-speak").removeClass("active"),$el.find(".play").removeClass("disabled").show(),$el.find(".pause").removeClass("disabled").hide(),
options.$contentArea.css("cursor","default"))}).on("play",function(){var $el=this.getElement();$el.find(".play").hide(),$el.find(".pause").show()}).on("pause",function(){var $el=this.getElement();$el.find(".play").show(),$el.find(".pause").hide()}).on("stop",function(){var $el=this.getElement();$el.find(".play").show(),$el.find(".pause").hide()})}}),define("taoQtiTest/runner/plugins/tools/textToSpeech/plugin",["jquery","lodash","i18n","taoTests/runner/plugin","ui/hider","ui/stacker","taoQtiTest/runner/plugins/tools/textToSpeech/textToSpeech"],function($,_,__,pluginFactory,hider,stackerFactory,ttsFactory){"use strict";return pluginFactory({
name:"textToSpeech",init:function(){var self=this,testRunner=this.getTestRunner();return this.ttsButton=this.getAreaBroker().getToolbox().createEntry({control:"tts",icon:"headphones",text:__("Text to Speech"),title:__("Text to Speech")}).on("render",function(){var stacker=stackerFactory("test-runner");self.tts=ttsFactory({$contentArea:testRunner.getAreaBroker().getContentArea(),$navigationArea:testRunner.getAreaBroker().getNavigationArea()}).render(self.ttsButton.getElement()).disable().hide(),stacker.autoBringToFront(self.tts.getElement())}).on("click",function(e){var ttsEl=self.tts.getElement();$(e.target).closest(ttsEl).length||(hider.toggle(ttsEl),
ttsEl.hasClass("hidden")?self.tts.disable():self.tts.enable())}).disable().hide(),testRunner.on("loaditem",function(){self.disable(),self.ttsButton.hide()}).on("enabletools",function(){self.enable()}).on("renderitem",function(){var context=testRunner.getTestContext(),config=testRunner.getConfig();context.options.textToSpeech&&(self.enable(),self.ttsButton.show(),self.tts.updateTexthelpCache(config.serviceCallId,this.itemRunner._item.attributes.identifier))}).on("disabletools unloaditem",function(){self.disable()}),this},enable:function(){this.tts&&this.tts.enable(),this.ttsButton&&this.ttsButton.enable()},disable:function(){this.tts&&this.tts.disable(),
this.ttsButton&&this.ttsButton.disable()}})}),define("taoQtiTest/runner/plugins/tools/zoom",["jquery","i18n","ui/hider","ui/transformer","util/shortcut","util/namespace","taoTests/runner/plugin"],function($,__,hider,transformer,shortcut,namespaceHelper,pluginFactory){"use strict";var standard=100,threshold={lower:10,upper:200},setZoomLevel=function($target,level){transformer.setTransformOrigin($target,"0 0"),transformer.scale($target,level/100)},resetZoom=function($target){transformer.reset($target)},forceRepaint=function($target){var sel=$target[0];sel&&(sel.style.display="none",sel.offsetHeight,sel.style.display="")};return pluginFactory({name:"zoom",
init:function(){function isConfigured(){return!!((testRunner.getTestContext()||{}).options||{}).zoom}function togglePlugin(){isConfigured()?self.show():self.hide()}function zoomAction(dir){var el,sx,sy,before,after,inc=10*dir;self.$zoomTarget&&(el=self.$zoomTarget[0],before=el.getBoundingClientRect(),sx=self.$container.scrollLeft(),sy=self.$container.scrollTop(),self.zoom=Math.max(threshold.lower,Math.min(threshold.upper,self.zoom+inc)),self.zoom===standard?resetZoom(self.$zoomTarget):setZoomLevel(self.$zoomTarget,self.zoom),forceRepaint(self.$zoomTarget),after=el.getBoundingClientRect(),sx=Math.max(0,sx+(after.width-before.width)/2),
sy=Math.max(0,sy+(after.height-before.height)/2),self.$container.scrollLeft(sx).scrollTop(sy))}function zoomIn(){!1!==self.getState("enabled")&&zoomAction(1)}function zoomOut(){!1!==self.getState("enabled")&&zoomAction(-1)}var self=this,testRunner=this.getTestRunner(),testData=testRunner.getTestData()||{},testConfig=testData.config||{},pluginShortcuts=(testConfig.shortcuts||{})[this.getName()]||{};this.buttonZoomOut=this.getAreaBroker().getToolbox().createEntry({control:"zoomOut",title:__("Zoom out"),icon:"remove"}),this.buttonZoomIn=this.getAreaBroker().getToolbox().createEntry({control:"zoomIn",title:__("Zoom in"),icon:"add"}),
this.buttonZoomIn.on("click",function(e){e.preventDefault(),testRunner.trigger("tool-zoomin")}),this.buttonZoomOut.on("click",function(e){e.preventDefault(),testRunner.trigger("tool-zoomout")}),testConfig.allowShortcuts&&(pluginShortcuts.in&&shortcut.add(namespaceHelper.namespaceAll(pluginShortcuts.in,this.getName(),!0),function(){testRunner.trigger("tool-zoomin")},{avoidInput:!0}),pluginShortcuts.out&&shortcut.add(namespaceHelper.namespaceAll(pluginShortcuts.out,this.getName(),!0),function(){testRunner.trigger("tool-zoomout")},{avoidInput:!0})),togglePlugin(),this.disable(),testRunner.on("loaditem",function(){self.zoom=standard,togglePlugin(),self.disable()
}).on("renderitem",function(){self.$container=$("#qti-content"),self.$zoomTarget=$(".qti-item"),self.enable()}).on("enabletools",function(){self.enable()}).on("disabletools unloaditem",function(){self.disable()}).on("tool-zoomin",zoomIn).on("tool-zoomout",zoomOut)},destroy:function(){shortcut.remove("."+this.getName())},enable:function(){this.buttonZoomIn.enable(),this.buttonZoomOut.enable()},disable:function(){this.buttonZoomIn.disable(),this.buttonZoomOut.disable()},show:function(){this.buttonZoomIn.show(),this.buttonZoomOut.show()},hide:function(){this.buttonZoomIn.hide(),this.buttonZoomOut.hide()}})}),function(c){
