...
testRunner.trigger("disconnect",source),self.$element.removeClass("connected").addClass("disconnected"))}).on("reconnect",function(){testRunner.getState("disconnected")&&(testRunner.setState("disconnected",!1),testRunner.trigger("reconnect"),self.$element.removeClass("disconnected").addClass("connected"))}),testRunner.before("error",function(e,err){var dialog;return!proxy.isConnectivityError(err)&&(proxy.isOffline()?(waiting||(waiting=!0,dialog=waitingDialog({message:__("You are encountering a prolonged connectivity loss"),waitContent:__("Please wait while we try to restore the connection."),
proceedContent:__("The connection seems to be back, please proceed")}).on("proceed",function(){testRunner.trigger("pause",{reasons:{category:__("technical"),subCategory:__("network")}})}).on("render",function(){proxy.off("reconnect.waiting").on("reconnect.waiting",function(){waiting=!1,dialog.endWait()})})),!1):void 0)})},render:function(){this.getAreaBroker().getControlArea().append(this.$element)}})}),define("taoQtiTest/runner/plugins/controls/duration/duration",["lodash","core/polling","core/timer","core/store","core/promise","taoTests/runner/plugin"],function(_,pollingFactory,timerFactory,store,Promise,pluginFactory){"use strict";return pluginFactory({
name:"duration",init:function(){var self=this,testRunner=this.getTestRunner(),initContext=testRunner.getTestContext(),initItemAttemptId=initContext.itemIdentifier+"#"+initContext.attempt;return store("duration-"+testRunner.getConfig().serviceCallId).then(function(durationStore){function getItemDuration(attemptId){return/^(.*)+#+\d+$/.test(attemptId)?durationStore.getItem(attemptId):Promise.reject(new Error('Is it really an attempt id, like "itemid#attempt"'))}return self.stopwatch=timerFactory({autoStart:!1}),durationStore.getItem(initItemAttemptId).then(function(initDuration){
if(!(initDuration=_.isNumber(initDuration)?initDuration:0)&&initContext.attemptDuration)return durationStore.setItem(initItemAttemptId,initContext.attemptDuration||0)}).then(function(){self.polling=pollingFactory({action:function(){var elapsed=self.stopwatch.tick(),context=testRunner.getTestContext(),itemAttemptId=context.itemIdentifier+"#"+context.attempt;durationStore.getItem(itemAttemptId).then(function(duration){duration=_.isNumber(duration)?duration:0,elapsed=_.isNumber(elapsed)&&elapsed>0?elapsed/1e3:0,durationStore.setItem(itemAttemptId,duration+elapsed)})},interval:1e3,autoStart:!1}),testRunner.after("renderitem enableitem",function(){self.enable()
}).before("move disableitem skip error",function(){self.getState("enabled")&&self.disable()}).before("move skip exit timeout",function(){var context=testRunner.getTestContext();return getItemDuration(context.itemIdentifier+"#"+context.attempt).then(function(duration){var params={itemDuration:0};_.isNumber(duration)&&duration>0&&(params.itemDuration=duration),testRunner.getProxy().addCallActionParams(params)})}).on("plugin-get.duration",function(e,attemptId,getDuration){_.isFunction(getDuration)&&getDuration(getItemDuration(attemptId))}).before("finish",function(){return new Promise(function(resolve){durationStore.removeStore().then(resolve).catch(resolve)})
...
$progressControl=$('[data-control="progress-bar"]',this.$element),$progressControl.progressbar(),update(),testRunner.on("ready",update).on("loaditem",update)},render:function(){this.getAreaBroker().getControlArea().append(this.$element)}})}),define("taoQtiTest/runner/plugins/controls/testState/testState",["lodash","taoTests/runner/plugin"],function(_,pluginFactory){"use strict";return pluginFactory({name:"testState",install:function(){var testRunner=this.getTestRunner();testRunner.getProxy().use(function(req,res,next){var data=res&&res.data;if(data&&data.type&&"TestState"===data.type)return testRunner.setState("closedOrSuspended",!0),
void(testRunner.getState("ready")?!_.isEmpty(data.messages)&&_.find(data.messages,{channel:"teststate"})||testRunner.trigger("leave",data):testRunner.trigger("destroy"));next()})},init:function(){var testRunner=this.getTestRunner(),isLeaving=!1;testRunner.getProxy().channel("teststate",function(data){isLeaving||!data||"close"!==data.type&&"pause"!==data.type||testRunner.getState("closedOrSuspended")||(isLeaving=!0,testRunner.setState("closedOrSuspended",!0),testRunner.trigger("leave",data))})}})}),define("tpl!taoQtiTest/runner/plugins/controls/timer/timer",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){
function program1(depth0,data){return" disabled"}this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,buffer="",escapeExpression=this.escapeExpression,self=this;return buffer+='<span class="qti-timer qti-timer__type-',(helper=helpers.type)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.type,stack1="function"==typeof helper?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+" qti-controls ",stack1=helpers.unless.call(depth0,depth0&&depth0.running,{hash:{},inverse:self.noop,fn:self.program(1,program1,data),data:data}),
(stack1||0===stack1)&&(buffer+=stack1),buffer+='" data-control="',(helper=helpers.id)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.id,stack1="function"==typeof helper?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+'" title="',(helper=helpers.label)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.label,stack1="function"==typeof helper?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+'">\n    <span class="qti-timer_label truncate">',(helper=helpers.label)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.label,
stack1="function"==typeof helper?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+'</span>\n    <span class="qti-timer_time"></span>\n</span>\n'})}),define("taoQtiTest/runner/plugins/controls/timer/timerComponent",["jquery","lodash","i18n","moment","core/encoder/time","tpl!taoQtiTest/runner/plugins/controls/timer/timer"],function($,_,__,moment,timeEncoder,timerTpl){"use strict";var hasAnimationFrame="requestAnimationFrame"in window,timerTypes={test:"assessmentTest",testPart:"testPart",section:"assessmentSection",item:"assessmentItemRef"};return function(config){var data
;if(!_.isPlainObject(config))throw new TypeError("A timer needs to be configured using a config object");if(_.isEmpty(config.id))throw new TypeError("A timer needs and identifier");if(!_.contains(timerTypes,config.type))throw new TypeError("Invalid type configured");return data=_.defaults(config,{label:"",remaining:0,warnings:{},running:!0}),{init:function(){return this.$element=$(timerTpl(data)),this.$display=$(".qti-timer_time",this.$element),this},render:function($container){return $container.append(this.$element),this},destroy:function(){return this.$element.remove(),this},refresh:function(){var self=this,update=function(){
self.$display.text(timeEncoder.encode(data.remaining/1e3))};return hasAnimationFrame?window.requestAnimationFrame(update):update(),this},warn:function(){var remaining,message={},warning=_.findLast(data.warnings,{showed:!1});if(!_.isEmpty(warning)&&_.isFinite(warning.point)&&data.remaining<=warning.point){switch(remaining=moment.duration(data.remaining/1e3,"seconds").humanize(),this.$display.removeClass("txt-info txt-warning txt-danger").addClass("txt-"+warning.type),data.type){case"assessmentItemRef":message.text=__("Warning – You have %s remaining to complete this item.",remaining);break;case"assessmentSection":
message.text=__("Warning – You have %s remaining to complete this section.",remaining);break;case"testPart":message.text=__("Warning – You have %s remaining to complete this test part.",remaining);break;case"assessmentTest":message.text=__("Warning – You have %s remaining to complete the test.",remaining)}var showed=_.find(data.warnings,{point:warning.point});showed&&showed.point&&data.warnings[showed.point/1e3]&&(data.warnings[showed.point/1e3].showed=!0),message.type=warning.type}return message},id:function(){return data.id},val:function(value){return void 0!==value?(data.remaining=value,this):data.remaining},running:function(value){
return void 0!==value?(data.running=!!value,this):!!data.running}}}}),define("taoQtiTest/runner/helpers/map",["lodash"],function(_){"use strict";function getEmptyStats(){return{questions:0,answered:0,flagged:0,viewed:0,total:0}}return{getJumps:function(map){return map&&map.jumps},getParts:function(map){return map&&map.parts},getSections:function(map){var parts=this.getParts(map),result={};return _.forEach(parts,function(part){var sections=part.sections;sections&&_.forEach(sections,function(section){result[section.id]=section})}),result},getJump:function(map,position){var jumps=this.getJumps(map);return jumps&&jumps[position]},getPart:function(map,partName){
...
;return section&&section.stats},getScopeStats:function(map,position,scope){var jump=this.getJump(map,position);switch(scope){case"section":case"testSection":return this.getSectionStats(map,jump&&jump.section);case"part":case"testPart":return this.getPartStats(map,jump&&jump.part);default:case"test":return this.getTestStats(map)}},getScopeMap:function(map,position,scope){var scopeMap=_.cloneDeep(map||{}),jump=this.getJump(map,position),part=this.getPart(scopeMap,jump&&jump.part),section=this.getSection(scopeMap,jump&&jump.section);return scope&&"test"!==scope&&(scopeMap.parts={},part&&(scopeMap.parts[jump.part]=part)),
...
return acc.questions+=item.stats.questions,acc.answered+=item.stats.answered,acc.flagged+=item.stats.flagged,acc.viewed+=item.stats.viewed,acc.total+=item.stats.total,acc},getEmptyStats())}}}),define("taoQtiTest/runner/helpers/currentItem",["lodash"],function(_){"use strict";var responseCardinalities={single:"base",multiple:"list",ordered:"list",record:"record"},currentItemHelper={getDeclarations:function(runner){var itemRunner=runner.itemRunner;return itemRunner._item&&itemRunner._item.responses},getResponseDeclaration:function(runner,identifier){var found=null;return _.forEach(currentItemHelper.getDeclarations(runner),function(declaration){
...
var itemRunner=runner.itemRunner,responses=itemRunner&&itemRunner.getResponses(),count=0,empty=0;return itemRunner&&_.forEach(currentItemHelper.getDeclarations(runner),function(declaration){var attributes=declaration.attributes||{},response=responses[attributes.identifier],baseType=attributes.baseType,cardinality=attributes.cardinality;count++,currentItemHelper.isQuestionAnswered(response,baseType,cardinality,declaration.defaultValue)||empty++}),!1===partially?count>0&&0===empty:count>0&&empty<count}};return currentItemHelper}),
define("taoQtiTest/runner/helpers/stats",["lodash","taoQtiTest/runner/helpers/map","taoQtiTest/runner/helpers/currentItem"],function(_,mapHelper,currentItemHelper){"use strict";function getInstantStats(scope,runner){var map=runner.getTestMap(),context=runner.getTestContext(),stats=_.clone(mapHelper.getScopeStats(map,context.itemPosition,scope)),isItemCurrentlyAnswered=currentItemHelper.isAnswered(runner);return isItemCurrentlyAnswered&&(!isItemCurrentlyAnswered&&context.itemAnswered&&stats.answered--,isItemCurrentlyAnswered&&!context.itemAnswered&&stats.answered++),stats}return{getInstantStats:getInstantStats}}),
define("taoQtiTest/runner/helpers/messages",["lodash","i18n","taoQtiTest/runner/helpers/map","taoQtiTest/runner/helpers/stats","taoQtiTest/runner/helpers/currentItem"],function(_,__,mapHelper,statsHelper,currentItemHelper){"use strict";function getExitMessage(message,scope,runner){var itemsCountMessage="",testData=runner.getTestData(),testConfig=testData&&testData.config;return(!testConfig||testConfig.enableUnansweredItemsWarning)&&(itemsCountMessage=getUnansweredItemsWarning(scope,runner)),(itemsCountMessage+" "+message).trim()}function getUnansweredItemsWarning(scope,runner){
var belowSections,stats=statsHelper.getInstantStats(scope,runner),unansweredCount=stats&&stats.questions-stats.answered,flaggedCount=stats&&stats.flagged,itemsCountMessage="",map=runner.getTestMap(),jump=mapHelper.getJump(map,runner.getTestContext().itemPosition);return"section"===scope||"testSection"===scope?(0===unansweredCount?itemsCountMessage+=__("You answered all %s question(s) in this section",stats.questions.toString()):itemsCountMessage=__("You answered only %s of the %s question(s) in this section",stats.answered.toString(),stats.questions.toString()),flaggedCount&&(itemsCountMessage+=", ",
itemsCountMessage+=__("and flagged %s of them",flaggedCount.toString()))):"test"===scope?(belowSections=getNextSections(map,jump.section),stats=_.clone(mapHelper.getSectionStats(runner.getTestMap(),jump.section)),_.forEach(belowSections,function(section){_.forEach(mapHelper.getSectionStats(runner.getTestMap(),section.id),function(sectionStats,statsKey){stats[statsKey]+=sectionStats})}),unansweredCount=stats&&stats.questions-stats.answered,flaggedCount=stats&&stats.flagged,currentItemHelper.isAnswered(runner)&&unansweredCount--,
itemsCountMessage=0===unansweredCount?__("You answered all %s question(s) in this test",stats.questions.toString()):__("You have %s unanswered question(s)",unansweredCount.toString()),flaggedCount&&(itemsCountMessage+=" "+__("and you flagged %s item(s) that you can review now",flaggedCount.toString()))):"part"===scope&&(itemsCountMessage=0===unansweredCount?__("You answered all %s question(s)",stats.questions.toString()):__("You have %s unanswered question(s)",unansweredCount.toString()),flaggedCount&&(itemsCountMessage+=" "+__("and you flagged %s item(s) that you can review now",flaggedCount.toString()))),itemsCountMessage&&(itemsCountMessage+="."),
itemsCountMessage}function getNextSections(map,currentSectionId){var sections=mapHelper.getSections(map),result={},below=!1;return _.forEach(sections,function(section){below&&(result[section.id]=section),section.id===currentSectionId&&(below=!0)}),result}return{getExitMessage:getExitMessage}}),define("tpl!taoQtiTest/runner/plugins/controls/timer/timers",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var helper,options,buffer="",helperMissing=helpers.helperMissing
;return buffer+='<div class="timer-box">\n    <a href="#" class="timer-toggler hidden" title="'+(0,this.escapeExpression)((helper=helpers.__||depth0&&depth0.__,options={hash:{},data:data},helper?helper.call(depth0,"Hide timers",options):helperMissing.call(depth0,"__","Hide timers",options)))+'"><span class="icon-clock"></span></a>\n    <div class="timer-wrapper"></div>\n</div>\n'})}),
define("taoQtiTest/runner/plugins/controls/timer/timer",["jquery","lodash","i18n","core/promise","core/polling","core/timer","core/store","ui/hider","taoTests/runner/plugin","taoQtiTest/runner/plugins/controls/timer/timerComponent","taoQtiTest/runner/helpers/messages","tpl!taoQtiTest/runner/plugins/controls/timer/timers"],function($,_,__,Promise,pollingFactory,timerFactory,store,hider,pluginFactory,timerComponentFactory,messages,timerBoxTpl){"use strict";var exitMessage=__("Once you close this section, you cannot return to it or change your answers."),timerTypes={test:"assessmentTest",testPart:"testPart",section:"assessmentSection",item:"assessmentItemRef"
},isTimedSection=function(context){var timeConstraints=context.timeConstraints||[];return _.some(timeConstraints,function(constraint){return"assessmentSection"===constraint.qtiClassName})};return pluginFactory({name:"timer",install:function(){var self=this;this.getTestRunner().on("storechange",function(){self.shouldClearStorage=!0})},init:function(){function doEnable(){self.getState("enabled")||self.enable()}function doDisable(){self.getState("enabled")&&self.disable()}function toggleZenMode(){self.$element.hasClass("zen-mode")?(self.$element.removeClass("zen-mode"),self.$toggler.attr("title",__("Hide timers")),
self.storage.removeItem("zen-mode")):(self.$element.addClass("zen-mode"),self.$toggler.attr("title",__("Show timers")),self.storage.setItem("zen-mode",!0))}function toggleToggler(){self.$toggler.length&&(_.size(timers)>0?hider.show(self.$toggler):hider.hide(self.$toggler))}var self=this,testRunner=this.getTestRunner(),testData=testRunner.getTestData()||{},itemStates=testData.itemStates||{},timerWarning=testData.config&&testData.config.timerWarning||{},displayedTimers={},timers={},extraTime=0,consumedExtraTime=0,lastConsumedExtraTime=0,getRemainingExtraTime=function(extra){return extra&&(extraTime=extra),1e3*Math.max(0,extraTime-consumedExtraTime)
},setRemainingTime=function(timerConfig,remaining){return timerConfig.remaining=remaining+getRemainingExtraTime(timerConfig.extra),timerConfig.regular=remaining,timerConfig},getTimerConfig=function(type){var timeConstraint,timer,closestPreviousWarning,context=testRunner.getTestContext();return context.isTimeout||context.itemSessionState!==itemStates.interacting||(timeConstraint=_.findLast(context.timeConstraints,{qtiClassName:type}))&&(timer=setRemainingTime({label:timeConstraint.label,extra:timeConstraint.extraTime,type:timeConstraint.qtiClassName,id:timeConstraint.source,running:!0,warnings:{}},1e3*timeConstraint.seconds),
_(timerWarning[timeConstraint.qtiClassName]).forEach(function(value,key){_.contains(["info","warning","danger"],value)&&(timer.warnings[key]={type:value,showed:timer.remaining/1e3<=key,point:1e3*parseInt(key,10)})}),closestPreviousWarning=_.find(timer.warnings,{showed:!0}),!_.isEmpty(closestPreviousWarning)&&closestPreviousWarning.point&&timer.warnings[closestPreviousWarning.point/1e3]&&(timer.warnings[closestPreviousWarning.point/1e3].showed=!1)),timer},leaveTimedSection=function(type,scope,position){
var context=testRunner.getTestContext(),map=testRunner.getTestMap(),section=map.parts[context.testPartId].sections[context.sectionId],nbItems=_.size(section.items),item=_.find(section.items,{position:context.itemPosition});return!(context.isTimeout||context.itemSessionState===itemStates.closed||!isTimedSection(context)||!item)&&!!("next"===type&&("assessmentSection"===scope||item.positionInSection+1===nbItems)||"previous"===type&&0===item.positionInSection||"jump"===type&&position>0&&(position<section.position||position>=section.position+nbItems))},removeTimer=function(type){displayedTimers[type]&&(self.storage.removeItem(displayedTimers[type].id()),
displayedTimers[type].destroy(),displayedTimers=_.omit(displayedTimers,type),timers=_.omit(timers,type),self.trigger("removetimer",type))},addTimer=function(type,config){config&&(timers[type]=config.regular,displayedTimers[type]=timerComponentFactory(config),displayedTimers[type].init().render(self.$element.find(".timer-wrapper")),self.trigger("addtimer",type,displayedTimers[type]))},updateTimers=function(checkStorage){var timerUpdatePromises=[],context=testRunner.getTestContext();return context.extraTime&&(extraTime=context.extraTime.total,consumedExtraTime=Math.max(consumedExtraTime,context.extraTime.consumed)),_.forEach(timerTypes,function(type){
timerUpdatePromises.push(new Promise(function(resolve){var timerConfig=getTimerConfig(type);return displayedTimers[type]?(timerConfig?displayedTimers[type].id()!==timerConfig.id?(removeTimer(type),addTimer(type,timerConfig)):(setRemainingTime(timerConfig,timers[type]),displayedTimers[type].val(timerConfig.remaining)):removeTimer(type),resolve()):timerConfig?checkStorage?void self.storage.getItem(timerConfig.id).then(function(savedTime){return _.isNumber(savedTime)&&savedTime>=0&&setRemainingTime(timerConfig,savedTime),addTimer(type,timerConfig),resolve()}).catch(function(){return addTimer(type,timerConfig),resolve()}):(addTimer(type,timerConfig),
resolve()):resolve()}))}),Promise.all(timerUpdatePromises).then(function(data){return toggleToggler(),data})};return store("timer-"+testRunner.getConfig().serviceCallId).then(function(timeStore){return self.shouldClearStorage?timeStore.clear().then(function(){return timeStore}):Promise.resolve(timeStore)}).then(function(timeStore){self.storage=timeStore,self.$element=$(timerBoxTpl()),self.$toggler=self.$element.find(".timer-toggler"),self.storage.getItem("zen-mode").then(function(zenMode){zenMode&&!self.$element.hasClass("zen-mode")&&toggleZenMode()}),self.$toggler.on("click",function(e){e.preventDefault(),toggleZenMode()}),self.stopwatch=timerFactory({
autoStart:!1}),self.polling=pollingFactory({action:function(){var timeoutScope,timeoutRef,elapsed=self.stopwatch.tick(),elapsedExtraTime=0,timeout=!1;_.forEach(displayedTimers,function(timer,type){var regularVal,displayedVal,warning,runTimeout=function(){testRunner.timeout(timeoutScope,timeoutRef)};timer.running()&&(regularVal=timers[type]-elapsed,displayedVal=timer.val()-elapsed,timers[type]=Math.max(regularVal,0),timer.val(Math.max(displayedVal,0)).refresh(),regularVal<=0&&(elapsedExtraTime=Math.max(elapsedExtraTime,Math.abs(regularVal))),displayedVal<=0?(timer.running(!1),timeout=!0,timeoutRef=timer.id(),timeoutScope=type,
self.storage.setItem(timer.id(),0).then(runTimeout).catch(runTimeout)):(self.storage.setItem(timer.id(),timers[type]),warning=timer.warn(),_.isEmpty(warning)||testRunner.trigger(warning.type,warning.text)))}),elapsedExtraTime&&(consumedExtraTime+=elapsedExtraTime/1e3,lastConsumedExtraTime+=elapsedExtraTime),timeout&&self.disable()},interval:1e3,autoStart:!1}),testRunner.before("renderitem resumeitem",function(){return updateTimers(!0)}).on("enableitem",doEnable).on("disableitem",doDisable).after("renderitem",doEnable).before("move",function(e,type,scope,position){
...
unansweredWarning:testOptions.unansweredWarning,stats:statsHelper.getInstantStats(warningScope,testRunner),unansweredOnly:unansweredOnly});!1!==self.getState("enabled")&&(testRunner.trigger("disablenav disabletools"),
warningHelper.shouldWarnBeforeEnd()?testRunner.trigger("confirm.endTest",messages.getExitMessage(__("You are about to submit the test. You will not be able to access this test once submitted. Click OK to continue and submit the test."),warningScope,testRunner),_.partial(triggerNextAction,context),enable):warningHelper.shouldWarnBeforeNext()?testRunner.trigger("confirm.next",__("You are about to go to the next item. Click OK to continue and go to the next item."),_.partial(triggerNextAction,context),enable):triggerNextAction(context))}function triggerNextAction(context){context.isLast&&self.trigger("end"),testRunner.next()}
...
return params=params||{},"move"===action&&"jump"===params.direction&&"item"===params.scope}};return navigationHelper}),define("taoQtiTest/runner/plugins/navigation/previous",["jquery","lodash","i18n","ui/hider","taoTests/runner/plugin","util/shortcut","util/namespace","taoQtiTest/runner/helpers/navigation","tpl!taoQtiTest/runner/plugins/templates/button"],function($,_,__,hider,pluginFactory,shortcut,namespaceHelper,navigationHelper,buttonTpl){"use strict";return pluginFactory({name:"previous",init:function(){function doPrevious(previousItemWarning){function enable(){testRunner.trigger("enablenav enabletools")}var context=testRunner.getTestContext()
;!1!==self.getState("enabled")&&(testRunner.trigger("disablenav disabletools"),previousItemWarning&&-1!==context.remainingAttempts?testRunner.trigger("confirm.previous",__("You are about to go to the previous item. Click OK to continue and go to the previous item."),testRunner.previous,enable):testRunner.previous())}var self=this,testRunner=this.getTestRunner(),testData=testRunner.getTestData(),testConfig=testData.config||{},pluginShortcuts=(testConfig.shortcuts||{})[this.getName()]||{},canDoPrevious=function(){var testMap=testRunner.getTestMap(),context=testRunner.getTestContext(),canMove=!navigationHelper.isFirst(testMap,context.itemIdentifier)
;return canMove&&context.isCatAdaptive&&(canMove=!navigationHelper.isFirstOf(testMap,context.itemIdentifier,"section")),canMove&&!1===context.isLinear&&!0===context.canMoveBackward},toggle=function(){canDoPrevious()?self.show():self.hide()};this.$element=$(buttonTpl({control:"move-backward",title:__("Submit and go to the previous item"),icon:"backward",text:__("Previous")})),this.$element.on("click",function(e){e.preventDefault(),testRunner.trigger("nav-previous")}),testConfig.allowShortcuts&&pluginShortcuts.trigger&&shortcut.add(namespaceHelper.namespaceAll(pluginShortcuts.trigger,this.getName(),!0),function(e){
canDoPrevious()&&!0===self.getState("enabled")&&testRunner.trigger("nav-previous",[!0])},{avoidInput:!0,prevent:!0}),toggle(),self.disable(),testRunner.on("loaditem",toggle).on("enablenav",function(){self.enable()}).on("disablenav",function(){self.disable()}).on("nav-previous",function(previousItemWarning){doPrevious(previousItemWarning)})},render:function(){this.getAreaBroker().getNavigationArea().append(this.$element)},destroy:function(){shortcut.remove("."+this.getName()),this.$element.remove()},enable:function(){this.$element.removeProp("disabled").removeClass("disabled")},disable:function(){this.$element.prop("disabled",!0).addClass("disabled")},
show:function(){hider.show(this.$element)},hide:function(){hider.hide(this.$element)}})}),define("tpl!taoQtiTest/runner/plugins/navigation/review/navigator",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){function program1(depth0,data){return" hidden"}function program3(depth0,data){var helper,options,buffer="";return buffer+='\n    <div class="qti-navigator-info collapsible">\n                <span class="qti-navigator-label">\n                    <span class="qti-navigator-text">'+escapeExpression((helper=helpers.__||depth0&&depth0.__,options={hash:{},data:data},
...
height:dimensions.outerHeight-2,"border-top-width":dimensions.topHeight-2,"border-right-width":dimensions.rightWidth-2,"border-bottom-width":dimensions.bottomHeight-2,"border-left-width":dimensions.leftWidth-2}),visualGuides.$innerWindow.css({width:dimensions.innerWidth,height:dimensions.innerHeight,left:dimensions.leftWidth-2,top:dimensions.topHeight-2})},removeVisualGuides:function(){visualGuides.$maskBg.remove(),visualGuides.$innerWindow.remove()}},overlayConfig),makeDraggable(overlay),makeStackable(overlay,stackingOptions),overlay.on("render",function(){var self=this
;this.getElement().addClass("line-reader-overlay "+overlayConfig.id).on("mousedown",function(){bringAllToFront(),self.transformOverlay()}).on("mouseup",function(){self.restoreOverlay()})}).on("dragstart",function(){innerDrag.hide(),closer.hide(),invokeOnMasks("hide"),this.appendVisualGuides()}).on("dragmove",function(xOffsetRelative,yOffsetRelative){position.outerX+=xOffsetRelative,position.outerY+=yOffsetRelative,position.innerX+=xOffsetRelative,position.innerY+=yOffsetRelative}).on("dragend",function(){this.removeVisualGuides(),invokeOnAll("show"),innerDrag.show(),closer.show(),applyTransforms()}).init()}function createInnerDragHandle(){
innerDrag=componentFactory(),makeStackable(innerDrag,stackingOptions),makeDraggable(innerDrag,{dragRestriction:function(){var fixedXY=allParts.nw.mask.getElement().offset();return{x:fixedXY.left+constrains.minWidth,y:fixedXY.top+(constrains.minHeight+dimensions.innerHeight+options.resizeHandleSize),width:dimensions.outerWidth-2*constrains.minWidth,height:dimensions.outerHeight-(dimensions.innerHeight+constrains.minHeight+constrains.minBottomHeight-options.innerDragHeight)}}}).on("render",function(){var $element=this.getElement(),$dragIcon=$("<div>",{class:"icon icon-move"});$element.addClass("line-reader-inner-drag"),$element.append($dragIcon),
$element.on("mousedown",function(e){e.stopPropagation(),bringAllToFront()})}).on("dragstart",function(){closer.hide(),invokeOnOverlays("hide"),invokeOnMasks("setState",["resizing",!0])}).on("dragmove",function(xOffsetRelative,yOffsetRelative){position.innerX+=xOffsetRelative,position.innerY+=yOffsetRelative,dimensions.leftWidth+=xOffsetRelative,dimensions.topHeight+=yOffsetRelative,dimensions.rightWidth-=xOffsetRelative,dimensions.bottomHeight-=yOffsetRelative,applyTransformsToMasks()}).on("dragend",function(){invokeOnOverlays("show"),innerDrag.bringToFront(),closer.show(),invokeOnMasks("setState",["resizing",!1]),applyTransformsToOverlays()}).init()}
function createCloser(){closer=componentFactory(),makeStackable(closer,stackingOptions),makePlaceable(closer).on("render",function(){var self=this,$element=this.getElement(),$closeIcon=$("<div>",{class:"icon icon-result-nok"});$element.append($closeIcon),$element.addClass("line-reader-closer"),$element.on("mousedown",function(){bringAllToFront()}),$element.on("click",function(e){e.stopPropagation(),self.trigger("click")})}).init()}function bringAllToFront(){invokeOnAll("bringToFront"),innerDrag.bringToFront(),closer.bringToFront()}function invokeOnAll(fn,args){invokeOnMasks(fn,args),invokeOnOverlays(fn,args)}function invokeOnMasks(fn,args){
invokeOn("mask",fn,args)}function invokeOnOverlays(fn,args){invokeOn("overlay",fn,args)}function invokeOn(target,fn,args){_.forOwn(allParts,function(part){_.isObject(part[target])&&_.isFunction(part[target][fn])&&part[target][fn].apply(part[target],args)})}function applyTransforms(){applyTransformsToMasks(),applyTransformsToOverlays(),applyTransformsToInnerDrag(),applyTransformsToCloser()}function applyTransformsToMasks(){invokeOnMasks("place")}function applyTransformsToOverlays(){_.forOwn(allParts,function(part){part.mask.placeOverlay(part.overlay)})}function applyTransformsToInnerDrag(){
