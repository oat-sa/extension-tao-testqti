...
testPart:__("Warning – You have %s remaining to complete this test part."),test:__("Warning – You have %s remaining to complete this test.")};return function(timeConstraints,isLinear,config,extraTime){var timers={},constraintsWarnings=_.reduce(config.warnings,function(acc,warnings,qtiScope){var scope=getScope(qtiScope);return acc[scope]=_.map(warnings,function(value,key){return{threshold:parseInt(key,10)*precision,message:function(remainingTime){var displayRemaining=moment.duration(remainingTime/precision,"seconds").humanize();return format(warningMessages[scope],displayRemaining)},level:value,shown:!1}}),acc},{}),buildTimer=function(type,constraintData){var timer=_.pick(constraintData,["label","scope","source","extraTime","qtiClassName"]);
return timer.type=type,timer.allowLateSubmission=constraintData.allowLateSubmission,"max"===type?timer.id=constraintData.source:timer.id=type+"-"+constraintData.scope+"-"+constraintData.source,"min"===type?(timer.originalTime=constraintData.minTime*precision,timer.remainingTime=constraintData.minTimeRemaining*precision):(timer.originalTime=constraintData.maxTime*precision,timer.remainingTime=constraintData.maxTimeRemaining*precision),timer.extraTime>0&&(timer.extraTime=timer.extraTime*precision),extraTime&&(timer.remainingTime+=extraTime.remaining*precision),"max"===type&&_.isArray(constraintsWarnings[timer.scope])&&(timer.warnings=constraintsWarnings[timer.scope]),
timer};return _.forEach(timeConstraints,function(timeConstraint){var newTimer,constraintData=_.clone(timeConstraint);constraintData.scope=getScope(timeConstraint.scope||timeConstraint.qtiClassName),constraintData.scope?constraintData.minTime===!1&&constraintData.maxTime===!1?logger.warning("Time constraint defined with no time, skipping"):config.guidedNavigation&&isLinear&&constraintData.maxTime&&constraintData.minTime&&constraintData.minTime===constraintData.maxTime&&constraintData.maxTime>0?(newTimer=buildTimer("locked",constraintData),timers[newTimer.id]=newTimer):(isLinear&&constraintData.minTime&&constraintData.minTime>0&&(newTimer=buildTimer("min",constraintData),
timers[newTimer.id]=newTimer),constraintData.maxTime&&constraintData.maxTime>0&&(newTimer=buildTimer("max",constraintData),timers[newTimer.id]=newTimer)):logger.warning("Wrong data, a time constraint should apply to a valid scope, skipping")}),logger.debug("Timers built from timeConstraints",timers),timers}}),define("taoQtiTest/runner/plugins/controls/timer/plugin",["jquery","lodash","core/promise","taoTests/runner/plugin","taoQtiTest/runner/plugins/controls/timer/strategy/strategyHandler","taoQtiTest/runner/plugins/controls/timer/component/timerbox","taoQtiTest/runner/plugins/controls/timer/timers"],function($,_,Promise,pluginFactory,getStrategyHandler,timerboxFactory,timersFactory){
"use strict";return pluginFactory({name:"timer",install:function(){var testRunner=this.getTestRunner();this.loadTimers=function(timeStore,config){var testContext=testRunner.getTestContext(),timeConstraints=testContext.timeConstraints,isLinear=!!testContext.isLinear,extraTime=testContext.extraTime,timers=timersFactory(timeConstraints,isLinear,config,extraTime);return Promise.all(_.map(timers,function(timer){return timeStore.getItem(timer.id).then(function(savedTime){var consumed,total=timer.originalTime+timer.extraTime;timer.remainingTime!==timer.originalTime&&0!==timer.remainingTime||savedTime||(timer.remainingTime=timer.originalTime+timer.extraTime),_.isNumber(savedTime)&&savedTime>=0&&config.restoreTimerFromClient&&(timer.remainingTime=savedTime),
consumed=total-timer.remainingTime,_.isNumber(timer.extraTime)&&timer.extraTime>0&&(timer.remainingWithoutExtraTime=consumed>timer.originalTime?0:timer.originalTime-consumed)})})).then(function(){return timers})},this.saveTimers=function(timeStore,timers){return Promise.all(_.map(timers,function(timer){return timeStore.setItem(timer.id,timer.remainingTime)}))},testRunner.getTestStore().setVolatile(this.getName())},init:function(){var self=this,testRunner=this.getTestRunner(),testData=testRunner.getTestData(),config=_.merge({},this.getConfig(),{contextualWarnings:!1,warnings:testData&&testData.config&&testData.config.timerWarning||{},guidedNavigation:testData&&testData.config&&testData.config.guidedNavigation,
restoreTimerFromClient:testData&&testData.config&&testData.config.timer.restoreTimerFromClient}),strategyHandler=getStrategyHandler(testRunner),handleError=function(err){testRunner.trigger("error",err)};return new Promise(function(resolve){return testRunner.getPluginStore(self.getName()).then(function(timeStore){testRunner.before("renderitem resumeitem",function(){var testContext=testRunner.getTestContext();if(self.timerbox&&testContext.timeConstraints)return self.loadTimers(timeStore,config).then(function(timers){return self.timerbox.update(timers)}).catch(handleError)}).on("enableitem",function(){self.timerbox&&self.timerbox.start()}).after("renderitem",function(){
self.timerbox&&self.timerbox.start()}).on("disableitem move skip",function(){self.timerbox&&self.timerbox.stop()}),timeStore.getItem("zen-mode").then(function(startZen){self.timerbox=timerboxFactory({zenMode:{enabled:!0,startHidden:!!startZen},displayWarning:config.contextualWarnings}).on("change",_.throttle(function(){self.saveTimers(timeStore,this.getTimers())},1e3)).on("timeradd",function(timer){strategyHandler.setUp(timer).catch(handleError)}).on("timerremove",function(timer){strategyHandler.tearDown(timer).catch(handleError)}).on("timerstart",function(timer){strategyHandler.start(timer).catch(handleError)}).on("timerstop",function(timer){strategyHandler.stop(timer).catch(handleError);
}).on("timerend",function(timer){strategyHandler.complete(timer).catch(handleError)}).on("timerchange",function(action,timer){self.trigger(action+"timer",timer.qtiClassName,timer)}).on("zenchange",function(isZen){timeStore.setItem("zen-mode",!!isZen)}).on("init",resolve).on("error",handleError),config.contextualWarnings||self.timerbox.on("warn",function(message,level){level&&message&&testRunner.trigger(level,message)})}).catch(handleError)})})},render:function(){this.timerbox.render(this.getAreaBroker().getControlArea())},destroy:function(){this.timerbox&&this.timerbox.stop().destroy()},show:function(){this.timerbox&&this.timerbox.show()},hide:function(){this.timerbox&&this.timerbox.hide();
