...
closeDialogs(".@"),dialogShortcut.clear(),dialogShortcut=null})}})}),define("taoQtiTest/runner/plugins/content/dialog/exitMessages",["lodash","core/promise","taoTests/runner/plugin"],function(_,Promise,pluginFactory){"use strict";return pluginFactory({name:"exitMessages",init:function(){},install:function(){var testRunner=this.getTestRunner();testRunner.before("leave",function(e,data){return _.isObject(data)&&data.message?new Promise(function(resolve){var context=testRunner.getTestContext();context&&context.itemIdentifier&&testRunner.disableItem(context.itemIdentifier),testRunner.trigger("alert.leave",data.message,resolve)}):void 0})}})}),define("taoQtiTest/runner/plugins/content/dialog/itemAlertMessage",["jquery","i18n","ui/hider","taoTests/runner/plugin"],function($,__,hider,pluginFactory){
...
"use strict";return pluginFactory({name:"connectivity",init:function(){},install:function(){function disconnect(){testRunner.getState("disconnected")||testRunner.trigger("disconnect")}function reconnect(){testRunner.getState("disconnected")&&testRunner.trigger("reconnect")}var testRunner=this.getTestRunner();$(window).on("offline.connectivity",function(){disconnect()}),$(window).on("online.connectivity",function(){reconnect()}),testRunner.on("disconnect",function(){testRunner.setState("disconnected",!0)}).on("reconnect",function(){testRunner.setState("disconnected",!1)}).before("error",function(e,error){return _.isObject(error)&&"network"===error.source&&"undefined"==typeof error.code?(disconnect(),
...
}),partially===!1?count>0&&0===empty:count>0&&count>empty}};return currentItemHelper}),define("taoQtiTest/runner/helpers/stats",["lodash","taoQtiTest/runner/helpers/map","taoQtiTest/runner/helpers/currentItem"],function(_,mapHelper,currentItemHelper){"use strict";function getInstantStats(scope,runner){var map=runner.getTestMap(),context=runner.getTestContext(),stats=_.clone(mapHelper.getScopeStats(map,context.itemPosition,scope)),isItemCurrentlyAnswered=currentItemHelper.isAnswered(runner);return isItemCurrentlyAnswered&&(!isItemCurrentlyAnswered&&context.itemAnswered&&stats.answered--,isItemCurrentlyAnswered&&!context.itemAnswered&&stats.answered++),stats}return{
...
self.enable()}).on("disablenav",function(){self.disable()})},render:function(){var $container=this.getAreaBroker().getNavigationArea();$container.append(this.$element)},destroy:function(){this.$element.remove()},enable:function(){this.$element.removeProp("disabled").removeClass("disabled")},disable:function(){this.$element.prop("disabled",!0).addClass("disabled")},show:function(){hider.show(this.$element)},hide:function(){hider.hide(this.$element)}})}),define("taoQtiTest/runner/plugins/navigation/preventSkipping",["lodash","i18n","taoTests/runner/plugin","taoQtiTest/runner/helpers/currentItem"],function(_,__,pluginFactory,currentItemHelper){"use strict";return pluginFactory({
name:"preventSkipping",init:function(){function toggle(){var context=testRunner.getTestContext();context.preventEmptyResponses===!0?self.enable():self.disable()}var self=this,testRunner=this.getTestRunner();toggle(),testRunner.before("move",function(){return self.getState("enabled")===!0?(this.trigger("disablenav disabletools"),new Promise(function(resolve,reject){return currentItemHelper.isAnswered(testRunner)?resolve():void testRunner.trigger("alert.notallowed",__("A response to this item is required."),function(){testRunner.trigger("resumeitem"),reject()})})):void 0}).after("move skip",function(){toggle()})}})}),define("taoQtiTest/runner/plugins/navigation/previous",["jquery","lodash","i18n","ui/hider","taoTests/runner/plugin","util/shortcut","util/namespace","tpl!taoQtiTest/runner/plugins/templates/button"],function($,_,__,hider,pluginFactory,shortcut,namespaceHelper,buttonTpl){
