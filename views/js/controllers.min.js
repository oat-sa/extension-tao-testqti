...
hash:{},inverse:self.noop,fn:self.program(1,program1,data),data:data}),(stack1||0===stack1)&&(buffer+=stack1),buffer+="\n</div>\n"})}),define("tpl!taoQtiTest/controller/creator/templates/testpart-props",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,options,buffer="",functionType="function",escapeExpression=this.escapeExpression,helperMissing=helpers.helperMissing;return buffer+='<div id="testpart-props-',(helper=helpers.identifier)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.identifier,
stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+'" class="testpart-props props clearfix">\n    <h3>',(helper=helpers.identifier)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.identifier,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+'</h3>\n\n    <form autocomplete="off">\n\n<!-- assessmentTest/testPart/identifier -->\n        <div class="grid-row">\n            <div class="col-5">\n                <label for="testpart-identifier">'+escapeExpression((helper=helpers.__||depth0&&depth0.__,options={hash:{},
data:data},helper?helper.call(depth0,"Identifier",options):helperMissing.call(depth0,"__","Identifier",options)))+' <abbr title="'+escapeExpression((helper=helpers.__||depth0&&depth0.__,options={hash:{},data:data},helper?helper.call(depth0,"Required field",options):helperMissing.call(depth0,"__","Required field",options)))+'">*</abbr></label>\n            </div>\n            <div class="col-6">\n                <input type="text" name="testpart-identifier" data-bind="identifier" data-validate="$notEmpty; $idFormat; $testIdAvailable(original=',(helper=helpers.identifier)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.identifier,stack1=typeof helper===functionType?helper.call(depth0,{
...
options={hash:{},data:data},helper?helper.call(depth0,"Feedback block",options):helperMissing.call(depth0,"__","Feedback block",options)))+"</h4>\n\n    <!--assessmentTest/testPart/assessmentSection/sectionPart/rubricBlock/feedback -->\n    ",stack1=helpers.with.call(depth0,depth0&&depth0.feedback,{hash:{},inverse:self.noop,fn:self.program(1,program1,data),data:data}),(stack1||0===stack1)&&(buffer+=stack1),buffer+="\n</div>"})}),define("taoQtiTest/controller/creator/templates/index",["tpl!taoQtiTest/controller/creator/templates/testpart","tpl!taoQtiTest/controller/creator/templates/section","tpl!taoQtiTest/controller/creator/templates/rubricblock","tpl!taoQtiTest/controller/creator/templates/itemref","tpl!taoQtiTest/controller/creator/templates/item","tpl!taoQtiTest/controller/creator/templates/test-props","tpl!taoQtiTest/controller/creator/templates/testpart-props","tpl!taoQtiTest/controller/creator/templates/section-props","tpl!taoQtiTest/controller/creator/templates/itemref-props","tpl!taoQtiTest/controller/creator/templates/itemref-props-weight","tpl!taoQtiTest/controller/creator/templates/rubricblock-props"],function(testPart,section,rubricBlock,itemRef,item,testProps,testPartProps,sectionProps,itemRefProps,itemRefPropsWeight,rubricBlockProps){
"use strict";return{testpart:testPart,section:section,itemref:itemRef,item:item,rubricblock:rubricBlock,properties:{test:testProps,testpart:testPartProps,section:sectionProps,itemref:itemRefProps,itemrefweight:itemRefPropsWeight,rubricblock:rubricBlockProps}}}),define("taoQtiTest/controller/creator/views/item",["jquery","lodash","taoQtiTest/controller/creator/templates/index"],function($,_,templates){"use strict";var itemTemplate=templates.item,itemView=function(loadItems,getCategories){function setUpLiveSearch(){var timeout,liveSearch=function(){var pattern=$search.val();(pattern.length>1||0===pattern.length)&&(clearTimeout(timeout),timeout=setTimeout(function(){
getItems(pattern)},300))};$search.keyup(liveSearch).siblings(".ctrl").click(liveSearch)}function update(items){disableSelection(),$itemBox.empty().append(itemTemplate(items)),enableSelection()}function disableSelection(){$panel.data("selectable")&&$panel.selectable("disable")}function enableSelection(){$panel.data("selectable")?$panel.selectable("enable"):$panel.selectable({filter:"li",selected:function(event,ui){$(ui.selected).addClass("selected")},unselected:function(event,ui){$(ui.unselected).removeClass("selected")},stop:function(){$(this).trigger("itemselect.creator",$(".selected"))}})}var $panel=$(".test-creator-items .item-selection"),$search=$("#item-filter"),$itemBox=$(".item-box",$panel),getItems=function(pattern){
return loadItems(pattern).then(function(items){return items&&items.length?getCategories(_.pluck(items,"uri")).then(function(categories){update(_.map(items,function(item){return item.categories=_.isArray(categories[item.uri])?categories[item.uri]:[],item}))}):update()})};getItems().then(setUpLiveSearch)};return itemView}),define("taoQtiTest/controller/creator/views/property",["jquery","ui","core/databinder","taoQtiTest/controller/creator/templates/index"],function($,ui,DataBinder,templates){"use strict";var propView=function(tmplName,model){function propValidation(){$view.on("validated.group",function(e,isValid){if("group"===e.namespace){var $togglers=$("#test-creator .property-toggler, #saver");
...
_.forEach(outcomeHelper.getOutcomeDeclarations(testModel),cb)},eachOutcomeProcessingRules:function(testModel,cb){_.forEach(outcomeHelper.getOutcomeProcessingRules(testModel),cb)},eachOutcomeProcessingRuleExpressions:function(testModel,cb){function browseExpressions(processingRule){_.isArray(processingRule)?_.forEach(processingRule,browseExpressions):processingRule&&(cb(processingRule),processingRule.expression?browseExpressions(processingRule.expression):processingRule.expressions&&browseExpressions(processingRule.expressions))}browseExpressions(outcomeHelper.getOutcomeProcessingRules(testModel))},listOutcomes:function(testModel,cb){var outcomes=[];return _.isFunction(cb)||(cb=null),
outcomeHelper.eachOutcomeDeclarations(testModel,function(outcome){cb&&!cb(outcome)||outcomes.push(outcomeHelper.getOutcomeIdentifier(outcome))}),outcomes},removeOutcomes:function(testModel,outcomes){var declarations=outcomeHelper.getOutcomeDeclarations(testModel),rules=outcomeHelper.getOutcomeProcessingRules(testModel);outcomes=_.indexBy(_.isArray(outcomes)?outcomes:[outcomes],function(outcome){return outcome}),declarations&&_.remove(declarations,function(outcomeDeclaration){return!!outcomes[outcomeHelper.getOutcomeIdentifier(outcomeDeclaration)]}),rules&&_.remove(rules,function(outcomeRule){return!!outcomes[outcomeHelper.getOutcomeIdentifier(outcomeRule)]})},
createOutcome:function(identifier,type,cardinality){if(!outcomeValidator.validateIdentifier(identifier))throw new TypeError("You must provide a valid identifier!");return qtiElementHelper.create("outcomeDeclaration",identifier,{views:[],interpretation:"",longInterpretation:"",normalMaximum:!1,normalMinimum:!1,masteryValue:!1,cardinality:cardinalityHelper.getValid(cardinality,cardinalityHelper.SINGLE),baseType:baseTypeHelper.getValid(type,baseTypeHelper.FLOAT)})},addOutcomeProcessing:function(testModel,processingRule){var outcomeProcessing=testModel.outcomeProcessing;if(!outcomeValidator.validateOutcome(processingRule))throw new TypeError("You must provide a valid outcome processing rule!");
return outcomeProcessing?outcomeProcessing.outcomeRules||(outcomeProcessing.outcomeRules=[]):(outcomeProcessing={"qti-type":"outcomeProcessing",outcomeRules:[]},testModel.outcomeProcessing=outcomeProcessing),outcomeProcessing.outcomeRules.push(processingRule),processingRule},addOutcome:function(testModel,outcome,processingRule){var declarations=testModel.outcomeDeclarations;if(!outcomeValidator.validateOutcome(outcome,!0,"outcomeDeclaration"))throw new TypeError("You must provide a valid outcome!");if(processingRule){if(!outcomeValidator.validateOutcome(processingRule)||processingRule.identifier!==outcome.identifier)throw new TypeError("You must provide a valid outcome processing rule!");
outcomeHelper.addOutcomeProcessing(testModel,processingRule)}return declarations||(declarations=[],testModel.outcomeDeclarations=declarations),declarations.push(outcome),outcome}};return outcomeHelper}),define("taoQtiTest/controller/creator/helpers/qtiTest",["lodash","i18n","taoQtiTest/controller/creator/helpers/outcome","taoQtiTest/controller/creator/helpers/qtiElement"],function(_,__,outcomeHelper,qtiElementHelper){"use strict";var qtiTestHelper={extractIdentifiers:function(obj){var self=this,identifiers=[];return _.has(obj,"identifier")&&(identifiers=identifiers.concat(obj.identifier.toLowerCase())),_.flatten(_.forEach(obj,function(value){identifiers=identifiers.concat("object"==typeof value?self.extractIdentifiers(value):[]);
}),!0),identifiers},getIdentifier:function(qtiType,lockedIdentifiers){var suggestion,index=1,glue="-";do suggestion=qtiType+glue+index++;while(_.contains(lockedIdentifiers,suggestion.toLowerCase()));return lockedIdentifiers.push(suggestion.toLowerCase()),suggestion},idFormatValidator:function(){var qtiIdPattern=/^[_a-zA-Z]{1}[a-zA-Z0-9\-._]{0,31}$/i;return{name:"idFormat",message:__("is not a valid identifier (alphanum, underscore, dash and dots)"),validate:function(value,callback){"function"==typeof callback&&callback(qtiIdPattern.test(value))}}},testidFormatValidator:function(){var qtiTestIdPattern=/^\S+$/;return{name:"testIdFormat",message:__("is not a valid identifier (everything except spaces)"),
validate:function(value,callback){"function"==typeof callback&&callback(qtiTestIdPattern.test(value))}}},idAvailableValidator:function(lockedIdentifiers){return{name:"testIdAvailable",message:__("is already used in the test."),validate:function(value,callback,options){"function"==typeof callback&&callback(!_.contains(_.values(lockedIdentifiers),value.toLowerCase())||options.original&&value===options.original)}}},filterQtiType:function(value,type){return value["qti-type"]&&value["qti-type"]===type},addMissingQtiType:function(collection,parentType){var self=this;_.forEach(collection,function(value,key){!_.isObject(value)||_.isArray(value)||_.has(value,"qti-type")||(_.isNumber(key)?parentType&&(value["qti-type"]=parentType):value["qti-type"]=key),
_.isArray(value)?self.addMissingQtiType(value,key.replace(/s$/,"")):_.isObject(value)&&self.addMissingQtiType(value)})},consolidateModel:function(model){return model&&model.testParts&&_.isArray(model.testParts)&&_.forEach(model.testParts,function(testPart){testPart.assessmentSections&&_.isArray(testPart.assessmentSections)&&_.forEach(testPart.assessmentSections,function(assessmentSection){assessmentSection.ordering&&void 0!==assessmentSection.ordering.shuffle&&assessmentSection.ordering.shuffle===!1&&delete assessmentSection.ordering,assessmentSection.sectionParts&&_.isArray(assessmentSection.sectionParts)&&_.forEach(assessmentSection.sectionParts,function(part){
part.categories&&_.isArray(part.categories)&&(0===part.categories.length||0===part.categories[0].length)&&(part.categories=[])}),assessmentSection.rubricBlocks&&_.isArray(assessmentSection.rubricBlocks)&&(0===assessmentSection.rubricBlocks.length||1===assessmentSection.rubricBlocks.length&&0===assessmentSection.rubricBlocks[0].content.length?delete assessmentSection.rubricBlocks:assessmentSection.rubricBlocks.length>0&&_.forEach(assessmentSection.rubricBlocks,function(rubricBlock){rubricBlock.views=["candidate"]}))})}),model},validateModel:function(model){var outcomes=_.indexBy(outcomeHelper.listOutcomes(model));_.forEach(model.testParts,function(testPart){_.forEach(testPart.assessmentSections,function(assessmentSection){
_.forEach(assessmentSection.rubricBlocks,function(rubricBlock){var feedbackBlock=qtiElementHelper.lookupElement(rubricBlock,"rubricBlock.div.feedbackBlock","content");if(feedbackBlock&&!outcomes[feedbackBlock.outcomeIdentifier])throw new Error(__('The outcome "%s" does not exist, but it is referenced by a feedback block!',feedbackBlock.outcomeIdentifier))})})})}};return qtiTestHelper}),define("taoQtiTest/controller/creator/views/itemref",["jquery","lodash","i18n","taoQtiTest/controller/creator/views/actions","taoQtiTest/controller/creator/helpers/sectionCategory","taoQtiTest/controller/creator/helpers/qtiTest","tpl!taoQtiTest/controller/creator/templates/itemref-props-weight"],function($,_,__,actions,sectionCategory,qtiTestHelper,weightTpl){
"use strict";var resize=_.throttle(function(){var $refs=$(".itemrefs").first(),$actions=$(".itemref .actions").first(),width=$refs.innerWidth()-$actions.outerWidth();$(".itemref > .title").width(width)},100),setUp=function($itemRef,model){function propHandler(propView){function removePropHandler(){null!==propView&&propView.destroy()}categoriesProperty(propView.getView()),weightsProperty(propView),$itemRef.parents(".testpart").on("delete",removePropHandler),$itemRef.parents(".section").on("delete",removePropHandler),$itemRef.on("delete",removePropHandler)}function categoriesProperty($view){function initCategories(){$select.select2("val",model.categories)}var $select=$view.find("[name=itemref-category]");
$select.select2({width:"100%",tags:_.pluck(sectionCategory.getTaoOptionCategories(),"name"),multiple:!0,tokenSeparators:[","," ",";"],formatNoMatches:function(){return __("Enter a category")},maximumInputLength:32}),initCategories(),$view.on("propopen.propview",function(){initCategories()})}function weightsProperty(propView){var $view=propView.getView(),$weightList=$view.find('[data-bind-each="weights"]');$view.find(".itemref-weight-add").on("click",function(e){var defaultData={value:1,identifier:0===model.weights.length?"WEIGHT":qtiTestHelper.getIdentifier("WEIGHT",qtiTestHelper.extractIdentifiers(model))};e.preventDefault(),$weightList.append(weightTpl(defaultData)),
$weightList.trigger("add.internalbinder"),$view.groupValidator()})}var $actionContainer=$(".actions",$itemRef);actions.properties($actionContainer,"itemref",model,propHandler),actions.move($actionContainer,"itemrefs","itemref"),resize()},listenActionState=function(){$(".itemrefs").each(function(){actions.movable($(".itemref",$(this)),"itemref",".actions")}),$(document).on("delete",function(e){var $parent,$target=$(e.target);$target.hasClass("itemref")&&($parent=$target.parents(".itemrefs"),actions.disable($parent.find(".itemref"),".actions"))}).on("add change undo.deleter deleted.deleter",".itemrefs",function(e){var $parent,$target=$(e.target);($target.hasClass("itemref")||$target.hasClass("itemrefs"))&&($parent=$(".itemref",$target.hasClass("itemrefs")?$target:$target.parents(".itemrefs")),
actions.enable($parent,".actions"),actions.movable($parent,"itemref",".actions"))})};return{setUp:setUp,listenActionState:listenActionState,resize:resize}}),define("taoQtiTest/controller/creator/encoders/dom2qti",["jquery","lodash","taoQtiTest/controller/creator/helpers/qtiElement"],function($,_,qtiElementHelper){"use strict";function getAttributes(object){return _.omit(object,["qti-type","content","xmlBase","lang","label"])}function attrToStr(attributes){return _.reduce(attributes,function(acc,value,key){return _.isNumber(value)||_.isString(value)&&!_.isEmpty(value)?acc+" "+key+'="'+value+'" ':acc},"")}function normalizeNodeName(nodeName){var normalized=nodeName.toLocaleLowerCase();
return normalizedNodes[normalized]||normalized}var normalizedNodes={feedbackblock:"feedbackBlock",outcomeidentifier:"outcomeIdentifier",showhide:"showHide"};return{encode:function(modelValue){var self=this;if(_.isArray(modelValue))return _.reduce(modelValue,function(result,value){return result+self.encode(value)},"");if(_.isObject(modelValue)&&modelValue["qti-type"]){if("textRun"===modelValue["qti-type"])return modelValue.content;var startTag="<"+modelValue["qti-type"]+attrToStr(getAttributes(modelValue));return modelValue.content?startTag+">"+self.encode(modelValue.content)+"</"+modelValue["qti-type"]+">":startTag+"/>"}return""+modelValue},decode:function(nodeValue){
var self=this,$nodeValue=nodeValue instanceof $?nodeValue:$(nodeValue),result=[];return _.forEach($nodeValue,function(elt){var object;3===elt.nodeType?_.isEmpty($.trim(elt.nodeValue))||result.push(qtiElementHelper.create("textRun",{content:elt.nodeValue,xmlBase:""})):1===elt.nodeType&&(object=_.merge(qtiElementHelper.create(normalizeNodeName(elt.nodeName),{id:"",class:"",xmlBase:"",lang:"",label:""}),_.transform(elt.attributes,function(acc,value){value.nodeName&&(acc[normalizeNodeName(value.nodeName)]=value.nodeValue)})),elt.childNodes.length>0&&(object.content=self.decode(elt.childNodes)),result.push(object))}),result}}}),define("taoQtiTest/controller/creator/helpers/ckConfigurator",["ui/ckeditor/ckConfigurator","mathJax"],function(ckConfigurator,mathJax){
"use strict";var getConfig=function(editor,toolbarType,options){return options=options||{},options.underline=!0,ckConfigurator.getConfig(editor,toolbarType,options)};return{getConfig:getConfig}}),define("taoQtiTest/controller/creator/views/rubricblock",["jquery","lodash","ui/hider","util/namespace","taoQtiTest/controller/creator/views/actions","helpers","ckeditor","taoQtiTest/controller/creator/encoders/dom2qti","taoQtiTest/controller/creator/helpers/ckConfigurator","taoQtiTest/controller/creator/helpers/qtiElement"],function($,_,hider,namespaceHelper,actions,helpers,ckeditor,Dom2QtiEncoder,ckConfigurator,qtiElementHelper){"use strict";function filterPlugin(plugin){
return _.contains(["taoqtiimage","taoqtimedia","taoqtimaths","taoqtiinclude"],plugin)}var ckConfig=ckConfigurator.getConfig(ckeditor,"qtiBlock");return ckConfig.plugins=_.reject(ckConfig.plugins.split(","),filterPlugin).join(","),ckConfig.extraPlugins=_.reject(ckConfig.extraPlugins.split(","),filterPlugin).join(","),{setUp:function($rubricBlock,model){function bindEvent($el,eventName,cb){return eventName=namespaceHelper.namespaceAll(eventName,model.uid),$el.off(eventName).on(eventName,cb)}function updateContent(){$rubricBlockContent.html(Dom2QtiEncoder.encode(model.content)),syncRubricBlockContent()}function propHandler(propView){function changeFeedback(feedback){
var activated=feedback&&feedback.activated,wrapper=qtiElementHelper.lookupElement(model,"rubricBlock.div.feedbackBlock","content");hider.toggle($feedbackOutcomeLine,activated),hider.toggle($feedbackMatchLine,activated),activated?(wrapper?(wrapper.outcomeIdentifier=feedback.outcome,wrapper.identifier=feedback.matchValue):model.content=[qtiElementHelper.create("div",{content:[qtiElementHelper.create("feedbackBlock",{outcomeIdentifier:feedback.outcome,identifier:feedback.matchValue,content:model.content})]})],updateContent()):wrapper&&(model.content=wrapper.content,updateContent())}function removePropHandler(){model.feedback={},null!==propView&&propView.destroy();
}function changeHandler(e,changedModel){"binder"===e.namespace&&"rubricBlock"===changedModel["qti-type"]&&changeFeedback(changedModel.feedback)}var $view=propView.getView(),$feedbackOutcomeLine=$(".rubric-feedback-outcome",$view),$feedbackMatchLine=$(".rubric-feedback-match-value",$view);$("[name=type]",$view).select2({minimumResultsForSearch:-1,width:"100%"}),$view.on("change.binder",changeHandler),bindEvent($rubricBlock.parents(".testpart"),"delete",removePropHandler),bindEvent($rubricBlock.parents(".section"),"delete",removePropHandler),bindEvent($rubricBlock,"delete",removePropHandler),changeFeedback(model.feedback),rbViews($view)}function rbViews($propContainer){
var $select=$("[name=view]",$propContainer);bindEvent($select.select2({width:"100%"}),"select2-removed",function(){0===$select.select2("val").length&&$select.select2("val",[1])}),0===$select.select2("val").length&&$select.select2("val",[1])}var editor,$rubricBlockBinding=$(".rubricblock-binding",$rubricBlock),$rubricBlockContent=$(".rubricblock-content",$rubricBlock),syncRubricBlockContent=_.throttle(function(){$rubricBlockBinding.html($rubricBlockContent.html()).trigger("change")},100);model.orderIndex=model.index+1,model.uid=_.uniqueId("rb"),model.feedback={activated:!!qtiElementHelper.lookupElement(model,"rubricBlock.div.feedbackBlock","content"),outcome:qtiElementHelper.lookupProperty(model,"rubricBlock.div.feedbackBlock.outcomeIdentifier","content"),
matchValue:qtiElementHelper.lookupProperty(model,"rubricBlock.div.feedbackBlock.identifier","content")},actions.properties($rubricBlock,"rubricblock",model,propHandler),$rubricBlockContent.empty().html($rubricBlockBinding.html()),editor=ckeditor.inline($rubricBlockContent[0],ckConfig),editor.on("change",function(){syncRubricBlockContent()})}}}),define("taoQtiTest/controller/creator/helpers/sectionBlueprints",["lodash","i18n","core/errorHandler"],function(_,__,errorHandler){"use strict";function setBlueprint(model,blueprint){model.blueprint=blueprint}function getBlueprint(getUrl,model){return $.ajax({url:getUrl,type:"GET",data:{section:model.identifier},dataType:"json"
...
$selectionSwitcher.prop("checked")||($selectionSelect.val(0),$selectionWithRep.prop("checked",!1),delete model.selection)}),$selectionSwitcher.prop("checked",!!model.selection).trigger("change"),$title=$("[data-bind=title]",$section),$view.on("change.binder",function(e){"binder"===e.namespace&&"assessmentSection"===model["qti-type"]&&$title.text(model.title)}),$section.parents(".testpart").on("deleted.deleter",removePropHandler),$section.on("deleted.deleter",removePropHandler),categoriesProperty($view),void 0!==model.hasBlueprint&&blueprintProperty($view)}function itemRefs(){model.sectionParts||(model.sectionParts=[]),$(".itemref",$section).each(function(){var $itemRef=$(this),index=$itemRef.data("bind-index");
model.sectionParts[index]||(model.sectionParts[index]={}),itemRefView.setUp($itemRef,model.sectionParts[index]),$itemRef.find(".title").text(data.labels[uri.encode($itemRef.data("uri"))])})}function acceptItemRefs(){var $selected,$items=$(".test-creator-items");$items.on("itemselect.creator",function(){var selection=Array.prototype.slice.call(arguments,1),$placeholder=$(".itemref-placeholder",$section),$placeholders=$(".itemref-placeholder");selection.length>0?$placeholder.show().off("click").on("click",function(){var categories,defaultItemData={};model.itemSessionControl&&!_.isUndefined(model.itemSessionControl.maxAttempts)&&(defaultItemData.itemSessionControl=_.clone(model.itemSessionControl)),
categories=sectionCategory.getCategories(model),defaultItemData.categories=_.clone(categories.propagated),_.forEach(selection,function(item){var $item=$(item),itemCategories=$item.data("categories"),itemData=_.defaults({href:uri.decode($item.data("uri")),label:$.trim($item.clone().children().remove().end().text()),"qti-type":"assessmentItemRef"},defaultItemData);_.isEmpty(itemCategories)||(itemData.categories=_.merge(itemData.categories,itemCategories.trim().split(","))),addItemRef($(".itemrefs",$section),void 0,itemData)}),$(".ui-selected",$items).removeClass("ui-selected").removeClass("selected"),$placeholders.hide().off("click")}):$placeholders.hide().off("click");
}),$(document).off("add.binder","#"+$section.attr("id")+" .itemrefs").on("add.binder","#"+$section.attr("id")+" .itemrefs",function(e,$itemRef){var index;"binder"===e.namespace&&$itemRef.hasClass("itemref")&&(index=$itemRef.data("bind-index"),itemRefView.setUp($itemRef,model.sectionParts[index]))}),$selected=$(".selected",$items),$selected.length>0&&$items.trigger("itemselect.creator",$selected)}function addItemRef($refList,index,itemData){var $itemRef,$items=$refList.children("li");index=index||$items.length,itemData.identifier=qtiTestHelper.getIdentifier("item",data.identifiers),itemData.index=index+1,$itemRef=$(templates.itemref(itemData)),index>0?$itemRef.insertAfter($items.eq(index-1)):$itemRef.appendTo($refList),
$refList.trigger("add",[$itemRef,itemData])}function rubricBlocks(){model.rubricBlocks||(model.rubricBlocks=[]),$(".rubricblock",$section).each(function(){var $rubricBlock=$(this),index=$rubricBlock.data("bind-index");model.rubricBlocks[index]||(model.rubricBlocks[index]={}),rubricBlockView.setUp($rubricBlock,model.rubricBlocks[index],data)}),model.rubricBlocks.length>0&&$(".rub-toggler",$section).trigger("click")}function addRubricBlock(){$(".rublock-adder",$section).adder({target:$(".rubricblocks",$section),content:templates.rubricblock,templateData:function(cb){cb({"qti-type":"rubricBlock",index:$(".rubricblock",$section).length,content:[],views:[1]})}}),$(document).on("add.binder","#"+$section.attr("id")+" .rubricblocks",function(e,$rubricBlock){
var index;"binder"===e.namespace&&$rubricBlock.hasClass("rubricblock")&&(index=$rubricBlock.data("bind-index"),$(".rubricblock-binding",$rubricBlock).html("<p>&nbsp;</p>"),rubricBlockView.setUp($rubricBlock,model.rubricBlocks[index],data))})}function categoriesProperty($view){function initCategories(){var categories=sectionCategory.getCategories(model);$select.select2("val",categories.all),$select.siblings(".select2-container").find(".select2-search-choice").each(function(){var $li=$(this),content=$li.find("div").text();_.indexOf(categories.partial,content)>=0&&$li.addClass("partial")})}function setCategories(categories){sectionCategory.setCategories(model,categories);
}var $select=$("[name=section-category]",$view);$select.select2({width:"100%",tags:_.pluck(sectionCategory.getTaoOptionCategories(),"name"),multiple:!0,tokenSeparators:[","," ",";"],formatNoMatches:function(){return __("Enter a category")},maximumInputLength:32}).on("change",function(e){setCategories(e.val)}),initCategories(),$view.on("propopen.propview",function(){initCategories()})}function blueprintProperty($view){function initBlueprint(){void 0===model.blueprint&&sectionBlueprint.getBlueprint(data.routes.blueprintByTestSection,model).success(function(data){_.isEmpty(data)||""!==model.blueprint&&(model.blueprint=data.uri,$select.select2("data",{id:data.uri,
text:data.text}),$select.trigger("change"))})}function setBlueprint(blueprint){sectionBlueprint.setBlueprint(model,blueprint)}var $select=$("[name=section-blueprint]",$view);$select.select2({ajax:{url:data.routes.blueprintsById,dataType:"json",delay:350,method:"POST",data:function(params){return{identifier:params}},results:function(data){return data}},minimumInputLength:3,width:"100%",multiple:!1,formatNoMatches:function(){return __("Enter a blueprint")},maximumInputLength:32}).on("change",function(e){setBlueprint(e.val)}),initBlueprint(),$view.on("propopen.propview",function(){initBlueprint()})}var $actionContainer=$("h2",$section);_.isEmpty(data.routes.blueprintsById)||(model.hasBlueprint=!0),
...
"use strict";var setUp=function($testPart,model,data){function propHandler(propView){var $view=propView.getView(),$identifier=$("[data-bind=identifier]",$testPart);$view.on("change.binder",function(e,model){"binder"===e.namespace&&"testPart"===model["qti-type"]&&$identifier.text(model.identifier)}),$testPart.on("delete",function(e){null!==propView&&propView.destroy()})}function sections(){model.assessmentSections||(model.assessmentSections=[]),$(".section",$testPart).each(function(){var $section=$(this),index=$section.data("bind-index");model.assessmentSections[index]||(model.assessmentSections[index]={}),sectionView.setUp($section,model.assessmentSections[index],data);
})}function addSection(){$(".section-adder",$testPart).adder({target:$(".sections",$testPart),content:templates.section,templateData:function(cb){var sectionIndex=$(".section",$testPart).length;cb({"qti-type":"assessmentSection",identifier:qtiTestHelper.getIdentifier("assessmentSection",data.identifiers),title:"Section "+(sectionIndex+1),index:0,sectionParts:[]})}}),$(document).off("add.binder","#"+$testPart.attr("id")+" .sections").on("add.binder","#"+$testPart.attr("id")+" .sections",function(e,$section){if("binder"===e.namespace&&$section.hasClass("section")){var index=$section.data("bind-index");sectionView.setUp($section,model.assessmentSections[index],data);
}})}var $actionContainer=$("h1",$testPart);actions.properties($actionContainer,"testpart",model,propHandler),actions.move($actionContainer,"testparts","testpart"),sections(),addSection()},listenActionState=function(){var $testParts=$(".testpart");actions.removable($testParts,"h1"),actions.movable($testParts,"testpart","h1"),$(".testparts").on("delete",function(e){var $target=$(e.target);$target.hasClass("testpart")&&actions.disable($(".testpart"),"h1")}).on("add change undo.deleter deleted.deleter",function(e){var $target=$(e.target);($target.hasClass("testpart")||$target.hasClass("testparts"))&&($testParts=$(".testpart"),actions.removable($testParts,"h1"),actions.movable($testParts,"testpart","h1"));
})};return{setUp:setUp,listenActionState:listenActionState}}),define("taoQtiTest/controller/creator/views/test",["jquery","lodash","ui/hider","taoQtiTest/controller/creator/views/actions","taoQtiTest/controller/creator/views/testpart","taoQtiTest/controller/creator/templates/index","taoQtiTest/controller/creator/helpers/qtiTest"],function($,_,hider,actions,testPartView,templates,qtiTestHelper){"use strict";var testView=function(model,data){function testParts(){model.testParts||(model.testParts=[]),$(".testpart").each(function(){var $testPart=$(this),index=$testPart.data("bind-index");model.testParts[index]||(model.testParts[index]={}),testPartView.setUp($testPart,model.testParts[index],data);
})}function propHandler(propView){function changeScoring(scoring){var noOptions=!!scoring&&["none","custom"].indexOf(scoring.outcomeProcessing)===-1;hider.toggle($cutScoreLine,!!scoring&&"cut"===scoring.outcomeProcessing),hider.toggle($categoryScoreLine,noOptions),hider.toggle($weightIdentifierLine,noOptions),hider.hide($descriptions),hider.show($descriptions.filter('[data-key="'+scoring.outcomeProcessing+'"]'))}var $view=propView.getView(),$categoryScoreLine=$(".test-category-score",$view),$cutScoreLine=$(".test-cut-score",$view),$weightIdentifierLine=$(".test-weight-identifier",$view),$descriptions=$(".test-outcome-processing-description",$view),$title=$(".test-creator-test > h1 [data-bind=title]");
$("[name=test-outcome-processing]",$view).select2({minimumResultsForSearch:-1,width:"100%"}),$view.on("change.binder",function(e,model){"binder"===e.namespace&&"assessmentTest"===model["qti-type"]&&(changeScoring(model.scoring),$title.text(model.title))}),changeScoring(model.scoring)}function addTestPart(){$(".testpart-adder").adder({target:$(".testparts"),content:templates.testpart,templateData:function(cb){var testPartIndex=$(".testpart").length;cb({"qti-type":"testPart",identifier:qtiTestHelper.getIdentifier("testPart",data.identifiers),index:testPartIndex,navigationMode:0,submissionMode:0,assessmentSections:[{"qti-type":"assessmentSection",identifier:qtiTestHelper.getIdentifier("assessmentSection",data.identifiers),
...
"use strict";function binaryOperator(type,left,right,baseType,cardinality){var processingRule=processingRuleHelper.create(type,null,[left,right]);return processingRule.minOperands=2,processingRule.maxOperands=2,_.isUndefined(baseType)&&(baseType=[baseTypeHelper.INTEGER,baseTypeHelper.FLOAT]),_.isUndefined(cardinality)&&(cardinality=[cardinalityHelper.SINGLE]),processingRule.acceptedCardinalities=forceArray(cardinality),processingRule.acceptedBaseTypes=forceArray(baseType),processingRule}function forceArray(value){return value||(value=[]),_.isArray(value)||(value=[value]),value}var processingRuleHelper={create:function(type,identifier,expression){var processingRule=qtiElementHelper.create(type,identifier);
if(identifier&&!outcomeValidator.validateIdentifier(identifier))throw new TypeError("You must provide a valid identifier!");return expression&&processingRuleHelper.setExpression(processingRule,expression),processingRule},setExpression:function(processingRule,expression){if(processingRule)if(_.isArray(expression)){if(!outcomeValidator.validateOutcomes(expression))throw new TypeError("You must provide valid QTI elements as expression!");processingRule.expression&&(processingRule.expression=null),processingRule.expressions=expression}else{if(!outcomeValidator.validateOutcome(expression))throw new TypeError("You must provide a valid QTI element as expression!");processingRule.expressions&&(processingRule.expressions=null),
expression&&(processingRule.expression=expression)}},addExpression:function(processingRule,expression){if(processingRule&&expression)if(processingRule.expression&&(processingRule.expressions=forceArray(processingRule.expression),processingRule.expression=null),_.isArray(expression)){if(!outcomeValidator.validateOutcomes(expression))throw new TypeError("You must provide valid QTI elements as expression!");processingRule.expressions=forceArray(processingRule.expressions).concat(expression)}else{if(!outcomeValidator.validateOutcome(expression))throw new TypeError("You must provide a valid QTI element as expression!");processingRule.expressions?processingRule.expressions.push(expression):processingRule.expression=expression;
}},setOutcomeValue:function(identifier,expression){if(!outcomeValidator.validateIdentifier(identifier))throw new TypeError("You must provide a valid identifier!");return processingRuleHelper.create("setOutcomeValue",identifier,expression)},gte:function(left,right){return binaryOperator("gte",left,right)},lte:function(left,right){return binaryOperator("lte",left,right)},divide:function(left,right){return binaryOperator("divide",left,right)},sum:function(terms){var processingRule=processingRuleHelper.create("sum",null,forceArray(terms));return processingRule.minOperands=1,processingRule.maxOperands=-1,processingRule.acceptedCardinalities=[cardinalityHelper.SINGLE,cardinalityHelper.MULTIPLE,cardinalityHelper.ORDERED],
processingRule.acceptedBaseTypes=[baseTypeHelper.INTEGER,baseTypeHelper.FLOAT],processingRule},testVariables:function(identifier,type,weightIdentifier,includeCategories,excludeCategories){var processingRule=processingRuleHelper.create("testVariables");if(!outcomeValidator.validateIdentifier(identifier))throw new TypeError("You must provide a valid identifier!");if(processingRule.variableIdentifier=identifier,weightIdentifier){if(!outcomeValidator.validateIdentifier(weightIdentifier))throw new TypeError("You must provide a valid weight identifier!");processingRule.weightIdentifier=weightIdentifier}else processingRule.weightIdentifier="";return processingRule.baseType=baseTypeHelper.getValid(type),
processingRule.sectionIdentifier="",processingRule.includeCategories=forceArray(includeCategories),processingRule.excludeCategories=forceArray(excludeCategories),processingRule},numberPresented:function(includeCategories,excludeCategories){var processingRule=processingRuleHelper.create("numberPresented");return processingRule.sectionIdentifier="",processingRule.includeCategories=forceArray(includeCategories),processingRule.excludeCategories=forceArray(excludeCategories),processingRule},baseValue:function(value,type){var processingRule=processingRuleHelper.create("baseValue");return processingRule.baseType=baseTypeHelper.getValid(type,baseTypeHelper.FLOAT),processingRule.value=baseTypeHelper.getValue(processingRule.baseType,value),
processingRule},variable:function(identifier,weightIdentifier){var processingRule=processingRuleHelper.create("variable",identifier);if(!outcomeValidator.validateIdentifier(identifier))throw new TypeError("You must provide a valid identifier!");if(weightIdentifier){if(!outcomeValidator.validateIdentifier(weightIdentifier))throw new TypeError("You must provide a valid weight identifier!");processingRule.weightIdentifier=weightIdentifier}else processingRule.weightIdentifier="";return processingRule},match:function(left,right){return binaryOperator("match",left,right,cardinalityHelper.SAME,cardinalityHelper.SAME)},outcomeCondition:function(outcomeIf,outcomeElse){
var processingRule=processingRuleHelper.create("outcomeCondition");if(!outcomeValidator.validateOutcome(outcomeIf,!1,"outcomeIf"))throw new TypeError("You must provide a valid outcomeIf element!");if(outcomeElse&&!outcomeValidator.validateOutcome(outcomeElse,!1,"outcomeElse"))throw new TypeError("You must provide a valid outcomeElse element!");return processingRule.outcomeIf=outcomeIf,processingRule.outcomeElseIfs=[],outcomeElse&&(processingRule.outcomeElse=outcomeElse),processingRule},outcomeIf:function(expression,instruction){var processingRule=processingRuleHelper.create("outcomeIf");if(!outcomeValidator.validateOutcome(expression))throw new TypeError("You must provide a valid outcomeIf element!");
if(_.isArray(instruction)||(instruction=[instruction]),!outcomeValidator.validateOutcomes(instruction))throw new TypeError("You must provide a valid instruction element!");return processingRule.expression=expression,processingRule.outcomeRules=instruction,processingRule},outcomeElse:function(instruction){var processingRule=processingRuleHelper.create("outcomeElse");if(_.isArray(instruction)||(instruction=[instruction]),!outcomeValidator.validateOutcomes(instruction))throw new TypeError("You must provide a valid instruction element!");return processingRule.outcomeRules=instruction,processingRule}};return processingRuleHelper}),define("taoQtiTest/controller/creator/helpers/category",["lodash"],function(_){
"use strict";function isCategoryOption(category){return category&&0===category.indexOf("x-tao-")}function eachCategories(testModel,cb){_.forEach(testModel.testParts,function(testPart){_.forEach(testPart.assessmentSections,function(assessmentSection){_.forEach(assessmentSection.sectionParts,function(itemRef){_.forEach(itemRef.categories,function(category){cb(category,itemRef)})})})})}return{eachCategories:eachCategories,listCategories:function(testModel){var categories={};return eachCategories(testModel,function(category){isCategoryOption(category)||(categories[category]=!0)}),_.keys(categories)},listOptions:function(testModel){var options={};return eachCategories(testModel,function(category){
isCategoryOption(category)&&(options[category]=!0)}),_.keys(options)}}}),define("taoQtiTest/controller/creator/helpers/scoring",["lodash","i18n","taoQtiTest/controller/creator/helpers/baseType","taoQtiTest/controller/creator/helpers/outcome","taoQtiTest/controller/creator/helpers/processingRule","taoQtiTest/controller/creator/helpers/category"],function(_,__,baseTypeHelper,outcomeHelper,processingRuleHelper,categoryHelper){"use strict";function getOutcomeIdentifier(processingMode,category){return processingMode.prefix&&category?processingMode.prefix+category.toUpperCase():processingMode.outcome}function addTotalScoreOutcomes(model,identifier,type,category){var outcome=outcomeHelper.createOutcome(identifier,type),processingRule=processingRuleHelper.setOutcomeValue(identifier,processingRuleHelper.sum(processingRuleHelper.testVariables(model.scoring.scoreIdentifier,-1,model.scoring.weightIdentifier,category)));
outcomeHelper.addOutcome(model,outcome,processingRule)}function addCutScoreOutcomes(model,identifier,type,category){var outcome=outcomeHelper.createOutcome(identifier,type),processingRule=processingRuleHelper.setOutcomeValue(identifier,processingRuleHelper.gte(processingRuleHelper.divide(processingRuleHelper.sum(processingRuleHelper.testVariables(model.scoring.scoreIdentifier,-1,model.scoring.weightIdentifier,category)),processingRuleHelper.numberPresented(category)),processingRuleHelper.baseValue(model.scoring.cutScore,baseTypeHelper.FLOAT)));outcomeHelper.addOutcome(model,outcome,processingRule)}function addFeedbackScoreOutcomes(model,identifier,type,variable,variableType,matchValue,passed,notPassed){
var outcome=outcomeHelper.createOutcome(identifier,type),processingRule=processingRuleHelper.outcomeCondition(processingRuleHelper.outcomeIf(processingRuleHelper.match(processingRuleHelper.variable(variable),processingRuleHelper.baseValue(matchValue,variableType)),processingRuleHelper.setOutcomeValue(identifier,processingRuleHelper.baseValue(passed,type))),processingRuleHelper.outcomeElse(processingRuleHelper.setOutcomeValue(identifier,processingRuleHelper.baseValue(notPassed,type))));outcomeHelper.addOutcome(model,outcome),outcomeHelper.addOutcomeProcessing(model,processingRule)}function getScoringMode(outcome){var identifier=outcomeHelper.getOutcomeIdentifier(outcome),mode=null;
return _.forEach(processingModes,function(processingMode){if(processingMode.prefix&&0===identifier.indexOf(processingMode.prefix)||processingMode.outcome&&processingMode.outcome===identifier||processingMode.feedback&&processingMode.feedback===identifier)return mode=processingMode,!1}),mode}function isScoringOutcome(outcome){return!!getScoringMode(outcome)}function listScoringModes(outcomes){var modes={};return _.forEach(outcomes,function(outcomeDeclaration){var mode=getScoringMode(outcomeDeclaration);mode&&(modes[mode.key]=!0)}),_.keys(modes)}function hasCategoryOutcome(model){var categoryOutcomes=!1;return _.forEach(outcomeHelper.getOutcomeDeclarations(model),function(outcomeDeclaration){
var identifier=outcomeHelper.getOutcomeIdentifier(outcomeDeclaration);_.forEach(processingModes,function(processingMode){if((!processingMode.outcome||processingMode.outcome!==identifier)&&(!processingMode.feedback||processingMode.feedback!==identifier)&&processingMode.prefix&&0===identifier.indexOf(processingMode.prefix))return categoryOutcomes=!0,!1})}),categoryOutcomes}function getCutScore(model){var values=_(outcomeHelper.getOutcomeProcessingRules(model)).map(function(outcome){return outcomeHelper.getProcessingRuleProperty(outcome,"setOutcomeValue.gte.baseValue.value")}).compact().uniq().value();return _.isEmpty(values)&&(values=[defaultCutScore]),Math.max(0,_.max(values));
}function getWeightIdentifier(model){var values=[];return outcomeHelper.eachOutcomeProcessingRuleExpressions(model,function(processingRule){"testVariables"===processingRule["qti-type"]&&processingRule.weightIdentifier&&values.push(processingRule.weightIdentifier)}),values=_(values).compact().uniq().value(),values.length?values[0]:""}function getOutcomeProcessing(model){var outcomeDeclarations=outcomeHelper.getOutcomeDeclarations(model),outcomeRules=outcomeHelper.getOutcomeProcessingRules(model),declarations=listScoringModes(outcomeDeclarations),processing=listScoringModes(outcomeRules),diff=_.difference(declarations,processing),count=_.size(declarations),outcomeProcessing="custom";
return count===_.size(processing)&&(count?1===count&&_.isEmpty(diff)&&(outcomeProcessing=processing[0]):outcomeProcessing="none"),outcomeProcessing}function removeScoring(model){var scoringOutcomes=outcomeHelper.listOutcomes(model,isScoringOutcome);outcomeHelper.removeOutcomes(model,scoringOutcomes)}var defaultCutScore=.5,processingModes={none:{key:"none",label:__("None"),description:__("No outcome processing. Erase the existing rules, if any.")},custom:{key:"custom",label:__("Custom"),description:__("Custom outcome processing. No changes will be made to the existing rules.")},total:{key:"total",label:__("Total score"),prefix:"SCORE_",outcome:"SCORE_TOTAL",outcomeType:baseTypeHelper.FLOAT,
description:__("The score will be processed for the entire test. A sum of all SCORE outcomes will be computed, the result will take place in the SCORE_TOTAL outcome.")+" "+__("If the category option is set, the score will also be processed per categories, and each results will take place in the SCORE_xxx outcome, where xxx is the name of the category.")},cut:{key:"cut",label:__("Cut score"),prefix:"PASS_",outcome:"PASS_TOTAL",outcomeType:baseTypeHelper.BOOLEAN,feedback:"PASS_TOTAL_RENDERING",feedbackSuffix:"_RENDERING",feedbackType:baseTypeHelper.IDENTIFIER,feedbackMatch:!0,feedbackOk:"passed",feedbackFailed:"not_passed",description:__("The score will be processed for the entire test. A sum of all SCORE outcomes will be computed, the result will be compared to the cut score, then the PASS_TOTAL outcome will be set accordingly.")+" "+__("If the category option is set, the score will also be processed per categories, and each results will take place in the PASS_xxx outcome, where xxx is the name of the category.")
}},processingModeWriters={none:function(model){removeScoring(model)},custom:function(){},total:function(model){var processingMode=processingModes.total;removeScoring(model),addTotalScoreOutcomes(model,getOutcomeIdentifier(processingMode),processingMode.outcomeType),model.scoring.categoryScore&&_.forEach(categoryHelper.listCategories(model),function(category){addTotalScoreOutcomes(model,getOutcomeIdentifier(processingMode,category),processingMode.outcomeType,category)})},cut:function(model){var processingMode=processingModes.cut,outcomeIdentifier=getOutcomeIdentifier(processingMode);removeScoring(model),addCutScoreOutcomes(model,outcomeIdentifier,processingMode.outcomeType),
processingMode.feedback&&addFeedbackScoreOutcomes(model,processingMode.feedback,processingMode.feedbackType,outcomeIdentifier,processingMode.outcomeType,processingMode.feedbackMatch,processingMode.feedbackOk,processingMode.feedbackFailed),model.scoring.categoryScore&&_.forEach(categoryHelper.listCategories(model),function(category){var categoryOutcomeIdentifier=getOutcomeIdentifier(processingMode,category);addCutScoreOutcomes(model,categoryOutcomeIdentifier,processingMode.outcomeType,category),addFeedbackScoreOutcomes(model,categoryOutcomeIdentifier+processingMode.feedbackSuffix,processingMode.feedbackType,categoryOutcomeIdentifier,processingMode.outcomeType,processingMode.feedbackMatch,processingMode.feedbackOk,processingMode.feedbackFailed);
})}};return{read:function(model){model.scoring={modes:processingModes,scoreIdentifier:"SCORE",weightIdentifier:getWeightIdentifier(model),cutScore:getCutScore(model),categoryScore:hasCategoryOutcome(model),outcomeProcessing:getOutcomeProcessing(model)}},write:function(model){var processingModeWriter,scoring=model.scoring;if(scoring){if(processingModeWriter=processingModeWriters[scoring.outcomeProcessing],!processingModeWriter)throw new Error("Unknown score processing mode: "+scoring.outcomeProcessing);processingModeWriter(model)}}}}),define("taoQtiTest/controller/creator/creator",["module","jquery","lodash","helpers","i18n","html5-history-api","ui/feedback","core/databindcontroller","taoQtiTest/controller/creator/views/item","taoQtiTest/controller/creator/views/test","taoQtiTest/controller/creator/views/testpart","taoQtiTest/controller/creator/views/section","taoQtiTest/controller/creator/views/itemref","taoQtiTest/controller/creator/encoders/dom2qti","taoQtiTest/controller/creator/templates/index","taoQtiTest/controller/creator/helpers/qtiTest","taoQtiTest/controller/creator/helpers/scoring","core/validator/validators","core/promise"],function(module,$,_,helpers,__,history,feedback,DataBindController,itemView,testView,testPartView,sectionView,itemrefView,Dom2QtiEncoder,templates,qtiTestHelper,scoringHelper,validators,Promise){
"use strict";var loadItems=function(url,search){return new Promise(function(resolve,reject){$.getJSON(url,{pattern:search,notempty:"true"}).done(resolve).fail(function(xhr){return reject(new Error(xhr.status+" : "+xhr.statusText))})})},getCategories=function(url,items){return new Promise(function(resolve,reject){items&&items.length&&$.getJSON(url,{uris:items}).done(resolve).fail(function(xhr){return reject(new Error(xhr.status+" : "+xhr.statusText))})})},Controller={routes:{},identifiers:[],start:function(options){var binder,binderOptions,self=this,$container=$("#test-creator"),$saver=$("#saver");self.identifiers=[],options=_.merge(module.config(),options||{}),
options.routes=options.routes||{},options.labels=options.labels||{},$("#authoringBack").on("click",function(e){e.preventDefault(),history&&history.back()}),itemView(_.partial(loadItems,options.routes.items),_.partial(getCategories,options.routes.categories)),binderOptions=_.merge(options.routes,{filters:{isItemRef:function(value){return qtiTestHelper.filterQtiType(value,"assessmentItemRef")},isSection:function(value){return qtiTestHelper.filterQtiType(value,"assessmentSection")}},encoders:{dom2qti:Dom2QtiEncoder},templates:templates,beforeSave:function(model){scoringHelper.write(model),qtiTestHelper.addMissingQtiType(model),qtiTestHelper.consolidateModel(model);
try{qtiTestHelper.validateModel(model)}catch(err){return $saver.attr("disabled",!1).removeClass("disabled"),feedback().error(__("The test has not been saved.")+" "+err),!1}return!0}}),binder=DataBindController.takeControl($container,binderOptions).get(function(model){scoringHelper.read(model),self.identifiers=qtiTestHelper.extractIdentifiers(model),validators.register("idFormat",qtiTestHelper.idFormatValidator()),validators.register("testIdFormat",qtiTestHelper.testidFormatValidator()),validators.register("testIdAvailable",qtiTestHelper.idAvailableValidator(self.identifiers),!0),testView(model,{uri:options.uri,identifiers:self.identifiers,labels:options.labels,
...
var $btn=$(this),_order=$btn.data("order");if("last"===_order)$before=$btn,$after=null;else if("first"===_order)$before=null,$after=$btn;else{if(_order=_.parseInt(_order),_.isNaN(_order)||_order>order)return $before=$btn,$after=null,!1;_order===order?$after=$btn:_order<order&&($after=$btn,$before=null)}}),$after?$after.after($button):$before?$before.before($button):$container.append($button)))}function isValidHook(hook){return _.isObject(hook)&&_(["init","render","clear","isVisible"]).reduce(function(result,method){return result&&_.isFunction(hook[method])},!0)}var _ns=".actionBarHook",$doc=$(document),tools={},itemIsLoaded=!1;return $doc.off(_ns).on("serviceloaded"+_ns,function(){
...
this.testReview&&this.testReview.show()},formatTime:function(totalSeconds){var sec_num=totalSeconds,hours=Math.floor(sec_num/3600),minutes=Math.floor((sec_num-3600*hours)/60),seconds=sec_num-3600*hours-60*minutes;hours<10&&(hours="0"+hours),minutes<10&&(minutes="0"+minutes),seconds<10&&(seconds="0"+seconds);var time=hours+":"+minutes+":"+seconds;return time},processError:function(error){var self=this;this.hideGui(),this.beforeTransition(),iframeNotifier.parent("messagealert",{message:error.message,action:function(){testMetaData&&testMetaData.clearData(),error.state===self.TEST_STATE_CLOSED?self.serviceApi.finish():self.serviceApi.exit()}})},actionCall:function(action,extraParams){
