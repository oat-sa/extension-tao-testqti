...
}).on("add change undo.deleter deleted.deleter",".itemrefs",function(e){var $parent,$target=$(e.target);($target.hasClass("itemref")||$target.hasClass("itemrefs"))&&($parent=$(".itemref",$target.hasClass("itemrefs")?$target:$target.parents(".itemrefs")),actions.enable($parent,".actions"),actions.movable($parent,"itemref",".actions"))})},resize=_.throttle(function(){var $refs=$(".itemrefs").first(),$actions=$(".itemref .actions").first(),width=$refs.innerWidth()-$actions.outerWidth();$(".itemref > .title").width(width)},100);return{setUp:setUp,listenActionState:listenActionState,resize:resize}}),define("taoQtiTest/controller/creator/helpers/ckConfigurator",["ui/ckeditor/ckConfigurator","mathJax"],function(ckConfigurator){var getConfig=function(editor,toolbarType,options){return options=options||{},options.underline=!0,ckConfigurator.getConfig(editor,toolbarType,options)
};return{getConfig:getConfig}}),define("taoQtiTest/controller/creator/views/rubricblock",["jquery","lodash","taoQtiTest/controller/creator/views/actions","helpers","ckeditor","taoQtiTest/controller/creator/helpers/ckConfigurator"],function($,_,actions,helpers,ckeditor,ckConfigurator){var ckConfig=ckConfigurator.getConfig(ckeditor,"qtiBlock"),filterPlugin=function(plugin){return _.contains(["taoqtiimage","taoqtimedia","taoqtimaths","taoqtiinclude"],plugin)};ckConfig.plugins=_.reject(ckConfig.plugins.split(","),filterPlugin).join(","),ckConfig.extraPlugins=_.reject(ckConfig.extraPlugins.split(","),filterPlugin).join(",");var setUp=function($rubricBlock,model){function propHandler(propView){function removePropHandler(){null!==propView&&propView.destroy()
}rbViews(propView.getView()),$rubricBlock.parents(".testpart").on("delete",removePropHandler),$rubricBlock.parents(".section").on("delete",removePropHandler),$rubricBlock.on("delete",removePropHandler)}function rbViews($propContainer){var $select=$("select",$propContainer);$select.select2({width:"100%"}).on("select2-removed",function(){0===$select.select2("val").length&&$select.select2("val",[1])}),0===$select.select2("val").length&&$select.select2("val",[1])}function setUpEditor(){var editor,$rubricBlockBinding=$(".rubricblock-binding",$rubricBlock),$rubricBlockContent=$(".rubricblock-content",$rubricBlock),syncRubricBlockContent=_.throttle(function(){$rubricBlockBinding.html($rubricBlockContent.html()).trigger("change")
},100);$rubricBlockContent.empty().html($rubricBlockBinding.html()),editor=ckeditor.inline($rubricBlockContent[0],ckConfig),editor.on("change",function(){syncRubricBlockContent()})}actions.properties($rubricBlock,"rubricblock",model,propHandler),setUpEditor()};return{setUp:setUp}}),define("taoQtiTest/controller/creator/helpers/qtiTest",["lodash","i18n"],function(_,__){var qtiTestHelper={extractIdentifiers:function(obj){var self=this,identifiers=[];return _.has(obj,"identifier")&&(identifiers=identifiers.concat(obj.identifier.toLowerCase())),_.flatten(_.forEach(obj,function(value){identifiers=identifiers.concat("object"==typeof value?self.extractIdentifiers(value):[])
...
3===elt.nodeType?_.isEmpty($.trim(elt.nodeValue))||result.push({"qti-type":"textRun",content:elt.nodeValue,xmlBase:""}):1===elt.nodeType&&(object=_.merge({"qti-type":elt.nodeName.toLowerCase(),id:"","class":"",xmlBase:"",lang:"",label:""},_.transform(elt.attributes,function(acc,value){value.nodeName&&(acc[value.nodeName]=value.nodeValue)})),elt.childNodes.length>0&&(object.content=self.decode(elt.childNodes)),result.push(object))}),result}};return Dom2QtiEncoder}),define("taoQtiTest/controller/creator/creator",["module","jquery","lodash","helpers","i18n","history","ui/feedback","core/databindcontroller","taoQtiTest/controller/creator/views/item","taoQtiTest/controller/creator/views/test","taoQtiTest/controller/creator/views/testpart","taoQtiTest/controller/creator/views/section","taoQtiTest/controller/creator/views/itemref","taoQtiTest/controller/creator/encoders/dom2qti","taoQtiTest/controller/creator/templates/index","taoQtiTest/controller/creator/helpers/qtiTest","core/validator/validators"],function(module,$,_,helpers,__,history,feedback,DataBindController,itemView,testView,testPartView,sectionView,itemrefView,Dom2QtiEncoder,templates,qtiTestHelper,validators){function loadItems(url,search,cb){$.getJSON(url,{pattern:search,notempty:"true"},function(data){data&&"function"==typeof cb&&cb(data)
