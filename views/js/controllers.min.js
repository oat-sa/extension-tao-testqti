...
return categoryHelper.listCategories(model)},getOptions:function(){return categoryHelper.listOptions(model)}};return config=_.isPlainObject(config)?config:_.assign({},config),statifier(eventifier(modelOverseer))}return modelOverseerFactory}),define("tpl!taoQtiTest/controller/creator/templates/testpart",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,buffer="",functionType="function",escapeExpression=this.escapeExpression;return buffer+='<div id="',(helper=helpers.identifier)?stack1=helper.call(depth0,{
...
}function removeCategories(model,categories){isValidSectionModel(model)?_.each(model.sectionParts,function(itemRef){"assessmentItemRef"===itemRef["qti-type"]&&_.isArray(itemRef.categories)&&(itemRef.categories=_.difference(itemRef.categories,categories))}):errorHandler["throw"](_ns,"invalid tool config format")}function getTaoOptionCategories(){return[{name:"x-tao-option-endTestWarning",description:__("displays a warning before the user finishes the test")},{name:"x-tao-option-unansweredWarning",description:__("displays a warning before the user finishes the test, only if there are unanswered/marked for review items")},{name:"x-tao-option-nextSectionWarning",
description:__("displays a next section button that warns the user that they will not be able to return to the section")},{name:"x-tao-option-nextSection",description:__("displays a next section button")},{name:"x-tao-option-noExitTimedSectionWarning",description:__("disable the warning automatically displayed upon exiting a timed section")},{name:"x-tao-option-exit",description:__("displays an exit button")},{name:"x-tao-option-markReview",description:__("displays a mark for review button. This option requires requires the x-tao-option-reviewScreen option")},{name:"x-tao-option-reviewScreen",description:__("displays the review screen / navigator")},{name:"x-tao-option-calculator",
description:__("enable calculator")},{name:"x-tao-option-areaMasking",description:__("enable area masking tool")},{name:"x-tao-option-highlighter",description:__("enable highlighter tool")},{name:"x-tao-option-magnifier",description:__("enable magnifier tool")},{name:"x-tao-option-eliminator",description:__("enable answer eliminator tool")},{name:"x-tao-itemusage-informational",description:__("describe the item as informational")}]}var _ns=".sectionCategory";return{isValidSectionModel:isValidSectionModel,setCategories:setCategories,getCategories:getCategories,addCategories:addCategories,removeCategories:removeCategories,getTaoOptionCategories:getTaoOptionCategories
}}),define("taoQtiTest/controller/creator/helpers/qtiTest",["lodash","i18n","taoQtiTest/controller/creator/helpers/outcome","taoQtiTest/controller/creator/helpers/qtiElement"],function(_,__,outcomeHelper,qtiElementHelper){"use strict";var qtiTestHelper={extractIdentifiers:function(obj){var self=this,identifiers=[];return _.has(obj,"identifier")&&(identifiers=identifiers.concat(obj.identifier.toLowerCase())),_.flatten(_.forEach(obj,function(value){identifiers=identifiers.concat("object"==typeof value?self.extractIdentifiers(value):[])}),!0),identifiers},getIdentifier:function(qtiType,lockedIdentifiers){var suggestion,index=1,glue="-";do suggestion=qtiType+glue+index++;while(_.contains(lockedIdentifiers,suggestion.toLowerCase()));
...
target:$(".rubricblocks",$section),content:templates.rubricblock,templateData:function(cb){cb({"qti-type":"rubricBlock",index:$(".rubricblock",$section).length,content:[],views:[1]})}}),$(document).on("add.binder","#"+$section.attr("id")+" .rubricblocks",function(e,$rubricBlock){var index,rubricModel;"binder"===e.namespace&&$rubricBlock.hasClass("rubricblock")&&(index=$rubricBlock.data("bind-index"),rubricModel=sectionModel.rubricBlocks[index],$(".rubricblock-binding",$rubricBlock).html("<p>&nbsp;</p>"),rubricBlockView.setUp(modelOverseer,rubricModel,$rubricBlock),modelOverseer.trigger("rubric-add",rubricModel))})}function categoriesProperty($view){function initCategories(){
var categories=sectionCategory.getCategories(sectionModel);$select.select2("val",categories.all),$select.siblings(".select2-container").find(".select2-search-choice").each(function(){var $li=$(this),content=$li.find("div").text();_.indexOf(categories.partial,content)>=0&&$li.addClass("partial")})}function setCategories(categories){sectionCategory.setCategories(sectionModel,categories),modelOverseer.trigger("category-change",categories)}var $select=$("[name=section-category]",$view);$select.select2({width:"100%",tags:_.pluck(sectionCategory.getTaoOptionCategories(),"name"),multiple:!0,tokenSeparators:[","," ",";"],formatNoMatches:function(){return __("Enter a category");
},maximumInputLength:32}).on("change",function(e){setCategories(e.val)}),initCategories(),$view.on("propopen.propview",function(){initCategories()})}function blueprintProperty($view){function initBlueprint(){"undefined"==typeof sectionModel.blueprint&&sectionBlueprint.getBlueprint(config.routes.blueprintByTestSection,sectionModel).success(function(data){_.isEmpty(data)||""!==sectionModel.blueprint&&(sectionModel.blueprint=data.uri,$select.select2("data",{id:data.uri,text:data.text}),$select.trigger("change"))})}function setBlueprint(blueprint){sectionBlueprint.setBlueprint(sectionModel,blueprint)}var $select=$("[name=section-blueprint]",$view);$select.select2({
ajax:{url:config.routes.blueprintsById,dataType:"json",delay:350,method:"POST",data:function(params){return{identifier:params}},results:function(data){return data}},minimumInputLength:3,width:"100%",multiple:!1,formatNoMatches:function(){return __("Enter a blueprint")},maximumInputLength:32}).on("change",function(e){setBlueprint(e.val)}),initBlueprint(),$view.on("propopen.propview",function(){initBlueprint()})}var $actionContainer=$("h2",$section),config=modelOverseer.getConfig();_.isEmpty(config.routes.blueprintsById)||(sectionModel.hasBlueprint=!0),actions.properties($actionContainer,"section",sectionModel,propHandler),actions.move($actionContainer,"sections","section"),
itemRefs(),acceptItemRefs(),rubricBlocks(),addRubricBlock()}function listenActionState(){var $sections;$(".sections").each(function(){$sections=$(".section",$(this)),actions.removable($sections,"h2"),actions.movable($sections,"section","h2")}),$(document).on("delete",function(e){var $parent,$target=$(e.target);$target.hasClass("section")&&($parent=$target.parents(".sections"),actions.disable($parent.find(".section"),"h2"))}).on("add change undo.deleter deleted.deleter",function(e){var $target=$(e.target);($target.hasClass("section")||$target.hasClass("sections"))&&($sections=$(".section",$target.hasClass("sections")?$target:$target.parents(".sections")),actions.removable($sections,"h2"),
actions.movable($sections,"section","h2"))}).on("open.toggler",".rub-toggler",function(e){"toggler"===e.namespace&&$(this).parents("h2").addClass("active")}).on("close.toggler",".rub-toggler",function(e){"toggler"===e.namespace&&$(this).parents("h2").removeClass("active")})}return{setUp:setUp,listenActionState:listenActionState}}),define("taoQtiTest/controller/creator/views/testpart",["jquery","lodash","taoQtiTest/controller/creator/views/actions","taoQtiTest/controller/creator/views/section","taoQtiTest/controller/creator/templates/index","taoQtiTest/controller/creator/helpers/qtiTest"],function($,_,actions,sectionView,templates,qtiTestHelper){"use strict";function setUp(modelOverseer,partModel,$testPart){
function propHandler(propView){var $view=propView.getView(),$identifier=$("[data-bind=identifier]",$testPart);$view.on("change.binder",function(e,model){"binder"===e.namespace&&"testPart"===model["qti-type"]&&$identifier.text(model.identifier)}),$testPart.on("delete",function(){null!==propView&&propView.destroy()})}function sections(){partModel.assessmentSections||(partModel.assessmentSections=[]),$(".section",$testPart).each(function(){var $section=$(this),index=$section.data("bind-index");partModel.assessmentSections[index]||(partModel.assessmentSections[index]={}),sectionView.setUp(modelOverseer,partModel.assessmentSections[index],$section)})}function addSection(){
...
templateData:function(cb){var testPartIndex=$(".testpart").length;cb({"qti-type":"testPart",identifier:qtiTestHelper.getIdentifier("testPart",config.identifiers),index:testPartIndex,navigationMode:0,submissionMode:0,assessmentSections:[{"qti-type":"assessmentSection",identifier:qtiTestHelper.getIdentifier("assessmentSection",config.identifiers),title:"Section 1",index:0,sectionParts:[]}]})}}),$(document).off("add.binder",".testparts").on("add.binder",".testparts",function(e,$testPart,added){var partModel;"binder"===e.namespace&&$testPart.hasClass("testpart")&&(partModel=testModel.testParts[added.index],testPartView.setUp(modelOverseer,partModel,$testPart),modelOverseer.trigger("part-add",partModel));
})}var testModel=modelOverseer.getModel(),config=modelOverseer.getConfig();actions.properties($(".test-creator-test > h1"),"test",testModel,propHandler),testParts(),addTestPart()}return testView}),define("taoQtiTest/controller/creator/helpers/processingRule",["lodash","taoQtiTest/controller/creator/helpers/outcomeValidator","taoQtiTest/controller/creator/helpers/qtiElement","taoQtiTest/controller/creator/helpers/baseType","taoQtiTest/controller/creator/helpers/cardinality"],function(_,outcomeValidator,qtiElementHelper,baseTypeHelper,cardinalityHelper){"use strict";function binaryOperator(type,left,right,baseType,cardinality){var processingRule=processingRuleHelper.create(type,null,[left,right]);
return processingRule.minOperands=2,processingRule.maxOperands=2,_.isUndefined(baseType)&&(baseType=[baseTypeHelper.INTEGER,baseTypeHelper.FLOAT]),_.isUndefined(cardinality)&&(cardinality=[cardinalityHelper.SINGLE]),processingRule.acceptedCardinalities=forceArray(cardinality),processingRule.acceptedBaseTypes=forceArray(baseType),processingRule}function forceArray(value){return value||(value=[]),_.isArray(value)||(value=[value]),value}var processingRuleHelper={create:function(type,identifier,expression){var processingRule=qtiElementHelper.create(type,identifier);if(identifier&&!outcomeValidator.validateIdentifier(identifier))throw new TypeError("You must provide a valid identifier!");
return expression&&processingRuleHelper.setExpression(processingRule,expression),processingRule},setExpression:function(processingRule,expression){if(processingRule)if(_.isArray(expression)){if(!outcomeValidator.validateOutcomes(expression))throw new TypeError("You must provide valid QTI elements as expression!");processingRule.expression&&(processingRule.expression=null),processingRule.expressions=expression}else{if(!outcomeValidator.validateOutcome(expression))throw new TypeError("You must provide a valid QTI element as expression!");processingRule.expressions&&(processingRule.expressions=null),expression&&(processingRule.expression=expression)}},addExpression:function(processingRule,expression){
if(processingRule&&expression)if(processingRule.expression&&(processingRule.expressions=forceArray(processingRule.expression),processingRule.expression=null),_.isArray(expression)){if(!outcomeValidator.validateOutcomes(expression))throw new TypeError("You must provide valid QTI elements as expression!");processingRule.expressions=forceArray(processingRule.expressions).concat(expression)}else{if(!outcomeValidator.validateOutcome(expression))throw new TypeError("You must provide a valid QTI element as expression!");processingRule.expressions?processingRule.expressions.push(expression):processingRule.expression=expression}},setOutcomeValue:function(identifier,expression){
...
"use strict";function addTotalScoreOutcomes(model,identifier,scoreIdentifier,weightIdentifier,category){var outcome=outcomeHelper.createOutcome(identifier,baseTypeHelper.FLOAT),processingRule=processingRuleHelper.setOutcomeValue(identifier,processingRuleHelper.sum(processingRuleHelper.testVariables(scoreIdentifier,-1,weightIdentifier,category)));outcomeHelper.addOutcome(model,outcome,processingRule)}function addCutScoreOutcomes(model,identifier,cutScore,scoreIdentifier,weightIdentifier,category){var outcome=outcomeHelper.createOutcome(identifier,baseTypeHelper.BOOLEAN),processingRule=processingRuleHelper.setOutcomeValue(identifier,processingRuleHelper.gte(processingRuleHelper.divide(processingRuleHelper.sum(processingRuleHelper.testVariables(scoreIdentifier,-1,weightIdentifier,category)),processingRuleHelper.numberPresented(category)),processingRuleHelper.baseValue(cutScore,baseTypeHelper.FLOAT)));
...
outcomeHelper.addOutcomeProcessing(model,processingRule)}function formatCategoryOutcome(category,template){return template.replace(/%s/g,category.toUpperCase())}function belongToScoringMode(identifier,processingMode,onlyCategories){var match=!1;return processingMode.signature&&processingMode.signature.test(identifier)&&(match=!0,onlyCategories&&_.forEach(processingMode.outcomes,function(outcome){return outcome.identifier===identifier||outcome.weighted&&outcome.weighted===identifier||outcome.feedback&&outcome.feedback===identifier?(match=!1,!1):void 0})),match}function matchScoringMode(processingMode,outcomes,categories){function matchScoringOutcome(mode,identifier){
var outcomeMatch=!1;return mode.signature&&mode.signature.test(identifier)&&_.forEach(mode.outcomes,function(outcome){return outcome.identifier!==identifier&&(!outcome.weighted||outcome.weighted&&outcome.weighted!==identifier)&&(!outcome.feedback||outcome.feedback&&outcome.feedback!==identifier)?categories&&_.forEach(categories,function(category){return outcome.categoryIdentifier&&identifier===formatCategoryOutcome(category,outcome.categoryIdentifier)?outcomeMatch=!0:outcome.categoryWeighted&&identifier===formatCategoryOutcome(category,outcome.categoryWeighted)?outcomeMatch=!0:outcome.categoryFeedback&&identifier===formatCategoryOutcome(category,outcome.categoryFeedback)&&(outcomeMatch=!0),
outcomeMatch?!1:void 0}):outcomeMatch=!0,outcomeMatch?!1:void 0}),!outcomeMatch&&mode.include&&(outcomeMatch=matchScoringOutcome(processingModes[mode.include],identifier)),outcomeMatch}var signatures=getSignatures(processingMode),match=!0;return _.forEach(outcomes,function(identifier){var signatureMatch=!1;return _.forEach(signatures,function(signature){return signature.test(identifier)?(signatureMatch=!0,!1):void 0}),signatureMatch&&(match=matchScoringOutcome(processingMode,identifier),!match)?!1:void 0}),match}function getSignatures(processingMode){for(var signatures=[];processingMode;)processingMode.signature&&signatures.push(processingMode.signature),processingMode=processingMode.include&&processingModes[processingMode.include];
return signatures}function getRecipes(processingMode){for(var descriptors=[];processingMode;)processingMode.outcomes&&(descriptors=descriptors.concat(processingMode.outcomes)),processingMode=processingMode.include&&processingModes[processingMode.include];return descriptors}function getScoringMode(outcome){var identifier=outcomeHelper.getOutcomeIdentifier(outcome),mode=null;return _.forEach(processingModes,function(processingMode){return belongToScoringMode(identifier,processingMode)?(mode=processingMode,!1):void 0}),mode}function isScoringOutcome(outcome){return!!getScoringMode(outcome)}function listScoringModes(outcomes){var modes={};return _.forEach(outcomes,function(outcome){
var mode=getScoringMode(outcome);mode&&(modes[mode.key]=!0)}),_.keys(modes)}function handleCategories(modelOverseer){var model=modelOverseer.getModel();return!(!model.scoring||!model.scoring.categoryScore)}function hasCategoryOutcome(model){var categoryOutcomes=!1;return _.forEach(outcomeHelper.getOutcomeDeclarations(model),function(outcomeDeclaration){var identifier=outcomeHelper.getOutcomeIdentifier(outcomeDeclaration);_.forEach(processingModes,function(processingMode){return belongToScoringMode(identifier,processingMode,!0)?(categoryOutcomes=!0,!1):void 0})}),categoryOutcomes}function getCutScore(model){var values=_(outcomeHelper.getOutcomeProcessingRules(model)).map(function(outcome){
return outcomeHelper.getProcessingRuleProperty(outcome,"setOutcomeValue.gte.baseValue.value")}).compact().uniq().value();return _.isEmpty(values)&&(values=[defaultCutScore]),Math.max(0,_.max(values))}function getWeightIdentifier(model){var values=[];return outcomeHelper.eachOutcomeProcessingRuleExpressions(model,function(processingRule){"testVariables"===processingRule["qti-type"]&&processingRule.weightIdentifier&&values.push(processingRule.weightIdentifier)}),values=_(values).compact().uniq().value(),values.length?values[0]:""}function getOutcomeProcessing(modelOverseer){var included,model=modelOverseer.getModel(),outcomeDeclarations=outcomeHelper.getOutcomeDeclarations(model),outcomeRules=outcomeHelper.getOutcomeProcessingRules(model),declarations=listScoringModes(outcomeDeclarations),processing=listScoringModes(outcomeRules),diff=_.difference(declarations,processing),count=_.size(declarations),outcomeProcessing="custom";
return count===_.size(processing)&&(count?_.isEmpty(diff)&&(count>1&&(included=[],_.forEach(declarations,function(mode){processingModes[mode]&&processingModes[mode].include&&included.push(processingModes[mode].include)}),processing=_.difference(processing,included),count=_.size(processing)),1===count&&(outcomeProcessing=processing[0],matchScoringMode(processingModes[outcomeProcessing],modelOverseer.getOutcomesNames(),modelOverseer.getCategories())||(outcomeProcessing="custom"))):outcomeProcessing="none"),outcomeProcessing}function detectScoring(modelOverseer){var model=modelOverseer.getModel();return{modes:processingModes,scoreIdentifier:defaultScoreIdentifier,
weightIdentifier:getWeightIdentifier(model),cutScore:getCutScore(model),categoryScore:hasCategoryOutcome(model),outcomeProcessing:getOutcomeProcessing(modelOverseer)}}function removeScoring(model){var scoringOutcomes=_.indexBy(outcomeHelper.listOutcomes(model,isScoringOutcome),function(outcome){return outcome});outcomeHelper.removeOutcomes(model,function(outcome){function browseExpressions(processingRule){_.isArray(processingRule)?_.forEach(processingRule,browseExpressions):processingRule&&(scoringOutcomes[outcomeHelper.getOutcomeIdentifier(processingRule)]&&(match=!0),!match&&processingRule.expression&&browseExpressions(processingRule.expression),!match&&processingRule.expressions&&browseExpressions(processingRule.expressions),
...
none:{key:"none",label:__("None"),clean:!0,description:__("No outcome processing. Erase the existing rules, if any.")},custom:{key:"custom",label:__("Custom"),description:__("Custom outcome processing. No changes will be made to the existing rules.")},total:{key:"total",label:__("Total score"),signature:/^SCORE_([a-zA-Z][a-zA-Z0-9_\.-]*)$/,outcomes:[{formula:"total",identifier:"SCORE_TOTAL",weighted:"SCORE_TOTAL_WEIGHTED",categoryIdentifier:"SCORE_CATEGORY_%s",categoryWeighted:"SCORE_CATEGORY_WEIGHTED_%s"}],clean:!0,description:__("The score will be processed for the entire test. A sum of all SCORE outcomes will be computed, the result will take place in the SCORE_TOTAL outcome.")+" "+__("If the category option is set, the score will also be processed per categories, and each results will take place in the SCORE_xxx outcome, where xxx is the name of the category.")
},cut:{key:"cut",label:__("Cut score"),include:"total",signature:/^PASS_([a-zA-Z][a-zA-Z0-9_\.-]*)$/,outcomes:[{formula:"cut",identifier:"PASS_ALL",feedback:"PASS_ALL_RENDERING",feedbackOk:"passed",feedbackFailed:"not_passed",categoryIdentifier:"PASS_CATEGORY_%s",categoryFeedback:"PASS_CATEGORY_%s_RENDERING"}],clean:!0,description:__("The score will be processed for the entire test. A sum of all SCORE outcomes will be computed, the result will be compared to the cut score, then the PASS_TOTAL outcome will be set accordingly.")+" "+__("If the category option is set, the score will also be processed per categories, and each results will take place in the PASS_xxx outcome, where xxx is the name of the category.")
}},formulaWriters={total:function(descriptor,scoring,outcomes,categories){var scoreIdentifier=scoring.scoreIdentifier,weightIdentifier=scoring.weightIdentifier;addTotalScoreOutcomes(outcomes,descriptor.identifier,scoreIdentifier),descriptor.weighted&&scoring.weightIdentifier&&addTotalScoreOutcomes(outcomes,descriptor.weighted,scoreIdentifier,weightIdentifier),descriptor.categoryIdentifier&&categories&&_.forEach(categories,function(category){addTotalScoreOutcomes(outcomes,formatCategoryOutcome(category,descriptor.categoryIdentifier),scoreIdentifier,null,category),descriptor.categoryWeighted&&scoring.weightIdentifier&&addTotalScoreOutcomes(outcomes,formatCategoryOutcome(category,descriptor.categoryWeighted),scoreIdentifier,weightIdentifier,category);
})},cut:function(descriptor,scoring,outcomes,categories){var cutScore=scoring.cutScore,scoreIdentifier=scoring.scoreIdentifier,weightIdentifier=scoring.weightIdentifier;return addCutScoreOutcomes(outcomes,descriptor.identifier,cutScore,scoreIdentifier,weightIdentifier),descriptor.feedback&&addFeedbackScoreOutcomes(outcomes,descriptor.feedback,descriptor.identifier,descriptor.feedbackOk,descriptor.feedbackFailed),descriptor.categoryIdentifier&&categories&&_.forEach(categories,function(category){var categoryOutcomeIdentifier=formatCategoryOutcome(category,descriptor.categoryIdentifier);addCutScoreOutcomes(outcomes,categoryOutcomeIdentifier,cutScore,scoreIdentifier,weightIdentifier,category),
descriptor.categoryFeedback&&addFeedbackScoreOutcomes(outcomes,formatCategoryOutcome(category,descriptor.categoryFeedback),categoryOutcomeIdentifier,descriptor.feedbackOk,descriptor.feedbackFailed)}),outcomes}},scoringHelper={init:function(modelOverseer){var model;if(!modelOverseer||!_.isFunction(modelOverseer.getModel))throw new TypeError("You must provide a valid modelOverseer");model=modelOverseer.getModel(),model.scoring=detectScoring(modelOverseer),modelOverseer.on("scoring-change category-change delete",function(){modelOverseer.trigger("scoring-generate",scoringHelper.generate(modelOverseer))}).on("scoring-generate",function(outcomes){outcomeHelper.replaceOutcomes(model,outcomes),
modelOverseer.trigger("scoring-write",model)}).trigger("scoring-init",model)},generate:function(modelOverseer){var model,scoring,outcomes,processingMode,descriptors,categories;if(!modelOverseer||!_.isFunction(modelOverseer.getModel))throw new TypeError("You must provide a valid modelOverseer");if(model=modelOverseer.getModel(),scoring=model.scoring,outcomes=getOutcomes(model),scoring){if(processingMode=processingModes[scoring.outcomeProcessing],!processingMode)throw new Error("Unknown score processing mode: "+scoring.outcomeProcessing);processingMode.clean&&removeScoring(outcomes),descriptors=getRecipes(processingMode),handleCategories(modelOverseer)&&(categories=modelOverseer.getCategories()),
_.forEach(descriptors,function(descriptor){var writer=formulaWriters[descriptor.formula];if(!_.isFunction(writer))throw new Error("Unknown score processing formula: "+descriptor.formula);writer(descriptor,scoring,outcomes,categories)})}return outcomes}};return scoringHelper}),define("taoQtiTest/controller/creator/creator",["module","jquery","lodash","helpers","i18n","html5-history-api","ui/feedback","core/databindcontroller","taoQtiTest/controller/creator/modelOverseer","taoQtiTest/controller/creator/views/item","taoQtiTest/controller/creator/views/test","taoQtiTest/controller/creator/views/testpart","taoQtiTest/controller/creator/views/section","taoQtiTest/controller/creator/views/itemref","taoQtiTest/controller/creator/encoders/dom2qti","taoQtiTest/controller/creator/templates/index","taoQtiTest/controller/creator/helpers/qtiTest","taoQtiTest/controller/creator/helpers/scoring","core/validator/validators","core/promise"],function(module,$,_,helpers,__,history,feedback,DataBindController,modelOverseerFactory,itemView,testView,testPartView,sectionView,itemrefView,Dom2QtiEncoder,templates,qtiTestHelper,scoringHelper,validators,Promise){
"use strict";var loadItems=function(url,search){return new Promise(function(resolve,reject){$.getJSON(url,{pattern:search,notempty:"true"}).done(resolve).fail(function(xhr){return reject(new Error(xhr.status+" : "+xhr.statusText))})})},getCategories=function(url,items){return new Promise(function(resolve,reject){items&&items.length&&$.getJSON(url,{uris:items}).done(resolve).fail(function(xhr){return reject(new Error(xhr.status+" : "+xhr.statusText))})})},Controller={routes:{},identifiers:[],start:function(options){var binder,binderOptions,modelOverseer,self=this,$container=$("#test-creator"),$saver=$("#saver");self.identifiers=[],options=_.merge(module.config(),options||{}),
options.routes=options.routes||{},options.labels=options.labels||{},$("#authoringBack").on("click",function(e){e.preventDefault(),history&&history.back()}),itemView(_.partial(loadItems,options.routes.items),_.partial(getCategories,options.routes.categories)),$container.on("change.binder delete.binder",function(e,model){"binder"===e.namespace&&model&&modelOverseer.trigger(e.type,model)}),binderOptions=_.merge(options.routes,{filters:{isItemRef:function(value){return qtiTestHelper.filterQtiType(value,"assessmentItemRef")},isSection:function(value){return qtiTestHelper.filterQtiType(value,"assessmentSection")}},encoders:{dom2qti:Dom2QtiEncoder},templates:templates,
beforeSave:function(model){qtiTestHelper.addMissingQtiType(model),qtiTestHelper.consolidateModel(model);try{qtiTestHelper.validateModel(model)}catch(err){return $saver.attr("disabled",!1).removeClass("disabled"),feedback().error(__("The test has not been saved.")+" "+err),!1}return!0}}),binder=DataBindController.takeControl($container,binderOptions).get(function(model){self.identifiers=qtiTestHelper.extractIdentifiers(model),modelOverseer=modelOverseerFactory(model,{uri:options.uri,identifiers:self.identifiers,labels:options.labels,routes:options.routes}),scoringHelper.init(modelOverseer),validators.register("idFormat",qtiTestHelper.idFormatValidator()),validators.register("testIdFormat",qtiTestHelper.testidFormatValidator()),
validators.register("testIdAvailable",qtiTestHelper.idAvailableValidator(self.identifiers),!0),testView(modelOverseer),testPartView.listenActionState(),sectionView.listenActionState(),itemrefView.listenActionState(),itemrefView.resize(),$(window).off("resize.qti-test-creator").on("resize.qti-test-creator",function(){itemrefView.resize()})}),$saver.on("click",function(event){event.preventDefault(),$saver.hasClass("disabled")||($saver.attr("disabled",!0).addClass("disabled"),binder.save(function(){$saver.attr("disabled",!1).removeClass("disabled"),feedback().success(__("Test Saved"))},function(){$saver.attr("disabled",!1).removeClass("disabled")}))})}};return Controller;
...
var self=this,options=options||{},scope=options.scope,confirmLabel=options.confirmLabel||__("Yes"),cancelLabel=options.cancelLabel||__("No"),showItemCount="undefined"!=typeof options.showItemCount?options.showItemCount:!0,$confirmBox=$(".exit-modal-feedback"),progression=this.getProgression(scope),unansweredCount=progression.total-progression.answered,flaggedCount=progression.flagged;return showItemCount&&(unansweredCount&&this.isCurrentItemAnswered()&&unansweredCount--,flaggedCount&&unansweredCount?message=__("You have %s unanswered question(s) and have %s item(s) marked for review.",unansweredCount.toString(),flaggedCount.toString())+" "+message:(flaggedCount&&(message=__("You have %s item(s) marked for review.",flaggedCount.toString())+" "+message),
...
switch($time.removeClass("txt-info txt-warning txt-danger").addClass("txt-"+warning.type),remaining=moment.duration(warning.point,"seconds").humanize(),cst.qtiClassName){case"assessmentItemRef":message=__("Warning – You have %s remaining to complete this item.",remaining);break;case"assessmentSection":message=__("Warning – You have %s remaining to complete this section.",remaining);break;case"testPart":message=__("Warning – You have %s remaining to complete this test part.",remaining);break;case"assessmentTest":message=__("Warning – You have %s remaining to complete the test.",remaining)}feedback()[warning.type](message),cst.warnings[warning.point].showed=!0},
