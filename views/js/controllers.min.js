...
options={hash:{},data:data},helper?helper.call(depth0,"Identifier",options):helperMissing.call(depth0,"__","Identifier",options)))+' <abbr title="'+escapeExpression((helper=helpers.__||depth0&&depth0.__,options={hash:{},data:data},helper?helper.call(depth0,"Required field",options):helperMissing.call(depth0,"__","Required field",options)))+'">*</abbr></label>\n            </div>\n            <div class="col-6">\n                <input type="text" name="testpart-identifier" data-bind="identifier" data-validate="$notEmpty; $testIdFormat; $testIdAvailable(original=',(helper=helpers.identifier)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.identifier,
...
options={hash:{},data:data},helper?helper.call(depth0,"Identifier",options):helperMissing.call(depth0,"__","Identifier",options)))+' <abbr title="'+escapeExpression((helper=helpers.__||depth0&&depth0.__,options={hash:{},data:data},helper?helper.call(depth0,"Required field",options):helperMissing.call(depth0,"__","Required field",options)))+'">*</abbr></label>\n        </div>\n        <div class="col-6">\n            <input type="text" name="section-identifier" data-bind="identifier" data-validate="$notEmpty; $testIdFormat; $testIdAvailable(original=',(helper=helpers.identifier)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.identifier,stack1=typeof helper===functionType?helper.call(depth0,{
...
rubricBlock.views=["candidate"]}))})}return model}};return qtiTestHelper}),define("taoQtiTest/controller/creator/views/section",["jquery","lodash","uri","taoQtiTest/controller/creator/views/actions","taoQtiTest/controller/creator/views/itemref","taoQtiTest/controller/creator/views/rubricblock","taoQtiTest/controller/creator/templates/index","taoQtiTest/controller/creator/helpers/qtiTest"],function($,_,uri,actions,itemRefView,rubricBlockView,templates,qtiTestHelper){"use strict";var setUp=function($section,model,data){function propHandler(propView){function removePropHandler(){null!==propView&&propView.destroy()}var $view=propView.getView(),$selectionSwitcher=$("[name=section-enable-selection]",$view),$selectionSelect=$("[name=section-select]",$view),$selectionWithRep=$("[name=section-with-replacement]",$view),switchSelection=function(){
$selectionSwitcher.prop("checked")===!0?($selectionSelect.incrementer("enable"),$selectionWithRep.removeClass("disabled")):($selectionSelect.incrementer("disable"),$selectionWithRep.addClass("disabled"))};$selectionSwitcher.on("change",switchSelection),$selectionSwitcher.on("change",function(){$selectionSwitcher.prop("checked")||($selectionSelect.val(0),$selectionWithRep.prop("checked",!1),delete model.selection)}),$selectionSwitcher.prop("checked",!!model.selection).trigger("change");var $title=$("[data-bind=title]",$section);$view.on("change.binder",function(e,model){"binder"===e.namespace&&"assessmentSection"===model["qti-type"]&&$title.text(model.title)}),
$section.parents(".testpart").on("deleted.deleter",removePropHandler),$section.on("deleted.deleter",removePropHandler)}function itemRefs(){model.sectionParts||(model.sectionParts=[]),$(".itemref",$section).each(function(){var $itemRef=$(this),index=$itemRef.data("bind-index");model.sectionParts[index]||(model.sectionParts[index]={}),itemRefView.setUp($itemRef,model.sectionParts[index]),$itemRef.find(".title").text(data.labels[uri.encode($itemRef.data("uri"))])})}function acceptItemRefs(){var $selected,$items=$(".test-creator-items");$items.on("itemselect.creator",function(e){var selection=Array.prototype.slice.call(arguments,1),$placeholder=$(".itemref-placeholder",$section),$placeholders=$(".itemref-placeholder");
selection.length>0?$placeholder.show().off("click").on("click",function(e){var defaultItemData={};model.itemSessionControl&&!_.isUndefined(model.itemSessionControl.maxAttempts)&&(defaultItemData.itemSessionControl=_.clone(model.itemSessionControl)),_.forEach(selection,function(item){var $item=$(item);addItemRef($(".itemrefs",$section),void 0,_.defaults({href:uri.decode($item.data("uri")),label:$.trim($item.clone().children().remove().end().text()),"qti-type":"assessmentItemRef"},defaultItemData))}),$(".ui-selected",$items).removeClass("ui-selected").removeClass("selected"),$placeholders.hide().off("click")}):$placeholders.hide().off("click")}),$(document).off("add.binder","#"+$section.attr("id")+" .itemrefs").on("add.binder","#"+$section.attr("id")+" .itemrefs",function(e,$itemRef){
if("binder"===e.namespace&&$itemRef.hasClass("itemref")){var index=$itemRef.data("bind-index");itemRefView.setUp($itemRef,model.sectionParts[index])}}),$selected=$(".selected",$items),$selected.length>0&&$items.trigger("itemselect.creator",$selected)}function addItemRef($refList,index,itemData){var $itemRef,$items=$refList.children("li");index=index||$items.length,itemData.identifier=qtiTestHelper.getIdentifier("item",data.identifiers),itemData.index=index+1,$itemRef=$(templates.itemref(itemData)),index>0?$itemRef.insertAfter($items.eq(index-1)):$itemRef.appendTo($refList),$refList.trigger("add",[$itemRef,itemData])}function rubricBlocks(){model.rubricBlocks||(model.rubricBlocks=[]),
...
})};return{setUp:setUp,listenActionState:listenActionState}}),define("taoQtiTest/controller/creator/views/test",["jquery","lodash","taoQtiTest/controller/creator/views/actions","taoQtiTest/controller/creator/views/testpart","taoQtiTest/controller/creator/templates/index","taoQtiTest/controller/creator/helpers/qtiTest"],function($,_,actions,testPartView,templates,qtiTestHelper){"use strict";var testView=function(model,data){function testParts(){model.testParts||(model.testParts=[]),$(".testpart").each(function(){var $testPart=$(this),index=$testPart.data("bind-index");model.testParts[index]||(model.testParts[index]={}),testPartView.setUp($testPart,model.testParts[index],data);
})}function propHandler(propView){var $view=propView.getView(),$title=$(".test-creator-test > h1 [data-bind=title]");$view.on("change.binder",function(e,model){"binder"===e.namespace&&"assessmentTest"===model["qti-type"]&&$title.text(model.title)})}function addTestPart(){$(".testpart-adder").adder({target:$(".testparts"),content:templates.testpart,templateData:function(cb){var testPartIndex=$(".testpart").length;cb({"qti-type":"testPart",identifier:qtiTestHelper.getIdentifier("testPart",data.identifiers),index:testPartIndex,navigationMode:0,submissionMode:0,assessmentSections:[{"qti-type":"assessmentSection",identifier:qtiTestHelper.getIdentifier("assessmentSection",data.identifiers),
...
},isSection:function(value){return qtiTestHelper.filterQtiType(value,"assessmentSection")}},encoders:{dom2qti:Dom2QtiEncoder},templates:templates,beforeSave:function(model){return qtiTestHelper.addMissingQtiType(model),qtiTestHelper.consolidateModel(model),!0}}),binder=DataBindController.takeControl($container,binderOptions).get(function(model){self.identifiers=qtiTestHelper.extractIdentifiers(model),validators.register("testIdFormat",qtiTestHelper.idFormatValidator()),validators.register("testIdAvailable",qtiTestHelper.idAvailableValidator(self.identifiers)),testView(model,{uri:options.uri,identifiers:self.identifiers,labels:options.labels}),testPartView.listenActionState(),
sectionView.listenActionState(),itemrefView.listenActionState(),itemrefView.resize(),$(window).off("resize.qti-test-creator").on("resize.qti-test-creator",function(){itemrefView.resize()})});$saver.on("click",function(event){event.preventDefault(),$saver.hasClass("disabled")||($saver.attr("disabled",!0).addClass("disabled"),binder.save(function(){$saver.attr("disabled",!1).removeClass("disabled"),feedback().success(__("Test Saved"))},function(){$saver.attr("disabled",!1).removeClass("disabled")}))})}};return Controller}),define("tpl!taoQtiTest/testRunner/tpl/button",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){
this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,buffer="",functionType="function",escapeExpression=this.escapeExpression;return buffer+='<li data-control="',(helper=helpers.id)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.id,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+'" class="small btn-info action" title="',(helper=helpers.title)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.title,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+'" data-order="',
(helper=helpers.order)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.order,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+'">\n    <a class="li-inner" href="#">\n        <span class="icon-',(helper=helpers.icon)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.icon,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+'"></span>\n        <span class="label">',(helper=helpers.label)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.label,stack1=typeof helper===functionType?helper.call(depth0,{
hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+"</span>\n    </a>\n</li>"})}),define("taoQtiTest/testRunner/actionBarHook",["jquery","lodash","core/errorHandler","tpl!taoQtiTest/testRunner/tpl/button"],function($,_,errorHandler,buttonTpl){"use strict";function isValidConfig(toolconfig){return _.isObject(toolconfig)&&toolconfig.label&&toolconfig.hook}function initQtiTool($toolsContainer,id,toolconfig,assessmentTestContext,testRunner){isValidConfig(toolconfig)?require([toolconfig.hook],function(hook){var order=_.parseInt(toolconfig.order);_.isNaN(order)&&(order=0);var tplData={id:id,navigation:!1,title:toolconfig.title||toolconfig.label,label:toolconfig.label,
icon:toolconfig.icon||"",order:order},$button=$(buttonTpl(tplData));if(isValidHook(hook)){var $existingBtn=$toolsContainer.children('[data-control="'+id+'"]');$existingBtn.length&&(hook.clear($button,toolconfig,assessmentTestContext),$existingBtn.remove()),hook.isVisible(toolconfig,assessmentTestContext)&&(hook.init($button,toolconfig,assessmentTestContext,testRunner),_appendInOrder($toolsContainer,$button),$button.trigger("ready"+_ns))}else errorHandler["throw"](_ns,"invalid hook format")},function(e){errorHandler["throw"](_ns,"the hook amd module cannot be found")}):errorHandler["throw"](_ns,"invalid tool config format")}function _appendInOrder($container,$button){
var $after,$before,order=parseInt($button.data("order"),10);order?($container.children(".action").each(function(){var $btn=$(this),_order=parseInt($btn.data("order"),10);return _order===order?($after=$btn,!1):void(_order>order?($before=$btn,$after=null):order>_order&&($after=$btn,$before=null))}),$after?$after.after($button):$before?$before.before($button):$container.append($button)):$container.append($button)}function isValidHook(hook){return _.isObject(hook)&&_.isFunction(hook.init)&&_.isFunction(hook.clear)&&_.isFunction(hook.isVisible)}var _ns=".actionBarHook";return{isValid:isValidConfig,initQtiTool:initQtiTool}}),define("tpl!taoQtiTest/testRunner/tpl/navigator",["handlebars"],function(hb){
return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,options,buffer="",functionType="function",escapeExpression=this.escapeExpression,helperMissing=helpers.helperMissing;return buffer+='        <aside class="test-sidebar test-sidebar-',(helper=helpers.region)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.region,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+' flex-container-navi qti-navigator">\n            <div class="qti-navigator-collapsible">\n                <span class="qti-navigator-collapse icon icon-left" title="'+escapeExpression((helper=helpers.__||depth0&&depth0.__,
...
answered:".answered",notFlagged:":not(.flagged)",notAnswered:":not(.answered)",masked:".masked"},_filterMap={all:"",unanswered:_selectors.answered,flagged:_selectors.notFlagged,answered:_selectors.notAnswered,filtered:_selectors.masked},_optionsMap={reviewScope:"reviewScope",reviewPreventsUnseen:"preventsUnseen",canCollapse:"canCollapse"},_reviewScopes={test:"test",testPart:"testPart",testSection:"testSection"},testReview={init:function(element,options){var initOptions=_.isObject(options)&&options||{},putOnRight="right"===initOptions.region,insertMethod=putOnRight?"append":"prepend";if(this.options=initOptions,this.disabled=!1,this.hidden=!1,this.$component&&this.$component.remove(),
this.$container=$(element),insertMethod=this.$container[insertMethod],!insertMethod)throw new Error("Unable to inject the component structure into the DOM");return insertMethod.call(this.$container,navigatorTpl({region:putOnRight?"right":"left"})),this._loadDOM(),this._initEvents(),this._updateDisplayOptions(),this},_loadDOM:function(){this.$component=this.$container.find(_selectors.component),this.$infoAnswered=this.$component.find(_selectors.infoAnswered),this.$infoViewed=this.$component.find(_selectors.infoViewed),this.$infoUnanswered=this.$component.find(_selectors.infoUnanswered),this.$infoFlagged=this.$component.find(_selectors.infoFlagged),this.$filterBar=this.$component.find(_selectors.filterBar),
this.$filters=this.$filterBar.find("li"),this.$tree=this.$component.find(_selectors.tree),this.$linearState=this.$component.find(_selectors.linearState)},_initEvents:function(){var self=this;this.$component.on("click"+_selectors.component,_selectors.collapseHandle,function(){self.disabled||(self.$component.toggleClass(_cssCls.collapsed),self.$component.hasClass(_cssCls.collapsed)&&self._openSelected())}),this.$component.on("click"+_selectors.component,_selectors.infoPanelLabels,function(){if(!self.disabled){var $panel=$(this).closest(_selectors.infoPanel);self._togglePanel($panel,_selectors.infoPanel)}}),this.$tree.on("click"+_selectors.component,_selectors.partLabels,function(){
if(!self.disabled){var $panel=$(this).closest(_selectors.parts),open=self._togglePanel($panel,_selectors.parts);open&&($panel.hasClass(_cssCls.active)?self._openSelected():self._openOnly($panel.find(_selectors.sections).first(),$panel))}}),this.$tree.on("click"+_selectors.component,_selectors.sectionLabels,function(){if(!self.disabled){var $panel=$(this).closest(_selectors.sections);self._togglePanel($panel,_selectors.sections)}}),this.$tree.on("click"+_selectors.component,_selectors.itemLabels,function(event){if(!self.disabled){var $target,$item=$(this).closest(_selectors.items);$item.hasClass(_cssCls.disabled)||($target=$(event.target),$target.is(_selectors.icons)&&!self.$component.hasClass(_cssCls.collapsed)?$item.hasClass(_cssCls.unseen)||self._mark($item):(self._select($item),
self._jump($item)))}}),this.$tree.on("click"+_selectors.component,_selectors.linearStart,function(){if(!self.disabled){var $btn=$(this);$btn.hasClass(_cssCls.disabled)||($btn.addClass(_cssCls.disabled),self._jump($btn))}}),this.$filterBar.on("click"+_selectors.component,"li",function(){if(!self.disabled){var $btn=$(this),mode=$btn.data("mode");self.$filters.removeClass(_cssCls.active),self.$component.removeClass(_cssCls.collapsed),$btn.addClass(_cssCls.active),self._filter(mode)}})},_filter:function(criteria){var $items=this.$tree.find(_selectors.items).removeClass(_cssCls.masked),filter=_filterMap[criteria];filter&&$items.filter(filter).addClass(_cssCls.masked),
this._updateSectionCounters(!!filter)},_select:function(position,open){var selected=position&&position.jquery?position:this.$tree.find("[data-position="+position+"]"),hierarchy=selected.parentsUntil(this.$tree);return open&&this._openOnly(hierarchy),this.$tree.find(_selectors.actives).removeClass(_cssCls.active),hierarchy.add(selected).addClass(_cssCls.active),selected},_openSelected:function(){var selected=this.$tree.find(_selectors.items+_selectors.actives),hierarchy=selected.parentsUntil(this.$tree);return this._openOnly(hierarchy),selected},_openOnly:function(opened,root){(root||this.$tree).find(_selectors.collapsible).addClass(_cssCls.collapsed),opened.removeClass(_cssCls.collapsed);
},_togglePanel:function(panel,collapseSelector){var collapsed=panel.hasClass(_cssCls.collapsed);return collapseSelector&&this.$tree.find(collapseSelector).addClass(_cssCls.collapsed),collapsed?panel.removeClass(_cssCls.collapsed):panel.addClass(_cssCls.collapsed),collapsed},_setItemIcon:function($item,icon){$item.find(_selectors.icons).attr("class",_cssCls.icon+" icon-"+icon)},_adjustItemIcon:function($item){var icon=null,defaultIcon=_cssCls.unseen,iconCls=[_cssCls.flagged,_cssCls.answered,_cssCls.viewed];_.forEach(iconCls,function(cls){return $item.hasClass(cls)?(icon=cls,!1):void 0}),this._setItemIcon($item,icon||defaultIcon)},_toggleFlag:function($item,flag){
$item.toggleClass(_cssCls.flagged,flag),this._adjustItemIcon($item)},_mark:function($item){var itemId=$item.data("id"),itemPosition=$item.data("position"),flag=!$item.hasClass(_cssCls.flagged);this._toggleFlag($item),this.trigger("mark",[flag,itemPosition,itemId])},_jump:function($item){var itemId=$item.data("id"),itemPosition=$item.data("position");this.trigger("jump",[itemPosition,itemId])},_updateSectionCounters:function(filtered){var filter=_filterMap[filtered?"filtered":"answered"];this.$tree.find(_selectors.sections).each(function(){var $section=$(this),$items=$section.find(_selectors.items),$filtered=$items.filter(filter),total=$items.length,nb=total-$filtered.length;
$section.find(_selectors.counters).html(nb+"/"+total)})},_updateDisplayOptions:function(){var reviewScope=_reviewScopes[this.options.reviewScope]||"test",scopeClass=_cssCls.scope[reviewScope],$root=this.$component;_.forEach(_cssCls.scope,function(cls){$root.removeClass(cls)}),scopeClass&&$root.addClass(scopeClass),$root.toggleClass(_cssCls.collapsible,this.options.canCollapse)},_updateOptions:function(testContext){var options=this.options;_.forEach(_optionsMap,function(optionKey,contextKey){void 0!==testContext[contextKey]&&(options[optionKey]=testContext[contextKey])})},_updateInfos:function(testContext){var progression=this.getProgression(testContext),unanswered=Number(progression.total)-Number(progression.answered);
this.$infoAnswered.text(progression.answered+"/"+progression.total),this.$infoUnanswered.text(unanswered+"/"+progression.total),this.$infoViewed.text(progression.viewed+"/"+progression.total),this.$infoFlagged.text(progression.flagged+"/"+progression.total)},_getProgressionOfTest:function(testContext){return{total:testContext.numberItems||0,answered:testContext.numberCompleted||0,viewed:testContext.numberPresented||0,flagged:testContext.numberFlagged||0}},_getProgressionOfTestPart:function(testContext){return{total:testContext.numberItemsPart||0,answered:testContext.numberCompletedPart||0,viewed:testContext.numberPresentedPart||0,flagged:testContext.numberFlaggedPart||0
...
},ServiceApi.prototype.onKill=function(callback){this.onKillCallback=callback},ServiceApi.prototype.kill=function(callback){"function"==typeof this.onKillCallback?this.onKillCallback(callback):callback(0)},ServiceApi.prototype.finish=function(valueArray){"function"==typeof this.onFinishCallback&&this.onFinishCallback(valueArray)},ServiceApi}),define("serviceApi/UserInfoService",["jquery"],function($){function UserInfoService(requestUrl,data){this.data=data,this.requestUrl=requestUrl}return UserInfoService.prototype.get=function(property,callback){this.data.hasOwnProperty(property)?"function"==typeof callback&&callback(this.data[property]):$.ajax({url:this.requestUrl,
data:{property:property},type:"post",dataType:"json",success:function(service,callback){return function(r){for(key in r.data)service.data[key]=r.data[key];"function"==typeof callback&&callback(service.data[property])}}(this,callback)})},UserInfoService}),define("serviceApi/StateStorage",["jquery"],function($){"use strict";function StateStorage(state,submitUrl){this.state=state,this.submitUrl=submitUrl}return StateStorage.prototype.get=function(callback){return"function"==typeof callback&&callback(this.state),this.state},StateStorage.prototype.set=function(state,callback){state===this.state?"function"==typeof callback&&callback():(this.state=state,$.ajax({url:this.submitUrl,
data:{state:state},type:"post",dataType:"json",success:"function"==typeof callback?callback:null}))},StateStorage}),define("taoQtiTest/controller/runtime/testRunner",["jquery","lodash","module","taoQtiTest/testRunner/actionBarHook","taoQtiTest/testRunner/testReview","taoQtiTest/testRunner/progressUpdater","serviceApi/ServiceApi","serviceApi/UserInfoService","serviceApi/StateStorage","iframeResizer","iframeNotifier","i18n","mathJax","ui/feedback","ui/deleter","moment","ui/modal","ui/progressbar"],function($,_,module,actionBarHook,testReview,progressUpdater,ServiceApi,UserInfoService,StateStorage,iframeResizer,iframeNotifier,__,MathJax,feedback,deleter,moment,modal){
"use strict";var timerIds=[],currentTimes=[],lastDates=[],timeDiffs=[],waitingTime=0,$timers,$controls,timerIndex,$doc=$(document),TestRunner={TEST_STATE_INITIAL:0,TEST_STATE_INTERACTING:1,TEST_STATE_MODAL_FEEDBACK:2,TEST_STATE_SUSPENDED:3,TEST_STATE_CLOSED:4,TEST_NAVIGATION_LINEAR:0,TEST_NAVIGATION_NONLINEAR:1,TEST_ITEM_STATE_INTERACTING:1,SECTION_EXIT_CODE:{COMPLETED_NORMALLY:700,QUIT:701,COMPLETE_TIMEOUT:703,TIMEOUT:704,FORCE_QUIT:705,IN_PROGRESS:706,ERROR:300},TEST_EXIT_CODE:{COMPLETE:"C",TERMINATED:"T",INCOMPLETE:"IC",INCOMPLETE_QUIT:"IQ",INACTIVE:"IA",CANDIDATE_DISAGREED_WITH_NDA:"DA"},beforeTransition:function(callback){iframeNotifier.parent("loading"),
this.disableGui(),$controls.$itemFrame.hide(),$controls.$rubricBlocks.hide(),$controls.$timerWrapper.hide(),"function"==typeof callback&&setTimeout(callback,waitingTime)},afterTransition:function(){this.enableGui(),iframeNotifier.parent("unloading")},jump:function(position){var self=this,action="jump",params={position:position};this.disableGui(),this.isJumpOutOfSection(position)&&this.isCurrentItemActive()&&this.isTimedSection()?this.exitTimedSection(action,params):this.itemServiceApi.kill(function(){self.actionCall(action,null,params)})},markForReview:function(flag,position){var self=this;iframeNotifier.parent("loading"),this.disableGui(),$.ajax({url:self.testContext.markForReviewUrl,
cache:!1,async:!0,type:"POST",dataType:"json",data:{flag:flag,position:position},success:function(data){self.testReview&&(self.testReview.setItemFlag(position,flag),self.testContext.itemPosition===position&&(self.testContext.itemFlagged=flag),self.updateTools(self.testContext)),self.enableGui(),iframeNotifier.parent("unloading")}})},moveForward:function(){var self=this,action="moveForward";this.disableGui(),this.testContext.numberItemsSection-this.testContext.itemPositionSection-1==0&&this.isCurrentItemActive()?this.isTimedSection()?this.exitTimedSection(action):this.exitSection(action):this.itemServiceApi.kill(function(){self.actionCall(action)})},moveBackward:function(){
var self=this,action="moveBackward";this.disableGui(),0==this.testContext.itemPositionSection&&this.isCurrentItemActive()&&this.isTimedSection()?this.exitTimedSection(action):this.itemServiceApi.kill(function(){self.actionCall(action)})},isJumpOutOfSection:function(jumpPosition){var items=this.getCurrentSectionItems(),isJumpToOtherSection=!0,isValidPosition=jumpPosition>=0&&jumpPosition<this.testContext.numberItems;if(isValidPosition){for(var i in items)if(items.hasOwnProperty(i)&&items[i].position==jumpPosition){isJumpToOtherSection=!1;break}}else isJumpToOtherSection=!1;return isJumpToOtherSection},exitSection:function(action,params){var self=this,metaData={
SECTION:{SECTION_EXIT_CODE:TestRunner.SECTION_EXIT_CODE.COMPLETED_NORMALLY}};self.itemServiceApi.kill(function(){self.actionCall(action,metaData,params)})},exitTimedSection:function(action,params){var self=this,$confirmBox=$(".exit-modal-feedback"),message=__("After you complete the section it would be impossible to return to this section to make changes. Are you sure you want to end the section?"),flaggedCount=this.testContext.numberFlaggedSection;this.getQtiRunner().updateItemApi(),void 0!==flaggedCount&&(message=__("You have %s item(s) marked for review.",flaggedCount.toString())+" "+message),$confirmBox.find(".message").html(message),$confirmBox.modal({width:500
}),$confirmBox.find(".js-exit-cancel, .modal-close").off("click").on("click",function(){self.enableGui(),$confirmBox.modal("close")}),$confirmBox.find(".js-exit-confirm").off("click").on("click",function(){$confirmBox.modal("close"),self.exitSection(action,params)})},isCurrentItemActive:function(){return 4!=this.testContext.itemSessionState},isCurrentItemAnswered:function(){var answered=!1;return _.each(this.getCurrentItemState(),function(state){return state&&_.isObject(state.response)&&null!==state.response.base?(answered=!0,!1):void 0}),answered},getQtiRunner:function(){var itemWindow,itemContainerWindow;return itemWindow=$("#qti-item")[0].contentWindow,itemContainerWindow=$(itemWindow.document).find("#item-container")[0].contentWindow,
itemContainerWindow.qtiRunner},isTimedSection:function(){var timeConstraints=this.testContext.timeConstraints,isTimedSection=!1;for(var index in timeConstraints)timeConstraints.hasOwnProperty(index)&&"assessmentSection"==timeConstraints[index].qtiClassName&&(isTimedSection=!0);return isTimedSection},getCurrentSectionItems:function(){var sectionItems,partId=this.testContext.testPartId,navMap=this.testContext.navigatorMap;for(var partIndex in navMap)if(navMap.hasOwnProperty(partIndex)&&navMap[partIndex].id===partId)for(var sectionIndex in navMap[partIndex].sections)if(navMap[partIndex].sections.hasOwnProperty(sectionIndex)&&navMap[partIndex].sections[sectionIndex].active===!0){
sectionItems=navMap[partIndex].sections[sectionIndex].items;break}return sectionItems},skip:function(){this.disableGui(),this.actionCall("skip")},timeout:function(){var self=this;this.disableGui(),this.testContext.isTimeout=!0,this.updateTimer(),this.itemServiceApi.kill(function(){var confirmBox=$(".timeout-modal-feedback"),testContext=self.testContext,confirmBtn=confirmBox.find(".js-timeout-confirm, .modal-close"),metaData={};metaData=testContext.numberCompletedSection===testContext.numberItemsSection?{SECTION:{SECTION_EXIT_CODE:TestRunner.SECTION_EXIT_CODE.COMPLETE_TIMEOUT}}:{SECTION:{SECTION_EXIT_CODE:TestRunner.SECTION_EXIT_CODE.TIMEOUT}},confirmBox.modal({
width:500}),confirmBtn.off("click").on("click",function(){confirmBox.modal("close"),self.actionCall("timeout",metaData)})})},comment:function(){$controls.$commentArea.is(":visible")||$controls.$commentText.val(""),$controls.$commentArea.toggle(),$controls.$commentText.focus()},closeComment:function(){$controls.$commentArea.hide()},emptyComment:function(){$controls.$commentText.val("")},storeComment:function(){var self=this,comment=$controls.$commentText.val();comment&&$.when($.post(self.testContext.commentUrl,{comment:comment})).done(function(){self.closeComment()})},setTestContext:function(testContext){this.testContext=testContext,this.itemServiceApi=eval(testContext.itemServiceApiCall);
},update:function(testContext){var self=this;$controls.$itemFrame.remove();var $runner=$("#runner");$runner.css("height","auto"),this.setTestContext(testContext),this.updateContext(),this.updateProgress(),this.updateNavigation(),this.updateTestReview(),this.updateInformation(),this.updateRubrics(),this.updateTools(testContext),this.updateTimer(),this.updateExitButton(),this.resetCurrentItemState(),$controls.$itemFrame=$('<iframe id="qti-item" frameborder="0"/>'),$controls.$itemFrame.appendTo($controls.$contentBox),iframeResizer.autoHeight($controls.$itemFrame,"body"),this.testContext.itemSessionState===this.TEST_ITEM_STATE_INTERACTING&&self.testContext.isTimeout===!1?($doc.off(".testRunner").on("serviceloaded.testRunner",function(){
self.afterTransition(),self.adjustFrame(),$controls.$itemFrame.css({visibility:"visible"})}),this.itemServiceApi.loadInto($controls.$itemFrame[0],function(){})):self.afterTransition()},updateInformation:function(){this.testContext.isTimeout===!0?feedback().error(__('Time limit reached for item "%s".',this.testContext.itemIdentifier)):this.testContext.itemSessionState!==this.TEST_ITEM_STATE_INTERACTING&&feedback().error(__('No more attempts allowed for item "%s".',this.testContext.itemIdentifier))},updateTools:function(testContext){var $toolsContainer,config=module.config();this.testContext.allowSkipping===!0?this.testContext.isLast===!1?($controls.$skip.show(),
$controls.$skipEnd.hide()):($controls.$skip.hide(),$controls.$skipEnd.show()):($controls.$skip.hide(),$controls.$skipEnd.hide()),config&&config.qtiTools&&($toolsContainer=$(".tools-box-list"),_.forIn(config.qtiTools,function(toolconfig,id){actionBarHook.initQtiTool($toolsContainer,id,toolconfig,testContext,TestRunner)}))},createTimer:function(cst){var $timer=$("<div>",{"class":"qti-timer qti-timer__type-"+cst.qtiClassName}),$label=$("<div>",{"class":"qti-timer_label truncate",text:cst.label}),$time=$("<div>",{"class":"qti-timer_time",text:this.formatTime(cst.seconds)});return $timer.append($label),$timer.append($time),$timer},updateTimer:function(){var hasTimers,self=this;
$controls.$timerWrapper.empty();for(var i=0;i<timerIds.length;i++)clearTimeout(timerIds[i]);if(timerIds=[],currentTimes=[],lastDates=[],timeDiffs=[],self.testContext.isTimeout===!1&&self.testContext.itemSessionState===self.TEST_ITEM_STATE_INTERACTING&&(hasTimers=!!this.testContext.timeConstraints.length,$controls.$topActionBar.toggleClass("has-timers",hasTimers),self.adjustFrame(),hasTimers)){for(i=0;i<this.testContext.timeConstraints.length;i++){var cst=this.testContext.timeConstraints[i];cst.allowLateSubmission===!1&&($controls.$timerWrapper.append(self.createTimer(cst)),currentTimes[i]=cst.seconds,lastDates[i]=new Date,timeDiffs[i]=0,timerIndex=i,cst.warningTime=Number.NEGATIVE_INFINITY,
self.testContext.timerWarning&&self.testContext.timerWarning[cst.qtiClassName]&&(cst.warningTime=parseInt(self.testContext.timerWarning[cst.qtiClassName],10)),function(timerIndex,cst){timerIds[timerIndex]=setInterval(function(){if(timeDiffs[timerIndex]+=(new Date).getTime()-lastDates[timerIndex].getTime(),timeDiffs[timerIndex]>=1e3){var seconds=timeDiffs[timerIndex]/1e3;currentTimes[timerIndex]-=seconds,timeDiffs[timerIndex]=0}$timers.eq(timerIndex).html(self.formatTime(Math.round(currentTimes[timerIndex]))),currentTimes[timerIndex]<=0?(currentTimes[timerIndex]=0,clearInterval(timerIds[timerIndex]),$controls.$itemFrame.hide(),self.timeout()):lastDates[timerIndex]=new Date,
_.isFinite(cst.warningTime)&&currentTimes[timerIndex]<=cst.warningTime&&self.timeWarning(cst)},1e3)}(timerIndex,cst))}$timers=$controls.$timerWrapper.find(".qti-timer .qti-timer_time"),$controls.$timerWrapper.show()}},timeWarning:function(cst){var message="";$controls.$timerWrapper.find(".qti-timer__type-"+cst.qtiClassName).addClass("qti-timer__warning"),cst.seconds>cst.warningTime&&(message=moment.duration(cst.warningTime,"seconds").humanize(),feedback().warning(__("Warning – You have %s remaining to complete the test.",message))),cst.warningTime=Number.NEGATIVE_INFINITY},updateRubrics:function(){if($controls.$rubricBlocks.remove(),this.testContext.rubrics.length>0){
$controls.$rubricBlocks=$('<div id="qti-rubrics"/>');for(var i=0;i<this.testContext.rubrics.length;i++)$controls.$rubricBlocks.append(this.testContext.rubrics[i]);$controls.$rubricBlocks.find("a").bind("click keypress",function(){return window.open(this.href),!1}),$controls.$rubricBlocks.prependTo($controls.$contentBox),MathJax&&MathJax.Hub.Queue(["Typeset",MathJax.Hub],$controls.$rubricBlocks[0])}},updateNavigation:function(){$controls.$exit.show(),this.testContext.isLast===!0?($controls.$moveForward.hide(),$controls.$moveEnd.show()):($controls.$moveForward.show(),$controls.$moveEnd.hide()),this.testContext.navigationMode===this.TEST_NAVIGATION_LINEAR?$controls.$moveBackward.hide():($controls.$controls.show(),
this.testContext.canMoveBackward===!0?$controls.$moveBackward.show():$controls.$moveBackward.hide())},updateTestReview:function(){var considerProgress=this.testContext.considerProgress===!0;this.testReview&&(this.testReview.toggle(considerProgress),this.testReview.update(this.testContext))},updateProgress:function(){var considerProgress=this.testContext.considerProgress===!0;$controls.$progressBox.css("visibility",considerProgress?"visible":"hidden"),considerProgress&&this.progressUpdater.update(this.testContext)},updateContext:function(){$controls.$title.text(this.testContext.testTitle),$controls.$position.text(" - "+this.testContext.sectionTitle),$controls.$titleGroup.show();
},updateExitButton:function(){$controls.$logout.toggleClass("hidden",!this.testContext.logoutButton),$controls.$exit.toggleClass("hidden",!this.testContext.exitButton)},adjustFrame:function(){var finalHeight=$(window).innerHeight()-$controls.$topActionBar.outerHeight()-$controls.$bottomActionBar.outerHeight();$controls.$contentBox.height(finalHeight),$controls.$sideBars.length&&$controls.$sideBars.each(function(){var $sideBar=$(this);$sideBar.height(finalHeight-$sideBar.outerHeight()+$sideBar.height())})},disableGui:function(){$controls.$naviButtons.addClass("disabled"),this.testReview&&this.testReview.disable()},enableGui:function(){$controls.$naviButtons.removeClass("disabled"),
this.testReview&&this.testReview.enable()},formatTime:function(totalSeconds){var sec_num=totalSeconds,hours=Math.floor(sec_num/3600),minutes=Math.floor((sec_num-3600*hours)/60),seconds=sec_num-3600*hours-60*minutes;10>hours&&(hours="0"+hours),10>minutes&&(minutes="0"+minutes),10>seconds&&(seconds="0"+seconds);var time=hours+":"+minutes+":"+seconds;return time},actionCall:function(action,metaData,extraParams){var self=this,params=metaData?{metaData:metaData}:{};extraParams&&(params=_.assign(params,extraParams)),this.beforeTransition(function(){$.ajax({url:self.testContext[action+"Url"],cache:!1,data:params,async:!0,dataType:"json",success:function(testContext){
...
responseId&&response&&TestRunner.setCurrentItemState(responseId,{response:response})}).on("stateready",function(e,id,state){id&&state&&TestRunner.setCurrentItemState(id,state)})}}});
