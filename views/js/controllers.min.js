...
SECTION:{SECTION_EXIT_CODE:TestRunner.SECTION_EXIT_CODE.COMPLETED_NORMALLY}};self.itemServiceApi.kill(function(){self.actionCall(action,metaData,params)})},exitTimedSection:function(action,params){var message,self=this,$confirmBox=$(".exit-modal-feedback"),messageFlagged="",unansweredCount=this.testContext.numberItemsSection-this.testContext.numberCompletedSection,flaggedCount=this.testContext.numberFlaggedSection;this.isCurrentItemAnswered()&&unansweredCount--,this.getQtiRunner().updateItemApi(),void 0!==flaggedCount&&(messageFlagged=" and have %s item(s) marked for review"),message=__("You have %s unanswered question(s)"+messageFlagged+". After you complete the section it would be impossible to return to this section to make changes.  Are you sure you want to end the section?",(unansweredCount||0).toString(),(flaggedCount||0).toString()),
$confirmBox.find(".message").html(message),$confirmBox.modal({width:500}),$confirmBox.find(".js-exit-cancel, .modal-close").off("click").on("click",function(){self.enableGui(),$confirmBox.modal("close")}),$confirmBox.find(".js-exit-confirm").off("click").on("click",function(){$confirmBox.modal("close"),self.exitSection(action,params)})},isCurrentItemActive:function(){return 4!=this.testContext.itemSessionState},isCurrentItemAnswered:function(){var answered=!1;return _.each(this.getCurrentItemState(),function(state){return state&&_.isObject(state.response)&&null!==state.response.base?(answered=!0,!1):void 0}),answered},getQtiRunner:function(){var itemWindow,itemContainerWindow;
return itemWindow=$("#qti-item")[0].contentWindow,itemContainerWindow=$(itemWindow.document).find("#item-container")[0].contentWindow,itemContainerWindow.qtiRunner},isTimedSection:function(){var timeConstraints=this.testContext.timeConstraints,isTimedSection=!1;for(var index in timeConstraints)timeConstraints.hasOwnProperty(index)&&"assessmentSection"==timeConstraints[index].qtiClassName&&(isTimedSection=!0);return isTimedSection},getCurrentSectionItems:function(){var sectionItems,partId=this.testContext.testPartId,navMap=this.testContext.navigatorMap;for(var partIndex in navMap)if(navMap.hasOwnProperty(partIndex)&&navMap[partIndex].id===partId)for(var sectionIndex in navMap[partIndex].sections)if(navMap[partIndex].sections.hasOwnProperty(sectionIndex)&&navMap[partIndex].sections[sectionIndex].active===!0){
