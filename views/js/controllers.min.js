...
outcomeHelper.addOutcomeProcessing(model,processingRule)}function formatCategoryOutcome(category,template){return template.replace(/%s/g,category.toUpperCase())}function belongToScoringMode(identifier,processingMode,onlyCategories){var match=!1;return processingMode.signature&&processingMode.signature.test(identifier)&&(match=!0,onlyCategories&&_.forEach(processingMode.outcomes,function(outcome){if(outcome.identifier===identifier||outcome.weighted&&outcome.weighted===identifier||outcome.feedback&&outcome.feedback===identifier)return match=!1,!1})),match}function matchScoringMode(processingMode,outcomes,categories){function matchScoringOutcome(mode,identifier){
var outcomeMatch=!1;return mode.signature&&mode.signature.test(identifier)&&_.forEach(mode.outcomes,function(outcome){if(outcome.identifier!==identifier&&(!outcome.weighted||outcome.weighted&&outcome.weighted!==identifier)&&(!outcome.feedback||outcome.feedback&&outcome.feedback!==identifier)?categories&&_.forEach(categories,function(category){if(outcome.categoryIdentifier&&identifier===formatCategoryOutcome(category,outcome.categoryIdentifier)?outcomeMatch=!0:outcome.categoryWeighted&&identifier===formatCategoryOutcome(category,outcome.categoryWeighted)?outcomeMatch=!0:outcome.categoryFeedback&&identifier===formatCategoryOutcome(category,outcome.categoryFeedback)&&(outcomeMatch=!0),
outcomeMatch)return!1}):outcomeMatch=!0,outcomeMatch)return!1}),!outcomeMatch&&mode.include&&(outcomeMatch=matchScoringOutcome(processingModes[mode.include],identifier)),outcomeMatch}var signatures=getSignatures(processingMode),match=!0;return _.forEach(outcomes,function(identifier){var signatureMatch=!1;if(_.forEach(signatures,function(signature){if(signature.test(identifier))return signatureMatch=!0,!1}),signatureMatch&&(match=matchScoringOutcome(processingMode,identifier),!match))return!1}),match}function getSignatures(processingMode){for(var signatures=[];processingMode;)processingMode.signature&&signatures.push(processingMode.signature),processingMode=processingMode.include&&processingModes[processingMode.include];
return signatures}function getRecipes(processingMode){for(var descriptors=[];processingMode;)processingMode.outcomes&&(descriptors=descriptors.concat(processingMode.outcomes)),processingMode=processingMode.include&&processingModes[processingMode.include];return descriptors}function getScoringMode(outcome){var identifier=outcomeHelper.getOutcomeIdentifier(outcome),mode=null;return _.forEach(processingModes,function(processingMode){if(belongToScoringMode(identifier,processingMode))return mode=processingMode,!1}),mode}function isScoringOutcome(outcome){return!!getScoringMode(outcome)}function listScoringModes(outcomes){var modes={};return _.forEach(outcomes,function(outcome){
var mode=getScoringMode(outcome);mode&&(modes[mode.key]=!0)}),_.keys(modes)}function handleCategories(modelOverseer){var model=modelOverseer.getModel();return!(!model.scoring||!model.scoring.categoryScore)}function hasCategoryOutcome(model){var categoryOutcomes=!1;return _.forEach(outcomeHelper.getOutcomeDeclarations(model),function(outcomeDeclaration){var identifier=outcomeHelper.getOutcomeIdentifier(outcomeDeclaration);_.forEach(processingModes,function(processingMode){if(belongToScoringMode(identifier,processingMode,!0))return categoryOutcomes=!0,!1})}),categoryOutcomes}function getCutScore(model){var values=_(outcomeHelper.getOutcomeProcessingRules(model)).map(function(outcome){
