...
},updateProgress:function(){var considerProgress=this.assessmentTestContext.considerProgress;if($("#qti-test-progress").css("visibility",considerProgress===!0?"visible":"hidden"),considerProgress===!0){var ratio=Math.floor(this.assessmentTestContext.numberCompleted/this.assessmentTestContext.numberItems*100),label="Avancement de l'évaluation (%d%%)".replace("%d",ratio).replace("%%","%");$("#qti-progress-label").text(label),$("#qti-progressbar").progressbar("value",ratio)}},updateContext:function(){var sectionTitle=this.assessmentTestContext.sectionTitle;try{$("#qti-test-position").badonkatrunc("destroy")}catch(e){}$("#qti-test-position").empty().append('<span id="qti-section-title">'+sectionTitle+"</span>"),$("#qti-test-position").badonkatrunc().css("visibility","visible")
},adjustFrame:function(){var actionsHeight=$("#qti-actions").outerHeight(),windowHeight=window.innerHeight?window.innerHeight:$(window).height(),navigationHeight=$("#qti-navigation").outerHeight(),newContentHeight=windowHeight-actionsHeight-navigationHeight,$content=$("#qti-content");$content.height(newContentHeight-parseInt($content.css("paddingTop"))-parseInt($content.css("paddingBottom")))},disableGui:function(){$("#qti-navigation button").addClass("disabled")},enableGui:function(){$("#qti-navigation button").removeClass("disabled")},formatTime:function(totalSeconds){var sec_num=totalSeconds,hours=Math.floor(sec_num/3600),minutes=Math.floor((sec_num-3600*hours)/60),seconds=sec_num-3600*hours-60*minutes;
10>hours&&(hours="0"+hours),10>minutes&&(minutes="0"+minutes),10>seconds&&(seconds="0"+seconds);var time=hours+":"+minutes+":"+seconds;return"± "+time},actionCall:function(action){var self=this;this.beforeTransition(function(){$.ajax({url:self.assessmentTestContext[action+"Url"],cache:!1,async:!0,dataType:"json",success:function(assessmentTestContext){assessmentTestContext.state===self.TEST_STATE_CLOSED?self.serviceApi.finish():self.update(assessmentTestContext)}})})}};return{start:function(assessmentTestContext){$(document).ajaxError(function(event,jqxhr){403==jqxhr.status&&iframeNotifier.parent("serviceforbidden")}),window.onServiceApiReady=function(serviceApi){TestRunner.serviceApi=serviceApi,assessmentTestContext.state===TestRunner.TEST_STATE_CLOSED?serviceApi.finish():TestRunner.update(assessmentTestContext)
},TestRunner.beforeTransition(),TestRunner.assessmentTestContext=assessmentTestContext,$("#skip, #skip-end").click(function(){$(this).hasClass("disabled")||TestRunner.skip()}),$("#move-forward, #move-end").click(function(){$(this).hasClass("disabled")||TestRunner.moveForward()}),$("#move-backward").click(function(){$(this).hasClass("disabled")||TestRunner.moveBackward()}),$("#comment").click(function(){$(this).hasClass("disabled")||TestRunner.comment()}),$("#qti-comment-cancel").click(function(){TestRunner.closeComment()}),$("#qti-comment-send").click(function(){TestRunner.storeComment()}),$("#qti-comment > textarea").click(function(){TestRunner.emptyComment()
}),$(window).bind("resize",function(){TestRunner.adjustFrame(),$("#qti-test-title, #qti-test-position").badonkatrunc()}),$(document).bind("loading",function(){iframeNotifier.parent("loading")}),$(document).bind("unloading",function(){iframeNotifier.parent("unloading")}),$("#qti-progressbar").progressbar(),iframeNotifier.parent("serviceready")}}});
