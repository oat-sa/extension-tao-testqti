...
},removeAttributes:function(attrNames){"string"==typeof attrNames&&(attrNames=[attrNames]);for(var i in attrNames)delete this.attributes[attrNames[i]];return this},getComposingElements:function(){var elts={};if("function"==typeof this.initContainer){var container=this.getBody();elts[container.getSerial()]=container,elts=_.extend(elts,container.getComposingElements())}if("function"==typeof this.initObject){var object=this.getObject();elts[object.getSerial()]=object,elts=_.extend(elts,object.getComposingElements())}return elts},getUsedClasses:function(){var ret=[this.qtiClass],composingElts=this.getComposingElements();return _.each(composingElts,function(elt){ret.push(elt.qtiClass)
}),_.uniq(ret)},find:function(serial){var found=null;if("function"==typeof this.initObject){var object=this.getObject();object.serial===serial&&(found={parent:this,element:object,location:"object"})}return found||"function"!=typeof this.initContainer||(found=this.getBody().find(serial,this)),found},parent:function(){var item=this.getRelatedItem();if(item){var found=item.find(this.getSerial());if(found)return found.parent}return null},setRelatedItem:function(item,recursive){if(recursive="undefined"==typeof recursive?!0:recursive,Element.isA(item,"assessmentItem")){this.relatedItem=item;var composingElts=this.getComposingElements();for(var i in composingElts)composingElts[i].setRelatedItem(item,!1)
}},getRelatedItem:function(){var ret=null;return Element.isA(this.relatedItem,"assessmentItem")&&(ret=this.relatedItem),ret},setRenderer:function(renderer){if(!renderer||!renderer.isRenderer)throw"invalid qti rendering engine";this.renderer=renderer;var elts=this.getComposingElements();for(var serial in elts)elts[serial].setRenderer(renderer)},getRenderer:function(){return this.renderer},render:function(){var args=rendererConfig.getOptionsFromArguments(arguments),renderer=args.renderer||this.getRenderer(),tplData={},defaultData={tag:this.qtiClass,serial:this.serial,attributes:this.getAttributes()};if(!renderer)throw"render: no renderer found for the element "+this.qtiClass+":"+this.serial;
if("function"==typeof this.initContainer&&(defaultData.body=this.getBody().render(renderer)),"function"==typeof this.initObject){defaultData.object={attributes:this.object.getAttributes()};var url=defaultData.object.attributes.data,baseUrl=renderer.getOption("baseUrl");baseUrl&&(/^http(s)?:\/\//.test(url)||(defaultData.object.attributes.data=baseUrl+url))}tplData=_.merge(defaultData,args.data||{}),tplData=renderer.getData(this,tplData,args.subclass);var rendering=renderer.renderTpl(this,tplData,args.subclass);return args.placeholder&&args.placeholder.replaceWith(rendering),rendering},postRender:function(data,altClassName,renderer){if(renderer=renderer||this.getRenderer(),"function"==typeof this.initContainer&&this.getBody().postRender({},"",renderer),renderer)return renderer.postRender(this,data,altClassName);
...
}if(result==lastResult&&(sameResultCount++,void 0==extraOptimizeLimit||sameResultCount==extraOptimizeLimit))return void(null!=mostOptimalValue&&actionFunc(mostOptimalValue));if(result>0){currentMax=currentValue;var difference=currentValue-currentMin;currentValue-=difference/2}else 0>=result&&(currentMin=currentValue,currentValue=currentMax);lastResult=result}}}(jQuery),define("jquery.trunc",function(){}),define("taoQtiTest/controller/runtime/testRunner",["jquery","jqueryui","lodash","spin","serviceApi/ServiceApi","serviceApi/UserInfoService","serviceApi/StateStorage","iframeResizer","iframeNotifier","i18n","mathJax","jquery.trunc"],function($,$ui,_,Spinner,ServiceApi,UserInfoService,StateStorage,iframeResizer,iframeNotifier,__,MathJax){var timerIds=[],currentTimes=[],lastDates=[],timeDiffs=[],waitingTime=0,TestRunner={TEST_STATE_INITIAL:0,TEST_STATE_INTERACTING:1,TEST_STATE_MODAL_FEEDBACK:2,TEST_STATE_SUSPENDED:3,TEST_STATE_CLOSED:4,TEST_NAVIGATION_LINEAR:0,TEST_NAVIGATION_NONLINEAR:1,TEST_ITEM_STATE_INTERACTING:1,beforeTransition:function(callback){iframeNotifier.parent("loading"),this.disableGui(),$("#qti-item, #qti-info, #qti-rubrics, #qti-timers").css("display","none"),"function"==typeof callback&&setTimeout(callback,waitingTime)
