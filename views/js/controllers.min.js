...
this.testReview&&(this.testReview.toggle(considerProgress&&_.indexOf(this.testContext.categories,"x-tao-option-reviewScreen")>=0),this.testReview.update(this.testContext))},updateProgress:function(){var considerProgress=this.testContext.considerProgress===!0;$controls.$progressBox.css("visibility",considerProgress?"visible":"hidden"),considerProgress&&this.progressUpdater.update(this.testContext)},updateContext:function(){$controls.$title.text(this.testContext.testTitle);var sectionText=this.testContext.isDeepestSectionVisible===!0?" - "+this.testContext.sectionTitle:"";$controls.$position.text(sectionText),$controls.$titleGroup.show()},updateExitButton:function(){
$controls.$logout.toggleClass("hidden",!this.testContext.logoutButton),$controls.$exit.toggleClass("hidden",!this.testContext.exitButton)},adjustFrame:function(){var frameContentHeight,rubricHeight=$controls.$rubricBlocks.outerHeight(!0)||0,finalHeight=$(window).innerHeight()-$controls.$topActionBar.outerHeight()-$controls.$bottomActionBar.outerHeight();$controls.$contentBox.height(finalHeight),$controls.$sideBars.length&&$controls.$sideBars.each(function(){var $sideBar=$(this);$sideBar.height(finalHeight-$sideBar.outerHeight()+$sideBar.height())}),$controls.$itemFrame.length&&$controls.$itemFrame[0]&&$controls.$itemFrame[0].contentWindow&&(frameContentHeight=$controls.$itemFrame.contents().outerHeight(!0),
finalHeight>frameContentHeight&&(frameContentHeight=rubricHeight?Math.max(frameContentHeight,finalHeight-rubricHeight):finalHeight),$controls.$itemFrame[0].contentWindow.$("body").trigger("setheight",[frameContentHeight]),$controls.$itemFrame.height(frameContentHeight))},disableGui:function(){$controls.$naviButtons.addClass("disabled"),this.testReview&&this.testReview.disable()},enableGui:function(){$controls.$naviButtons.removeClass("disabled"),this.testReview&&this.testReview.enable()},formatTime:function(totalSeconds){var sec_num=totalSeconds,hours=Math.floor(sec_num/3600),minutes=Math.floor((sec_num-3600*hours)/60),seconds=sec_num-3600*hours-60*minutes;10>hours&&(hours="0"+hours),
10>minutes&&(minutes="0"+minutes),10>seconds&&(seconds="0"+seconds);var time=hours+":"+minutes+":"+seconds;return time},actionCall:function(action,extraParams){var self=this,params={metaData:testMetaData.getData()};extraParams&&(params=_.assign(params,extraParams)),this.beforeTransition(function(){$.ajax({url:self.testContext[action+"Url"],cache:!1,data:params,async:!0,dataType:"json",success:function(testContext){testContext.state===self.TEST_STATE_CLOSED?(self.serviceApi.finish(),testMetaData.clearData()):self.update(testContext)}})})},exit:function(){var self=this,$confirmBox=$(".exit-modal-feedback"),testProgression=TestRunner.testReview?TestRunner.testReview.getProgression(self.testContext):{
total:self.testContext.numberItems,answered:self.testContext.numberCompleted,flagged:self.testContext.numberFlagged||0},message=__("You have %s unanswered question(s) and have %s item(s) marked for review. Are you sure you want to end the test?",(testProgression.total-testProgression.answered).toString(),testProgression.flagged.toString());testMetaData.addData({TEST:{TEST_EXIT_CODE:testMetaData.TEST_EXIT_CODE.INCOMPLETE},SECTION:{SECTION_EXIT_CODE:testMetaData.SECTION_EXIT_CODE.QUIT}}),$confirmBox.find(".message").html(message),$confirmBox.modal({width:500}),$confirmBox.find(".js-exit-cancel, .modal-close").off("click").on("click",function(){$confirmBox.modal("close");
}),$confirmBox.find(".js-exit-confirm").off("click").on("click",function(){$confirmBox.modal("close"),self.killItemSession(function(){self.actionCall("endTestSession"),testMetaData.destroy()})})},setCurrentItemState:function(id,state){id&&(this.currentItemState[id]=state)},resetCurrentItemState:function(){this.currentItemState={}},getCurrentItemState:function(){return this.currentItemState}},config=module.config();return config&&actionBarTools.register(config.qtiTools),{start:function(testContext){$controls={$moveForward:$('[data-control="move-forward"]'),$moveEnd:$('[data-control="move-end"]'),$moveBackward:$('[data-control="move-backward"]'),$skip:$('[data-control="skip"]'),
$skipEnd:$('[data-control="skip-end"]'),$exit:$(window.parent.document).find('[data-control="exit"]'),$logout:$(window.parent.document).find('[data-control="logout"]'),$naviButtons:$(".bottom-action-bar .action"),$skipButtons:$(".navi-box .skip"),$forwardButtons:$(".navi-box .forward"),$commentToggle:$('[data-control="comment-toggle"]'),$commentArea:$('[data-control="qti-comment"]'),$commentText:$('[data-control="qti-comment-text"]'),$commentCancel:$('[data-control="qti-comment-cancel"]'),$commentSend:$('[data-control="qti-comment-send"]'),$progressBar:$('[data-control="progress-bar"]'),$progressLabel:$('[data-control="progress-label"]'),$progressBox:$(".progress-box"),
