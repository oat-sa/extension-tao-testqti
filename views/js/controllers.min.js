...
beforeSave:function(model){qtiTestHelper.addMissingQtiType(model),qtiTestHelper.consolidateModel(model);try{qtiTestHelper.validateModel(model)}catch(err){return $saver.attr("disabled",!1).removeClass("disabled"),feedback().error(__("The test has not been saved.")+" "+err),!1}return!0}}),binder=DataBindController.takeControl($container,binderOptions).get(function(model){modelOverseer=modelOverseerFactory(model,{uri:options.uri,identifiers:self.identifiers,labels:options.labels,routes:options.routes}),scoringHelper.init(modelOverseer),self.identifiers=qtiTestHelper.extractIdentifiers(model),validators.register("idFormat",qtiTestHelper.idFormatValidator()),validators.register("testIdFormat",qtiTestHelper.testidFormatValidator()),
