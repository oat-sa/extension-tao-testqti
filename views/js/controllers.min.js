...
return processingRule.minOperands=2,processingRule.maxOperands=2,processingRule.acceptedCardinalities=[cardinalityHelper.SINGLE],processingRule.acceptedBaseTypes=[baseTypeHelper.INTEGER,baseTypeHelper.FLOAT],processingRule}function forceArray(value){return value||(value=[]),_.isArray(value)||(value=[value]),value}var processingRuleHelper={create:function(type,identifier,expression){var processingRule={"qti-type":type};if(!type||!_.isString(type))throw new TypeError("You must provide a valid processing type!");if(identifier){if(!validateIdentifier(identifier))throw new TypeError("You must provide a valid identifier!");processingRule.identifier=identifier}return expression&&processingRuleHelper.setExpression(processingRule,expression),
processingRule},setExpression:function(processingRule,expression){processingRule&&(_.isArray(expression)?(processingRule.expression&&(processingRule.expression=null),processingRule.expressions=expression):(processingRule.expressions&&(processingRule.expressions=null),expression&&(processingRule.expression=expression)))},addExpression:function(processingRule,expression){processingRule&&expression&&(processingRule.expression&&(processingRule.expressions=forceArray(processingRule.expression),processingRule.expression=null),_.isArray(expression)?processingRule.expressions=forceArray(processingRule.expressions).concat(expression):processingRule.expressions?processingRule.expressions.push(expression):processingRule.expression=expression);
