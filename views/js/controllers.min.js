...
return _.isNumber(value)||_.isString(value)&&!_.isEmpty(value)?acc+" "+key+'="'+value+'" ':acc},"")},Dom2QtiEncoder={encode:function(modelValue){var self=this;if(_.isArray(modelValue))return _.reduce(modelValue,function(result,value){return result+self.encode(value)},"");if(_.isObject(modelValue)&&modelValue["qti-type"]){if("textRun"===modelValue["qti-type"])return modelValue.content;var startTag="<"+modelValue["qti-type"]+attrToStr(getAttributes(modelValue));return modelValue.content?startTag+">"+self.encode(modelValue.content)+"</"+modelValue["qti-type"]+">":startTag+"/>"}return modelValue},decode:function(nodeValue){var self=this,$nodeValue=nodeValue instanceof $?nodeValue:$(nodeValue),result=[];
return _.forEach($nodeValue,function(elt){var object;3===elt.nodeType?_.isEmpty($.trim(elt.nodeValue))||result.push({"qti-type":"textRun",content:elt.nodeValue,xmlBase:""}):1===elt.nodeType&&(object=_.merge({"qti-type":elt.nodeName.toLowerCase(),id:"",class:"",xmlBase:"",lang:"",label:""},_.transform(elt.attributes,function(acc,value){value.nodeName&&(acc[value.nodeName]=value.nodeValue)})),elt.childNodes.length>0&&(object.content=self.decode(elt.childNodes)),result.push(object))}),result}};return Dom2QtiEncoder}),define("taoQtiTest/controller/creator/helpers/baseType",["lodash"],function(_){"use strict";var baseTypeEnum={IDENTIFIER:0,BOOLEAN:1,INTEGER:2,FLOAT:3,
STRING:4,POINT:5,PAIR:6,DIRECTED_PAIR:7,DURATION:8,FILE:9,URI:10,INT_OR_IDENTIFIER:11,COORDS:12},baseTypeHelper=_({asArray:function(){return baseTypeEnum},getValid:function(type,defaultType){return _.isFinite(type)?baseTypeHelper.getNameByConstant(type)||(type=!1):type=baseTypeHelper.getConstantByName(type),!1===type&&(type=_.isUndefined(defaultType)||defaultType===-1?-1:baseTypeHelper.getValid(defaultType,-1)),type},getConstantByName:function(name){switch(String(name).trim().toLowerCase()){case"identifier":return baseTypeEnum.IDENTIFIER;case"boolean":return baseTypeEnum.BOOLEAN;case"integer":return baseTypeEnum.INTEGER;case"float":return baseTypeEnum.FLOAT;case"string":
return baseTypeEnum.STRING;case"point":return baseTypeEnum.POINT;case"pair":return baseTypeEnum.PAIR;case"directedpair":return baseTypeEnum.DIRECTED_PAIR;case"duration":return baseTypeEnum.DURATION;case"file":return baseTypeEnum.FILE;case"uri":return baseTypeEnum.URI;case"intoridentifier":return baseTypeEnum.INT_OR_IDENTIFIER;case"coords":return baseTypeEnum.COORDS;default:return!1}},getNameByConstant:function(constant){switch(constant){case baseTypeEnum.IDENTIFIER:return"identifier";case baseTypeEnum.BOOLEAN:return"boolean";case baseTypeEnum.INTEGER:return"integer";case baseTypeEnum.FLOAT:return"float";case baseTypeEnum.STRING:return"string";case baseTypeEnum.POINT:
return"point";case baseTypeEnum.PAIR:return"pair";case baseTypeEnum.DIRECTED_PAIR:return"directedPair";case baseTypeEnum.DURATION:return"duration";case baseTypeEnum.FILE:return"file";case baseTypeEnum.URI:return"uri";case baseTypeEnum.INT_OR_IDENTIFIER:return"intOrIdentifier";case baseTypeEnum.COORDS:return"coords";default:return!1}}}).assign(baseTypeEnum).value();return baseTypeHelper}),define("taoQtiTest/controller/creator/helpers/cardinality",["lodash"],function(_){"use strict";var cardinalityEnum={SINGLE:0,MULTIPLE:1,ORDERED:2,RECORD:3},cardinalityHelper=_({asArray:function(){return cardinalityEnum},getValid:function(cardinality,defaultCardinality){return _.isFinite(cardinality)?cardinalityHelper.getNameByConstant(cardinality)||(cardinality=!1):cardinality=cardinalityHelper.getConstantByName(cardinality),
!1===cardinality&&(cardinality=_.isUndefined(defaultCardinality)||defaultCardinality===cardinalityEnum.SINGLE?cardinalityEnum.SINGLE:cardinalityHelper.getValid(defaultCardinality,cardinalityEnum.SINGLE)),cardinality},getConstantByName:function(name){switch(String(name).trim().toLowerCase()){case"single":return cardinalityEnum.SINGLE;case"multiple":return cardinalityEnum.MULTIPLE;case"ordered":return cardinalityEnum.ORDERED;case"record":return cardinalityEnum.RECORD;default:return!1}},getNameByConstant:function(constant){switch(constant){case cardinalityEnum.SINGLE:return"single";case cardinalityEnum.MULTIPLE:return"multiple";case cardinalityEnum.ORDERED:return"ordered";
case cardinalityEnum.RECORD:return"record";default:return!1}}}).assign(cardinalityEnum).value();return cardinalityHelper}),define("taoQtiTest/controller/creator/helpers/outcome",["lodash","taoQtiTest/controller/creator/helpers/baseType","taoQtiTest/controller/creator/helpers/cardinality"],function(_,baseTypeHelper,cardinalityHelper){"use strict";function validateIdentifier(identifier){return identifier&&_.isString(identifier)&&identifierValidator.test(identifier)}var outcomeHelper={findQtiType:function(expression,type){function checkType(qti){if(qti["qti-type"]===type)return found=qti,!1}var found=null;return _.isArray(expression)?_.forEach(expression,checkType):expression&&checkType(expression),
found},getProcessingRuleExpression:function(outcomeRule,path){for(var steps=_.isArray(path)?path:path.split("."),len=steps.length,expression=outcomeRule,i=0;expression&&i<len;)expression=outcomeHelper.findQtiType(expression,steps[i++]),expression&&i<len&&(expression=expression.expression||expression.expressions);return expression||null},getProcessingRuleProperty:function(outcomeRule,path){var result=null,steps=_.isArray(path)?path:path.split("."),property=steps.pop(),expression=outcomeHelper.getProcessingRuleExpression(outcomeRule,steps);return expression&&expression[property]&&(result=expression[property]),result},getOutcomeIdentifier:function(outcome){return String(_.isObject(outcome)?outcome.identifier:outcome);
},getOutcomeDeclarations:function(testModel){return testModel&&testModel.outcomeDeclarations},getOutcomeProcessingRules:function(testModel){return testModel&&testModel.outcomeProcessing&&testModel.outcomeProcessing.outcomeRules},eachOutcomeDeclarations:function(testModel,cb){_.forEach(outcomeHelper.getOutcomeDeclarations(testModel),cb)},eachOutcomeProcessingRules:function(testModel,cb){_.forEach(outcomeHelper.getOutcomeProcessingRules(testModel),cb)},eachOutcomeProcessingRuleExpressions:function(testModel,cb){function browseExpressions(processingRule){_.isArray(processingRule)?_.forEach(processingRule,browseExpressions):processingRule&&(cb(processingRule),processingRule.expression?browseExpressions(processingRule.expression):processingRule.expressions&&browseExpressions(processingRule.expressions));
}browseExpressions(outcomeHelper.getOutcomeProcessingRules(testModel))},listOutcomes:function(testModel,cb){var outcomes=[];return _.isFunction(cb)||(cb=null),outcomeHelper.eachOutcomeDeclarations(testModel,function(outcome){cb&&!cb(outcome)||outcomes.push(outcomeHelper.getOutcomeIdentifier(outcome))}),outcomes},removeOutcomes:function(testModel,outcomes){var declarations=outcomeHelper.getOutcomeDeclarations(testModel),rules=outcomeHelper.getOutcomeProcessingRules(testModel);outcomes=_.indexBy(_.isArray(outcomes)?outcomes:[outcomes],function(outcome){return outcome}),declarations&&_.remove(declarations,function(outcomeDeclaration){return!!outcomes[outcomeHelper.getOutcomeIdentifier(outcomeDeclaration)];
}),rules&&_.remove(rules,function(outcomeRule){return!!outcomes[outcomeHelper.getOutcomeIdentifier(outcomeRule)]})},createOutcome:function(identifier,type,cardinality){if(!validateIdentifier(identifier))throw new TypeError("You must provide a valid identifier!");return{"qti-type":"outcomeDeclaration",views:[],interpretation:"",longInterpretation:"",normalMaximum:!1,normalMinimum:!1,masteryValue:!1,identifier:identifier,cardinality:cardinalityHelper.getValid(cardinality,cardinalityHelper.SINGLE),baseType:baseTypeHelper.getValid(type,baseTypeHelper.FLOAT)}},addOutcomeProcessing:function(testModel,processingRule){var outcomeProcessing=testModel.outcomeProcessing;
if(!processingRule||!processingRule["qti-type"]||!_.isString(processingRule["qti-type"]))throw new TypeError("You must provide a valid outcome processing rule!");return outcomeProcessing?outcomeProcessing.outcomeRules||(outcomeProcessing.outcomeRules=[]):(outcomeProcessing={"qti-type":"outcomeProcessing",outcomeRules:[]},testModel.outcomeProcessing=outcomeProcessing),outcomeProcessing.outcomeRules.push(processingRule),processingRule},addOutcome:function(testModel,outcome,processingRule){var declarations=testModel.outcomeDeclarations;if(!outcome||"outcomeDeclaration"!==outcome["qti-type"]||!validateIdentifier(outcome.identifier))throw new TypeError("You must provide a valid outcome!");
if(processingRule){if(!validateIdentifier(processingRule.identifier)||processingRule.identifier!==outcome.identifier)throw new TypeError("You must provide a valid outcome processing rule!");outcomeHelper.addOutcomeProcessing(testModel,processingRule)}return declarations||(declarations=[],testModel.outcomeDeclarations=declarations),declarations.push(outcome),outcome}},identifierValidator=/^[a-zA-Z_][a-zA-Z0-9_\.-]*$/;return outcomeHelper}),define("taoQtiTest/controller/creator/helpers/processingRule",["lodash","taoQtiTest/controller/creator/helpers/baseType","taoQtiTest/controller/creator/helpers/cardinality"],function(_,baseTypeHelper,cardinalityHelper){"use strict";
function validateIdentifier(identifier){return identifier&&_.isString(identifier)&&identifierValidator.test(identifier)}function binaryOperator(type,left,right){var processingRule=processingRuleHelper.create(type,null,[left,right]);return processingRule.minOperands=2,processingRule.maxOperands=2,processingRule.acceptedCardinalities=[cardinalityHelper.SINGLE],processingRule.acceptedBaseTypes=[baseTypeHelper.INTEGER,baseTypeHelper.FLOAT],processingRule}function forceArray(value){return value||(value=[]),_.isArray(value)||(value=[value]),value}var processingRuleHelper={create:function(type,identifier,expression){var processingRule={"qti-type":type};if(!type||!_.isString(type))throw new TypeError("You must provide a valid processing type!");
if(identifier){if(!validateIdentifier(identifier))throw new TypeError("You must provide a valid identifier!");processingRule.identifier=identifier}return expression&&processingRuleHelper.setExpression(processingRule,expression),processingRule},setExpression:function(processingRule,expression){processingRule&&(_.isArray(expression)?(processingRule.expression&&(processingRule.expression=null),processingRule.expressions=expression):(processingRule.expressions&&(processingRule.expressions=null),expression&&(processingRule.expression=expression)))},addExpression:function(processingRule,expression){processingRule&&expression&&(processingRule.expression&&(processingRule.expressions=forceArray(processingRule.expression),
processingRule.expression=null),_.isArray(expression)?processingRule.expressions=forceArray(processingRule.expressions).concat(expression):processingRule.expressions?processingRule.expressions.push(expression):processingRule.expression=expression)},setOutcomeValue:function(identifier,expression){if(!validateIdentifier(identifier))throw new TypeError("You must provide a valid identifier!");return processingRuleHelper.create("setOutcomeValue",identifier,expression)},gte:function(left,right){return binaryOperator("gte",left,right)},lte:function(left,right){return binaryOperator("lte",left,right)},divide:function(left,right){return binaryOperator("divide",left,right);
},sum:function(terms){var processingRule=processingRuleHelper.create("sum",null,forceArray(terms));return processingRule.minOperands=1,processingRule.maxOperands=-1,processingRule.acceptedCardinalities=[cardinalityHelper.SINGLE,cardinalityHelper.MULTIPLE,cardinalityHelper.ORDERED],processingRule.acceptedBaseTypes=[baseTypeHelper.INTEGER,baseTypeHelper.FLOAT],processingRule},testVariables:function(identifier,type,weight,includeCategories,excludeCategories){var processingRule=processingRuleHelper.create("testVariables");if(!validateIdentifier(identifier))throw new TypeError("You must provide a valid identifier!");if(processingRule.variableIdentifier=identifier,
weight){if(!validateIdentifier(weight))throw new TypeError("You must provide a valid weight identifier!");processingRule.weightIdentifier=weight}else processingRule.weightIdentifier="";return processingRule.baseType=baseTypeHelper.getValid(type),processingRule.sectionIdentifier="",processingRule.includeCategories=forceArray(includeCategories),processingRule.excludeCategories=forceArray(excludeCategories),processingRule},numberPresented:function(includeCategories,excludeCategories){var processingRule=processingRuleHelper.create("numberPresented");return processingRule.sectionIdentifier="",processingRule.includeCategories=forceArray(includeCategories),processingRule.excludeCategories=forceArray(excludeCategories),
processingRule},baseValue:function(value,type){var processingRule=processingRuleHelper.create("baseValue");return processingRule.baseType=baseTypeHelper.getValid(type,baseTypeHelper.FLOAT),processingRule.value=parseFloat(value)||0,processingRule}},identifierValidator=/^[a-zA-Z_][a-zA-Z0-9_\.-]*$/;return processingRuleHelper}),define("taoQtiTest/controller/creator/helpers/category",["lodash"],function(_){"use strict";function isCategoryOption(category){return category&&0===category.indexOf("x-tao-")}function eachCategories(testModel,cb){_.forEach(testModel.testParts,function(testPart){_.forEach(testPart.assessmentSections,function(assessmentSection){_.forEach(assessmentSection.sectionParts,function(itemRef){
_.forEach(itemRef.categories,function(category){cb(category,itemRef)})})})})}return{eachCategories:eachCategories,listCategories:function(testModel){var categories={};return eachCategories(testModel,function(category){isCategoryOption(category)||(categories[category]=!0)}),_.keys(categories)},listOptions:function(testModel){var options={};return eachCategories(testModel,function(category){isCategoryOption(category)&&(options[category]=!0)}),_.keys(options)}}}),define("taoQtiTest/controller/creator/helpers/scoring",["lodash","i18n","taoQtiTest/controller/creator/helpers/baseType","taoQtiTest/controller/creator/helpers/outcome","taoQtiTest/controller/creator/helpers/processingRule","taoQtiTest/controller/creator/helpers/category"],function(_,__,baseTypeHelper,outcomeHelper,processingRuleHelper,categoryHelper){
"use strict";function getOutcomeIdentifier(processingMode,category){return processingMode.prefix&&category?processingMode.prefix+category.toUpperCase():processingMode.outcome}function addTotalScoreOutcomes(model,identifier,type,category){var outcome=outcomeHelper.createOutcome(identifier,type),processingRule=processingRuleHelper.setOutcomeValue(identifier,processingRuleHelper.sum(processingRuleHelper.testVariables(model.scoring.scoreIdentifier,-1,model.scoring.weightIdentifier,category)));outcomeHelper.addOutcome(model,outcome,processingRule)}function addCutScoreOutcomes(model,identifier,type,category){var outcome=outcomeHelper.createOutcome(identifier,type),processingRule=processingRuleHelper.setOutcomeValue(identifier,processingRuleHelper.gte(processingRuleHelper.divide(processingRuleHelper.sum(processingRuleHelper.testVariables(model.scoring.scoreIdentifier,-1,model.scoring.weightIdentifier,category)),processingRuleHelper.numberPresented(category)),processingRuleHelper.baseValue(model.scoring.cutScore,baseTypeHelper.FLOAT)));
outcomeHelper.addOutcome(model,outcome,processingRule)}function getScoringMode(outcome){var identifier=outcomeHelper.getOutcomeIdentifier(outcome),mode=null;return _.forEach(processingModes,function(processingMode){if(processingMode.outcome===identifier||0===identifier.indexOf(processingMode.prefix))return mode=processingMode,!1}),mode}function isScoringOutcome(outcome){return!!getScoringMode(outcome)}function listScoringModes(outcomes){var modes={};return _.forEach(outcomes,function(outcomeDeclaration){var mode=getScoringMode(outcomeDeclaration);mode&&(modes[mode.key]=!0)}),_.keys(modes)}function hasCategoryOutcome(model){var categoryOutcomes=!1;return _.forEach(outcomeHelper.getOutcomeDeclarations(model),function(outcomeDeclaration){
var identifier=outcomeHelper.getOutcomeIdentifier(outcomeDeclaration);_.forEach(processingModes,function(processingMode){if(processingMode.prefix&&processingMode.outcome!==identifier&&0===identifier.indexOf(processingMode.prefix))return categoryOutcomes=!0,!1})}),categoryOutcomes}function getCutScore(model){var values=_(outcomeHelper.getOutcomeProcessingRules(model)).map(function(outcome){return outcomeHelper.getProcessingRuleProperty(outcome,"setOutcomeValue.gte.baseValue.value")}).compact().uniq().value();return _.isEmpty(values)&&(values=[defaultCutScore]),Math.max(0,_.max(values))}function getWeightIdentifier(model){var values=[];return outcomeHelper.eachOutcomeProcessingRuleExpressions(model,function(processingRule){
"testVariables"===processingRule["qti-type"]&&processingRule.weightIdentifier&&values.push(processingRule.weightIdentifier)}),values=_(values).compact().uniq().value(),values.length?values[0]:""}function getOutcomeProcessing(model){var outcomeDeclarations=outcomeHelper.getOutcomeDeclarations(model),outcomeRules=outcomeHelper.getOutcomeProcessingRules(model),declarations=listScoringModes(outcomeDeclarations),processing=listScoringModes(outcomeRules),diff=_.difference(declarations,processing),count=_.size(declarations),outcomeProcessing="custom";return count===_.size(processing)&&(count?1===count&&_.isEmpty(diff)&&(outcomeProcessing=processing[0]):outcomeProcessing="none"),
outcomeProcessing}function removeScoring(model){var scoringOutcomes=outcomeHelper.listOutcomes(model,isScoringOutcome);outcomeHelper.removeOutcomes(model,scoringOutcomes)}var defaultCutScore=.5,processingModes={none:{key:"none",label:__("None"),description:__("No outcome processing. Erase the existing rules, if any.")},custom:{key:"custom",label:__("Custom"),description:__("Custom outcome processing. No changes will be made to the existing rules.")},total:{key:"total",label:__("Total score"),outcome:"SCORE_TOTAL",prefix:"SCORE_",type:baseTypeHelper.FLOAT,description:__("The score will be processed for the entire test. A sum of all SCORE outcomes will be computed, the result will take place in the SCORE_TOTAL outcome.")+" "+__("If the category option is set, the score will also be processed per categories, and each results will take place in the SCORE_xxx outcome, where xxx is the name of the category.")
},cut:{key:"cut",label:__("Cut score"),outcome:"PASS_TOTAL",prefix:"PASS_",type:baseTypeHelper.BOOLEAN,description:__("The score will be processed for the entire test. A sum of all SCORE outcomes will be computed, the result will be compared to the cut score, then the PASS_TOTAL outcome will be set accordingly.")+" "+__("If the category option is set, the score will also be processed per categories, and each results will take place in the PASS_xxx outcome, where xxx is the name of the category.")}},processingModeWriters={none:function(model){removeScoring(model)},custom:function(){},total:function(model){var processingMode=processingModes.total;removeScoring(model),
addTotalScoreOutcomes(model,getOutcomeIdentifier(processingMode),processingMode.type),model.scoring.categoryScore&&_.forEach(categoryHelper.listCategories(model),function(category){addTotalScoreOutcomes(model,getOutcomeIdentifier(processingMode,category),processingMode.type,category)})},cut:function(model){var processingMode=processingModes.cut;removeScoring(model),addCutScoreOutcomes(model,getOutcomeIdentifier(processingMode),processingMode.type),model.scoring.categoryScore&&_.forEach(categoryHelper.listCategories(model),function(category){addCutScoreOutcomes(model,getOutcomeIdentifier(processingMode,category),processingMode.type,category)})}};return{read:function(model){
model.scoring={modes:processingModes,scoreIdentifier:"SCORE",weightIdentifier:getWeightIdentifier(model),cutScore:getCutScore(model),categoryScore:hasCategoryOutcome(model),outcomeProcessing:getOutcomeProcessing(model)}},write:function(model){var processingModeWriter,scoring=model.scoring;if(scoring){if(processingModeWriter=processingModeWriters[scoring.outcomeProcessing],!processingModeWriter)throw new Error("Unknown score processing mode: "+scoring.outcomeProcessing);processingModeWriter(model)}}}}),define("taoQtiTest/controller/creator/creator",["module","jquery","lodash","helpers","i18n","html5-history-api","ui/feedback","core/databindcontroller","taoQtiTest/controller/creator/views/item","taoQtiTest/controller/creator/views/test","taoQtiTest/controller/creator/views/testpart","taoQtiTest/controller/creator/views/section","taoQtiTest/controller/creator/views/itemref","taoQtiTest/controller/creator/encoders/dom2qti","taoQtiTest/controller/creator/templates/index","taoQtiTest/controller/creator/helpers/qtiTest","taoQtiTest/controller/creator/helpers/scoring","core/validator/validators","core/promise"],function(module,$,_,helpers,__,history,feedback,DataBindController,itemView,testView,testPartView,sectionView,itemrefView,Dom2QtiEncoder,templates,qtiTestHelper,scoringHelper,validators,Promise){
