...
;this.$element.addClass("overlay"),overlayConfig.full&&this.$element.addClass("overlay-full")},disable:function(){this.$element.removeClass("overlay overlay-full")},show:function(){this.enable()},hide:function(){this.disable()}})}),define("taoQtiTest/runner/plugins/content/responsiveness/collapser",["lodash","jquery","taoTests/runner/plugin"],function(_,$,pluginFactory){"use strict";var ns=".collapser",defaults={collapseTools:!0,collapseNavigation:!1,collapseInOrder:!1,hover:!1,collaspeOrder:[]},$window=$(window);return pluginFactory({name:"collapser",init:function(){function buildCollapsiblesList(){
shouldCollapseInOrder()?(allCollapsibles=config.collapseOrder.map(function(selector){var $elements=$(selector).not(".no-tool-label"),extraWidth=0;return!!$elements.length&&($elements.each(function(){extraWidth+=getExtraWidth($(this))}),{$elements:$elements,extraWidth:extraWidth})}),allCollapsibles=_.compact(allCollapsibles)):(allCollapsibles=[],config.collapseTools&&allCollapsibles.push({$elements:$toolbox,extraWidth:getExtraWidth($toolbox)}),config.collapseNavigation&&allCollapsibles.push({$elements:$navigation,extraWidth:getExtraWidth($navigation)}),totalExtraWidth=allCollapsibles.reduce(function(total,collapsible){return total+=collapsible.extraWidth},0))
}function getExtraWidth($element){var expandedWidth,collapsedWidth;return $element.removeClass(collapseCls),expandedWidth=$element.outerWidth(!0),$element.addClass(collapseCls),collapsedWidth=$element.outerWidth(!0),$element.removeClass(collapseCls),expandedWidth-collapsedWidth}function toggleCollapsibles(){availableWidth=getAvailableWidth(),availableWidth<previousAvailableWidth?shouldCollapseInOrder()?collapseInOrder():collapseAll(collapseNeeded()):shouldCollapseInOrder()?expandInOrder():expandAll(),previousAvailableWidth=availableWidth}function shouldCollapseInOrder(){
return config.collapseInOrder&&_.isArray(config.collapseOrder)&&config.collapseOrder.length}function collapseAll(yes){config.collapseTools&&$toolbox.toggleClass(collapseCls,yes),config.collapseNavigation&&$navigation.toggleClass(collapseCls,yes)}function collapseInOrder(){for(var toCollapse,collapsiblesCopy=_.clone(allCollapsibles);collapseNeeded()&&collapsiblesCopy.length;)toCollapse=collapsiblesCopy.shift(),toCollapse.$elements.addClass(collapseCls)}function collapseNeeded(){return getToolbarWidth()>getAvailableWidth()}function expandAll(){expandPossible(totalExtraWidth)&&allCollapsibles.forEach(function(collapsible){
collapsible.$elements.removeClass(collapseCls)})}function expandInOrder(){_.forEachRight(allCollapsibles,function(toExpand){if(toExpand.$elements.hasClass(collapseCls)){if(!expandPossible(toExpand.extraWidth))return!1;toExpand.$elements.removeClass(collapseCls)}})}function expandPossible(extraWidth){return getToolbarWidth()+extraWidth<=getAvailableWidth()}function getAvailableWidth(){return $actionsBar.width()}function getToolbarWidth(){return $toolbox.outerWidth(!0)+$navigation.outerWidth(!0)}
var allCollapsibles,availableWidth,previousAvailableWidth,totalExtraWidth,testRunner=this.getTestRunner(),testData=testRunner.getTestData()||{},testConfig=testData.config||{},pluginsConfig=testConfig.plugins||{},config=_.defaults(pluginsConfig.collapser||{},defaults),collapseCls=config.hover?"tool-label-collapsed-hover":"tool-label-collapsed",areaBroker=testRunner.getAreaBroker(),$actionsBar=areaBroker.getArea("actionsBar"),$toolbox=areaBroker.getToolboxArea(),$navigation=areaBroker.getNavigationArea();$window.on("resize"+ns,_.throttle(function(){testRunner.trigger("collapseTools")},100)),testRunner.after("renderitem loaditem",function(){
previousAvailableWidth=1/0,buildCollapsiblesList(),testRunner.trigger("collapseTools")}).on("collapseTools"+ns,function(){toggleCollapsibles()})},destroy:function(){$window.off(ns)}})}),define("tpl!taoQtiTest/runner/plugins/content/rubricBlock/rubricBlock",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){return this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{},'<div id="qti-rubrics"></div>\n'})}),
...
this.$element.removeProp("disabled").removeClass("disabled")},disable:function(){this.$element.prop("disabled",!0).addClass("disabled")},show:function(){hider.show(this.$element)},hide:function(){hider.hide(this.$element)}})}),define("tpl!taoQtiTest/runner/plugins/controls/connectivity/connectivity",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,options,buffer="",escapeExpression=this.escapeExpression,helperMissing=helpers.helperMissing;return buffer+='<div class="connectivity-box ',
(helper=helpers.state)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.state,stack1="function"==typeof helper?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+'">\n    <span data-control="connectivity-connected" class="qti-controls icon-connect" title="'+escapeExpression((helper=helpers.__||depth0&&depth0.__,options={hash:{},data:data},
...
define("taoQtiTest/runner/plugins/controls/connectivity/connectivity",["jquery","i18n","ui/waitingDialog/waitingDialog","taoTests/runner/plugin","tpl!taoQtiTest/runner/plugins/controls/connectivity/connectivity"],function($,__,waitingDialog,pluginFactory,connectivityTpl){"use strict";return pluginFactory({name:"connectivity",init:function(){},install:function(){var self=this,waiting=!1,testRunner=this.getTestRunner(),proxy=testRunner.getProxy();this.$element=$(connectivityTpl({state:proxy.isOnline()?"connected":"disconnected"})),proxy.on("disconnect",function(source){testRunner.getState("disconnected")||(testRunner.setState("disconnected",!0),
testRunner.trigger("disconnect",source),self.$element.removeClass("connected").addClass("disconnected"))}).on("reconnect",function(){testRunner.getState("disconnected")&&(testRunner.setState("disconnected",!1),testRunner.trigger("reconnect"),self.$element.removeClass("disconnected").addClass("connected"))}),testRunner.before("error",function(e,err){var dialog;return!proxy.isConnectivityError(err)&&(proxy.isOffline()?(waiting||(waiting=!0,testRunner.before("pause.waiting",function(){return new Promise(function(resolve){proxy.off("reconnect.pausing").after("reconnect.pausing",resolve)})}),dialog=waitingDialog({
message:__("You are encountering a prolonged connectivity loss. ")+err.message,waitContent:__("Please wait while we try to restore the connection."),proceedContent:__("The connection seems to be back, please proceed")}).on("proceed",function(){var testContext=testRunner.getTestContext();"nav"===err.type&&testRunner.loadItem(testContext.itemIdentifier),"finish"===err.type&&testRunner.finish()}).on("render",function(){proxy.off("reconnect.waiting").after("reconnect.waiting",function(){testRunner.off("pause.waiting"),waiting=!1,dialog.endWait()})})),!1):void 0)})},render:function(){this.getAreaBroker().getControlArea().append(this.$element)}})}),
define("taoQtiTest/runner/plugins/controls/duration/duration",["lodash","core/polling","core/timer","core/store","core/promise","taoTests/runner/plugin"],function(_,pollingFactory,timerFactory,store,Promise,pluginFactory){"use strict";return pluginFactory({name:"duration",install:function(){var self=this;this.getTestRunner().on("storechange",function(){self.shouldClearStorage=!0})},init:function(){var self=this,testRunner=this.getTestRunner();return store("duration-"+testRunner.getConfig().serviceCallId).then(function(durationStore){return self.shouldClearStorage?durationStore.clear().then(function(){return durationStore}):durationStore
}).then(function(durationStore){function getItemDuration(attemptId){return/^(.*)+#+\d+$/.test(attemptId)?durationStore.getItem(attemptId):Promise.reject(new Error('Is it really an attempt id, like "itemid#attempt"'))}self.stopwatch=timerFactory({autoStart:!1}),self.polling=pollingFactory({action:function(){var context=testRunner.getTestContext(),itemAttemptId=context.itemIdentifier+"#"+context.attempt;durationStore.getItem(itemAttemptId).then(function(duration){var elapsed=self.stopwatch.tick();duration=_.isNumber(duration)?duration:0,elapsed=_.isNumber(elapsed)&&elapsed>0?elapsed/1e3:0,durationStore.setItem(itemAttemptId,duration+elapsed)})},interval:1e3,
autoStart:!1}),testRunner.after("renderitem enableitem",function(){self.enable()}).before("move disableitem skip error",function(){self.getState("enabled")&&self.disable()}).before("move skip exit timeout",function(){var context=testRunner.getTestContext();return getItemDuration(context.itemIdentifier+"#"+context.attempt).then(function(duration){var params={itemDuration:0};_.isNumber(duration)&&duration>0&&(params.itemDuration=duration),testRunner.getProxy().addCallActionParams(params)})}).on("plugin-get.duration",function(e,attemptId,getDuration){_.isFunction(getDuration)&&getDuration(getItemDuration(attemptId))}).before("finish",function(){
return new Promise(function(resolve){durationStore.removeStore().then(resolve).catch(resolve)})})})},destroy:function(){this.polling.stop(),this.stopwatch.stop()},enable:function(){this.polling.start(),this.stopwatch.resume()},disable:function(){this.polling.stop(),this.stopwatch.pause()}})}),define("tpl!taoQtiTest/runner/plugins/controls/progressbar/progressbar",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){return this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{},
'<div class="progress-box">\n    <div data-control="progress-bar" class="qti-controls progressbar info"></div>\n    <div data-control="progress-label" class="qti-controls"></div>\n</div>\n'})}),define("taoQtiTest/runner/plugins/controls/progressbar/progressbar",["jquery","lodash","i18n","taoTests/runner/plugin","tpl!taoQtiTest/runner/plugins/controls/progressbar/progressbar","ui/progressbar"],function($,_,__,pluginFactory,progressTpl){"use strict";var progressUpdater=function(testContext,testMap,progressConfig){return{percentage:function(){var total=Math.max(1,testContext.numberItems),ratio=Math.floor(testContext.numberCompleted/total*100);return{
ratio:ratio,label:ratio+"%"}},position:function(){var getTestPart=function(){if(testMap&&testMap.parts)return testMap.parts[testContext.testPartId]},getTestSection=function(){var testPart=getTestPart();if(testPart&&testPart.sections)return testPart.sections[testContext.sectionId]},progressScopeCounter={test:{total:function(){return Math.max(1,testContext.numberItems)},position:function(){return testContext.itemPosition+1}},testPart:{total:function(){return getTestPart()?_.reduce(testMap.parts[testContext.testPartId].sections,function(acc,section){return acc+_.size(section.items)},0):0},position:function(){var testSection=getTestSection()
;return testSection?testSection.items[testContext.itemIdentifier].positionInPart+1:0}},testSection:{total:function(){var testSection=getTestSection();return testSection?_.size(testSection.items):0},position:function(){var testSection=getTestSection();return testSection&&testSection.items[testContext.itemIdentifier]?testSection.items[testContext.itemIdentifier].positionInSection+1:0}}},counter=progressScopeCounter[progressConfig.scope]||progressScopeCounter.test,total=counter.total(),currentPosition=counter.position();return{ratio:total>0?Math.floor(currentPosition/total*100):0,
label:progressConfig.showTotal?__("Item %d of %d",currentPosition,total):__("Item %d",currentPosition)}}}[progressConfig.indicator]()};return pluginFactory({name:"progressBar",init:function(){var $progressLabel,$progressControl,testRunner=this.getTestRunner(),testData=testRunner.getTestData(),config=testData.config.progressIndicator||{},progressConfig={indicator:config.type||"percentage",scope:config.scope||"test",showTotal:!!config.showTotal},update=function(){var progressData=progressUpdater(testRunner.getTestContext(),testRunner.getTestMap(),progressConfig);progressData&&$progressLabel&&$progressControl&&($progressLabel.text(progressData.label),
$progressControl.progressbar("value",progressData.ratio))};this.$element=$(progressTpl()),$progressLabel=$('[data-control="progress-label"]',this.$element),$progressControl=$('[data-control="progress-bar"]',this.$element),$progressControl.progressbar(),update(),testRunner.on("ready",update).on("loaditem",update)},render:function(){this.getAreaBroker().getControlArea().append(this.$element)}})}),define("taoQtiTest/runner/plugins/controls/testState/testState",["lodash","taoTests/runner/plugin"],function(_,pluginFactory){"use strict";return pluginFactory({name:"testState",install:function(){var testRunner=this.getTestRunner()
;testRunner.getProxy().use(function(req,res,next){var data=res&&res.data;if(data&&data.type&&"TestState"===data.type)return testRunner.setState("closedOrSuspended",!0),void(testRunner.getState("ready")?!_.isEmpty(data.messages)&&_.find(data.messages,{channel:"teststate"})||testRunner.trigger("leave",data):testRunner.trigger("destroy"));next()})},init:function(){var testRunner=this.getTestRunner(),isLeaving=!1;testRunner.getProxy().channel("teststate",function(data){isLeaving||!data||"close"!==data.type&&"pause"!==data.type||testRunner.getState("closedOrSuspended")||(isLeaving=!0,testRunner.setState("closedOrSuspended",!0),testRunner.trigger("leave",data))})}
})}),define("tpl!taoQtiTest/runner/plugins/controls/timer/timer",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){function program1(depth0,data){return" disabled"}this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,buffer="",escapeExpression=this.escapeExpression,self=this;return buffer+='<span class="qti-timer qti-timer__type-',(helper=helpers.type)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.type,stack1="function"==typeof helper?helper.call(depth0,{hash:{},data:data}):helper),
buffer+=escapeExpression(stack1)+" qti-controls ",stack1=helpers.unless.call(depth0,depth0&&depth0.running,{hash:{},inverse:self.noop,fn:self.program(1,program1,data),data:data}),(stack1||0===stack1)&&(buffer+=stack1),buffer+='" data-control="',(helper=helpers.id)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.id,stack1="function"==typeof helper?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+'" title="',(helper=helpers.label)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.label,stack1="function"==typeof helper?helper.call(depth0,{hash:{},data:data}):helper),
buffer+=escapeExpression(stack1)+'">\n    <span class="qti-timer_label truncate">',(helper=helpers.label)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.label,stack1="function"==typeof helper?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+'</span>\n    <span class="qti-timer_time"></span>\n</span>\n'})}),define("taoQtiTest/runner/plugins/controls/timer/timerComponent",["jquery","lodash","i18n","moment","core/encoder/time","tpl!taoQtiTest/runner/plugins/controls/timer/timer"],function($,_,__,moment,timeEncoder,timerTpl){"use strict";var hasAnimationFrame="requestAnimationFrame"in window,timerTypes={
test:"assessmentTest",testPart:"testPart",section:"assessmentSection",item:"assessmentItemRef"};return function(config){var data;if(!_.isPlainObject(config))throw new TypeError("A timer needs to be configured using a config object");if(_.isEmpty(config.id))throw new TypeError("A timer needs and identifier");if(!_.contains(timerTypes,config.type))throw new TypeError("Invalid type configured");return data=_.defaults(config,{label:"",remaining:0,warnings:{},running:!0}),{init:function(){return this.$element=$(timerTpl(data)),this.$display=$(".qti-timer_time",this.$element),this},render:function($container){return $container.append(this.$element),this},
destroy:function(){return this.$element.remove(),this},refresh:function(){var self=this,update=function(){self.$display.text(timeEncoder.encode(data.remaining/1e3))};return hasAnimationFrame?window.requestAnimationFrame(update):update(),this},warn:function(){var remaining,message={},warning=_.findLast(data.warnings,{showed:!1});if(!_.isEmpty(warning)&&_.isFinite(warning.point)&&data.remaining<=warning.point){switch(remaining=moment.duration(data.remaining/1e3,"seconds").humanize(),this.$display.removeClass("txt-info txt-warning txt-danger").addClass("txt-"+warning.type),data.type){case"assessmentItemRef":
message.text=__("Warning – You have %s remaining to complete this item.",remaining);break;case"assessmentSection":message.text=__("Warning – You have %s remaining to complete this section.",remaining);break;case"testPart":message.text=__("Warning – You have %s remaining to complete this test part.",remaining);break;case"assessmentTest":message.text=__("Warning – You have %s remaining to complete the test.",remaining)}var showed=_.find(data.warnings,{point:warning.point});showed&&showed.point&&data.warnings[showed.point/1e3]&&(data.warnings[showed.point/1e3].showed=!0),message.type=warning.type}return message},id:function(){return data.id},
val:function(value){return void 0!==value?(data.remaining=value,this):data.remaining},running:function(value){return void 0!==value?(data.running=!!value,this):!!data.running}}}}),define("taoQtiTest/runner/helpers/map",["lodash"],function(_){"use strict";function getEmptyStats(){return{questions:0,answered:0,flagged:0,viewed:0,total:0}}return{getJumps:function(map){return map&&map.jumps},getParts:function(map){return map&&map.parts},getSections:function(map){var parts=this.getParts(map),result={};return _.forEach(parts,function(part){var sections=part.sections;sections&&_.forEach(sections,function(section){result[section.id]=section})}),result},
...
return"boolean"===baseType?!0===v||"true"===v:v}),mappedCardinality&&("base"===mappedCardinality?0===value.length?response.base=null:(response.base={},response.base[baseType]=value[0]):(response[mappedCardinality]={},response[mappedCardinality][baseType]=value)),response},isQtiValueNull:function(value,baseType,cardinality){var mappedCardinality=responseCardinalities[cardinality];return _.isObject(value)&&null===value[mappedCardinality]&&(value=null),_.isObject(value)&&value[mappedCardinality]&&void 0!==value[mappedCardinality][baseType]&&(value=value[mappedCardinality][baseType]),
null===value||"string"===baseType&&_.isEmpty(value)||"single"!==cardinality&&_.isEmpty(value)},isQuestionAnswered:function(response,baseType,cardinality,defaultValue){return defaultValue=defaultValue||null,!currentItemHelper.isQtiValueNull(response,baseType,cardinality)&&!_.isEqual(response,currentItemHelper.toResponse(defaultValue,baseType,cardinality))},isAnswered:function(runner,partially){var itemRunner=runner.itemRunner,responses=itemRunner&&itemRunner.getResponses(),count=0,empty=0;return itemRunner&&_.forEach(currentItemHelper.getDeclarations(runner),function(declaration){
var attributes=declaration.attributes||{},response=responses[attributes.identifier],baseType=attributes.baseType,cardinality=attributes.cardinality;count++,currentItemHelper.isQuestionAnswered(response,baseType,cardinality,declaration.defaultValue)||empty++}),!1===partially?count>0&&0===empty:count>0&&empty<count}};return currentItemHelper}),define("taoQtiTest/runner/helpers/stats",["lodash","taoQtiTest/runner/helpers/map","taoQtiTest/runner/helpers/currentItem"],function(_,mapHelper,currentItemHelper){"use strict";function getInstantStats(scope,runner){
var map=runner.getTestMap(),context=runner.getTestContext(),stats=_.clone(mapHelper.getScopeStats(map,context.itemPosition,scope)),isItemCurrentlyAnswered=currentItemHelper.isAnswered(runner);return!isItemCurrentlyAnswered&&context.itemAnswered?stats.answered--:isItemCurrentlyAnswered&&!context.itemAnswered&&stats.answered++,stats}return{getInstantStats:getInstantStats}}),define("taoQtiTest/runner/helpers/messages",["lodash","i18n","taoQtiTest/runner/helpers/map","taoQtiTest/runner/helpers/stats","taoQtiTest/runner/helpers/currentItem"],function(_,__,mapHelper,statsHelper,currentItemHelper){"use strict";function getExitMessage(message,scope,runner){
var itemsCountMessage="",testData=runner.getTestData(),testConfig=testData&&testData.config;return(!testConfig||testConfig.enableUnansweredItemsWarning)&&(itemsCountMessage=getUnansweredItemsWarning(scope,runner)),(itemsCountMessage+" "+message).trim()}function getUnansweredItemsWarning(scope,runner){var belowSections,stats=statsHelper.getInstantStats(scope,runner),unansweredCount=stats&&stats.questions-stats.answered,flaggedCount=stats&&stats.flagged,itemsCountMessage="",map=runner.getTestMap(),jump=mapHelper.getJump(map,runner.getTestContext().itemPosition)
;return"section"===scope||"testSection"===scope?(0===unansweredCount?itemsCountMessage+=__("You answered all %s question(s) in this section",stats.questions.toString()):itemsCountMessage=__("You answered only %s of the %s question(s) in this section",stats.answered.toString(),stats.questions.toString()),flaggedCount&&(itemsCountMessage+=", ",itemsCountMessage+=__("and flagged %s of them",flaggedCount.toString()))):"test"===scope?(belowSections=getNextSections(map,jump.section),_.forEach(belowSections,function(section){_.forEach(mapHelper.getSectionStats(runner.getTestMap(),section.id),function(sectionStats,statsKey){stats[statsKey]+=sectionStats})}),
unansweredCount=stats&&stats.questions-stats.answered,flaggedCount=stats&&stats.flagged,itemsCountMessage=0===unansweredCount?__("You answered all %s question(s) in this test",stats.questions.toString()):__("You have %s unanswered question(s)",unansweredCount.toString()),flaggedCount&&(itemsCountMessage+=" "+__("and you flagged %s item(s) that you can review now",flaggedCount.toString()))):"part"===scope&&(itemsCountMessage=0===unansweredCount?__("You answered all %s question(s)",stats.questions.toString()):__("You have %s unanswered question(s)",unansweredCount.toString()),
flaggedCount&&(itemsCountMessage+=" "+__("and you flagged %s item(s) that you can review now",flaggedCount.toString()))),itemsCountMessage&&(itemsCountMessage+="."),itemsCountMessage}function getNextSections(map,currentSectionId){var sections=mapHelper.getSections(map),result={},below=!1;return _.forEach(sections,function(section){below&&(result[section.id]=section),section.id===currentSectionId&&(below=!0)}),result}return{getExitMessage:getExitMessage}}),define("taoQtiTest/runner/helpers/navigation",["lodash","taoQtiTest/runner/helpers/map"],function(_,mapHelper){"use strict";var navigationHelper={
isLeavingSection:function(testContext,testMap,direction,scope,position){var section,sectionStats,nbItems,item;if(_.isPlainObject(testContext)&&_.isPlainObject(testMap)&&!_.isEmpty(testContext.sectionId)&&!_.isEmpty(testContext.itemIdentifier))return section=mapHelper.getSection(testMap,testContext.sectionId),sectionStats=mapHelper.getSectionStats(testMap,testContext.sectionId),nbItems=sectionStats&&sectionStats.total,item=mapHelper.getItem(testMap,testContext.itemIdentifier),
"section"===scope||"testPart"===scope||"next"===direction&&item.positionInSection+1===nbItems||"previous"===direction&&0===item.positionInSection||"jump"===direction&&position>0&&(position<section.position||position>=section.position+nbItems);throw new TypeError("Invalid test context and test map")},isLeavingTestPart:function(testContext,testMap,direction,scope,position){var testPart,testPartStats,nbItems,item,section,sectionStats
;if(_.isPlainObject(testContext)&&_.isPlainObject(testMap)&&!_.isEmpty(testContext.testPartId)&&!_.isEmpty(testContext.sectionId)&&!_.isEmpty(testContext.itemIdentifier))return testPart=mapHelper.getPart(testMap,testContext.testPartId),testPartStats=mapHelper.getPartStats(testMap,testContext.testPartId),nbItems=testPartStats&&testPartStats.total,item=mapHelper.getItem(testMap,testContext.itemIdentifier),"section"===scope&&(section=mapHelper.getSection(testMap,testContext.sectionId),sectionStats=mapHelper.getSectionStats(testMap,testContext.sectionId)),
"testPart"===scope||"next"===direction&&"item"===scope&&item.positionInPart+1===nbItems||"next"===direction&&"section"===scope&&section.position+sectionStats.total>=nbItems||"previous"===direction&&"item"===scope&&0===item.positionInPart||"previous"===direction&&"section"===scope&&section.position===testPart.position||"jump"===direction&&position>0&&(position<testPart.position||position>=testPart.position+nbItems);throw new TypeError("Invalid test context and test map")},isLast:function(testMap,itemIdentifier){return this.isLastOf(testMap,itemIdentifier,"test")},isFirst:function(testMap,itemIdentifier){return this.isFirstOf(testMap,itemIdentifier,"test")},
isLastOf:function(testMap,itemIdentifier,scope){var item,stats;if(!_.isPlainObject(testMap))throw new TypeError("Invalid test map");if(_.isEmpty(itemIdentifier))throw new TypeError("Invalid item identifier");if(scope=scope||"test",item=mapHelper.getItem(testMap,itemIdentifier),(stats=mapHelper.getScopeStats(testMap,item.position,scope))&&_.isNumber(stats.total)){if("test"===scope)return item.position+1===stats.total;if("section"===scope||"assessmentSection"===scope||"testSection"===scope)return item.positionInSection+1===stats.total;if("part"===scope||"testPart"===scope)return item.positionInPart+1===stats.total}return!1},
isFirstOf:function(testMap,itemIdentifier,scope){var item;if(!_.isPlainObject(testMap))throw new TypeError("Invalid test map");if(_.isEmpty(itemIdentifier))throw new TypeError("Invalid item identifier");return scope=scope||"test",item=mapHelper.getItem(testMap,itemIdentifier),"test"===scope?0===item.position:"section"===scope||"assessmentSection"===scope||"testSection"===scope?0===item.positionInSection:("part"===scope||"testPart"===scope)&&0===item.positionInPart},getSiblingItems:function(testMap,itemPosition,direction,size){
var directions,itemId=mapHelper.getItemIdentifier(testMap,itemPosition),previous=null,siblings=[],itemChain=_.reduce(testMap&&testMap.jumps,function(map,jump){var ref=jump.identifier;return previous&&(map[previous].next=ref),map[ref]={identifier:ref,previous:previous,next:null},previous=ref,map},{});return size=_.isFinite(size)?parseInt(size,10):3,directions=direction&&"both"!==direction?[direction]:["previous","next"],_.forEach(directions,function(link){var id=itemId;_.times(size,function(){if(!(id=itemChain[id]&&itemChain[id][link]))return!1;siblings.push(mapHelper.getItem(testMap,id))})}),siblings},getNextItem:function(testMap,itemPosition){
var siblings=navigationHelper.getSiblingItems(testMap,itemPosition,"next",1);return siblings.length?siblings[0]:null},getPreviousItem:function(testMap,itemPosition){var siblings=navigationHelper.getSiblingItems(testMap,itemPosition,"previous",1);return siblings.length?siblings[0]:null},isMovingToNextItem:function(action,params){return params=params||{},"timeout"===action||"skip"===action||"move"===action&&"next"===params.direction&&"item"===params.scope},isMovingToPreviousItem:function(action,params){return params=params||{},"move"===action&&"previous"===params.direction&&"item"===params.scope},isJumpingToItem:function(action,params){
return params=params||{},"move"===action&&"jump"===params.direction&&"item"===params.scope}};return navigationHelper}),define("tpl!taoQtiTest/runner/plugins/controls/timer/timers",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var helper,options,buffer="",helperMissing=helpers.helperMissing;return buffer+='<div class="timer-box">\n    <a href="#" class="timer-toggler hidden" title="'+(0,this.escapeExpression)((helper=helpers.__||depth0&&depth0.__,options={hash:{},data:data},
...
define("taoQtiTest/runner/plugins/controls/timer/timer",["jquery","lodash","i18n","core/promise","core/polling","core/timer","core/store","ui/hider","taoTests/runner/plugin","taoQtiTest/runner/plugins/controls/timer/timerComponent","taoQtiTest/runner/helpers/messages","taoQtiTest/runner/helpers/navigation","tpl!taoQtiTest/runner/plugins/controls/timer/timers"],function($,_,__,Promise,pollingFactory,timerFactory,store,hider,pluginFactory,timerComponentFactory,messages,navigationHelper,timerBoxTpl){"use strict";var exitMessage=__("Once you close this section, you cannot return to it or change your answers."),timerTypes={test:"assessmentTest",
testPart:"testPart",section:"assessmentSection",item:"assessmentItemRef"},isTimedSection=function(context){var timeConstraints=context.timeConstraints||[];return _.some(timeConstraints,function(constraint){return"assessmentSection"===constraint.qtiClassName})};return pluginFactory({name:"timer",install:function(){var self=this;this.getTestRunner().on("storechange",function(){self.shouldClearStorage=!0})},init:function(){function doEnable(){self.getState("enabled")||self.enable()}function doDisable(){self.getState("enabled")&&self.disable()}function toggleZenMode(){self.$element.hasClass("zen-mode")?(self.$element.removeClass("zen-mode"),
self.$toggler.attr("title",__("Hide timers")),self.storage.removeItem("zen-mode")):(self.$element.addClass("zen-mode"),self.$toggler.attr("title",__("Show timers")),self.storage.setItem("zen-mode",!0))}function toggleToggler(){self.$toggler.length&&(_.size(timers)>0?hider.show(self.$toggler):hider.hide(self.$toggler))}var self=this,testRunner=this.getTestRunner(),testData=testRunner.getTestData()||{},itemStates=testData.itemStates||{},timerWarning=testData.config&&testData.config.timerWarning||{},displayedTimers={},timers={},extraTime=0,consumedExtraTime=0,lastConsumedExtraTime=0,getRemainingExtraTime=function(extra){return extra&&(extraTime=extra),
1e3*Math.max(0,extraTime-consumedExtraTime)},setRemainingTime=function(timerConfig,remaining){return timerConfig.remaining=remaining+getRemainingExtraTime(timerConfig.extra),timerConfig.regular=remaining,timerConfig},getTimerConfig=function(type){var timeConstraint,timer,closestPreviousWarning,context=testRunner.getTestContext();return context.isTimeout||context.itemSessionState!==itemStates.interacting||(timeConstraint=_.findLast(context.timeConstraints,{qtiClassName:type}))&&(timer=setRemainingTime({label:timeConstraint.label,extra:timeConstraint.extraTime,type:timeConstraint.qtiClassName,id:timeConstraint.source,running:!0,warnings:{}
},1e3*timeConstraint.seconds),_(timerWarning[timeConstraint.qtiClassName]).forEach(function(value,key){_.contains(["info","warning","danger"],value)&&(timer.warnings[key]={type:value,showed:timer.remaining/1e3<=key,point:1e3*parseInt(key,10)})}),closestPreviousWarning=_.find(timer.warnings,{showed:!0}),!_.isEmpty(closestPreviousWarning)&&closestPreviousWarning.point&&timer.warnings[closestPreviousWarning.point/1e3]&&(timer.warnings[closestPreviousWarning.point/1e3].showed=!1)),timer},leaveTimedSection=function(direction,scope,position){var context=testRunner.getTestContext(),map=testRunner.getTestMap()
;return!(context.isTimeout||context.itemSessionState===itemStates.closed||!isTimedSection(context))&&navigationHelper.isLeavingSection(context,map,direction,scope,position)},removeTimer=function(type){displayedTimers[type]&&(displayedTimers[type].destroy(),displayedTimers=_.omit(displayedTimers,type),timers=_.omit(timers,type),self.trigger("removetimer",type))},addTimer=function(type,config){config&&(timers[type]=config.regular,displayedTimers[type]=timerComponentFactory(config),displayedTimers[type].init().render(self.$element.find(".timer-wrapper")),self.trigger("addtimer",type,displayedTimers[type]))},updateTimers=function(checkStorage){
var timerUpdatePromises=[],context=testRunner.getTestContext();return context.extraTime&&(extraTime=context.extraTime.total,consumedExtraTime=Math.max(consumedExtraTime,context.extraTime.consumed)),_.forEach(timerTypes,function(type){timerUpdatePromises.push(new Promise(function(resolve){var timerConfig=getTimerConfig(type),timerExists=void 0!==displayedTimers[type],addAndResolve=function(){return addTimer(type,timerConfig),resolve()};return timerConfig?timerExists&&displayedTimers[type].id()===timerConfig.id?(setRemainingTime(timerConfig,timers[type]),resolve()):(removeTimer(type),
checkStorage?void self.storage.getItem(timerConfig.id).then(function(savedTime){return _.isNumber(savedTime)&&savedTime>=0&&setRemainingTime(timerConfig,savedTime),addAndResolve()}).catch(function(){return addAndResolve()}):addAndResolve()):(removeTimer(type),resolve())}))}),Promise.all(timerUpdatePromises).then(function(data){return toggleToggler(),data})};return store("timer-"+testRunner.getConfig().serviceCallId).then(function(timeStore){return self.shouldClearStorage?timeStore.clear().then(function(){return timeStore}):Promise.resolve(timeStore)}).then(function(timeStore){self.storage=timeStore,self.$element=$(timerBoxTpl()),
self.$toggler=self.$element.find(".timer-toggler"),self.storage.getItem("zen-mode").then(function(zenMode){zenMode&&!self.$element.hasClass("zen-mode")&&toggleZenMode()}),self.$toggler.on("click",function(e){e.preventDefault(),toggleZenMode()}),self.stopwatch=timerFactory({autoStart:!1}),self.polling=pollingFactory({action:function(){var timeoutScope,timeoutRef,elapsed=self.stopwatch.tick(),elapsedExtraTime=0,timeout=!1;_.forEach(displayedTimers,function(timer,type){var regularVal,displayedVal,warning,runTimeout=function(){testRunner.timeout(timeoutScope,timeoutRef)};timer.running()&&(regularVal=timers[type]-elapsed,displayedVal=timer.val()-elapsed,
timers[type]=Math.max(regularVal,0),timer.val(Math.max(displayedVal,0)).refresh(),regularVal<=0&&(elapsedExtraTime=Math.max(elapsedExtraTime,Math.abs(regularVal))),displayedVal<=0?(timer.running(!1),timeout=!0,timeoutRef=timer.id(),timeoutScope=type,self.storage.setItem(timer.id(),0).then(runTimeout).catch(runTimeout)):(self.storage.setItem(timer.id(),timers[type]),warning=timer.warn(),_.isEmpty(warning)||testRunner.trigger(warning.type,warning.text)))}),elapsedExtraTime&&(consumedExtraTime+=elapsedExtraTime/1e3,lastConsumedExtraTime+=elapsedExtraTime),timeout&&self.disable()},interval:1e3,autoStart:!1}),testRunner.before("renderitem resumeitem",function(){
return updateTimers(!0)}).on("enableitem",doEnable).on("disableitem",doDisable).after("renderitem",doEnable).before("move skip",function(e,type,scope,position){var context=testRunner.getTestContext(),testDataBeforeMove=testRunner.getTestData(),config=testDataBeforeMove&&testDataBeforeMove.config,timerConfig=config&&config.timer||{},options=context&&context.options||{};type=type||"next";var movePromise=new Promise(function(resolve,reject){
context.isLast&&options.endTestWarning?resolve():!leaveTimedSection(type,scope,position)||options.noExitTimedSectionWarning||timerConfig.keepUpToTimeout?resolve():testRunner.trigger("confirm.exittimed",messages.getExitMessage(exitMessage,"section",testRunner),resolve,reject,{buttons:{labels:{ok:__("Close this Section"),cancel:__("Review my Answers")}}})});return movePromise.catch(function(){_.defer(function(){testRunner.trigger("enableitem enablenav")})}),movePromise}).on("move",function(){doDisable(),removeTimer(timerTypes.item)}).before("move skip exit timeout",function(){testRunner.getProxy().addCallActionParams({
consumedExtraTime:lastConsumedExtraTime/1e3}),lastConsumedExtraTime=0}).before("finish",function(){return new Promise(function(resolve){self.storage.removeStore().then(resolve).catch(resolve)})})})},render:function(){this.getAreaBroker().getControlArea().append(this.$element)},destroy:function(){this.polling.stop(),this.stopwatch.stop(),this.$element.remove()},enable:function(){this.polling.start(),this.stopwatch.resume()},disable:function(){this.polling.stop(),this.stopwatch.pause()},show:function(){hider.show(this.$element)},hide:function(){hider.hide(this.$element)}})}),
define("tpl!taoQtiTest/runner/plugins/controls/title/title",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){function program1(depth0,data){var stack1,helper,buffer="";return buffer+='\n        <span data-control="',(helper=helpers.control)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.control,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+'" class="qti-controls" title="',(helper=helpers.text)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.text,
stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+'">',stack1=helpers.unless.call(depth0,null==data||!1===data?data:data.first,{hash:{},inverse:self.noop,fn:self.program(2,program2,data),data:data}),(stack1||0===stack1)&&(buffer+=stack1),(helper=helpers.text)?stack1=helper.call(depth0,{hash:{},data:data}):(helper=depth0&&depth0.text,stack1=typeof helper===functionType?helper.call(depth0,{hash:{},data:data}):helper),buffer+=escapeExpression(stack1)+"</span>\n    "}function program2(depth0,data){return" - "}this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),
data=data||{};var stack1,buffer="",functionType="function",escapeExpression=this.escapeExpression,self=this;return buffer+='<div class="title-box truncate">\n    ',stack1=helpers.each.call(depth0,depth0&&depth0.titles,{hash:{},inverse:self.noop,fn:self.program(1,program1,data),data:data}),(stack1||0===stack1)&&(buffer+=stack1),buffer+="\n</div>\n"})}),define("taoQtiTest/runner/plugins/controls/title/title",["jquery","i18n","taoTests/runner/plugin","tpl!taoQtiTest/runner/plugins/controls/title/title"],function($,__,pluginFactory,titleTpl){"use strict";return pluginFactory({name:"title",init:function(){
var self=this,testRunner=this.getTestRunner(),testData=testRunner.getTestData(),createElement=function(){var context=testRunner.getTestContext(),titles=[{control:"qti-test-title",text:testData.title}];return context.isDeepestSectionVisible&&titles.push({control:"qti-test-position",text:context.sectionTitle}),$(titleTpl({titles:titles}))};this.$element=createElement(),testRunner.after("renderitem",function(){var $element=createElement();self.$element.replaceWith($element),self.$element=$element})},render:function(){this.getAreaBroker().getControlArea().append(this.$element)}})}),
define("taoQtiTest/runner/plugins/controls/trace/itemTraceVariables",["lodash","moment","core/store","taoTests/runner/plugin"],function(_,moment,storeFactory,pluginFactory){"use strict";function timestamp(){return Date.now()/precision}var precision=1e3;return pluginFactory({name:"itemTraceVariables",install:function(){var self=this;self.getTestRunner().on("storechange",function(){self.shouldClearStorage=!0})},init:function(){function onError(err){testRunner.trigger("error",err)}var self=this,testRunner=this.getTestRunner(),variables={};return storeFactory("trace-"+testRunner.getConfig().serviceCallId).then(function(tracesStore){
return self.shouldClearStorage?tracesStore.clear().then(function(){return tracesStore}):tracesStore}).then(function(tracesStore){testRunner.after("renderitem enableitem",function(){var context=testRunner.getTestContext();variables={ITEM_START_TIME_CLIENT:timestamp()},tracesStore.getItem(context.itemIdentifier).then(function(data){return data&&_.merge(variables,data),tracesStore.setItem(context.itemIdentifier,variables)}).catch(onError)}).before("move skip exit timeout",function(){var context=testRunner.getTestContext();return variables.ITEM_END_TIME_CLIENT=timestamp(),variables.ITEM_TIMEZONE=moment().utcOffset(moment().utcOffset()).format("Z"),
tracesStore.setItem(context.itemIdentifier,variables).catch(onError)}).before("unloaditem",function(){var context=testRunner.getTestContext();return testRunner.getProxy().callItemAction(context.itemIdentifier,"storeTraceData",{traceData:JSON.stringify(variables)})}).before("finish",function(){return new Promise(function(resolve){tracesStore.removeStore().then(resolve).catch(resolve)})})})}})}),define("taoQtiTest/runner/plugins/navigation/allowSkipping",["lodash","i18n","taoTests/runner/plugin","taoQtiTest/runner/helpers/currentItem"],function(_,__,pluginFactory,currentItemHelper){"use strict";return pluginFactory({name:"allowSkipping",init:function(){
this.getTestRunner().before("move",function(){var self=this,testContext=this.getTestContext();if(!this.getItemState(testContext.itemIdentifier,"disabled")&&testContext.enableAllowSkipping&&!testContext.allowSkipping)return new Promise(function(resolve,reject){return 0===_.size(currentItemHelper.getDeclarations(self))?resolve():currentItemHelper.isAnswered(self,!0)?resolve():void(self.getState("alerted.notallowed")||(self.setState("alerted.notallowed",!0),self.trigger("alert.notallowed",__("A response to this item is required."),function(){self.trigger("resumeitem"),reject(),self.setState("alerted.notallowed",!1)})))})})}})}),
...
icon:"forward",text:__("Next")},end:{control:"move-end",title:__("Submit and go to the end of the test"),icon:"fast-forward",text:__("End test")}},createElement=function(context){var dataType=context.isLast?"end":"next";return $(buttonTpl(buttonData[dataType]))},updateElement=function($element,context){var dataType=context.isLast?"end":"next";$element.data("control")!==buttonData[dataType].control&&($element.data("control",buttonData[dataType].control).attr("title",buttonData[dataType].title).find(".text").text(buttonData[dataType].text),
"next"===dataType?$element.find(".icon-"+buttonData.end.icon).removeClass("icon-"+buttonData.end.icon).addClass("icon-"+buttonData.next.icon):$element.find(".icon-"+buttonData.next.icon).removeClass("icon-"+buttonData.next.icon).addClass("icon-"+buttonData.end.icon))};return pluginFactory({name:"next",init:function(){function doNext(nextItemWarning){
...
unansweredWarning:testOptions.unansweredWarning,stats:statsHelper.getInstantStats(warningScope,testRunner),unansweredOnly:unansweredOnly})
;!1!==self.getState("enabled")&&(warningHelper.shouldWarnBeforeEnd()?testRunner.trigger("confirm.endTest",messages.getExitMessage(__("You are about to submit the test. You will not be able to access this test once submitted. Click OK to continue and submit the test."),warningScope,testRunner),_.partial(triggerNextAction,context)):warningHelper.shouldWarnBeforeNext()?testRunner.trigger("confirm.next",__("You are about to go to the next item. Click OK to continue and go to the next item."),_.partial(triggerNextAction,context)):triggerNextAction(context))}function triggerNextAction(context){context.isLast&&self.trigger("end"),testRunner.next()}
...
testRunner.on("loaditem",toggle).on("enablenav",function(){self.enable()}).on("disablenav",function(){self.disable()})},render:function(){this.getAreaBroker().getNavigationArea().append(this.$element)},destroy:function(){this.$element.remove()},enable:function(){this.$element.removeProp("disabled").removeClass("disabled")},disable:function(){this.$element.prop("disabled",!0).addClass("disabled")},show:function(){hider.show(this.$element)},hide:function(){hider.hide(this.$element)}})}),
define("taoQtiTest/runner/plugins/navigation/previous",["jquery","lodash","i18n","ui/hider","taoTests/runner/plugin","util/shortcut","util/namespace","taoQtiTest/runner/helpers/navigation","taoQtiTest/runner/helpers/map","tpl!taoQtiTest/runner/plugins/templates/button"],function($,_,__,hider,pluginFactory,shortcut,namespaceHelper,navigationHelper,mapHelper,buttonTpl){"use strict";return pluginFactory({name:"previous",init:function(){function doPrevious(previousItemWarning){var context=testRunner.getTestContext()
;!1!==self.getState("enabled")&&(previousItemWarning&&-1!==context.remainingAttempts?testRunner.trigger("confirm.previous",__("You are about to go to the previous item. Click OK to continue and go to the previous item."),testRunner.previous):testRunner.previous())}var self=this,testRunner=this.getTestRunner(),testData=testRunner.getTestData(),testConfig=testData.config||{},pluginShortcuts=(testConfig.shortcuts||{})[this.getName()]||{},canDoPrevious=function(){var previousSection,previousPart,testMap=testRunner.getTestMap(),context=testRunner.getTestContext();if(navigationHelper.isFirst(testMap,context.itemIdentifier))return!1
;if(navigationHelper.isFirstOf(testMap,context.itemIdentifier,"section")){if(context.isCatAdaptive)return!1;if(previousSection=mapHelper.getItemSection(testMap,context.itemPosition-1),previousSection.isCatAdaptive||previousSection.timeConstraint&&!context.options.noExitTimedSectionWarning)return!1}return(!navigationHelper.isFirstOf(testMap,context.itemIdentifier,"part")||(previousPart=mapHelper.getItemPart(testMap,context.itemPosition-1),!previousPart.isLinear))&&(!1===context.isLinear&&!0===context.canMoveBackward)},toggle=function(){canDoPrevious()?self.show():self.hide()};this.$element=$(buttonTpl({control:"move-backward",
title:__("Submit and go to the previous item"),icon:"backward",text:__("Previous")})),this.$element.on("click",function(e){e.preventDefault(),testRunner.trigger("nav-previous")}),testConfig.allowShortcuts&&pluginShortcuts.trigger&&shortcut.add(namespaceHelper.namespaceAll(pluginShortcuts.trigger,this.getName(),!0),function(e){canDoPrevious()&&!0===self.getState("enabled")&&testRunner.trigger("nav-previous",[!0])},{avoidInput:!0,prevent:!0}),toggle(),self.disable(),testRunner.on("loaditem",toggle).on("enablenav",function(){self.enable()}).on("disablenav",function(){self.disable()}).on("nav-previous",function(previousItemWarning){
doPrevious(previousItemWarning)})},render:function(){this.getAreaBroker().getNavigationArea().append(this.$element)},destroy:function(){shortcut.remove("."+this.getName()),this.$element.remove()},enable:function(){this.$element.removeProp("disabled").removeClass("disabled")},disable:function(){this.$element.prop("disabled",!0).addClass("disabled")},show:function(){hider.show(this.$element)},hide:function(){hider.hide(this.$element)}})}),define("tpl!taoQtiTest/runner/plugins/navigation/review/navigator",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){function program1(depth0,data){return" hidden"}
...
this.controls.$tree.empty()):(this.controls.$filterBar.show(),this.controls.$linearState.hide(),this.controls.$tree.html(navigatorTreeTpl(scopedMap)),this.autoScroll(),this.setState("prevents-unseen",this.config.preventsUnseen),this.config.preventsUnseen&&this.controls.$tree.find(_selectors.unseen).addClass(_cssCls.disabled)),this.filter(this.controls.$filters.filter(_selectors.actives).data("mode")),this.trigger("update"),this},getScopedMap:function(map,context){
var scopedMap=mapHelper.getScopeMapFromContext(map,context,this.config.scope),testPart=mapHelper.getPart(scopedMap,context.testPartId)||{},section=mapHelper.getSection(scopedMap,context.sectionId)||{},item=mapHelper.getItem(scopedMap,context.itemIdentifier)||{};return testPart.active=!0,section.active=!0,item.active=!0,mapHelper.each(scopedMap,function(itm){var cls=[],icon="";itm.active&&cls.push("active"),itm.informational&&(cls.push("info"),icon=icon||"info"),itm.flagged&&(cls.push("flagged"),icon=icon||"flagged"),itm.answered&&(cls.push("answered"),icon=icon||"answered"),itm.viewed?(cls.push("viewed"),icon=icon||"viewed"):(cls.push("unseen"),
icon=icon||"unseen"),itm.cls=cls.join(" "),itm.icon=icon})},writeCount:function($place,count,total){var display=0;$place.parent().hasClass("qti-navigator-tab")?display=Math.max(count,0):total>0&&(display=Math.min(count,total)+"/"+total),$place.text(display)},select:function(position,open){var $tree=this.controls.$tree,selected=position&&position.jquery?position:$tree.find("[data-position="+position+"]"),hierarchy=selected.parentsUntil($tree),previousPosition=0,$previous=$tree.find(_selectors.activeItem);return $previous.length&&(previousPosition=$previous.data("position")),open&&this.openOnly(hierarchy),
$tree.find(_selectors.actives).removeClass(_cssCls.active),hierarchy.add(selected).addClass(_cssCls.active),position=selected.data("position"),this.trigger("selected",position,previousPosition),selected},openSelected:function(){var $tree=this.controls.$tree,selected=$tree.find(_selectors.items+_selectors.actives),hierarchy=selected.parentsUntil($tree);return this.openOnly(hierarchy),selected},openOnly:function(opened,root){(root||this.controls.$tree).find(_selectors.collapsible).addClass(_cssCls.collapsed),opened.removeClass(_cssCls.collapsed)},togglePanel:function(panel,collapseSelector){var collapsed=panel.hasClass(_cssCls.collapsed)
;return collapseSelector&&this.controls.$tree.find(collapseSelector).addClass(_cssCls.collapsed),collapsed?panel.removeClass(_cssCls.collapsed):panel.addClass(_cssCls.collapsed),collapsed},toggle:function(show){return void 0===show&&(show=this.is("hidden")),show?this.show():this.hide(),this}};return navigatorFactory}),define("taoQtiTest/runner/plugins/navigation/review/review",["jquery","lodash","i18n","ui/hider","util/shortcut","util/namespace","taoTests/runner/plugin","taoQtiTest/runner/helpers/map","taoQtiTest/runner/plugins/navigation/review/navigator"],function($,_,__,hider,shortcut,namespaceHelper,pluginFactory,mapHelper,navigatorFactory){"use strict"
...
var context=testRunner.getTestContext(),map=testRunner.getTestMap(),item=mapHelper.getItemAt(map,context.itemPosition);return!(context.isLinear||!context.options.markReview||item&&item.informational)}var buttonData={setFlag:{control:"set-item-flag",title:__("Flag the current item for later review"),icon:"anchor",text:__("Flag for Review")},unsetFlag:{control:"unset-item-flag",title:__("Do not flag the current item for later review"),icon:"anchor",text:__("Unflag for Review")},showReview:{control:"show-review",title:__("Show the review screen"),icon:"right",text:__("Show Review")},hideReview:{control:"hide-review",title:__("Hide the review screen"),
icon:"left",text:__("Hide Review")}};return pluginFactory({name:"review",init:function(){function isPluginAllowed(){var context=testRunner.getTestContext();return navigatorConfig.enabled&&context&&context.options&&context.options.reviewScreen}function flagItem(position,flag){return self.disable(),testRunner.getProxy().callTestAction("flagItem",{position:position,flag:flag}).then(function(){var context=testRunner.getTestContext();context.itemPosition===position&&(context.itemFlagged=flag),updateButton(self.flagItemButton,getFlagItemButtonData(context)),self.navigator.setItemFlag(position,flag),self.enable()}).catch(function(){
self.navigator.setItemFlag(position,!flag),self.enable()})}function flagCurrentItem(){var context=testRunner.getTestContext();!1!==self.getState("enabled")&&flagItem(context.itemPosition,!context.itemFlagged)}function togglePanel(forcedState){(_.isUndefined(forcedState)?self.navigator.is("hidden"):forcedState)?(self.explicitlyHidden=!1,self.navigator.show()):(self.explicitlyHidden=!0,self.navigator.hide()),updateButton(self.toggleButton,getToggleButtonData(self.navigator))}
var previousItemPosition,self=this,testRunner=this.getTestRunner(),testData=testRunner.getTestData(),testContext=testRunner.getTestContext(),testMap=testRunner.getTestMap(),testConfig=testData.config||{},pluginShortcuts=(testConfig.shortcuts||{})[this.getName()]||{},navigatorConfig=testConfig.review||{};this.navigator=navigatorFactory(navigatorConfig,testMap,testContext).on("selected",function(position,previousPosition){previousItemPosition=previousPosition}).on("jump",function(position){!1!==self.getState("enabled")&&(self.disable(),testRunner.jump(position,"item"))}).on("flag",function(position,flag){!1!==self.getState("enabled")&&flagItem(position,flag)
}).render(),testRunner.on("alert.notallowed",function(){self.navigator.select(previousItemPosition)}),this.explicitlyHidden=!1,this.toggleButton=this.getAreaBroker().getToolbox().createEntry(getToggleButtonData(this.navigator)),this.toggleButton.on("click",function(e){e.preventDefault(),testRunner.trigger("tool-reviewpanel")}),this.flagItemButton=this.getAreaBroker().getToolbox().createEntry(getFlagItemButtonData(testContext)),this.flagItemButton.on("click",function(e){e.preventDefault(),testRunner.trigger("tool-flagitem")}),
testConfig.allowShortcuts&&(pluginShortcuts.flag&&shortcut.add(namespaceHelper.namespaceAll(pluginShortcuts.flag,this.getName(),!0),function(){testRunner.trigger("tool-flagitem")},{avoidInput:!0}),pluginShortcuts.toggle&&shortcut.add(namespaceHelper.namespaceAll(pluginShortcuts.toggle,this.getName(),!0),function(){testRunner.trigger("tool-reviewpanel")},{avoidInput:!0})),isPluginAllowed()||this.hide(),this.disable(),togglePanel(testConfig.review.defaultOpen),testRunner.on("render",function(){isPluginAllowed()?(self.show(),updateButton(self.toggleButton,getToggleButtonData(self.navigator))):self.hide()}).on("loaditem",function(){
var context=testRunner.getTestContext(),map=testRunner.getTestMap();isPluginAllowed()?(updateButton(self.flagItemButton,getFlagItemButtonData(context)),self.navigator.update(map,context).updateConfig({canFlag:!context.isLinear&&context.options.markReview}),self.show(),updateButton(self.toggleButton,getToggleButtonData(self.navigator))):self.hide()}).on("enabletools",function(){isPluginAllowed()&&self.enable()}).on("disabletools",function(){isPluginAllowed()&&self.disable()}).on("tool-flagitem",function(){isPluginAllowed()&&canFlag(testRunner)&&flagCurrentItem()}).on("tool-reviewpanel",function(){isPluginAllowed()&&self.getState("enabled")&&togglePanel()})},
...
this.toggleButton.show(),this.explicitlyHidden?this.navigator.hide():this.navigator.show()},hide:function(){this.flagItemButton.hide(),this.toggleButton.hide(),this.navigator.hide()}})}),define("taoQtiTest/runner/plugins/navigation/skip",["jquery","lodash","i18n","ui/hider","taoTests/runner/plugin","taoQtiTest/runner/helpers/messages","tpl!taoQtiTest/runner/plugins/templates/button"],function($,_,__,hider,pluginFactory,messages,buttonTpl){"use strict";var buttonData={skip:{control:"skip",title:__("Skip  and go to the next item"),icon:"external",text:__("Skip")},end:{control:"skip-end",title:__("Skip and go to the end of the test"),icon:"external",
text:__("Skip and end test")}},createElement=function(context){var dataType=context.isLast?"end":"skip";return $(buttonTpl(buttonData[dataType]))},updateElement=function($element,context){var dataType=context.isLast?"end":"skip";$element.data("control")!==buttonData[dataType].control&&$element.data("control",buttonData[dataType].control).attr("title",buttonData[dataType].title).find(".text").text(buttonData[dataType].text)};return pluginFactory({name:"skip",init:function(){function doSkip(){testRunner.skip()}var self=this,testRunner=this.getTestRunner(),toggle=function(){return!0===testRunner.getTestContext().options.allowSkipping?(self.show(),
!0):(self.hide(),!1)};this.$element=createElement(testRunner.getTestContext()),this.$element.on("click",function(e){var enable=_.bind(self.enable,self),context=testRunner.getTestContext();e.preventDefault(),!1!==self.getState("enabled")&&(self.disable(),context.options.endTestWarning&&context.isLast?testRunner.trigger("confirm.endTest",messages.getExitMessage(__("You are about to submit the test. You will not be able to access this test once submitted. Click OK to continue and submit the test."),"test",testRunner),doSkip,enable):doSkip())}),toggle(),self.disable(),testRunner.on("loaditem",function(){
toggle()&&updateElement(self.$element,testRunner.getTestContext())}).on("enablenav",function(){self.enable()}).on("disablenav",function(){self.disable()})},render:function(){this.getAreaBroker().getNavigationArea().append(this.$element)},destroy:function(){this.$element.remove()},enable:function(){this.$element.removeProp("disabled").removeClass("disabled")},disable:function(){this.$element.prop("disabled",!0).addClass("disabled")},show:function(){hider.show(this.$element)},hide:function(){hider.hide(this.$element)}})}),
...
this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var helper,options,buffer="",helperMissing=helpers.helperMissing,escapeExpression=this.escapeExpression;return buffer+='<div class="mask">\n   <div class="inner"></div>\n   <div class="controls">\n        <a href="#" class="view"  title="'+escapeExpression((helper=helpers.__||depth0&&depth0.__,options={hash:{},data:data},
helper?helper.call(depth0,"Preview the covered area",options):helperMissing.call(depth0,"__","Preview the covered area",options)))+'"><span class="icon-preview"></span></a>\n        <a href="#" class="close" title="'+escapeExpression((helper=helpers.__||depth0&&depth0.__,options={hash:{},data:data},helper?helper.call(depth0,"Close the mask",options):helperMissing.call(depth0,"__","Close the mask",options)))+'"><span class="icon-result-nok"></span></a>\n   </div>\n</div>\n'})}),
define("taoQtiTest/runner/plugins/tools/areaMasking/mask",["lodash","ui/movableComponent","tpl!taoQtiTest/runner/plugins/tools/areaMasking/mask"],function(_,movableComponent,areaMaskingTpl){"use strict";function maskingComponentFactory(){var maskComponent=movableComponent({preview:function(){var self=this,delay=this.config.previewDelay||1e3;return!this.is("rendered")||this.is("disabled")||this.is("previewing")||(this.setState("previewing",!0).trigger("preview"),_.delay(function(){self.setState("previewing",!1)},delay)),this}},defaultConfig);return maskComponent.setTemplate(areaMaskingTpl).on("render",function(){var self=this
;this.getElement().on("click touchstart",".view",function(e){e.preventDefault(),self.preview()}).on("click touchstart",".close",function(e){e.preventDefault(),self.destroy()})}),maskComponent}var defaultConfig={previewDelay:3e3,stackingScope:"test-runner"};return maskingComponentFactory}),define("taoQtiTest/runner/plugins/tools/areaMasking/areaMasking",["jquery","lodash","i18n","ui/hider","util/shortcut","util/namespace","taoTests/runner/plugin","taoQtiTest/runner/plugins/tools/areaMasking/mask"],function($,_,__,hider,shortcut,namespaceHelper,pluginFactory,maskComponent){"use strict";var defaultConfig={max:5,foo:!0};return pluginFactory({name:"area-masking",
init:function(){function addMask(){maskComponent().on("render",function(){self.masks.push(this),self.button.turnOn(),self.trigger("maskadd")}).on("destroy",function(){self.masks=_.without(self.masks,this),self.masks.length<config.max&&self.enable(),0===self.masks.length&&(self.button.turnOff(),self.trigger("close")),self.trigger("maskclose")}).init({x:10*self.masks.length,y:10*self.masks.length}).render($container)}function isEnabled(){return!!(testRunner.getTestContext().options||{}).areaMasking}function togglePlugin(){isEnabled()?self.show():self.hide()}
var self=this,testRunner=this.getTestRunner(),$container=testRunner.getAreaBroker().getContentArea().parent(),testConfig=testRunner.getTestData().config||{},config=_.defaults(_.clone((testConfig.plugins||{})["area-masking"])||{},defaultConfig),pluginShortcuts=(testConfig.shortcuts||{})["area-masking"]||{};this.masks=[],this.button=this.getAreaBroker().getToolbox().createEntry({control:"area-masking",text:__("Masking"),title:__("Covers parts of the item"),icon:"eye-slash"}),this.button.on("click",function(e){e.preventDefault(),testRunner.trigger("tool-area-masking-toggle")}),testConfig.allowShortcuts&&_.forEach(pluginShortcuts,function(command,key){
shortcut.add(namespaceHelper.namespaceAll(command,"area-masking",!0),function(){testRunner.trigger("tool-area-masking-"+key)},{avoidInput:!0})}),this.disable(),testRunner.on("loaditem",togglePlugin).on("enabletools renderitem",function(){self.enable()}).on("disabletools unloaditem",function(){self.disable(),_.invoke(self.masks,"destroy")}).on("tool-area-masking-toggle",function(){isEnabled()&&(0===self.masks.length&&self.trigger("open"),self.masks.length<config.max?addMask():1===config.max&&_.invoke(self.masks,"destroy"))})},destroy:function(){shortcut.remove(".area-masking")},enable:function(){this.button.enable()},disable:function(){this.button.disable()},
show:function(){this.button.show()},hide:function(){this.button.hide()}})}),define("taoQtiTest/runner/plugins/tools/calculator",["jquery","lodash","i18n","ui/hider","ui/calculator","util/shortcut","util/namespace","taoTests/runner/plugin"],function($,_,__,hider,calculatorFactory,shortcut,namespaceHelper,pluginFactory){"use strict";var _default={height:360,width:240,top:50,left:10,stackingScope:"test-runner"};return pluginFactory({name:"calculator",init:function(){function isEnabled(){return!!((testRunner.getTestContext()||{}).options||{}).calculator}function togglePlugin(){isEnabled()?self.show():self.hide()}function buildCalculator(calcTpl){
self.calculator=calculatorFactory(_.defaults({renderTo:self.$calculatorContainer,replace:!0,draggableContainer:areaBroker.getContainer(),alternativeTemplate:calcTpl||null},_default)).on("show",function(){self.trigger("open"),self.button.turnOn()}).on("hide",function(){self.trigger("close"),self.button.turnOff()}).show()}function toggleCalculator(){!1!==self.getState("enabled")&&(self.calculator?self.calculator.is("hidden")?self.calculator.show():self.calculator.hide():config.template?require(["tpl!"+config.template.replace(/\.tpl$/,"")],function(calcTpl){buildCalculator(calcTpl)},function(){buildCalculator()}):buildCalculator())}
var self=this,testRunner=this.getTestRunner(),areaBroker=this.getAreaBroker(),testData=testRunner.getTestData()||{},testConfig=testData.config||{},pluginsConfig=testConfig.plugins||{},config=pluginsConfig.calculator||{},pluginShortcuts=(testConfig.shortcuts||{})[this.getName()]||{};this.button=this.getAreaBroker().getToolbox().createEntry({control:"calculator",title:__("Open Calculator"),icon:"table",text:__("Calculator")}),this.$calculatorContainer=$('<div class="widget-calculator">'),this.calculator=null,this.button.on("click",function(e){$(e.target).closest(".widget-calculator").length||(e.preventDefault(),testRunner.trigger("tool-calculator"))}),
testConfig.allowShortcuts&&pluginShortcuts.toggle&&shortcut.add(namespaceHelper.namespaceAll(pluginShortcuts.toggle,this.getName(),!0),function(){testRunner.trigger("tool-calculator")},{avoidInput:!0,allowIn:".widget-calculator"}),togglePlugin(),this.disable(),testRunner.on("loaditem",togglePlugin).on("enabletools renderitem",function(){self.enable()}).on("disabletools unloaditem",function(){self.disable(),self.calculator&&(self.calculator.destroy(),self.calculator=null)}).on("tool-calculator",function(){isEnabled()&&toggleCalculator()})},render:function(){this.getAreaBroker().getContainer().append(this.$calculatorContainer)},destroy:function(){
shortcut.remove("."+this.getName()),this.$calculatorContainer.remove(),this.calculator&&this.calculator.destroy()},enable:function(){this.button.enable()},disable:function(){this.button.disable(),this.calculator&&this.calculator.hide()},show:function(){this.button.show()},hide:function(){this.button.hide(),this.calculator&&this.calculator.hide()}})}),define("tpl!taoQtiTest/runner/plugins/tools/comment/comment",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{}
;var helper,options,buffer="",helperMissing=helpers.helperMissing,escapeExpression=this.escapeExpression;return buffer+='<div data-control="qti-comment" class="hidden">\n    <textarea data-control="qti-comment-text" placeholder="'+escapeExpression((helper=helpers.__||depth0&&depth0.__,options={hash:{},data:data},helper?helper.call(depth0,"Your comment…",options):helperMissing.call(depth0,"__","Your comment…",options)))+'"></textarea>\n    <button data-control="qti-comment-cancel" class="btn-info small"></span>'+escapeExpression((helper=helpers.__||depth0&&depth0.__,options={hash:{},data:data},
helper?helper.call(depth0,"Cancel",options):helperMissing.call(depth0,"__","Cancel",options)))+'</button>\n    <button data-control="qti-comment-send" class="btn-info small">'+escapeExpression((helper=helpers.__||depth0&&depth0.__,options={hash:{},data:data},helper?helper.call(depth0,"Send",options):helperMissing.call(depth0,"__","Send",options)))+"</button>\n</div>\n"})}),
...
}).on("renderitem",function(){var testContext=testRunner.getTestContext();highlighter.restoreHighlight(testContext.itemIdentifier)}).on("beforeunloaditem",function(){var testContext=testRunner.getTestContext();highlighter.saveHighlight(testContext.itemIdentifier)}).on("disabletools unloaditem",function(){self.disable(),highlighter.toggleHighlighting(!1)}).on("tool-highlight",function(){isEnabled()&&highlighter.trigger()}).on("tool-highlight-remove",function(){highlighter.clearHighlights()}).on("plugin-start.highlighter",function(){self.buttonMain.turnOn()}).on("plugin-end.highlighter",function(){self.buttonMain.turnOff()})},destroy:function(){
shortcut.remove("."+this.getName()),$(document).off(".highlighter")},enable:function(){this.buttonMain.enable(),this.buttonRemove.enable()},disable:function(){this.buttonMain.disable(),this.buttonRemove.disable()},show:function(){this.buttonMain.show(),this.buttonRemove.show()},hide:function(){this.buttonMain.hide(),this.buttonRemove.hide()}})}),define("taoQtiTest/runner/plugins/tools/itemThemeSwitcher/itemThemeSwitcher",["jquery","lodash","i18n","core/store","taoTests/runner/plugin","ui/hider","ui/themes","util/shortcut","util/namespace"],function($,_,__,storeFactory,pluginFactory,hider,themeHandler,shortcut,namespaceHelper){"use strict"
;return pluginFactory({name:"itemThemeSwitcher",install:function(){var self=this;this.getTestRunner().on("storechange",function(){self.shouldClearStorage=!0})},init:function(){function changeTheme(themeId){var $qtiItem=$(".qti-item");state.selectedTheme=themeId,$qtiItem&&_.defer(function(){$qtiItem.trigger("themechange",[state.selectedTheme])}),self.storage&&self.storage.setItem("itemThemeId",themeId)}var self=this,testRunner=this.getTestRunner(),testData=testRunner.getTestData()||{},testConfig=testData.config||{},pluginShortcuts=(testConfig.shortcuts||{})[this.getName()]||{},themesConfig=themeHandler.get("items")||{},state={availableThemes:[],
defaultTheme:"",selectedTheme:""},allMenuEntries=[];return themesConfig&&(themesConfig.default&&(state.defaultTheme=themesConfig.default,state.selectedTheme=themesConfig.default),themesConfig.available&&_.forEach(themesConfig.available,function(theme){state.availableThemes.push({id:theme.id,label:theme.name})})),this.menuButton=this.getAreaBroker().getToolbox().createMenu({control:"color-contrast",title:__("Change the current color preset"),icon:"contrast",text:__("Contrast")}),this.menuButton.on("click",function(e){e.preventDefault(),testRunner.trigger("tool-themeswitcher-toggle")}),state.availableThemes.forEach(function(theme){
var themeEntry=self.getAreaBroker().getToolbox().createEntry({control:theme.id,title:theme.label,icon:"preview",text:__(theme.label)});themeEntry.setMenuId("color-contrast"),themeEntry.on("click",function(e){var themeId=this.config.control;e.preventDefault(),self.menuButton.turnOffAll(),this.turnOn(),changeTheme(themeId)}),state.defaultTheme===theme.id&&themeEntry.on("render",function(){this.turnOn()}),allMenuEntries.push(themeEntry)}),testConfig.allowShortcuts&&pluginShortcuts.toggle&&shortcut.add(namespaceHelper.namespaceAll(pluginShortcuts.toggle,this.getName(),!0),function(){testRunner.trigger("tool-themeswitcher-toggle")},{avoidInput:!0}),
this.disable(),testRunner.on("loaditem",function(){self.show()}).on("renderitem",function(){self.enable(),changeTheme(state.selectedTheme)}).on("enabletools",function(){self.enable()}).on("disabletools unloaditem",function(){self.disable()}).on("tool-themeswitcher-toggle",function(){!1!==self.getState("enabled")&&self.menuButton.toggleMenu()}),storeFactory("itemThemeSwitcher-"+testRunner.getConfig().serviceCallId).then(function(itemThemesStore){return self.shouldClearStorage?itemThemesStore.clear().then(function(){return itemThemesStore}):itemThemesStore}).then(function(itemThemesStore){testRunner.after("renderitem enableitem",function(){
self.storage=itemThemesStore,self.storage.getItem("itemThemeId").then(function(itemThemeId){itemThemeId&&state.selectedTheme!==itemThemeId&&(changeTheme(itemThemeId),allMenuEntries.forEach(function(menuEntry){menuEntry.getId()===itemThemeId?menuEntry.turnOn():menuEntry.turnOff()}))})}).before("finish",function(){return new Promise(function(resolve){self.storage.removeStore().then(resolve).catch(resolve)})})})},destroy:function(){shortcut.remove("."+this.getName())},enable:function(){this.menuButton.enable()},disable:function(){this.menuButton.disable()},show:function(){this.menuButton.show()},hide:function(){this.menuButton.hide()}})}),
define("taoQtiTest/runner/plugins/tools/lineReader/compoundMask",["jquery","lodash","core/statifier","core/eventifier","ui/component","ui/component/placeable","ui/component/draggable","ui/component/resizable","ui/component/stackable"],function($,_,statifier,eventifier,componentFactory,makePlaceable,makeDraggable,makeResizable,makeStackable){"use strict";var constrains,defaultDimensions={outerWidth:600,outerHeight:400,innerWidth:500,innerHeight:20},defaultPosition={outerX:0,outerY:0,innerX:50,innerY:50},defaultOptions={dragMinWidth:10,dragMinHeight:10,resizeHandleSize:10,innerDragHeight:20},stackingOptions={stackingScope:"test-runner"}
;return function(options,dimensions,position){function createMask(maskConfig){var mask,maskAPI={place:maskConfig.place,placeOverlay:maskConfig.placeOverlay,styleResizableEdges:function(){var $element=this.getElement();_.forOwn(this.config.edges,function(isResizable,edgeId){isResizable&&$element.addClass("border-"+edgeId)})}};return mask=componentFactory(maskAPI,maskConfig),makeResizable(mask),makeStackable(mask,stackingOptions),mask.on("render",function(){var $element=this.getElement();this.styleResizableEdges(),$element.addClass("line-reader-mask "+maskConfig.id).on("mousedown",function(){bringAllToFront()})}).on("resizestart",function(){innerDrag.hide(),
...
this.setSize(dimensions.outerWidth,dimensions.outerHeight).moveTo(position.outerX,position.outerY),$element.addClass("moving"),this.setState("transformed",!0)},restoreOverlay:function(){var $element=this.getElement();this.is("transformed")&&(this.setSize(this._sizeBackup.width,this._sizeBackup.height).moveTo(this._posBackup.x,this._posBackup.y),$element.removeClass("moving"),this._sizeBackup=null,this._posBackup=null,this.setState("transformed",!1))},appendVisualGuides:function(){var $element=this.getElement();$element.append(visualGuides.$maskBg),$element.append(visualGuides.$innerWindow),visualGuides.$maskBg.css({width:dimensions.outerWidth-2,
height:dimensions.outerHeight-2,"border-top-width":dimensions.topHeight-2,"border-right-width":dimensions.rightWidth-2,"border-bottom-width":dimensions.bottomHeight-2,"border-left-width":dimensions.leftWidth-2}),visualGuides.$innerWindow.css({width:dimensions.innerWidth,height:dimensions.innerHeight,left:dimensions.leftWidth-2,top:dimensions.topHeight-2})},removeVisualGuides:function(){visualGuides.$maskBg.remove(),visualGuides.$innerWindow.remove()}},overlayConfig),makeDraggable(overlay),makeStackable(overlay,stackingOptions),overlay.on("render",function(){var self=this,$element=this.getElement(),pointerEventsPrefix=window.PointerEvent?"pointer":"mouse"
;$element.addClass("line-reader-overlay "+overlayConfig.id).on(pointerEventsPrefix+"down",function(){bringAllToFront(),self.transformOverlay()}).on(pointerEventsPrefix+"up",function(){self.restoreOverlay()})}).on("dragstart",function(){innerDrag.hide(),closer.hide(),invokeOnMasks("hide"),this.appendVisualGuides()}).on("dragmove",function(xOffsetRelative,yOffsetRelative){position.outerX+=xOffsetRelative,position.outerY+=yOffsetRelative,position.innerX+=xOffsetRelative,position.innerY+=yOffsetRelative}).on("dragend",function(){this.removeVisualGuides(),invokeOnAll("show"),innerDrag.show(),closer.show(),applyTransforms()}).init()}
function createInnerDragHandle(){innerDrag=componentFactory(),makeStackable(innerDrag,stackingOptions),makeDraggable(innerDrag,{dragRestriction:function(){var fixedXY=allParts.nw.mask.getElement().offset();return{x:fixedXY.left+constrains.minWidth,y:fixedXY.top+(constrains.minHeight+dimensions.innerHeight+options.resizeHandleSize),width:dimensions.outerWidth-2*constrains.minWidth,height:dimensions.outerHeight-(dimensions.innerHeight+constrains.minHeight+constrains.minBottomHeight-options.innerDragHeight)}}}).on("render",function(){var $element=this.getElement(),$dragIcon=$("<div>",{class:"icon icon-move"});$element.addClass("line-reader-inner-drag"),
$element.append($dragIcon),$element.on("mousedown",function(e){e.stopPropagation(),bringAllToFront()})}).on("dragstart",function(){closer.hide(),invokeOnOverlays("hide"),invokeOnMasks("setState",["resizing",!0])}).on("dragmove",function(xOffsetRelative,yOffsetRelative){position.innerX+=xOffsetRelative,position.innerY+=yOffsetRelative,dimensions.leftWidth+=xOffsetRelative,dimensions.topHeight+=yOffsetRelative,dimensions.rightWidth-=xOffsetRelative,dimensions.bottomHeight-=yOffsetRelative,applyTransformsToMasks()}).on("dragend",function(){invokeOnOverlays("show"),innerDrag.bringToFront(),closer.show(),invokeOnMasks("setState",["resizing",!1]),
applyTransformsToOverlays()}).init()}function createCloser(){closer=componentFactory(),makeStackable(closer,stackingOptions),makePlaceable(closer).on("render",function(){var self=this,$element=this.getElement(),$closeIcon=$("<div>",{class:"icon icon-result-nok"});$element.append($closeIcon),$element.addClass("line-reader-closer"),$element.on("mousedown",function(){bringAllToFront()}),$element.on("click",function(e){e.stopPropagation(),self.trigger("click")})}).init()}function bringAllToFront(){invokeOnAll("bringToFront"),innerDrag.bringToFront(),closer.bringToFront()}function invokeOnAll(fn,args){invokeOnMasks(fn,args),invokeOnOverlays(fn,args)}
...
}).on(actionPrefix+"close",function(){self.getState("active")&&hideMagnifier()})},destroy:function(){shortcut.remove("."+pluginName)},enable:function(){this.button.enable()},disable:function(){this.button.disable()},show:function(){this.button.show()},hide:function(){this.button.hide()}})}),define("tpl!taoQtiTest/runner/plugins/tools/textToSpeech/textToSpeech",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var helper,options,buffer="",helperMissing=helpers.helperMissing,escapeExpression=this.escapeExpression
;return buffer+='<div class="text-to-speech hidden">\n    <div class="action click-to-speak" title="'+escapeExpression((helper=helpers.__||depth0&&depth0.__,options={hash:{},data:data},helper?helper.call(depth0,"Click to speak",options):helperMissing.call(depth0,"__","Click to speak",options)))+'"><span class="icon-click-to-speak"></span></div>\n\n    <div class="action play" title="'+escapeExpression((helper=helpers.__||depth0&&depth0.__,options={hash:{},data:data},
helper?helper.call(depth0,"Play",options):helperMissing.call(depth0,"__","Play",options)))+'"><span class="icon-play"></span></div>\n    <div class="action pause" title="'+escapeExpression((helper=helpers.__||depth0&&depth0.__,options={hash:{},data:data},helper?helper.call(depth0,"Pause",options):helperMissing.call(depth0,"__","Pause",options)))+'"><span class="icon-pause"></span></div>\n\n    <div class="action stop" title="'+escapeExpression((helper=helpers.__||depth0&&depth0.__,options={hash:{},data:data},
helper?helper.call(depth0,"Stop",options):helperMissing.call(depth0,"__","Stop",options)))+'"><span class="icon-stop"></span></div>\n\n    <div class="action settings" title="'+escapeExpression((helper=helpers.__||depth0&&depth0.__,options={hash:{},data:data},helper?helper.call(depth0,"Settings",options):helperMissing.call(depth0,"__","Settings",options)))+'">\n        <span class="icon-settings"></span>\n        <div class="settings-menu">\n            <div class="option volume" title="'+escapeExpression((helper=helpers.__||depth0&&depth0.__,options={hash:{},data:data},
helper?helper.call(depth0,"Volume",options):helperMissing.call(depth0,"__","Volume",options)))+'">\n                <span class="icon-volume"></span>\n                <div class="slider-container">\n                    <div class="slider"></div>\n                </div>\n                <div class="hover-container"></div>\n            </div>\n            <div class="option speed" title="'+escapeExpression((helper=helpers.__||depth0&&depth0.__,options={hash:{},data:data},
helper?helper.call(depth0,"Speech speed",options):helperMissing.call(depth0,"__","Speech speed",options)))+'">\n                <span class="icon-speed"></span>\n                <div class="slider-container">\n                    <div class="slider"></div>\n                </div>\n                <div class="hover-container"></div>\n            </div>\n        </div>\n    </div>\n</div>\n'})}),define("css!taoQtiTest/runner/plugins/tools/textToSpeech/textToSpeech",[],function(){}),
define("taoQtiTest/runner/plugins/tools/textToSpeech/textToSpeech",["jquery","lodash","i18n","ui/component","tpl!taoQtiTest/runner/plugins/tools/textToSpeech/textToSpeech","css!taoQtiTest/runner/plugins/tools/textToSpeech/textToSpeech","nouislider"],function($,_,__,componentFactory,tpl){"use strict";return function(options,config){var speed,volume,texthelpMapping={properties:{pageCompleteCallback:"eba_page_complete_callback",texthelpSpeechStream:"TexthelpSpeechStream",volumeValue:"eba_volume_value"},functions:{barDynamicStart:"$rw_barDynamicStart",barCacheInit:"$rw_barCacheInit",barInit:"$rw_barInit",cachePage:"$rw_cachePage",
disableSpeech:"$rw_disableSpeech",enableClickToSpeak:"$rw_enableClickToSpeak",enableContinuousReading:"$rw_enableContinuousReading",enableSpeech:"$rw_enableSpeech",event_pause:"$rw_event_pause",event_play:"$rw_event_play",getSpeed:"$rw_getSpeed",getVoice:"$rw_getVoice",hasReachedEnd:"$rw_hasReachedEnd",isPaused:"$rw_isPaused",isSpeaking:"$rw_isSpeaking",isTextSelectedForPlay:"$rw_isTextSelectedForPlay",isUsingMathjax:"$rw_isUsingMathjax",setSpeedValue:"$rw_setSpeedValue",setUsingMath:"$rw_setUsingMath",setUsingMathjax:"$rw_setUsingMathjax",setUsingMaths:"$rw_setUsingMaths",setVolumeValue:"$rw_setVolumeValue",stopSpeech:"$rw_stopSpeech",
tagSentences:"$rw_tagSentences",userParameters:"$rw_userParameters"}};return _.assign(options||{},{contentArea:$("body"),enableClickToSpeak:!1,ignoreEls:["header",".left-bar",".right-bar",".modal","footer",".action-bar",".sts-scope",".media-container"],toolbarUrl:"//taotoolbar.speechstream.net/tao/configQA.js"}),componentFactory({_exec:function(action){var args=[].slice.call(arguments,1),fn=window[texthelpMapping.functions[action]];if(fn&&_.isFunction(fn))return fn.apply(this,args)},_get:function(property){return window[texthelpMapping.properties[property]]},_set:function(property,value){var prop=texthelpMapping.properties[property]
;if(prop)return window[prop]=value},_init:function(){var self=this;return this._exec("barDynamicStart"),this._exec("barInit"),this._exec("enableClickToSpeak",options.enableClickToSpeak),this._exec("setSpeedValue",speed),this._exec("setVolumeValue",volume),this._set("pageCompleteCallback",function(){self.trigger("stop")}),this},updateTexthelpCache:function(deliveryId,itemId){var tss=this._get("texthelpSpeechStream");return tss.g_strBookId=deliveryId,tss.g_strPageId=itemId,this._exec("tagSentences",options.$contentArea.selector),this},enable:function(){return this._exec("enableSpeech"),this},disable:function(){return this._exec("disableSpeech"),this},
play:function(){return this._exec("isSpeaking")&&!this._exec("isPaused")||(this._exec("event_play"),this.trigger("play")),this},pause:function(){return this._exec("event_pause"),this.trigger("pause"),this},stop:function(){return this._exec("stopSpeech"),this.trigger("stop"),this},setSpeed:function(e,value){return speed=+value,this._exec("setSpeedValue",speed),this.trigger("setSpeed",speed),this},setVolume:function(e,value){return volume=+value,this._exec("setVolumeValue",volume),this.trigger("setVolume",volume),this},clickToSpeak:function(){return options.enableClickToSpeak=!options.enableClickToSpeak,
this._exec("enableClickToSpeak",options.enableClickToSpeak),this._exec("enableContinuousReading",!options.enableClickToSpeak),this.trigger("clickToSpeak"),this}},{}).setTemplate(tpl).on("init",function(){var self=this;speed=40,volume=40,$(options.ignoreEls.join(",")).attr("ignore",!0),require([options.toolbarUrl],function(){var tss=self._get("texthelpSpeechStream");require(["//"+tss.g_strServer+"/SpeechStream/v"+tss.g_strBuild+"/texthelpMain.js"],function(){self._init()})})}).init(config).on("render",function(){var $this=this.getElement();$this.on("mousedown",function(e){return e.preventDefault(),!1}),$this.find(".action").on("click",function(e){
var $action=$(this);$action.hasClass("disabled")&&e.stopImmediatePropagation(),$action.closest(".settings").length||($this.find(".settings").removeClass("active"),$this.find(".settings > .settings-menu").hide())}),$this.find(".settings").on("click",function(){$(this).toggleClass("active"),$this.find(".settings-menu").toggle()}),$this.find(".settings > .settings-menu").on("click",function(e){e.stopPropagation()}).hide(),$this.find(".settings > .settings-menu > .option").on("hover",function(){$(this).find(".slider-container").show()}).on("mouseleave",function(){$(this).find(".slider-container").hide()}),
$this.find(".settings > .settings-menu > .option > .slider-container").hide(),$this.find(".click-to-speak").on("click",this.clickToSpeak),$this.find(".play").on("click",this.play),$this.find(".pause").on("click",this.pause).hide(),$this.find(".stop").on("click",this.stop),$this.find(".settings > .settings-menu > .volume .slider").noUiSlider({animate:!0,connected:!0,range:{min:0,max:100},start:volume,step:10}).on("change",this.setVolume),$this.find(".settings > .settings-menu > .speed .slider").noUiSlider({animate:!0,connected:!0,range:{min:0,max:100},start:speed,step:10}).on("change",this.setSpeed)}).on("clickToSpeak",function(){var $el=this.getElement()
;options.enableClickToSpeak?($el.find(".click-to-speak").addClass("active"),$el.find(".play").addClass("disabled").show(),$el.find(".pause").addClass("disabled").hide(),options.$contentArea.css("cursor","pointer")):($el.find(".click-to-speak").removeClass("active"),$el.find(".play").removeClass("disabled").show(),$el.find(".pause").removeClass("disabled").hide(),options.$contentArea.css("cursor","default"))}).on("play",function(){var $el=this.getElement();$el.find(".play").hide(),$el.find(".pause").show()}).on("pause",function(){var $el=this.getElement();$el.find(".play").show(),$el.find(".pause").hide()}).on("stop",function(){var $el=this.getElement()
;$el.find(".play").show(),$el.find(".pause").hide()})}}),define("taoQtiTest/runner/plugins/tools/textToSpeech/plugin",["jquery","lodash","i18n","taoTests/runner/plugin","ui/hider","ui/stacker","taoQtiTest/runner/plugins/tools/textToSpeech/textToSpeech"],function($,_,__,pluginFactory,hider,stackerFactory,ttsFactory){"use strict";return pluginFactory({name:"textToSpeech",init:function(){var self=this,testRunner=this.getTestRunner();return this.ttsButton=this.getAreaBroker().getToolbox().createEntry({control:"tts",icon:"headphones",text:__("Text to Speech"),title:__("Text to Speech")}).on("render",function(){var stacker=stackerFactory("test-runner")
;self.tts=ttsFactory({$contentArea:testRunner.getAreaBroker().getContentArea()}).render(self.ttsButton.getElement()).disable().hide(),stacker.autoBringToFront(self.tts.getElement())}).on("click",function(e){var ttsEl=self.tts.getElement();$(e.target).closest(ttsEl).length||(hider.toggle(ttsEl),ttsEl.hasClass("hidden")?self.tts.disable():self.tts.enable())}).disable().hide(),testRunner.on("loaditem",function(){self.disable(),self.ttsButton.hide()}).on("enabletools",function(){self.enable()}).on("renderitem",function(){var context=testRunner.getTestContext(),config=testRunner.getConfig();context.options.textToSpeech&&(self.enable(),self.ttsButton.show(),
self.tts.updateTexthelpCache(config.serviceCallId,this.itemRunner._item.attributes.identifier))}).on("disabletools unloaditem",function(){self.disable()}),this},enable:function(){this.tts&&this.tts.enable(),this.ttsButton&&this.ttsButton.enable()},disable:function(){this.tts&&this.tts.disable(),this.ttsButton&&this.ttsButton.disable()}})}),define("taoQtiTest/runner/plugins/tools/zoom",["jquery","i18n","ui/hider","ui/transformer","util/shortcut","util/namespace","taoTests/runner/plugin"],function($,__,hider,transformer,shortcut,namespaceHelper,pluginFactory){"use strict";var standard=100,threshold={lower:10,upper:200},setZoomLevel=function($target,level){
transformer.setTransformOrigin($target,"0 0"),transformer.scale($target,level/100)},resetZoom=function($target){transformer.reset($target)},forceRepaint=function($target){var sel=$target[0];sel&&(sel.style.display="none",sel.offsetHeight,sel.style.display="")};return pluginFactory({name:"zoom",init:function(){function isConfigured(){return!!((testRunner.getTestContext()||{}).options||{}).zoom}function togglePlugin(){isConfigured()?self.show():self.hide()}function zoomAction(dir){var el,sx,sy,before,after,inc=10*dir;self.$zoomTarget&&(el=self.$zoomTarget[0],before=el.getBoundingClientRect(),sx=self.$container.scrollLeft(),sy=self.$container.scrollTop(),
self.zoom=Math.max(threshold.lower,Math.min(threshold.upper,self.zoom+inc)),self.zoom===standard?resetZoom(self.$zoomTarget):setZoomLevel(self.$zoomTarget,self.zoom),forceRepaint(self.$zoomTarget),after=el.getBoundingClientRect(),sx=Math.max(0,sx+(after.width-before.width)/2),sy=Math.max(0,sy+(after.height-before.height)/2),self.$container.scrollLeft(sx).scrollTop(sy))}function zoomIn(){!1!==self.getState("enabled")&&zoomAction(1)}function zoomOut(){!1!==self.getState("enabled")&&zoomAction(-1)}
var self=this,testRunner=this.getTestRunner(),testData=testRunner.getTestData()||{},testConfig=testData.config||{},pluginShortcuts=(testConfig.shortcuts||{})[this.getName()]||{};this.buttonZoomOut=this.getAreaBroker().getToolbox().createEntry({control:"zoomOut",title:__("Zoom out"),icon:"remove"}),this.buttonZoomIn=this.getAreaBroker().getToolbox().createEntry({control:"zoomIn",title:__("Zoom in"),icon:"add"}),this.buttonZoomIn.on("click",function(e){e.preventDefault(),testRunner.trigger("tool-zoomin")}),this.buttonZoomOut.on("click",function(e){e.preventDefault(),testRunner.trigger("tool-zoomout")}),
testConfig.allowShortcuts&&(pluginShortcuts.in&&shortcut.add(namespaceHelper.namespaceAll(pluginShortcuts.in,this.getName(),!0),function(){testRunner.trigger("tool-zoomin")},{avoidInput:!0}),pluginShortcuts.out&&shortcut.add(namespaceHelper.namespaceAll(pluginShortcuts.out,this.getName(),!0),function(){testRunner.trigger("tool-zoomout")},{avoidInput:!0})),togglePlugin(),this.disable(),testRunner.on("loaditem",function(){self.zoom=standard,togglePlugin(),self.disable()}).on("renderitem",function(){self.$container=$("#qti-content"),self.$zoomTarget=$(".qti-item"),self.enable()}).on("enabletools",function(){self.enable()
