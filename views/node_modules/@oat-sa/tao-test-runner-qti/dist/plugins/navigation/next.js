define(['jquery', 'lodash', 'i18n', 'ui/hider', 'taoTests/runner/plugin', 'taoQtiTest/runner/plugins/navigation/next/nextWarningHelper', 'taoQtiTest/runner/helpers/messages', 'taoQtiTest/runner/helpers/map', 'taoQtiTest/runner/helpers/stats', 'util/shortcut', 'util/namespace', 'handlebars'], function ($$1, _, __, hider, pluginFactory, nextWarningHelper, messages, mapHelper, statsHelper, shortcut, namespaceHelper, Handlebars) { 'use strict';
    ...
    nextWarningHelper = nextWarningHelper && nextWarningHelper.hasOwnProperty('default') ? nextWarningHelper['default'] : nextWarningHelper;
    ...
    statsHelper = statsHelper && statsHelper.hasOwnProperty('default') ? statsHelper['default'] : statsHelper;
    ...
    var updateElement = function updateElement($element, context) {
      ...
      if ($element.attr('data-control') !== buttonData[dataType].control) {
        $element.attr('data-control', buttonData[dataType].control).attr('title', buttonData[dataType].title).find('.text').text(buttonData[dataType].text);
    ...
    var next = pluginFactory({
      ...
      init: function init() {
        ...
        var pluginShortcuts = (testConfig.shortcuts || {})[this.getName()] || {}; //plugin behavior
        ...
        function doNext(nextItemWarning) {
          ...
          var nextItemPosition = context.itemPosition + 1; // x-tao-option-unansweredWarning is a deprecated option whose behavior now matches the one of
          ...
          var unansweredOnly = !testOptions.endTestWarning && // this check to avoid an edge case where having both endTestWarning
          ...
          if (self.getState('enabled') !== false) {
            ...
            if (warningHelper.shouldWarnBeforeEnd()) {
              testRunner.trigger('confirm.endTest', messages.getExitMessage(__('You are about to submit the test. You will not be able to access this test once submitted. Click OK to continue and submit the test.'), warningScope, testRunner), _.partial(triggerNextAction, context), // if the test taker accept
              enableNav // if he refuse
              );
            } else if (warningHelper.shouldWarnBeforeNext()) {
              testRunner.trigger('confirm.next', __('You are about to go to the next item. Click OK to continue and go to the next item.'), _.partial(triggerNextAction, context), // if the test taker accept
              enableNav // if he refuse
              );
            } else {
        ...
        this.$element = createElement(testRunner.getTestContext()); //attach behavior
