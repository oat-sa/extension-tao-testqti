define(['jquery', 'lodash', 'ui/keyNavigation/navigator', 'ui/keyNavigation/navigableDomElement', 'ui/keyNavigation/navigableGroupElement', 'util/shortcut', 'taoTests/runner/plugin', 'css!taoQtiTest/runner/plugins/content/accessibility/css/key-navigation.css'], function ($, _, keyNavigator, navigableDomElement, navigableGroupElement, shortcut, pluginFactory, keyNavigation_css) { 'use strict';
    ...
    navigableGroupElement = navigableGroupElement && navigableGroupElement.hasOwnProperty('default') ? navigableGroupElement['default'] : navigableGroupElement;
    shortcut = shortcut && shortcut.hasOwnProperty('default') ? shortcut['default'] : shortcut;
    ...
    function initNavigatorNavigation(testRunner) {
      ...
      var itemsNavigator;
      var $filters, $trees, navigableFilters, navigableTrees; //the tag to identify if the item listing has been browsed, to only "smart jump" to active item only on the first visit
      ...
      if ($navigator.length && !$navigator.hasClass('disabled')) {
        ...
        if (navigableFilters.length) {
          filtersNavigator = keyNavigator({
            ...
            cursor.navigable.getElement().click(); //reset the item listing browsed tag whenever the focus on the filter happens after a focus on another element
            ...
            defaultPosition: function defaultPosition(navigables) {
              ...
              if (filterCursor && filterCursor.navigable.getElement().data('mode') !== 'flagged') {
                _.forIn(navigables, function (navigable, i) {
                  var $parent = navigable.getElement().parent('.qti-navigator-item'); //find the first active and visible item
    ...
    function initDefaultContentNavigation(testRunner) {
      var itemNavigators = [];
      var $content = testRunner.getAreaBroker().getContentArea(); //the item focusable body elements are considered scrollable
    ...
    function initAllContentButtonsNavigation(testRunner) {
      ...
      var $qtiChoiceNodesList = $qtiIteractionsNodeList.find('.qti-choice'); //the item focusable body elements are considered scrollable
      ...
      $qtiChoiceNodesList.each(function () {
        ...
        keyNavigatorItem.on('activate', function (cursor) {
          var $elt = cursor.navigable.getElement(); //jQuery <= 1.9.0 the checkbox values are set
    ...
    function initInteractionNavigation($interaction) {
      ...
      var interactionNavigators = []; //add navigable elements from prompt
      ...
      $interaction.off('.keyNavigation'); //search for inputs that represent the interaction focusable choices
    ...
    function initRubricNavigation() {
      ...
      $itemElements.each(function () {
        var $itemElement = $(this);
        var id = "rubric_element_navigation_group_".concat(rubricNavigators.length);
    ...
    function initTestRunnerNavigation(testRunner, config) {
      ...
      var navigators; //blur current focused element, to reinitialize keyboard navigation
      ...
      switch (config.contentNavigatorType) {
        case 'linear':
          navigators = _.union(initRubricNavigation(), initAllContentButtonsNavigation(testRunner), initToolbarNavigation(), initNavigatorNavigation(testRunner), initHeaderNavigation());
          break;

        default:
          navigators = _.union(initRubricNavigation(), initDefaultContentNavigation(testRunner), initToolbarNavigation(), initNavigatorNavigation(testRunner), initHeaderNavigation());
          break;
      }
    ...
    var keyNavigation = pluginFactory({
      name: 'keyNavigation',
      ...
      init: function init() {
        ...
        var pluginConfig = _.defaults((testConfig.plugins || {})[self.getName()] || {}, defaultPluginConfig); //start disabled
