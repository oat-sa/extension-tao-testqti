define(['lodash', 'taoQtiTest/runner/navigator/navigator', 'taoQtiTest/runner/helpers/map', 'taoQtiTest/runner/helpers/navigation', 'taoQtiTest/runner/provider/dataUpdater', 'taoQtiTest/runner/proxy/qtiServiceProxy', 'taoQtiTest/runner/proxy/cache/itemStore', 'taoQtiTest/runner/proxy/cache/actionStore', 'taoQtiTest/runner/helpers/offlineErrorHelper'], function (_, testNavigatorFactory, mapHelper, navigationHelper, dataUpdater, qtiServiceProxy, itemStoreFactory, actionStoreFactory, offlineErrorHelper) { 'use strict';
    ...
    testNavigatorFactory = testNavigatorFactory && testNavigatorFactory.hasOwnProperty('default') ? testNavigatorFactory['default'] : testNavigatorFactory;
    mapHelper = mapHelper && mapHelper.hasOwnProperty('default') ? mapHelper['default'] : mapHelper;
    navigationHelper = navigationHelper && navigationHelper.hasOwnProperty('default') ? navigationHelper['default'] : navigationHelper;
    dataUpdater = dataUpdater && dataUpdater.hasOwnProperty('default') ? dataUpdater['default'] : dataUpdater;
    qtiServiceProxy = qtiServiceProxy && qtiServiceProxy.hasOwnProperty('default') ? qtiServiceProxy['default'] : qtiServiceProxy;
    itemStoreFactory = itemStoreFactory && itemStoreFactory.hasOwnProperty('default') ? itemStoreFactory['default'] : itemStoreFactory;
    actionStoreFactory = actionStoreFactory && actionStoreFactory.hasOwnProperty('default') ? actionStoreFactory['default'] : actionStoreFactory;
    offlineErrorHelper = offlineErrorHelper && offlineErrorHelper.hasOwnProperty('default') ? offlineErrorHelper['default'] : offlineErrorHelper;
    ...
    var proxy = _.defaults({
      ...
      install: function install(config) {
        var self = this; //install the parent proxy
        ...
        this.actionPromises = {}; //let's you update test data (testData, testContext and testMap)
        ...
        this.offlineAction = function offlineAction(action, actionParams) {
          ...
          if (actionParams.direction && actionParams.scope) {
            ...
            newTestContext = testNavigator.navigate(actionParams.direction, actionParams.scope, actionParams.ref); //we are really not able to navigate
        ...
        this.scheduleAction = function scheduleAction(action, actionParams) {
          actionParams.actionId = "".concat(action, "_").concat(new Date().getTime());
          return self.actiontStore.push(action, self.prepareParams(_.defaults(actionParams || {}, self.requestConfig))).then(function () {
            return {
              action: action,
              params: actionParams
            };
        ...
        this.requestNetworkThenOffline = function requestNetworkThenOffline(url, action, actionParams, deferred) {
          var testContext = this.getDataHolder().get('testContext');
          var communicationConfig = self.configStorage.getCommunicationConfig(); //perform the request, but fallback on offline if the request itself fails
          ...
          if (this.isOffline()) {
            ...
            return this.telemetry(testContext.itemIdentifier, 'up').then(function () {
      ...
      init: function init(config, params) {
        ...
        this.actiontStore = actionStoreFactory(config.serviceCallId); //we resync as soon as the connection is back
      ...
      getItem: function getItem(itemIdentifier, params) {
        ...
        var loadNextItem = function loadNextItem() {
          ...
          if (self.isOnline() && missing.length) {
            _.delay(function () {
              self.requestNetworkThenOffline(self.configStorage.getTestActionUrl('getNextItemData'), 'getNextItemData', {
        ...
        return this.request(this.configStorage.getItemActionUrl(itemIdentifier, 'getItem'), params).then(function (response) {
      ...
      callItemAction: function callItemAction(itemIdentifier, action, params, deferred) {
        ...
        return updateStatePromise.then(function () {
          return self.requestNetworkThenOffline(self.configStorage.getItemActionUrl(itemIdentifier, action), action, _.merge({
