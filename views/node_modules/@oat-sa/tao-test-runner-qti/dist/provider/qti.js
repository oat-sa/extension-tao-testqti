define(['jquery', 'lodash', 'i18n', 'core/cachedStore', 'taoTests/runner/areaBroker', 'taoTests/runner/proxy', 'taoTests/runner/probeOverseer', 'taoTests/runner/testStore', 'taoQtiTest/runner/provider/dataUpdater', 'taoQtiTest/runner/provider/toolStateBridge', 'taoQtiTest/runner/helpers/currentItem', 'taoQtiTest/runner/helpers/map', 'taoQtiTest/runner/ui/toolbox/toolbox', 'taoQtiItem/runner/qtiItemRunner', 'taoQtiTest/runner/config/assetManager', 'handlebars'], function ($$1, _, __, cachedStore, areaBrokerFactory, proxyFactory, probeOverseerFactory, testStoreFactory, dataUpdater, toolStateBridgeFactory, currentItemHelper, mapHelper, toolboxFactory, qtiItemRunner, getAssetManager, Handlebars) { 'use strict';

    $$1 = $$1 && $$1.hasOwnProperty('default') ? $$1['default'] : $$1;
    _ = _ && _.hasOwnProperty('default') ? _['default'] : _;
    __ = __ && __.hasOwnProperty('default') ? __['default'] : __;
    cachedStore = cachedStore && cachedStore.hasOwnProperty('default') ? cachedStore['default'] : cachedStore;
    areaBrokerFactory = areaBrokerFactory && areaBrokerFactory.hasOwnProperty('default') ? areaBrokerFactory['default'] : areaBrokerFactory;
    proxyFactory = proxyFactory && proxyFactory.hasOwnProperty('default') ? proxyFactory['default'] : proxyFactory;
    probeOverseerFactory = probeOverseerFactory && probeOverseerFactory.hasOwnProperty('default') ? probeOverseerFactory['default'] : probeOverseerFactory;
    testStoreFactory = testStoreFactory && testStoreFactory.hasOwnProperty('default') ? testStoreFactory['default'] : testStoreFactory;
    dataUpdater = dataUpdater && dataUpdater.hasOwnProperty('default') ? dataUpdater['default'] : dataUpdater;
    toolStateBridgeFactory = toolStateBridgeFactory && toolStateBridgeFactory.hasOwnProperty('default') ? toolStateBridgeFactory['default'] : toolStateBridgeFactory;
    ...
    toolboxFactory = toolboxFactory && toolboxFactory.hasOwnProperty('default') ? toolboxFactory['default'] : toolboxFactory;
    qtiItemRunner = qtiItemRunner && qtiItemRunner.hasOwnProperty('default') ? qtiItemRunner['default'] : qtiItemRunner;
    getAssetManager = getAssetManager && getAssetManager.hasOwnProperty('default') ? getAssetManager['default'] : getAssetManager;
    ...
    var qtiProvider = {
      ...
      init: function init() {
        ...
        function computeNext(action, params, loadPromise) {
          var context = self.getTestContext(); //catch server errors
        ...
        this.on('ready', function () {
          ...
          if (timer && timer.allowLateSubmission) {
            ...
            computeNext('timeout', _.merge(getItemResults(), {
              ...
              if (context.options && Object.prototype.hasOwnProperty.call(context.options, 'noAlertTimeout') && context.options.noAlertTimeout) {
                resolve();
              } else {
        ...
        return this.getTestStore().getStorageIdentifier().then(function (storeId) {
          ...
          return self.getProxy().init({
            ...
            self.dataUpdater.update(response); //set the plugin config from the test data

            self.dataUpdater.updatePluginsConfig(self.getPlugins()); //this checks the received storeId and clear the volatiles stores
      ...
      loadItem: function loadItem(itemIdentifier) {
        return this.getProxy().getItem(itemIdentifier).then(function (data) {
