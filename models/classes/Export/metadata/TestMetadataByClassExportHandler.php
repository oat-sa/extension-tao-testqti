...
class TestMetadataByClassExportHandler extends ItemMetadataByClassExportHandler
{
    ...
    public function export($formValues, $destPath): Report
    {
        if (!isset($formValues['filename'])) {
            return Report::createError('Missing filename for export using ' . __CLASS__);
        }

        if (!isset($formValues['uri'])) {
            return Report::createError('No uri selected for export using ' . __CLASS__);
        }

        $report = Report::createSuccess();

        $instance = $this->getResource(\tao_helpers_Uri::decode($formValues['uri']));
