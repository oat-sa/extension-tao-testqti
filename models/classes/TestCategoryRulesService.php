 ...
 *
 * @see oat\taoQtiTest\models\TestCategoryRulesGenerator class for a in-depth documentation of the category-based rule
 *                                                       generation process.
 */
class TestCategoryRulesService extends ConfigurableService
{
     ...
     *
     * This constructor allows you to instantiate a new TestCategoryRulesService object with the following options
     * available:
     *
     * * 'score-variable-identifier' (string) : The identifier of the item session outcome variable to be used when
     *   generating <testVariables> based rules.
     * * 'weight-identifier' (string) : The identifier of the item reference weight to be used when generating
     *   <testVariables> based rules.
     * * 'category-exclusions' (array) : An array of PCREs describing what are the categories to be excluded from the
     *   rule generation process.
     * * 'flags' (integer) : A binary flags configuration of the rule generation process composed by values described
     *   by the TestCategoryRulesGenerator class constants.
     *
    ...
    public function __construct(array $options = [])
    {
        ...
        $generator->setScoreVariableIdentifier(
            empty($options['score-variable-identifier']) ? 'SCORE' : (string) $options['score-variable-identifier']
        );
        $generator->setWeightIdentifier(
            array_key_exists('weight-identifier', $options) ? (string) $options['weight-identifier'] : ''
        );
        $generator->setCategoryExclusions(
            empty($options['category-exclusions']) ? [] : $options['category-exclusions']
        );
