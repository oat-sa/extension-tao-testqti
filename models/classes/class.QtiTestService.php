...
class taoQtiTest_models_classes_QtiTestService extends taoTests_models_classes_TestsService
{
    ...
    public function save(core_kernel_classes_Resource $test, array $items)
    {
        try {
            ...
            throw new taoQtiTest_models_classes_QtiTestServiceException("An error occured while dealing with the QTI-XML test: " . $e->getMessage(), taoQtiTest_models_classes_QtiTestServiceException::TEST_WRITE_ERROR);
        }
                ...
                if (count($dependencies) > 0) {
                    ...
                    if ($itemError === false) {
                        ...
                        if ($testContent !== false) {
                            ...
                            $testResource->setLabel($testDefinition->getDocumentComponent()
                                ->getTitle());
                            $targetClass->setLabel($testDefinition->getDocumentComponent()
                                ->getTitle());
                        }
                    } else {
    ...
    {
        if (is_null($test)) {
            throw new taoQtiTest_models_classes_QtiTestServiceException('The selected test is null', taoQtiTest_models_classes_QtiTestServiceException::TEST_READ_ERROR);
        }
    ...
    public function getDoc(core_kernel_classes_Resource $test)
    {
        ...
        try {
            ...
            throw new taoQtiTest_models_classes_QtiTestServiceException("An error occured while loading QTI-XML test file for test '" . $test->getUri() . "' : " . $e->getMessage(), taoQtiTest_models_classes_QtiTestServiceException::TEST_READ_ERROR);
        }
