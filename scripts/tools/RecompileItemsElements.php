        ...
        if ($extManager->isInstalled('taoDeliveryRdf') === true && $extManager->isEnabled('taoDeliveryRdf') === true) {
            ...
            if ($compiledDeliveryClass->exists() === true) {
                foreach ($compiledDeliveryClass->getInstances(true) as $compiledDelivery) {
                    /** @var UnionAssignmentService $nccer */
                    $nccer = $this->getServiceLocator()->get(UnionAssignmentService::SERVICE_ID);
                    $runtime = $nccer->getRuntime($compiledDelivery);
                    ...
                    $report->add(new Report(Report::TYPE_INFO, "Starting to recompile items for delivery {$compiledDelivery->getLabel()} with identifier {$compiledDelivery->getUri()}:"));
                    $count = 0;
                    ...
                    foreach ($assessmentItemRefs as $assessmentItemRef) {
                        ...
                        foreach ($languages as $lang) {
                            if (!$directory->has($lang.DIRECTORY_SEPARATOR.QtiJsonItemCompiler::METADATA_FILE_NAME)) {
                                ...
                                $report->add(new Report(Report::TYPE_SUCCESS, "Metadata for assessmentItemRef '{$identifier}' with identifier '{$uri}' and lang '{$lang}' successfully compiled."));
                                $count++;
                            }
                        }
                    }
                    $report->add(new Report(Report::TYPE_INFO, "Was updated {$count} items."));
                }
            }
        } else {
            $report->add(
